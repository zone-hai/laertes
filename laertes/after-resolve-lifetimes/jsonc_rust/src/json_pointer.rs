...
pub use crate::src::json_object::{
    json_object, json_object_array_add, json_object_array_get_idx, json_object_array_length,
    json_object_array_put_idx, json_object_is_type, json_object_object_add,
...
extern "C" fn string_replace_all_occurrences_with_char(
    ...
    let mut slen: u64 = (unsafe { strlen(s) });
    let mut skip: u64 = (unsafe { (strlen(occur)) }).wrapping_sub(1 as i32 as u64);
    let mut p: *mut i8 = s;
    loop {
...
extern "C" fn is_valid_index<'a1>(
    ...
    if len == 1 as i32 as u64 {
        ...
        {
            *(borrow_mut(&mut idx)).unwrap() =
                ((unsafe { *path.offset(0 as i32 as isize) }) as i32 - '0' as i32) as size_t;
        } else {
...
extern "C" fn json_pointer_get_recursive<'a1>(
    mut obj: *mut crate::src::json_object::json_object,
    mut path: *mut i8,
...
#[no_mangle]
pub extern "C" fn json_pointer_get<'a1>(
    mut obj: *mut crate::src::json_object::json_object,
    mut path: *const i8,
...
#[no_mangle]
pub extern "C" fn json_pointer_set<'a1>(
        ...
        *__errno_location() = 22 as i32;
        ...
        if endp == path_copy {
            ...
            *endp = '\u{0}' as i32 as i8;
            rc = json_pointer_get_recursive(
                *(borrow_mut(&mut obj)).unwrap(),
                path_copy,
                Some(&mut set),
            );
            if rc != 0 {
