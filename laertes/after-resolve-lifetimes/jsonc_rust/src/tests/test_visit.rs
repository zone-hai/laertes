...
pub use crate::src::{
    debug::mc_set_debug,
    json_object::{json_object, json_object_get_type, json_object_put, json_object_to_json_string},
...
fn main_0() -> i32 {
    let mut input : * const i8 = b"{\t\t\"obj1\": 123,\t\t\"obj2\": {\t\t\t\"subobj1\": \"aaa\",\t\t\t\"subobj2\": \"bbb\",\t\t\t\"subobj3\": [ \"elem1\", \"elem2\", true ],\t\t},\t\t\"obj3\": 1.234,\t\t\"obj4\": [ true, false, null ]\t}\0" as * const u8 as * const i8 ;
...
extern "C" fn pop_and_stop(
    ...
    emit_object(jso, flags, parent_jso, jso_key, jso_index, userarg);
    if !jso_key.is_null() && (unsafe { strcmp(jso_key, b"subobj1\0" as *const u8 as *const i8) }) == 0 as i32 {
        (unsafe { printf(b"POP after handling subobj1\n\0" as *const u8 as *const i8) });
        return 767 as i32;
    }
    if !jso_key.is_null() && (unsafe { strcmp(jso_key, b"obj3\0" as *const u8 as *const i8) }) == 0 as i32 {
        (unsafe { printf(b"STOP after handling obj3\n\0" as *const u8 as *const i8) });
        return 7867 as i32;
    }
    return 0 as i32;
}
extern "C" fn err_on_subobj2(
    mut jso: *mut crate::src::json_object::json_object,
    mut flags: i32,
    mut parent_jso: *mut crate::src::json_object::json_object,
    mut jso_key: *const i8,
    ...
    emit_object(jso, flags, parent_jso, jso_key, jso_index, userarg);
    if !jso_key.is_null() && (unsafe { strcmp(jso_key, b"subobj2\0" as *const u8 as *const i8) }) == 0 as i32 {
        (unsafe { printf(b"ERROR after handling subobj1\n\0" as *const u8 as *const i8) });
        return -(1 as i32);
    }
    return 0 as i32;
}
extern "C" fn pop_array(
    mut jso: *mut crate::src::json_object::json_object,
    mut flags: i32,
    mut parent_jso: *mut crate::src::json_object::json_object,
    mut jso_key: *const i8,
    ...
    emit_object(jso, flags, parent_jso, jso_key, jso_index, userarg);
    if !jso_index.is_null() && (unsafe { *jso_index }) == 0 as i32 as u64 {
        (unsafe { printf(b"POP after handling array[0]\n\0" as *const u8 as *const i8) });
        return 767 as i32;
    }
    return 0 as i32;
}
extern "C" fn stop_array(
    mut jso: *mut crate::src::json_object::json_object,
    mut flags: i32,
    mut parent_jso: *mut crate::src::json_object::json_object,
    mut jso_key: *const i8,
    ...
    emit_object(jso, flags, parent_jso, jso_key, jso_index, userarg);
    if !jso_index.is_null() && (unsafe { *jso_index }) == 0 as i32 as u64 {
        (unsafe { printf(b"STOP after handling array[1]\n\0" as *const u8 as *const i8) });
        return 7867 as i32;
    }
    return 0 as i32;
}
extern "C" fn err_return(
    mut jso: *mut crate::src::json_object::json_object,
    mut flags: i32,
    mut parent_jso: *mut crate::src::json_object::json_object,
    mut jso_key: *const i8,
