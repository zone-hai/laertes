...
extern "C" {
    ...
    fn psl_free(psl: * mut crate::src::lib::urlapi::psl_ctx_st);
    fn psl_builtin() -> * const crate::src::lib::urlapi::psl_ctx_st;
    fn psl_latest(fname: * const i8) -> * mut crate::src::lib::urlapi::psl_ctx_st;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_psl_destroy(mut pslcache: * mut crate::src::lib::http2::PslCache) {
    if !((*pslcache).psl).is_null() {
        if (*pslcache).dynamic {
            psl_free((*pslcache).psl as *mut psl_ctx_t);
        }
        let mut fresh0 = &mut ((*pslcache).psl);
        *fresh0 = 0 as *const psl_ctx_t;
        (*pslcache).dynamic = 0 as i32 != 0;
    }
}
unsafe extern "C" fn now_seconds() -> i64 {
    let mut now: crate::src::lib::http2::curltime = Curl_now();
    return now.tv_sec;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_psl_use(mut easy: * mut crate::src::lib::http2::Curl_easy) -> * const crate::src::lib::urlapi::psl_ctx_st {
    let mut pslcache: * mut crate::src::lib::http2::PslCache = (*easy).psl;
    let mut psl: * const crate::src::lib::urlapi::psl_ctx_st = 0 as *const psl_ctx_t;
    let mut now: i64 = 0;
    if pslcache.is_null() {
    ...
    if ((*pslcache).psl).is_null() || (*pslcache).expires <= now {
        ...
        if ((*pslcache).psl).is_null() || (*pslcache).expires <= now {
            ...
            let mut expires: i64 = 0x7fffffffffffffff as i64;
            ...
            if !psl.is_null() {
                ...
                let mut fresh1 = &mut ((*pslcache).psl);
                *fresh1 = psl;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_psl_release(mut easy: * mut crate::src::lib::http2::Curl_easy) {
    Curl_share_unlock(easy, CURL_LOCK_DATA_PSL);
}
