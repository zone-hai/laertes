...
pub use crate::src::lib::asyn_thread::Curl_resolver_getaddrinfo;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_addrinfo_callback(
    ...
    (*data).state.async_0.status = status;
    if CURLE_OK as i32 == status {
        if !ai.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn Curl_getaddrinfo<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut hostname: * const i8,
    mut port: i32,
    mut waitp: Option<&'a1 mut i32>,
) -> * mut crate::src::lib::http2::Curl_addrinfo {
    return Curl_resolver_getaddrinfo(data, hostname, port, borrow_mut(&mut waitp));
}
