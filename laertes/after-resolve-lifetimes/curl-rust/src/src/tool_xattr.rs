...
impl std::default::Default for xattr_mapping {
    fn default() -> Self { xattr_mapping::new() }
}

static mut mappings: [crate::src::src::tool_xattr::xattr_mapping; 4] = [
    {
...
unsafe extern "C" fn stripcredentials<'a1>(mut url: Option<&'a1 mut * mut i8>) -> bool {
    let mut u: * mut crate::src::lib::urlapi::Curl_URL = 0 as *mut CURLU;
    let mut uc: u32 = CURLUE_OK;
    let mut nurl: * mut i8 = 0 as *mut i8;
    u = curl_url();
    if !u.is_null() {
        uc = curl_url_set(u, CURLUPART_URL, *(borrow(& url)).unwrap(), 0 as i32 as u32);
        if !(uc as u64 != 0) {
            uc = curl_url_set(
                u,
                CURLUPART_USER,
            ...
            if !(uc as u64 != 0) {
                ...
                if !(uc as u64 != 0) {
                    ...
                    if !(uc as u64 != 0) {
                        curl_url_cleanup(u);
                        *(borrow_mut(&mut url)).unwrap() = nurl;
                        return 1 as i32 != 0;
                    }
                }
            }
...
pub unsafe extern "C" fn fwrite_xattr(
    ...
    while err == 0 as i32 && !(mappings[i as usize].attr).is_null() {
        ...
        let mut result: u32 = curl_easy_getinfo(
            curl,
        ...
        if result as u64 == 0 && !value.is_null() {
            ...
            {
                freeptr = stripcredentials(Some(&mut value));
            }
            if !value.is_null() {
                err = fsetxattr(
                    fd,
