...
pub struct speedcount {
    pub dl: i64,
    pub ul: i64,
    pub stamp: crate::src::lib::openldap::timeval,
}
impl speedcount {
    pub const fn new() -> Self {
        ...
        dl: 0,
        ul: 0,
        stamp: crate::src::lib::openldap::timeval::new()
        }
    }
}

impl std::default::Default for speedcount {
    fn default() -> Self { speedcount::new() }
}
...
pub unsafe extern "C" fn xferinfo_cb(
    mut clientp: * mut core::ffi::c_void,
    mut dltotal: i64,
    mut dlnow: i64,
    mut ultotal: i64,
    mut ulnow: i64,
) -> i32 {
    let mut per: * mut crate::src::src::tool_cb_hdr::per_transfer<'_> = clientp as *mut per_transfer;
...
static mut all_dltotal: i64 = 0 as i32 as curl_off_t;
static mut all_ultotal: i64 = 0 as i32 as curl_off_t;
static mut all_dlalready: i64 = 0 as i32 as curl_off_t;
static mut all_ulalready: i64 = 0 as i32 as curl_off_t;
#[no_mangle]
pub static mut all_xfers: i64 = 0 as i32 as curl_off_t;
static mut speedindex: u32 = 0;
static mut indexwrapped: bool = false;
static mut speedstore: [crate::src::src::tool_progress::speedcount; 10] = [speedcount {
    dl: 0,
    ul: 0,
    stamp: timeval { tv_sec: 0, tv_usec: 0 },
}; 10];
#[no_mangle]
pub unsafe extern "C" fn progress_meter<'a1>(
    mut global: * mut crate::src::src::tool_cb_dbg::GlobalConfig,
    mut start: Option<&'a1 mut crate::src::lib::openldap::timeval>,
    mut final_0: bool,
) -> bool {
    static mut stamp: crate::src::lib::openldap::timeval = timeval { tv_sec: 0, tv_usec: 0 };
    ...
    if final_0 as i32 != 0 || diff > 500 as i32 as i64 {
        ...
        let mut spent: i64 = tvdiff(now, *(borrow_mut(&mut start)).unwrap())
            / 1000 as i32 as i64;
        let mut dlpercen: [i8; 4] = *core::intrinsics::transmute::<&'_ [u8; 4], &'_ mut [i8; 4]>(b"--\0\0");
        let mut ulpercen: [i8; 4] = *core::intrinsics::transmute::<&'_ [u8; 4], &'_ mut [i8; 4]>(b"--\0\0");
        let mut per: * mut crate::src::src::tool_cb_hdr::per_transfer<'_> = 0 as *mut per_transfer;
        let mut all_dlnow: i64 = 0 as i32 as curl_off_t;
        let mut all_ulnow: i64 = 0 as i32 as curl_off_t;
        let mut dlknown: bool = 1 as i32 != 0;
        let mut ulknown: bool = 1 as i32 != 0;
        let mut all_running: i64 = 0 as i32 as curl_off_t;
        let mut all_queued: i64 = 0 as i32 as curl_off_t;
        let mut speed: i64 = 0 as i32 as curl_off_t;
        let mut i: u32 = 0;
        stamp = now;
        ...
        if indexwrapped {
            ...
            deltams = tvdiff(now, *(borrow_mut(&mut start)).unwrap());
            dl = all_dlnow;
            ul = all_ulnow;
        }
        ...
        if dlknown as i32 != 0 && speed != 0 {
            let mut est: i64 = all_dltotal / speed;
            let mut left: i64 = (all_dltotal - all_dlnow) / speed;
...
#[no_mangle]
pub unsafe extern "C" fn progress_finalize<'a1>(mut per: * mut crate::src::src::tool_cb_hdr::per_transfer<'a1>) {
