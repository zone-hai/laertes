...
pub extern "C" fn lh_table_new(
    ...
    (unsafe { (*t).size = size });
    let (unsafe { ref mut fresh1 }) = (*t).table;
    *fresh1 = (unsafe { calloc(size as u64, ::std::mem::size_of::<lh_entry>() as u64) }) as *mut lh_entry;
    ...
    let (unsafe { ref mut fresh2 }) = (*t).free_fn;
    *fresh2 = free_fn;
    let (unsafe { ref mut fresh3 }) = (*t).hash_fn;
    *fresh3 = hash_fn;
    let (unsafe { ref mut fresh4 }) = (*t).equal_fn;
    *fresh4 = equal_fn;
    i = 0 as i32;
    while i < size {
        let (unsafe { ref mut fresh5 }) = (*(unsafe { ((*t).table).offset(i as isize) })).k;
...
pub extern "C" fn lh_table_resize(
    ...
    let (unsafe { ref mut fresh6 }) = (*t).table;
    *fresh6 = (unsafe { (*new_t).table });
    (unsafe { (*t).size = new_size });
    let (unsafe { ref mut fresh7 }) = (*t).head;
    *fresh7 = (unsafe { (*new_t).head });
    let (unsafe { ref mut fresh8 }) = (*t).tail;
...
pub extern "C" fn lh_table_insert_w_hash(
    ...
    let (unsafe { ref mut fresh9 }) = (*(unsafe { ((*t).table).offset(n as isize) })).k;
    *fresh9 = k;
    (unsafe { (*((*t).table).offset(n as isize)).k_is_constant =
        (opts & ((1 as i32) << 2 as i32) as u32) as i32 });
    let (unsafe { ref mut fresh10 }) = (*(unsafe { ((*t).table).offset(n as isize) })).v;
    *fresh10 = v;
    let (unsafe { ref mut fresh11 }) = (*t).count;
    *fresh11 += 1;
    if (unsafe { ((*t).head) }).is_null() {
        let (unsafe { ref mut fresh12 }) = (*t).tail;
        *fresh12 = (unsafe { &mut *((*t).table).offset(n as isize) }) as *mut lh_entry;
        let (unsafe { ref mut fresh13 }) = (*t).head;
        *fresh13 = *fresh12;
        let (unsafe { ref mut fresh14 }) = (*(unsafe { ((*t).table).offset(n as isize) })).prev;
        *fresh14 = 0 as *mut lh_entry;
        let (unsafe { ref mut fresh15 }) = (*(unsafe { ((*t).table).offset(n as isize) })).next;
        *fresh15 = *fresh14;
    } else {
        let (unsafe { ref mut fresh16 }) = (*(*t).tail).next;
        *fresh16 = (unsafe { &mut *((*t).table).offset(n as isize) }) as *mut lh_entry;
        let (unsafe { ref mut fresh17 }) = (*(unsafe { ((*t).table).offset(n as isize) })).prev;
        *fresh17 = (unsafe { (*t).tail });
        let (unsafe { ref mut fresh18 }) = (*(unsafe { ((*t).table).offset(n as isize) })).next;
        *fresh18 = 0 as *mut lh_entry;
        let (unsafe { ref mut fresh19 }) = (*t).tail;
...
pub extern "C" fn lh_table_delete_entry(
    ...
    let (unsafe { ref mut fresh20 }) = (*t).count;
    *fresh20 -= 1;
    if (unsafe { ((*t).free_fn).is_some() }) {
        (unsafe { ((*t).free_fn).expect("non-null function pointer")(e) });
    }
    let (unsafe { ref mut fresh21 }) = (*(unsafe { ((*t).table).offset(n as isize) })).v;
    *fresh21 = 0 as *const libc::c_void;
    let (unsafe { ref mut fresh22 }) = (*(unsafe { ((*t).table).offset(n as isize) })).k;
    *fresh22 = -(2 as i32) as *mut libc::c_void;
    ...
    {
        let (unsafe { ref mut fresh23 }) = (*t).tail;
        *fresh23 = 0 as *mut lh_entry;
        let (unsafe { ref mut fresh24 }) = (*t).head;
        *fresh24 = *fresh23;
    } else if (unsafe { (*t).head }) == (unsafe { &mut *((*t).table).offset(n as isize) }) as *mut lh_entry {
        let (unsafe { ref mut fresh25 }) = (*(*(*t).head).next).prev;
        *fresh25 = 0 as *mut lh_entry;
        let (unsafe { ref mut fresh26 }) = (*t).head;
        *fresh26 = (unsafe { (*(*t).head).next });
    } else if (unsafe { (*t).tail }) == (unsafe { &mut *((*t).table).offset(n as isize) }) as *mut lh_entry {
        let (unsafe { ref mut fresh27 }) = (*(*(*t).tail).prev).next;
        *fresh27 = 0 as *mut lh_entry;
        let (unsafe { ref mut fresh28 }) = (*t).tail;
        *fresh28 = (unsafe { (*(*t).tail).prev });
    } else {
        let (unsafe { ref mut fresh29 }) = (*(*(unsafe { ((*t).table).offset(n as isize) })).prev).next;
        *fresh29 = (unsafe { (*((*t).table).offset(n as isize)).next });
        let (unsafe { ref mut fresh30 }) = (*(*(unsafe { ((*t).table).offset(n as isize) })).next).prev;
        *fresh30 = (unsafe { (*((*t).table).offset(n as isize)).prev });
    }
    let (unsafe { ref mut fresh31 }) = (*(unsafe { ((*t).table).offset(n as isize) })).prev;
    *fresh31 = 0 as *mut lh_entry;
    let (unsafe { ref mut fresh32 }) = (*(unsafe { ((*t).table).offset(n as isize) })).next;
    *fresh32 = *fresh31;
    return 0 as i32;
}
#[no_mangle]
