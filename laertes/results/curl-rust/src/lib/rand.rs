...
pub use crate::src::lib::vtls::vtls::Curl_ssl_random;
...
unsafe extern "C" fn randit(
    ...
    if !seeded {
        ...
        if fd > -(1 as i32) {
            let mut nread: i64 = read(
                fd,
                &mut randseed as *mut u32 as *mut libc::c_void,
                ::std::mem::size_of::<u32>() as u64,
            );
            if nread as u64
                == ::std::mem::size_of::<u32>() as u64
    ...
    if !seeded {
        ...
        Curl_infof(
            data,
            b"WARNING: Using weak random seed\0" as *const u8 as *const i8,
        );
        randseed = randseed
            .wrapping_add(
                (now.tv_usec as u32).wrapping_add(now.tv_sec as u32),
            );
        randseed = randseed
    ...
    r = randseed;
    *rnd = r << 16 as i32
        | r >> 16 as i32 & 0xffff as i32 as u32;
    return CURLE_OK;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_rand(
    ...
    while num != 0 {
        ...
        while left != 0 {
            let mut fresh0 = rnd;
            rnd = rnd.offset(1);
            *fresh0 = (r & 0xff as i32 as u32) as u8;
            r >>= 8 as i32;
...
pub unsafe extern "C" fn Curl_rand_hex(
    ...
    let mut bufp: * mut u8 = buffer.as_mut_ptr();
    if num.wrapping_div(2 as i32 as u64)
        >= ::std::mem::size_of::<[u8; 128]>() as u64
        || num & 1 as i32 as u64 == 0
    {
    ...
    result = Curl_rand(
        data,
        buffer.as_mut_ptr(),
        num.wrapping_div(2 as i32 as u64),
    );
    if result as u64 != 0 {
        return result;
    }
    while num != 0 {
        ...
        *fresh1 = *hex
            .offset(
                ((*bufp as i32 & 0xf0 as i32) >> 4 as i32)
                    as isize,
            ) as u8;
        let mut fresh2 = rnd;
        rnd = rnd.offset(1);
        *fresh2 = *hex.offset((*bufp as i32 & 0xf as i32) as isize)
            as u8;
        bufp = bufp.offset(1);
        num = (num as u64).wrapping_sub(2 as i32 as u64)
            as size_t as size_t;
    }
    *rnd = 0 as i32 as u8;
    return result;
