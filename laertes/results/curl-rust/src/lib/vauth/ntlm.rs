...
unsafe extern "C" fn ntlm_decode_type2_target<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut type2ref: Option<&'a1 crate::src::lib::bufref::bufref>,
    mut ntlm: * mut crate::src::lib::http2::ntlmdata,
) -> u32 {
    let mut target_info_len: u16 = 0 as i32 as u16;
    let mut target_info_offset: u32 = 0 as i32 as u32;
    let mut type2: * const u8 = Curl_bufref_ptr((type2ref).clone());
    let mut type2len: u64 = Curl_bufref_len((type2ref).clone());
    if type2len >= 48 as i32 as u64 {
        ...
        if target_info_len as i32 > 0 as i32 {
            ...
            let mut fresh0 = &mut ((*ntlm).target_info);
            *fresh0 = Curl_cmalloc
...
#[no_mangle]
pub unsafe extern "C" fn Curl_auth_decode_ntlm_type2_message<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut type2ref: Option<&'a1 crate::src::lib::bufref::bufref>,
    mut ntlm: * mut crate::src::lib::http2::ntlmdata,
) -> u32 {
    ...
    if (*ntlm).flags & ((1 as i32) << 23 as i32) as u32 != 0 {
        result = ntlm_decode_type2_target(data, (type2ref).clone(), ntlm);
        if result as u64 != 0 {
            Curl_infof(
                data,
                b"NTLM handshake failure (bad type-2 message)\0" as *const u8
...
#[no_mangle]
pub unsafe extern "C" fn Curl_auth_create_ntlm_type1_message<'a1, 'a2>(
    mut _data: * mut crate::src::lib::http2::Curl_easy,
    mut _userp: * const i8,
    mut _passwdp: * const i8,
    mut _service: * const i8,
    mut _hostname: * const i8,
    ...
    let mut hostlen: u64 = 0 as i32 as size_t;
    let mut domlen: u64 = 0 as i32 as size_t;
    let mut hostoff: u64 = 0 as i32 as size_t;
    let mut domoff: u64 = hostoff.wrapping_add(hostlen);
    Curl_auth_cleanup_ntlm(borrow_mut(&mut ntlm));
...
#[no_mangle]
pub unsafe extern "C" fn Curl_auth_create_ntlm_type3_message<'a1, 'a2>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut userp: * const i8,
    ...
    let mut unicode: bool = if (*(borrow(& ntlm)).unwrap()).flags
    ...
    let mut host: [i8; 1025] = *core::intrinsics::transmute::<&'_ [u8; 1025], &'_ mut [i8; 1025]>(
    ...
    let mut domain: * const i8 = b"\0" as *const u8 as *const i8;
    let mut hostoff: u64 = 0 as i32 as size_t;
    let mut useroff: u64 = 0 as i32 as size_t;
    let mut domoff: u64 = 0 as i32 as size_t;
    let mut hostlen: u64 = 0 as i32 as size_t;
    let mut userlen: u64 = 0 as i32 as size_t;
    let mut domlen: u64 = 0 as i32 as size_t;
    ...
    if (*(borrow(& ntlm)).unwrap()).flags & ((1 as i32) << 19 as i32) as u32 != 0 {
        ...
        (*(borrow_mut(&mut ntlm)).unwrap()).flags &= !((1 as i32) << 19 as i32) as u32;
    }
    if unicode {
    ...
    size = curl_msnprintf(
        ...
        0 as i32,
        0 as i32,
        ((*(borrow(& ntlm)).unwrap()).flags & 0xff as i32 as u32) as i32,
        ((*(borrow(& ntlm)).unwrap()).flags >> 8 as i32 & 0xff as i32 as u32)
            as i32,
        ((*(borrow(& ntlm)).unwrap()).flags >> 16 as i32 & 0xff as i32 as u32)
            as i32,
        ((*(borrow(& ntlm)).unwrap()).flags >> 24 as i32 & 0xff as i32 as u32)
            as i32,
    ) as size_t;
    ...
    result = Curl_bufref_memdup(borrow_mut(&mut out), ntlmbuf.as_mut_ptr() as *const libc::c_void, size);
    Curl_auth_cleanup_ntlm(borrow_mut(&mut ntlm));
    return result;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_auth_cleanup_ntlm<'a1>(mut ntlm: Option<&'a1 mut crate::src::lib::http2::ntlmdata>) {
    Curl_cfree.expect("non-null function pointer")((*(borrow_mut(&mut ntlm)).unwrap()).target_info);
    let mut fresh1 = &mut ((*(borrow_mut(&mut ntlm)).unwrap()).target_info);
    *fresh1 = 0 as *mut libc::c_void;
    (*(borrow_mut(&mut ntlm)).unwrap()).target_info_len = 0 as i32 as u32;
}
