...
#[no_mangle]
pub static mut Curl_handler_mqtt: crate::src::lib::http2::Curl_handler = unsafe {
    {
        let mut init = Curl_handler {
            scheme: b"MQTT\0" as *const u8 as *const i8,
            ...
            attach: None,
            defport: 1883 as i32,
            protocol: ((1 as i32) << 28 as i32) as u32,
            family: ((1 as i32) << 28 as i32) as u32,
...
unsafe extern "C" fn mqtt_setup_conn(
    ...
    let mut mq: * mut crate::src::lib::http2::MQTT = (0 as * mut crate::src::lib::http2::MQTT);
    mq = Curl_ccalloc
        .expect(
            "non-null function pointer",
        )(1 as i32 as size_t, ::std::mem::size_of::<MQTT>() as u64)
        as *mut MQTT;
    if mq.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    let mut fresh0 = &mut ((*data).req.p.mqtt);
    *fresh0 = mq;
    return CURLE_OK;
}
unsafe extern "C" fn mqtt_send(
    ...
    let mut mq: * mut crate::src::lib::http2::MQTT = (*data).req.p.mqtt;
    let mut n: i64 = 0;
    result = Curl_write(data, sockfd, buf as *const libc::c_void, len, Some(&mut n));
    if result as u64 == 0 {
        Curl_debug(data, CURLINFO_HEADER_OUT, buf, n as size_t);
    }
    if len != n as size_t {
        let mut nsend: u64 = len.wrapping_sub(n as u64);
        let mut sendleftovers: * mut i8 = Curl_memdup(
            &mut *buf.offset(n as isize) as *mut i8 as *const libc::c_void,
            nsend,
        ) as *mut i8;
        if sendleftovers.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        let mut fresh1 = &mut ((*mq).sendleftovers);
        *fresh1 = sendleftovers;
        (*mq).nsend = nsend;
    } else {
        let mut fresh2 = &mut ((*mq).sendleftovers);
        *fresh2 = 0 as *mut i8;
        (*mq).nsend = 0 as i32 as size_t;
    }
    return result;
}
...
unsafe extern "C" fn mqtt_encode_len(
    ...
    let mut i: i32 = 0;
    i = 0 as i32;
    while len > 0 as i32 as u64 && i < 4 as i32 {
        encoded = len.wrapping_rem(0x80 as i32 as u64)
            as u8;
        len = (len as u64).wrapping_div(0x80 as i32 as u64)
            as size_t as size_t;
        if len != 0 {
            encoded = (encoded as i32 | 0x80 as i32) as u8;
        }
        *buf.offset(i as isize) = encoded as i8;
        i += 1;
    }
    return i;
}
unsafe extern "C" fn add_passwd(
    ...
    let conn_flags_pos: u64 = (remain_pos + 8 as i32) as size_t;
    if plen > 0xffff as i32 as u64 {
        return 1 as i32;
    }
    let mut fresh3 = &mut (*pkt.offset(conn_flags_pos as isize));
    *fresh3 = (*fresh3 as i32 | 0x40 as i32) as i8;
    *pkt
        .offset(
            start as isize,
        ) = (plen >> 8 as i32 & 0xff as i32 as u64)
        as i8;
    *pkt
        .offset(
            start.wrapping_add(1 as i32 as u64) as isize,
        ) = (plen & 0xff as i32 as u64) as i8;
    memcpy(
        &mut *pkt.offset(start.wrapping_add(2 as i32 as u64) as isize)
...
unsafe extern "C" fn add_user(
    ...
    let conn_flags_pos: u64 = (remain_pos + 8 as i32) as size_t;
    if ulen > 0xffff as i32 as u64 {
        return 1 as i32;
    }
    let mut fresh4 = &mut (*pkt.offset(conn_flags_pos as isize));
    *fresh4 = (*fresh4 as i32 | 0x80 as i32) as u8;
    *pkt
        .offset(
            start as isize,
        ) = (ulen >> 8 as i32 & 0xff as i32 as u64)
        as u8;
    *pkt
        .offset(
            start.wrapping_add(1 as i32 as u64) as isize,
        ) = (ulen & 0xff as i32 as u64) as u8;
    memcpy(
        &mut *pkt.offset(start.wrapping_add(2 as i32 as u64) as isize)
...
unsafe extern "C" fn add_client_id(
    mut client_id: * const i8,
    client_id_len: u64,
    mut pkt: * mut i8,
    start: u64,
) -> i32 {
    if client_id_len != 12 as i32 as u64 {
        return 1 as i32;
    }
    *pkt.offset(start as isize) = 0 as i32 as i8;
    *pkt
        .offset(
            start.wrapping_add(1 as i32 as u64) as isize,
        ) = 12 as i32 as i8;
...
unsafe extern "C" fn init_connpack(
    ...
    *packet.offset(0 as i32 as isize) = 0x10 as i32 as i8;
    memcpy(
        &mut *packet.offset(1 as i32 as isize) as *mut i8
            as *mut libc::c_void,
    ...
    *packet
        .offset(
            (remain_pos + 1 as i32) as isize,
        ) = 0 as i32 as i8;
    *packet
        .offset(
            (remain_pos + 2 as i32) as isize,
        ) = 0x4 as i32 as i8;
    *packet
        .offset((remain_pos + 3 as i32) as isize) = 'M' as i32 as i8;
    *packet
        .offset((remain_pos + 4 as i32) as isize) = 'Q' as i32 as i8;
    *packet
        .offset((remain_pos + 5 as i32) as isize) = 'T' as i32 as i8;
    *packet
        .offset((remain_pos + 6 as i32) as isize) = 'T' as i32 as i8;
    *packet
        .offset(
            (remain_pos + 7 as i32) as isize,
        ) = 0x4 as i32 as i8;
    *packet
        .offset(
            (remain_pos + 8 as i32) as isize,
        ) = 0x2 as i32 as i8;
    *packet
        .offset(
            (remain_pos + 9 as i32) as isize,
        ) = 0 as i32 as i8;
    *packet
        .offset(
            (remain_pos + 10 as i32) as isize,
        ) = 0x3c as i32 as i8;
    return remain_pos + 10 as i32;
}
unsafe extern "C" fn mqtt_connect(mut data: * mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut current_block: u64;
    let mut result: u32 = CURLE_OK;
    let mut pos: i32 = 0 as i32;
    let mut rc: i32 = 0 as i32;
    let mut remain_pos: i32 = 0 as i32;
    let mut remain: [i8; 4] = [0 as i32 as i8, 0, 0, 0];
    let mut packetlen: u64 = 0 as i32 as size_t;
    let mut payloadlen: u64 = 0 as i32 as size_t;
    let mut start_user: u64 = 0 as i32 as size_t;
    let mut start_pwd: u64 = 0 as i32 as size_t;
    let mut client_id: [i8; 13] = *core::intrinsics::transmute::<&'_ [u8; 13], &'_ mut [i8; 13]>(b"curl\0\0\0\0\0\0\0\0\0");
    let clen: u64 = strlen(b"curl\0" as *const u8 as *const i8);
    let mut packet: * mut i8 = 0 as *mut i8;
    let mut username: * const i8 = if !((*data).state.aptr.user).is_null() {
        (*data).state.aptr.user as *const i8
    } else {
        b"\0" as *const u8 as *const i8
    };
    let ulen: u64 = strlen(username);
    let mut passwd: * const i8 = if !((*data).state.aptr.passwd).is_null() {
        (*data).state.aptr.passwd as *const i8
    } else {
        b"\0" as *const u8 as *const i8
    };
    let plen: u64 = strlen(passwd);
    payloadlen = ulen
        .wrapping_add(plen)
        .wrapping_add(12 as i32 as u64)
        .wrapping_add(2 as i32 as u64);
    if ulen != 0 {
        payloadlen = (payloadlen as u64)
    ...
    remain_pos = mqtt_encode_len(
        remain.as_mut_ptr(),
        payloadlen.wrapping_add(10 as i32 as u64),
    );
    packetlen = payloadlen
        .wrapping_add(10 as i32 as u64)
        .wrapping_add(remain_pos as u64)
        .wrapping_add(1 as i32 as u64);
    if packetlen > 268435455 as i32 as u64 {
    ...
    memset(packet as *mut libc::c_void, 0 as i32, packetlen);
    pos = init_connpack(packet, remain.as_mut_ptr(), remain_pos);
    result = Curl_rand_hex(
        data,
        &mut *client_id.as_mut_ptr().offset(clen as isize) as *mut i8
            as *mut u8,
        (12 as i32 as u64)
            .wrapping_sub(clen)
            .wrapping_add(1 as i32 as u64),
    ...
    rc = add_client_id(
        ...
        packet,
        (pos + 1 as i32) as size_t,
    );
    if rc != 0 {
        Curl_failf(
            data,
            b"Client ID length mismatched: [%lu]\0" as *const u8 as *const i8,
            strlen(client_id.as_mut_ptr()),
        );
        ...
        Curl_infof(
            data,
            b"Using client id '%s'\0" as *const u8 as *const i8,
            client_id.as_mut_ptr(),
        );
        start_user = (pos + 3 as i32 + 12 as i32) as size_t;
        start_pwd = start_user.wrapping_add(ulen);
        if ulen != 0 {
            start_pwd = (start_pwd as u64)
                .wrapping_add(2 as i32 as u64) as size_t as size_t;
            rc = add_user(
                username,
                ulen,
    ...
    let mut fresh5 = &mut ((*data).state.aptr.user);
    *fresh5 = 0 as *mut i8;
    Curl_cfree
        .expect(
            "non-null function pointer",
        )((*data).state.aptr.passwd as *mut libc::c_void);
    let mut fresh6 = &mut ((*data).state.aptr.passwd);
    *fresh6 = 0 as *mut i8;
    return result;
}
unsafe extern "C" fn mqtt_disconnect(mut data: * mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut result: u32 = CURLE_OK;
    result = mqtt_send(
        data,
        b"\xE0\0\0" as *const u8 as *const i8 as *mut i8,
        2 as i32 as size_t,
    );
    return result;
}
unsafe extern "C" fn mqtt_verify_connack(mut data: * mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut conn: * mut crate::src::lib::http2::connectdata = (*data).conn;
    let mut sockfd: i32 = (*conn).sock[0 as i32 as usize];
    ...
    if !(result as u64 != 0) {
        ...
        if nread < 2 as i32 as i64 {
            result = CURLE_WEIRD_SERVER_REPLY;
        } else if readbuf[0 as i32 as usize] as i32 != 0 as i32
                || readbuf[1 as i32 as usize] as i32 != 0 as i32
            {
            Curl_failf(
                data,
                ...
                0 as i32,
                0 as i32,
                readbuf[0 as i32 as usize] as i32,
                readbuf[1 as i32 as usize] as i32,
            );
            result = CURLE_WEIRD_SERVER_REPLY;
        }
    }
...
unsafe extern "C" fn mqtt_get_topic<'a1, 'a2>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut topic: Option<&'a1 mut * mut i8>,
    mut topiclen: Option<&'a2 mut u64>,
) -> u32 {
    let mut path: * mut i8 = (*data).state.up.path;
    if strlen(path) > 1 as i32 as u64 {
        return Curl_urldecode(
            data,
            path.offset(1 as i32 as isize),
            0 as i32 as size_t,
            borrow_mut(&mut topic),
            borrow_mut(&mut topiclen),
            REJECT_NADA,
        );
    }
    Curl_failf(
...
unsafe extern "C" fn mqtt_subscribe(mut data: * mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut topic: * mut i8 = 0 as *mut i8;
    let mut topiclen: u64 = 0;
    let mut packet: * mut u8 = 0 as *mut u8;
    let mut packetlen: u64 = 0;
    let mut encodedsize: [i8; 4] = [0; 4];
    let mut n: u64 = 0;
    let mut conn: * mut crate::src::lib::http2::connectdata = (*data).conn;
    result = mqtt_get_topic(data, Some(&mut topic), Some(&mut topiclen));
    if !(result as u64 != 0) {
        let mut fresh7 = &mut ((*conn).proto.mqtt.packetid);
        *fresh7 = (*fresh7).wrapping_add(1);
        packetlen = topiclen.wrapping_add(5 as i32 as u64);
        n = mqtt_encode_len(encodedsize.as_mut_ptr(), packetlen) as size_t;
        packetlen = (packetlen as u64)
            .wrapping_add(n.wrapping_add(1 as i32 as u64)) as size_t
            as size_t;
        packet = Curl_cmalloc.expect("non-null function pointer")(packetlen)
            as *mut u8;
        if packet.is_null() {
            ...
            *packet
            ...
            memcpy(
                &mut *packet.offset(1 as i32 as isize) as *mut u8
                    as *mut libc::c_void,
            ...
            *packet
                .offset(
                    (1 as i32 as u64).wrapping_add(n) as isize,
                ) = ((*conn).proto.mqtt.packetid >> 8 as i32
                & 0xff as i32 as u32) as u8;
            *packet
                .offset(
                    (2 as i32 as u64).wrapping_add(n) as isize,
                ) = ((*conn).proto.mqtt.packetid & 0xff as i32 as u32)
                as u8;
            *packet
                .offset(
                    (3 as i32 as u64).wrapping_add(n) as isize,
                ) = (topiclen >> 8 as i32 & 0xff as i32 as u64)
                as u8;
            *packet
                .offset(
                    (4 as i32 as u64).wrapping_add(n) as isize,
                ) = (topiclen & 0xff as i32 as u64) as u8;
            memcpy(
                &mut *packet
                    .offset((5 as i32 as u64).wrapping_add(n) as isize)
                    as *mut u8 as *mut libc::c_void,
            ...
            *packet
            ...
            result = mqtt_send(data, packet as *mut i8, packetlen);
        }
    }
...
unsafe extern "C" fn mqtt_verify_suback(mut data: * mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut conn: * mut crate::src::lib::http2::connectdata = (*data).conn;
    let mut sockfd: i32 = (*conn).sock[0 as i32 as usize];
    let mut readbuf: [u8; 3] = [0; 3];
    let mut nread: i64 = 0;
    let mut mqtt: Option<&'_ mut crate::src::lib::http2::mqtt_conn> = Some(&mut (*conn).proto.mqtt);
    result = Curl_read(
        data,
        sockfd,
        readbuf.as_mut_ptr() as *mut i8,
    ...
    if !(result as u64 != 0) {
        ...
        if nread < 3 as i32 as i64 {
            result = CURLE_WEIRD_SERVER_REPLY;
        } else if readbuf[0 as i32 as usize] as u32
                != (*(borrow(& mqtt)).unwrap()).packetid >> 8 as i32
                    & 0xff as i32 as u32
                || readbuf[1 as i32 as usize] as u32
                    != (*(borrow(& mqtt)).unwrap()).packetid & 0xff as i32 as u32
                || readbuf[2 as i32 as usize] as i32 != 0 as i32
            {
            result = CURLE_WEIRD_SERVER_REPLY;
        }
    }
...
unsafe extern "C" fn mqtt_publish(mut data: * mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut payload: * mut i8 = (*data).set.postfields as *mut i8;
    let mut payloadlen: u64 = 0;
    let mut topic: * mut i8 = 0 as *mut i8;
    let mut topiclen: u64 = 0;
    let mut pkt: * mut u8 = 0 as *mut u8;
    let mut i: u64 = 0 as i32 as size_t;
    let mut remaininglength: u64 = 0;
    let mut encodelen: u64 = 0;
    let mut encodedbytes: [i8; 4] = [0; 4];
    let mut postfieldsize: i64 = (*data).set.postfieldsize;
    if payload.is_null() {
        return CURLE_BAD_FUNCTION_ARGUMENT;
    }
    if postfieldsize < 0 as i32 as i64 {
    ...
    if !(result as u64 != 0) {
        ...
        if pkt.is_null() {
            ...
            let mut fresh8 = i;
            i = i.wrapping_add(1);
            *pkt.offset(fresh8 as isize) = 0x30 as i32 as u8;
            memcpy(
                &mut *pkt.offset(i as isize) as *mut u8 as *mut libc::c_void,
                encodedbytes.as_mut_ptr() as *const libc::c_void,
                encodelen,
            );
            i = (i as u64).wrapping_add(encodelen) as size_t as size_t;
            let mut fresh9 = i;
            i = i.wrapping_add(1);
            *pkt
                .offset(
            ...
            *pkt
            ...
            i = (i as u64).wrapping_add(topiclen) as size_t as size_t;
            memcpy(
                &mut *pkt.offset(i as isize) as *mut u8 as *mut libc::c_void,
                payload as *const libc::c_void,
                payloadlen,
            );
            i = (i as u64).wrapping_add(payloadlen) as size_t as size_t;
            result = mqtt_send(data, pkt as *mut i8, i);
        }
    }
...
unsafe extern "C" fn mqtt_decode_len<'a1>(
    mut buf: * mut u8,
    mut buflen: u64,
    mut lenbytes: Option<&'a1 mut u64>,
) -> u64 {
    let mut len: u64 = 0 as i32 as size_t;
    let mut mult: u64 = 1 as i32 as size_t;
    let mut i: u64 = 0;
    let mut encoded: u8 = 128 as i32 as u8;
    i = 0 as i32 as size_t;
    while i < buflen && encoded as i32 & 128 as i32 != 0 {
        encoded = *buf.offset(i as isize);
        len = (len as u64)
            .wrapping_add(
                ((encoded as i32 & 127 as i32) as u64)
                    .wrapping_mul(mult),
            ) as size_t as size_t;
        mult = (mult as u64).wrapping_mul(128 as i32 as u64)
            as size_t as size_t;
        i = i.wrapping_add(1);
    }
    if !borrow(& lenbytes).is_none() {
        *(borrow_mut(&mut lenbytes)).unwrap() = i;
    }
    return len;
}
unsafe extern "C" fn mqstate(
    ...
    let mut conn: * mut crate::src::lib::http2::connectdata = (*data).conn;
    let mut mqtt: Option<&'_ mut crate::src::lib::http2::mqtt_conn> = Some(&mut (*conn).proto.mqtt);
    (*(borrow_mut(&mut mqtt)).unwrap()).state = state;
    if state as u32 == MQTT_FIRST as i32 as u32 {
        (*(borrow_mut(&mut mqtt)).unwrap()).nextstate = nextstate;
    }
}
unsafe extern "C" fn mqtt_read_publish(
    ...
    let mut sockfd: i32 = (*conn).sock[0 as i32 as usize];
    let mut nread: i64 = 0;
    let mut pkt: * mut u8 = (*data).state.buffer as *mut u8;
    let mut remlen: u64 = 0;
    let mut mqtt: Option<&'_ mut crate::src::lib::http2::mqtt_conn> = Some(&mut (*conn).proto.mqtt);
    let mut mq: * mut crate::src::lib::http2::MQTT = (*data).req.p.mqtt;
    let mut packet: u8 = 0;
    match (*(borrow(& mqtt)).unwrap()).state as u32 {
        4 => {
            current_block = 16063587588522528629;
        }
        3 | 5 => {
            packet = ((*mq).firstbyte as i32 & 0xf0 as i32)
                as u8;
            if packet as i32 == 0x30 as i32 {
                mqstate(data, MQTT_PUB_REMAIN, MQTT_NOSTATE);
                remlen = (*mq).remaining_length;
                Curl_infof(
                    data,
                    b"Remaining length: %zd bytes\0" as *const u8 as *const i8,
                    remlen,
                );
                    ...
                    && remlen as curl_off_t > (*data).set.max_filesize
                {
                    ...
                    (*data).req.bytecount = 0 as i32 as curl_off_t;
                    (*data).req.size = remlen as curl_off_t;
                    (*mq).npacket = remlen;
                    current_block = 4488286894823169796;
                }
            } else if packet as i32 == 0x90 as i32 {
                mqstate(data, MQTT_SUBACK_COMING, MQTT_NOSTATE);
                current_block = 16063587588522528629;
            } else {
                if packet as i32 == 0xe0 as i32 {
                    Curl_infof(
                        data,
                        b"Got DISCONNECT\0" as *const u8 as *const i8,
                    );
                    *done = 1 as i32 != 0;
                } else {
                    result = CURLE_WEIRD_SERVER_REPLY;
    ...
    match current_block {
        4488286894823169796 => {
            ...
            let mut rest: u64 = (*mq).npacket;
            if rest > (*data).set.buffer_size as size_t {
                rest = (*data).set.buffer_size as size_t;
            }
            result = Curl_read(data, sockfd, pkt as *mut i8, rest, Some(&mut nread));
            if result as u64 != 0 {
                if CURLE_AGAIN as i32 as u32 == result as u32 {
                    Curl_infof(
                        data,
                        b"EEEE AAAAGAIN\0" as *const u8 as *const i8,
                    );
                }
            } else if nread == 0 {
                Curl_infof(
                    data,
                    b"server disconnected\0" as *const u8 as *const i8,
                );
                ...
                let mut fresh11 = &mut ((*mq).npacket);
                *fresh11 = (*fresh11 as u64)
                    .wrapping_sub(nread as u64) as size_t as size_t;
                let mut fresh12 = &mut ((*(borrow_mut(&mut k)).unwrap()).bytecount);
                *fresh12 += nread;
...
unsafe extern "C" fn mqtt_do(mut data: * mut crate::src::lib::http2::Curl_easy, mut done: * mut bool) -> u32 {
    let mut result: u32 = CURLE_OK;
    *done = 0 as i32 != 0;
    ...
    if result as u64 != 0 {
        Curl_failf(
            data,
            b"Error %d sending MQTT CONN request\0" as *const u8 as *const i8,
            result as u32,
        );
        return result;
    }
...
unsafe extern "C" fn mqtt_doing(
    ...
    *done = 0 as i32 != 0;
    if (*mq).nsend != 0 {
        let mut ptr: * mut i8 = (*mq).sendleftovers;
    ...
    Curl_infof(
        data,
        b"mqtt_doing: state [%d]\0" as *const u8 as *const i8,
        (*(borrow(& mqtt)).unwrap()).state as i32,
    );
    ...
    match (*(borrow(& mqtt)).unwrap()).state as u32 {
        0 => {
            result = Curl_read(
                data,
                sockfd,
                &mut (*mq).firstbyte as *mut u8 as *mut i8,
                1 as i32 as size_t,
                Some(&mut nread),
            );
            if nread == 0 {
        ...
        2 => {
            ...
            if result as u64 != 0 {
                    ...
                    == HTTPREQ_POST as i32 as u32
                {
                    ...
                    (*(borrow_mut(&mut mqtt)).unwrap()).nextstate = MQTT_FIRST;
                } else {
        ...
        _ => {
            Curl_failf(
                data,
                b"State not handled yet\0" as *const u8 as *const i8,
            );
            *done = 1 as i32 != 0;
            current_block_42 = 2500484646272006982;
        }
    ...
    match current_block_42 {
        9618971279722602572 => {
            loop {
                ...
                let mut fresh13 = &mut ((*mq).npacket);
                let mut fresh14 = *fresh13;
                *fresh13 = (*fresh13).wrapping_add(1);
                *pkt.offset(fresh14 as isize) = byte;
                if !(byte as i32 & 0x80 as i32 != 0
                    && (*mq).npacket < 4 as i32 as u64)
                {
                    break;
                }
            }
            if nread != 0 && byte as i32 & 0x80 as i32 != 0 {
                result = CURLE_WEIRD_SERVER_REPLY;
            }
            if !(result as u64 != 0) {
                (*mq)
                    .remaining_length = mqtt_decode_len(
                    &mut *pkt.offset(0 as i32 as isize),
                    (*mq).npacket,
                    Option::<&'_ mut u64>::None,
                );
                (*mq).npacket = 0 as i32 as size_t;
                if (*mq).remaining_length != 0 {
                    mqstate(data, (*(borrow_mut(&mut mqtt)).unwrap()).nextstate, MQTT_NOSTATE);
                } else {
                    mqstate(data, MQTT_FIRST, MQTT_FIRST);
                    if (*mq).firstbyte as i32 == 0xe0 as i32 {
                        Curl_infof(
                            data,
                            b"Got DISCONNECT\0" as *const u8 as *const i8,
                        );
