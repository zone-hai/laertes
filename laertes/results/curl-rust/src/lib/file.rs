...
pub struct stat {
    ...
    pub st_blksize: i64,
    pub st_blocks: i64,
    pub st_atim: crate::src::lib::file::timespec,
    pub st_mtim: crate::src::lib::file::timespec,
    pub st_ctim: crate::src::lib::file::timespec,
...
impl stat {
    pub const fn new() -> Self {
        ...
        st_blksize: 0,
        st_blocks: 0,
        st_atim: crate::src::lib::file::timespec::new(),
        st_mtim: crate::src::lib::file::timespec::new(),
        st_ctim: crate::src::lib::file::timespec::new(),
...
#[no_mangle]
pub static mut Curl_handler_file: crate::src::lib::http2::Curl_handler = unsafe {
    {
        let mut init = Curl_handler {
            scheme: b"FILE\0" as *const u8 as *const i8,
            ...
            attach: None,
            defport: 0 as i32,
            protocol: ((1 as i32) << 10 as i32) as u32,
            family: ((1 as i32) << 10 as i32) as u32,
...
unsafe extern "C" fn file_setup_connection(
    ...
    let mut fresh0 = &mut ((*data).req.p.file);
    *fresh0 = Curl_ccalloc
        .expect(
            "non-null function pointer",
        )(
        1 as i32 as size_t,
        ::std::mem::size_of::<FILEPROTO>() as u64,
    ) as *mut FILEPROTO;
...
unsafe extern "C" fn file_connect(
    ...
    let mut real_path: * mut i8 = 0 as *mut i8;
    let mut file: * mut crate::src::lib::http2::FILEPROTO = (*data).req.p.file;
    let mut fd: i32 = 0;
    let mut real_path_len: u64 = 0;
    let mut result: u32 = Curl_urldecode(
        data,
        (*data).state.up.path,
        0 as i32 as size_t,
        Some(&mut real_path),
        Some(&mut real_path_len),
        REJECT_ZERO,
    );
    if result as u64 != 0 {
        return result;
    }
    if !(memchr(real_path as *const libc::c_void, 0 as i32, real_path_len))
        .is_null()
    {
        Curl_cfree.expect("non-null function pointer")(real_path as *mut libc::c_void);
        real_path = 0 as *mut i8;
        return CURLE_URL_MALFORMAT;
    }
    fd = open(real_path, 0 as i32);
    let mut fresh1 = &mut ((*file).path);
    *fresh1 = real_path;
    let mut fresh2 = &mut ((*file).freepath);
    *fresh2 = real_path;
    (*file).fd = fd;
    if ((*data).set).upload() == 0 && fd == -(1 as i32) {
        Curl_failf(
            data,
            b"Couldn't open file %s\0" as *const u8 as *const i8,
            (*data).state.up.path,
        );
        file_done(data, CURLE_FILE_COULDNT_READ_FILE, 0 as i32 != 0);
...
unsafe extern "C" fn file_done(
    ...
    if !file.is_null() {
        ...
        let mut fresh3 = &mut ((*file).freepath);
        *fresh3 = 0 as *mut i8;
        let mut fresh4 = &mut ((*file).path);
        *fresh4 = 0 as *mut i8;
        if (*file).fd != -(1 as i32) {
            close((*file).fd);
        }
...
unsafe extern "C" fn file_disconnect(
    ...
    return file_done(data, CURLE_OK, 0 as i32 != 0);
}
unsafe extern "C" fn file_upload(mut data: * mut crate::src::lib::http2::Curl_easy) -> u32 {
    let mut file: * mut crate::src::lib::http2::FILEPROTO = (*data).req.p.file;
    let mut dir: * const i8 = strchr((*file).path, '/' as i32);
    let mut fd: i32 = 0;
    let mut mode: i32 = 0;
    let mut result: u32 = CURLE_OK;
    let mut buf: * mut i8 = (*data).state.buffer;
    let mut bytecount: i64 = 0 as i32 as curl_off_t;
    let mut file_stat: crate::src::lib::file::stat = stat {
        st_dev: 0,
        st_ino: 0,
        st_nlink: 0,
        st_mode: 0,
    ...
    let mut buf2: * const i8 = 0 as *const i8;
    let mut fresh5 = &mut ((*data).req.upload_fromhere);
    *fresh5 = buf;
    if dir.is_null() {
        return CURLE_FILE_COULDNT_READ_FILE;
    }
    if *dir.offset(1 as i32 as isize) == 0 {
        return CURLE_FILE_COULDNT_READ_FILE;
    }
    if (*data).state.resume_from != 0 {
        mode = 0o1 as i32 | 0o100 as i32 | 0o2000 as i32;
    } else {
        mode = 0o1 as i32 | 0o100 as i32 | 0o1000 as i32;
    }
    fd = open((*file).path, mode, (*data).set.new_file_perms);
    if fd < 0 as i32 {
        Curl_failf(
            data,
            b"Can't open %s for writing\0" as *const u8 as *const i8,
            (*file).path,
        );
    ...
    if (*data).state.resume_from < 0 as i32 as i64 {
        if fstat(fd, &mut file_stat) != 0 {
            ...
            Curl_failf(
                data,
                b"Can't get the size of %s\0" as *const u8 as *const i8,
                (*file).path,
            );
            return CURLE_WRITE_ERROR;
        }
    ...
    while result as u64 == 0 {
        ...
        if (*data).state.resume_from != 0 {
            if nread as curl_off_t <= (*data).state.resume_from {
                let mut fresh6 = &mut ((*data).state.resume_from);
                *fresh6 = (*fresh6 as u64).wrapping_sub(nread) as curl_off_t
                    as curl_off_t;
                nread = 0 as i32 as size_t;
                buf2 = buf;
            } else {
        ...
        if nwrite != nread {
            ...
            bytecount = (bytecount as u64).wrapping_add(nread) as curl_off_t
                as curl_off_t;
...
unsafe extern "C" fn file_do(mut data: * mut crate::src::lib::http2::Curl_easy, mut done: * mut bool) -> u32 {
    let mut result: u32 = CURLE_OK;
    ...
    let mut expected_size: i64 = -(1 as i32) as curl_off_t;
    let mut size_known: bool = false;
    let mut fstated: bool = 0 as i32 != 0;
    let mut buf: * mut i8 = (*data).state.buffer;
    let mut bytecount: i64 = 0 as i32 as curl_off_t;
    let mut fd: i32 = 0;
    let mut file: * mut crate::src::lib::http2::FILEPROTO = (0 as * mut crate::src::lib::http2::FILEPROTO);
    *done = 1 as i32 != 0;
    ...
    fd = (*file).fd;
    if -(1 as i32) != fstat(fd, &mut statbuf) {
    ...
    if fstated as i32 != 0 && ((*data).state.range).is_null()
    ...
    if fstated {
        ...
        let mut header: [i8; 80] = [0; 80];
        let mut headerlen: i32 = 0;
        let mut accept_ranges: [i8; 24] = *core::intrinsics::transmute::<&'_ [u8; 24], &'_ mut [i8; 24]>(b"Accept-ranges: bytes\r\n\0\0");
        if expected_size >= 0 as i32 as i64 {
    ...
    if (*data).state.resume_from < 0 as i32 as i64 {
        if !fstated {
            Curl_failf(
                data,
                b"Can't get the size of file.\0" as *const u8 as *const i8,
            );
            return CURLE_READ_ERROR;
        }
        let mut fresh7 = &mut ((*data).state.resume_from);
    ...
    if (*data).req.maxdownload > 0 as i32 as i64 {
        expected_size = (*data).req.maxdownload;
    }
    if !fstated || expected_size <= 0 as i32 as i64 {
        size_known = 0 as i32 != 0;
    } else {
        size_known = 1 as i32 != 0;
    }
    if size_known {
        Curl_pgrsSetDownloadSize(data, expected_size);
    }
    if (*data).state.resume_from != 0 {
        if (*data).state.resume_from
            != lseek(fd, (*data).state.resume_from, 0 as i32)
        {
            return CURLE_BAD_DOWNLOAD_RESUME;
        }
    }
    ...
    while result as u64 == 0 {
        ...
        if size_known {
            ...
            bytestoread = ((*data).set.buffer_size - 1 as i32 as i64)
                as size_t;
        }
