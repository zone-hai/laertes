...
#[no_mangle]
pub unsafe extern "C" fn Curl_speedinit(mut data: * mut crate::src::lib::http2::Curl_easy) {
    memset(
...
pub unsafe extern "C" fn Curl_speedcheck(
    ...
    if (*data).progress.current_speed >= 0 as i32 as i64
        && (*data).set.low_speed_time != 0
    {
        if (*data).progress.current_speed < (*data).set.low_speed_limit {
            if (*data).state.keeps_speed.tv_sec == 0 {
                ...
                let mut howlong: i64 = Curl_timediff(
                    now,
                    (*data).state.keeps_speed,
                );
                if howlong
                    >= (*data).set.low_speed_time * 1000 as i32 as i64
                {
                    Curl_failf(
                        data,
    ...
    if (*data).set.low_speed_limit != 0 {
        Curl_expire(data, 1000 as i32 as timediff_t, EXPIRE_SPEEDCHECK);
    }
    return CURLE_OK;
}
