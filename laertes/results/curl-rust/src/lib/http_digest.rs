...
pub use crate::src::lib::vauth::digest::Curl_auth_create_digest_http_message;
pub use crate::src::lib::vauth::digest::Curl_auth_decode_digest_http_message;
pub use crate::src::lib::vauth::digest::Curl_auth_digest_cleanup;
...
pub unsafe extern "C" fn Curl_input_digest(
    ...
    let mut digest: Option<&'_ mut crate::src::lib::http2::digestdata> = Option::<&'_ mut crate::src::lib::http2::digestdata>::None;
    if proxy {
        digest = Some(&mut (*data).state.proxydigest);
    } else {
        digest = Some(&mut (*data).state.digest);
    }
    if curl_strnequal(
        ...
        || Curl_isspace(
            *header.offset(6 as i32 as isize) as u8 as i32,
        ) == 0
    {
        return CURLE_BAD_CONTENT_ENCODING;
    }
    header = header
        .offset(strlen(b"Digest\0" as *const u8 as *const i8) as isize);
    while *header as i32 != 0
        && Curl_isspace(*header as u8 as i32) != 0
    {
        header = header.offset(1);
    }
    return Curl_auth_decode_digest_http_message(header, borrow_mut(&mut digest));
}
#[no_mangle]
pub unsafe extern "C" fn Curl_output_digest(
    ...
    Curl_cfree.expect("non-null function pointer")(*(borrow_mut(&mut allocuserpwd)).unwrap() as *mut libc::c_void);
    *(borrow_mut(&mut allocuserpwd)).unwrap() = 0 as *mut i8;
    ...
    have_chlg = if !((*(borrow_mut(&mut digest)).unwrap()).nonce).is_null() {
        1 as i32
    } else {
        0 as i32
    } != 0;
    ...
    if (*(borrow(& authp)).unwrap()).iestyle() != 0 {
        tmp = strchr(uripath as *mut i8, '?' as i32);
        if !tmp.is_null() {
            let mut urilen: u64 = tmp.offset_from(uripath as *mut i8)
                as i64 as size_t;
            path = curl_maprintf(
    ...
    if tmp.is_null() {
        path = Curl_cstrdup
            .expect("non-null function pointer")(uripath as *mut i8)
            as *mut u8;
    }
    if path.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn Curl_http_auth_cleanup_digest(mut data: * mut crate::src::lib::http2::Curl_easy) {
    Curl_auth_digest_cleanup(Some(&mut (*data).state.digest));
    Curl_auth_digest_cleanup(Some(&mut (*data).state.proxydigest));
}
