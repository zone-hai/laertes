...
pub use crate::src::lib::curl_ntlm_wb::Curl_http_auth_cleanup_ntlm_wb;
...
pub unsafe extern "C" fn Curl_input_ntlm(
    ...
    let mut ntlm: * mut crate::src::lib::http2::ntlmdata = 0 as *mut ntlmdata;
    let mut state: * mut u32 = 0 as *mut curlntlm;
    ...
    ) != 0
    {
        ...
        if *header != 0 {
            ...
            let mut hdrlen: u64 = 0;
            result = Curl_base64_decode(header, Some(&mut hdr), Some(&mut hdrlen));
            if result as u64 == 0 {
                let mut hdrbuf: crate::src::lib::bufref::bufref = bufref {
                    dtor: None,
                    ptr: 0 as *const u8,
                    len: 0,
                };
                Curl_bufref_init(Some(&mut hdrbuf));
                Curl_bufref_set(
                    Some(&mut hdrbuf),
                ...
                result = Curl_auth_decode_ntlm_type2_message(data, Some(&mut hdrbuf), ntlm);
                Curl_bufref_free(Some(&mut hdrbuf));
            }
            if result as u64 != 0 {
                return result;
            }
...
pub unsafe extern "C" fn Curl_output_ntlm(
    ...
    let mut result: u32 = CURLE_OK;
    let mut ntlmmsg: crate::src::lib::bufref::bufref = bufref {
        dtor: None,
        ptr: 0 as *const u8,
        len: 0,
    };
    ...
    let mut passwdp: * const i8 = 0 as *const i8;
    let mut service: * const i8 = 0 as *const i8;
    ...
    if passwdp.is_null() {
        passwdp = b"\0" as *const u8 as *const i8;
    }
    Curl_bufref_init(Some(&mut ntlmmsg));
    ...
    match *(borrow(& state)).unwrap() as u32 {
        2 => {
            ...
            if result as u64 == 0 && Curl_bufref_len(Some(&mut ntlmmsg)) != 0 {
                result = Curl_base64_encode(
                    data,
                    Curl_bufref_ptr(Some(&mut ntlmmsg)) as *const i8,
                    Curl_bufref_len(Some(&mut ntlmmsg)),
                    Some(&mut base64),
                    Some(&mut len),
                );
                if result as u64 == 0 {
                    Curl_cfree
                        ...
                        )(*(borrow_mut(&mut allocuserpwd)).unwrap() as *mut libc::c_void);
                    *(borrow_mut(&mut allocuserpwd)).unwrap() = curl_maprintf(
                    ...
                    if (*(borrow(& allocuserpwd)).unwrap()).is_null() {
        ...
        3 => {
            *(borrow_mut(&mut state)).unwrap() = NTLMSTATE_LAST;
            current_block_61 = 4609189292336033333;
        }
        4 => {
            current_block_61 = 4609189292336033333;
        }
    ...
    Curl_bufref_free(Some(&mut ntlmmsg));
    return result;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_http_auth_cleanup_ntlm(mut conn: * mut crate::src::lib::http2::connectdata) {
    Curl_auth_cleanup_ntlm(Some(&mut (*conn).ntlm));
    Curl_auth_cleanup_ntlm(Some(&mut (*conn).proxyntlm));
