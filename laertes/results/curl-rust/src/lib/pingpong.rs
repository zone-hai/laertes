...
pub use crate::src::lib::dynbuf::Curl_dyn_vaddf;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_state_timeout(
    ...
    let mut response_time: i64 = if (*data).set.server_response_timeout != 0 {
        (*data).set.server_response_timeout
    } else {
        (*pp).response_time
    };
    timeout_ms = response_time - Curl_timediff(Curl_now(), (*pp).response);
    if (*data).set.timeout != 0 && !disconnecting {
        let mut timeout2_ms: i64 = (*data).set.timeout
            - Curl_timediff(Curl_now(), (*conn).now);
        timeout_ms = if timeout_ms < timeout2_ms { timeout_ms } else { timeout2_ms };
    }
    return timeout_ms;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_statemach(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut pp: * mut crate::src::lib::http2::pingpong,
    mut block: bool,
    mut disconnecting: bool,
    ...
    let mut timeout_ms: i64 = Curl_pp_state_timeout(data, pp, disconnecting);
    let mut result: u32 = CURLE_OK;
    if timeout_ms <= 0 as i32 as i64 {
        Curl_failf(
            data,
            b"server response timeout\0" as *const u8 as *const i8,
        );
    ...
    if block {
        ...
        interval_ms = 0 as i32 as timediff_t;
    }
    if Curl_ssl_data_pending(conn, 0 as i32) {
        rc = 1 as i32;
    } else if Curl_pp_moredata(pp) {
        rc = 1 as i32;
    } else if (*pp).sendleft == 0
            && Curl_ssl_data_pending(conn, 0 as i32) as i32 != 0
        {
        rc = 1 as i32;
    } else {
        rc = Curl_socket_check(
            if (*pp).sendleft != 0 { -(1 as i32) } else { sock },
            -(1 as i32),
            if (*pp).sendleft != 0 { sock } else { -(1 as i32) },
            interval_ms,
        );
    }
    if block {
    ...
    if rc == -(1 as i32) {
        Curl_failf(data, b"select/poll error\0" as *const u8 as *const i8);
        result = CURLE_OUT_OF_MEMORY;
    } else if rc != 0 {
        result = ((*pp).statemachine)
            .expect("non-null function pointer")(data, (*data).conn);
    }
    return result;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_init<'a1>(mut data: * mut crate::src::lib::http2::Curl_easy, mut pp: Option<&'a1 mut crate::src::lib::http2::pingpong>) {
    (*(borrow_mut(&mut pp)).unwrap()).nread_resp = 0 as i32 as size_t;
    let mut fresh0 = &mut ((*(borrow_mut(&mut pp)).unwrap()).linestart_resp);
    *fresh0 = (*data).state.buffer;
    (*(borrow_mut(&mut pp)).unwrap()).pending_resp = 1 as i32 != 0;
    (*(borrow_mut(&mut pp)).unwrap()).response = Curl_now();
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_setup<'a1>(mut pp: Option<&'a1 mut crate::src::lib::http2::pingpong>) {
    Curl_dyn_init(
        &mut (*(borrow_mut(&mut pp)).unwrap()).sendbuf,
        (64 as i32 * 1024 as i32) as size_t,
    );
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_vsendf<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut pp: Option<&'a1 mut crate::src::lib::http2::pingpong>,
    mut fmt: * const i8,
    mut args: core::ffi::VaList,
) -> u32 {
    let mut bytes_written: i64 = 0 as i32 as ssize_t;
    ...
    Curl_dyn_reset(Some(&mut (*(borrow_mut(&mut pp)).unwrap()).sendbuf));
    result = Curl_dyn_vaddf(&mut (*(borrow_mut(&mut pp)).unwrap()).sendbuf, fmt, args.as_va_list());
    if result as u64 != 0 {
        return result;
    }
    ...
    write_len = Curl_dyn_len(&mut (*(borrow_mut(&mut pp)).unwrap()).sendbuf);
    s = Curl_dyn_ptr(&mut (*(borrow_mut(&mut pp)).unwrap()).sendbuf);
    ...
    Curl_debug(data, CURLINFO_HEADER_OUT, s, bytes_written as size_t);
    if bytes_written != write_len as ssize_t {
        let mut fresh1 = &mut ((*(borrow_mut(&mut pp)).unwrap()).sendthis);
        *fresh1 = s;
        (*(borrow_mut(&mut pp)).unwrap()).sendsize = write_len;
        (*(borrow_mut(&mut pp)).unwrap()).sendleft = write_len.wrapping_sub(bytes_written as u64);
    } else {
        let mut fresh2 = &mut ((*(borrow_mut(&mut pp)).unwrap()).sendthis);
        *fresh2 = 0 as *mut i8;
        let mut fresh3 = &mut ((*(borrow_mut(&mut pp)).unwrap()).sendsize);
        *fresh3 = 0 as i32 as size_t;
        (*(borrow_mut(&mut pp)).unwrap()).sendleft = *fresh3;
        (*(borrow_mut(&mut pp)).unwrap()).response = Curl_now();
    }
    return CURLE_OK;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_sendf<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut pp: Option<&'a1 mut crate::src::lib::http2::pingpong>,
    mut fmt: * const i8,
    mut args: ...
    ...
    ap = args.clone();
    result = Curl_pp_vsendf(data, borrow_mut(&mut pp), fmt, ap.as_va_list());
    return result;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_readresp<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut sockfd: i32,
    mut pp: * mut crate::src::lib::http2::pingpong,
    mut code: * mut i32,
    ...
    let mut perline: i64 = 0;
    let mut keepon: bool = 1 as i32 != 0;
    let mut gotbytes: i64 = 0;
    let mut ptr: * mut i8 = 0 as *mut i8;
    let mut conn: * mut crate::src::lib::http2::connectdata = (*data).conn;
    let buf: * mut i8 = (*data).state.buffer;
    let mut result: u32 = CURLE_OK;
    *code = 0 as i32;
    *(borrow_mut(&mut size)).unwrap() = 0 as i32 as size_t;
    ptr = buf.offset((*pp).nread_resp as isize);
    perline = ptr.offset_from((*pp).linestart_resp) as i64;
    while (*pp).nread_resp < (*data).set.buffer_size as size_t
        && (keepon as i32 != 0 && result as u64 == 0)
    {
        if !((*pp).cache).is_null() {
            if ptr.offset((*pp).cache_size as isize)
                > buf
                    .offset((*data).set.buffer_size as isize)
                    .offset(1 as i32 as isize)
            {
                Curl_failf(
                    data,
                    b"cached response data too big to handle\0" as *const u8
                        as *const i8,
                );
            ...
            memcpy(
                ...
                (*pp).cache as *const libc::c_void,
                (*pp).cache_size,
            );
            gotbytes = (*pp).cache_size as ssize_t;
            Curl_cfree
                .expect("non-null function pointer")((*pp).cache as *mut libc::c_void);
            let mut fresh4 = &mut ((*pp).cache);
            *fresh4 = 0 as *mut i8;
            ...
            result = Curl_read(
                ...
                sockfd,
                ptr,
                ((*data).set.buffer_size as u64)
                    .wrapping_sub((*pp).nread_resp),
                Some(&mut gotbytes),
            );
            if result as u32 == CURLE_AGAIN as i32 as u32 {
                return CURLE_OK;
            }
            if result as u64 == 0 && gotbytes > 0 as i32 as i64 {
                result = CURLE_OK as i32 as CURLcode;
            }
            if result as u64 != 0 {
                keepon = 0 as i32 != 0;
        ...
        if keepon {
            if gotbytes <= 0 as i32 as i64 {
                keepon = 0 as i32 != 0;
                result = CURLE_RECV_ERROR;
                Curl_failf(
                    data,
                    b"response reading failed\0" as *const u8 as *const i8,
                );
            } else {
                let mut i: i64 = 0;
                let mut clipamount: i64 = 0 as i32 as ssize_t;
                let mut restart: bool = 0 as i32 != 0;
                let mut fresh5 = &mut ((*data).req.headerbytecount);
                *fresh5 += gotbytes;
                let mut fresh6 = &mut ((*pp).nread_resp);
                *fresh6 = (*fresh6 as u64)
                    .wrapping_add(gotbytes as u64) as size_t as size_t;
                ...
                while i < gotbytes {
                    ...
                    if *ptr as i32 == '\n' as i32 {
                        ...
                        if ((*pp).endofresp)
                            ...
                            )(data, conn, (*pp).linestart_resp, perline as size_t, code)
                        {
                            let mut n: u64 = ptr.offset_from((*pp).linestart_resp)
                                as i64 as size_t;
                            memmove(
                                buf as *mut libc::c_void,
                                (*pp).linestart_resp as *const libc::c_void,
                                n,
                            );
                            *buf.offset(n as isize) = 0 as i32 as i8;
                            keepon = 0 as i32 != 0;
                            let mut fresh7 = &mut ((*pp).linestart_resp);
                            *fresh7 = ptr.offset(1 as i32 as isize);
                            i += 1;
                            *(borrow_mut(&mut size)).unwrap() = (*pp).nread_resp;
                            (*pp).nread_resp = 0 as i32 as size_t;
                            break;
                        } else {
                            perline = 0 as i32 as ssize_t;
                            let mut fresh8 = &mut ((*pp).linestart_resp);
                            *fresh8 = ptr.offset(1 as i32 as isize);
                        }
                    }
                    ptr = ptr.offset(1);
                    i += 1;
                ...
                if !keepon && i != gotbytes {
                    ...
                    if perline == gotbytes
                        && gotbytes
                            > (*data).set.buffer_size / 2 as i32 as i64
                    {
                        Curl_infof(
                            data,
                            b"Excessive server response line length received, %zd bytes. Stripping\0"
                                as *const u8 as *const i8,
                            gotbytes,
                        );
                        restart = 1 as i32 != 0;
                        clipamount = 40 as i32 as ssize_t;
                    } else if (*pp).nread_resp
                ...
                if clipamount != 0 {
                    (*pp).cache_size = clipamount as size_t;
                    let mut fresh9 = &mut ((*pp).cache);
                    *fresh9 = Curl_cmalloc
                        .expect("non-null function pointer")((*pp).cache_size)
                        as *mut i8;
                    if !((*pp).cache).is_null() {
                        memcpy(
                ...
                if restart {
                    (*pp).nread_resp = 0 as i32 as size_t;
                    let mut fresh10 = &mut ((*pp).linestart_resp);
                    *fresh10 = buf;
                    ptr = *fresh10;
                    perline = 0 as i32 as ssize_t;
                }
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_getsock<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut pp: Option<&'a1 mut crate::src::lib::http2::pingpong>,
    mut socks: * mut i32,
) -> i32 {
    ...
    *socks.offset(0 as i32 as isize) = (*conn).sock[0 as i32 as usize];
    if (*(borrow(& pp)).unwrap()).sendleft != 0 {
        return (1 as i32) << 16 as i32 + 0 as i32;
    }
    return (1 as i32) << 0 as i32;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_flushsend(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut pp: * mut crate::src::lib::http2::pingpong,
) -> u32 {
    let mut conn: * mut crate::src::lib::http2::connectdata = (*data).conn;
    ...
    let mut sock: i32 = (*conn).sock[0 as i32 as usize];
    let mut result: u32 = Curl_write(
        data,
        sock,
        ((*pp).sendthis)
            .offset((*pp).sendsize as isize)
            .offset(-((*pp).sendleft as isize)) as *const libc::c_void,
        (*pp).sendleft,
        Some(&mut written),
    );
    if result as u64 != 0 {
    ...
    if written != (*pp).sendleft as ssize_t {
        let mut fresh11 = &mut ((*pp).sendleft);
        *fresh11 = (*fresh11 as u64).wrapping_sub(written as u64)
            as size_t as size_t;
    } else {
        let mut fresh12 = &mut ((*pp).sendthis);
        *fresh12 = 0 as *mut i8;
        let mut fresh13 = &mut ((*pp).sendsize);
        *fresh13 = 0 as i32 as size_t;
        (*pp).sendleft = *fresh13;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_disconnect<'a1>(mut pp: Option<&'a1 mut crate::src::lib::http2::pingpong>) -> u32 {
    Curl_dyn_free(&mut (*(borrow_mut(&mut pp)).unwrap()).sendbuf);
    Curl_cfree.expect("non-null function pointer")((*(borrow_mut(&mut pp)).unwrap()).cache as *mut libc::c_void);
    let mut fresh14 = &mut ((*(borrow_mut(&mut pp)).unwrap()).cache);
    *fresh14 = 0 as *mut i8;
    return CURLE_OK;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_moredata(mut pp: * mut crate::src::lib::http2::pingpong) -> bool {
    return if (*pp).sendleft == 0 && !((*pp).cache).is_null()
        && (*pp).nread_resp < (*pp).cache_size
    {
        1 as i32
    } else {
        0 as i32
