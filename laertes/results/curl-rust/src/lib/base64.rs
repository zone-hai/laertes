...
pub use crate::src::lib::warnless::curlx_ultouc;
...
static mut base64url: [i8; 65] = unsafe {
    *core::intrinsics::transmute::<&'_ [u8; 65], &'_ [i8; 65]>(b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_\0")
};
unsafe extern "C" fn decodeQuantum(
    mut dest: * mut u8,
    mut src: * const i8,
) -> u64 {
    let mut padding: u64 = 0 as i32 as size_t;
    ...
    while i < 4 as i32 as u64 {
        if *s as i32 == '=' as i32 {
            ...
            p = base64.as_ptr();
            while *p as i32 != 0 && *p as i32 != *s as i32 {
                v = v.wrapping_add(1);
                p = p.offset(1);
            }
            if *p as i32 == *s as i32 {
                x = (x << 6 as i32).wrapping_add(v);
            } else {
    ...
    if padding < 1 as i32 as u64 {
        *dest
            .offset(2 as i32 as isize) = curlx_ultouc(x & 0xff as u64);
    }
    x >>= 8 as i32;
    if padding < 2 as i32 as u64 {
        *dest
            .offset(1 as i32 as isize) = curlx_ultouc(x & 0xff as u64);
    }
    x >>= 8 as i32;
    *dest.offset(0 as i32 as isize) = curlx_ultouc(x & 0xff as u64);
    return (3 as i32 as u64).wrapping_sub(padding);
}
#[no_mangle]
pub unsafe extern "C" fn Curl_base64_decode<'a1, 'a2>(
    mut src: * const i8,
    mut outptr: Option<&'a1 mut * mut u8>,
    mut outlen: Option<&'a2 mut u64>,
) -> u32 {
    let mut srclen: u64 = 0 as i32 as size_t;
    let mut length: u64 = 0 as i32 as size_t;
    let mut padding: u64 = 0 as i32 as size_t;
    let mut i: u64 = 0;
    let mut numQuantums: u64 = 0;
    let mut rawlen: u64 = 0 as i32 as size_t;
    let mut pos: * mut u8 = 0 as *mut u8;
    let mut newstr: * mut u8 = 0 as *mut u8;
    *(borrow_mut(&mut outptr)).unwrap() = 0 as *mut u8;
    *(borrow_mut(&mut outlen)).unwrap() = 0 as i32 as size_t;
    srclen = strlen(src);
    if srclen == 0 || srclen.wrapping_rem(4 as i32 as u64) != 0 {
        return CURLE_BAD_CONTENT_ENCODING;
    }
    while *src.offset(length as isize) as i32 != '=' as i32
        && *src.offset(length as isize) as i32 != 0
    {
        length = length.wrapping_add(1);
    }
    if *src.offset(length as isize) as i32 == '=' as i32 {
        padding = padding.wrapping_add(1);
        if *src.offset(length.wrapping_add(1 as i32 as u64) as isize)
            as i32 == '=' as i32
        {
            padding = padding.wrapping_add(1);
        }
    ...
    numQuantums = srclen.wrapping_div(4 as i32 as u64);
    rawlen = numQuantums
    ...
    newstr = Curl_cmalloc
        ...
        )(rawlen.wrapping_add(1 as i32 as u64)) as *mut u8;
    if newstr.is_null() {
    ...
    while i < numQuantums {
        let mut result: u64 = decodeQuantum(pos, src);
        if result == 0 {
            Curl_cfree.expect("non-null function pointer")(newstr as *mut libc::c_void);
            return CURLE_BAD_CONTENT_ENCODING;
        }
        pos = pos.offset(result as isize);
        src = src.offset(4 as i32 as isize);
        i = i.wrapping_add(1);
    }
    *pos = '\u{0}' as i32 as u8;
    *(borrow_mut(&mut outptr)).unwrap() = newstr;
    *(borrow_mut(&mut outlen)).unwrap() = rawlen;
    return CURLE_OK;
}
unsafe extern "C" fn base64_encode<'a1, 'a2>(
    mut table64: * const i8,
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut inputbuff: * const i8,
    mut insize: u64,
    mut outptr: Option<&'a1 mut * mut i8>,
    mut outlen: Option<&'a2 mut u64>,
) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut ibuf: [u8; 3] = [0; 3];
    let mut obuf: [u8; 4] = [0; 4];
    let mut i: i32 = 0;
    let mut inputparts: i32 = 0;
    let mut output: * mut i8 = 0 as *mut i8;
    let mut base64data: * mut i8 = 0 as *mut i8;
    let mut convbuf: * mut i8 = 0 as *mut i8;
    let mut indata: * const i8 = inputbuff;
    *(borrow_mut(&mut outptr)).unwrap() = 0 as *mut i8;
    *(borrow_mut(&mut outlen)).unwrap() = 0 as i32 as size_t;
    if insize == 0 {
    ...
    while insize > 0 as i32 as u64 {
        ...
        i = inputparts;
        while i < 3 as i32 {
            if insize > 0 as i32 as u64 {
                inputparts += 1;
                ibuf[i as usize] = *indata as u8;
                indata = indata.offset(1);
                insize = insize.wrapping_sub(1);
            } else {
                ibuf[i as usize] = 0 as i32 as u8;
            }
            i += 1;
        }
        obuf[0 as i32
            as usize] = ((ibuf[0 as i32 as usize] as i32
            & 0xfc as i32) >> 2 as i32) as u8;
        obuf[1 as i32
            as usize] = ((ibuf[0 as i32 as usize] as i32
            & 0x3 as i32) << 4 as i32
            | (ibuf[1 as i32 as usize] as i32 & 0xf0 as i32)
                >> 4 as i32) as u8;
        obuf[2 as i32
            as usize] = ((ibuf[1 as i32 as usize] as i32
            & 0xf as i32) << 2 as i32
            | (ibuf[2 as i32 as usize] as i32 & 0xc0 as i32)
                >> 6 as i32) as u8;
        obuf[3 as i32
            as usize] = (ibuf[2 as i32 as usize] as i32
            & 0x3f as i32) as u8;
        match inputparts {
            1 => {
                curl_msnprintf(
                    ...
                    *table64.offset(obuf[0 as i32 as usize] as isize)
                        as i32,
                    *table64.offset(obuf[1 as i32 as usize] as isize)
                        as i32,
                );
            }
            2 => {
                curl_msnprintf(
                    ...
                    *table64.offset(obuf[1 as i32 as usize] as isize)
                        as i32,
                    *table64.offset(obuf[2 as i32 as usize] as isize)
                        as i32,
                );
            }
            _ => {
                curl_msnprintf(
                    ...
                    *table64.offset(obuf[2 as i32 as usize] as isize)
                        as i32,
                    *table64.offset(obuf[3 as i32 as usize] as isize)
                        as i32,
                );
            }
        }
        output = output.offset(4 as i32 as isize);
    }
    *output = '\u{0}' as i32 as i8;
    *(borrow_mut(&mut outptr)).unwrap() = base64data;
    Curl_cfree.expect("non-null function pointer")(convbuf as *mut libc::c_void);
    *(borrow_mut(&mut outlen)).unwrap() = strlen(base64data);
    return CURLE_OK;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_base64_encode<'a1, 'a2>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut inputbuff: * const i8,
    mut insize: u64,
    mut outptr: Option<&'a1 mut * mut i8>,
    ...
    return base64_encode(base64.as_ptr(), data, inputbuff, insize, borrow_mut(&mut outptr), borrow_mut(&mut outlen));
}
#[no_mangle]
pub unsafe extern "C" fn Curl_base64url_encode<'a1, 'a2>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut inputbuff: * const i8,
    mut insize: u64,
    mut outptr: Option<&'a1 mut * mut i8>,
    ...
    return base64_encode(base64url.as_ptr(), data, inputbuff, insize, borrow_mut(&mut outptr), borrow_mut(&mut outlen));
}
