...
#[no_mangle]
pub unsafe extern "C" fn Curl_httpchunk_init(mut data: * mut crate::src::lib::http2::Curl_easy) {
    let mut conn: * mut crate::src::lib::http2::connectdata = (*data).conn;
    let mut chunk: Option<&'_ mut crate::src::lib::http2::Curl_chunker> = Some(&mut (*conn).chunk);
    (*(borrow_mut(&mut chunk)).unwrap()).hexindex = 0 as i32 as u8;
    (*(borrow_mut(&mut chunk)).unwrap()).state = CHUNK_HEX;
    Curl_dyn_init(&mut (*conn).trailer, 4096 as i32 as size_t);
}
#[no_mangle]
pub unsafe extern "C" fn Curl_httpchunk_read<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut datap: * mut i8,
    mut datalen: i64,
    mut wrotep: * mut i64,
    mut extrap: Option<&'a1 mut u32>,
) -> i32 {
    let mut result: u32 = CURLE_OK;
    let mut conn: * mut crate::src::lib::http2::connectdata = (*data).conn;
    let mut ch: * mut crate::src::lib::http2::Curl_chunker = &mut (*conn).chunk;
    let mut k: Option<&'_ mut crate::src::lib::http2::SingleRequest> = Some(&mut (*data).req);
    let mut piece: u64 = 0;
    let mut length: i64 = datalen;
    let mut wrote: * mut u64 = wrotep as *mut size_t;
    *wrote = 0 as i32 as size_t;
    if ((*data).set).http_te_skip() as i32 != 0 && (*(borrow(& k)).unwrap()).ignorebody() == 0 {
        ...
        if result as u64 != 0 {
            *(borrow_mut(&mut extrap)).unwrap() = result;
        ...
        match (*ch).state as u32 {
            ...
            1 => {
                if *datap as i32 == 0xa as i32 {
                    if 0 as i32 as i64 == (*ch).datasize {
            ...
            2 => {
                ...
                if ((*data).set).http_te_skip() == 0 && (*(borrow(& k)).unwrap()).ignorebody() == 0 {
                ...
                *wrote = (*wrote as u64).wrapping_add(piece) as size_t
                    as size_t;
                let mut fresh1 = &mut ((*ch).datasize);
                *fresh1 = (*fresh1 as u64).wrapping_sub(piece) as curl_off_t
                    as curl_off_t;
            ...
            5 => {
                ...
                {
                    let mut tr: * mut i8 = Curl_dyn_ptr(&mut (*conn).trailer);
                    if !tr.is_null() {
                        let mut trlen: u64 = 0;
                        result = Curl_dyn_add(
                            &mut (*conn).trailer,
                        ...
                        Curl_dyn_reset(Some(&mut (*conn).trailer));
...
#[no_mangle]
pub extern "C" fn Curl_chunked_strerror(
    mut code: i32,
) -> * const i8 {
    match code as i32 {
        1 => return b"Too long hexadecimal number\0" as *const u8 as *const i8,
        2 => {
            return b"Illegal or missing hexadecimal sequence\0" as *const u8
                as *const i8;
        }
        3 => return b"Malformed encoding found\0" as *const u8 as *const i8,
        6 => return b"\0" as *const u8 as *const i8,
        4 => return b"Bad content-encoding found\0" as *const u8 as *const i8,
        5 => return b"Out of memory\0" as *const u8 as *const i8,
        _ => return b"OK\0" as *const u8 as *const i8,
    };
}
