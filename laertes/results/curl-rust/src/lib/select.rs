...
#[no_mangle]
pub unsafe extern "C" fn Curl_wait_ms(mut timeout_ms: i64) -> i32 {
...
pub unsafe extern "C" fn Curl_socket_check(
    ...
    let mut pfd: [crate::src::lib::multi::pollfd; 3] = [pollfd {
        fd: 0,
        events: 0,
        revents: 0,
    }; 3];
...
pub unsafe extern "C" fn Curl_poll(
    mut ufds: * mut crate::src::lib::multi::pollfd,
    mut nfds: u32,
    mut timeout_ms: i64,
) -> i32 {
    ...
    while i < nfds {
        if !((*ufds.offset(i as isize)).fd == -(1 as i32)) {
            if (*ufds.offset(i as isize)).revents as i32 & 0x10 as i32
                != 0
            {
                let mut fresh0 = &mut ((*ufds.offset(i as isize)).revents);
                *fresh0 = (*fresh0 as i32 | 0x1 as i32) as i16;
            }
            if (*ufds.offset(i as isize)).revents as i32 & 0x8 as i32
                != 0
            {
                let mut fresh1 = &mut ((*ufds.offset(i as isize)).revents);
