...
impl std::default::Default for C2RustUnnamed_6 {
    fn default() -> Self { C2RustUnnamed_6::new() }
}
...
static mut mechtable: [crate::src::lib::curl_sasl::C2RustUnnamed_6; 12] = [
    {
...
pub unsafe extern "C" fn Curl_sasl_cleanup(
    ...
    {
        Curl_auth_gsasl_cleanup(Some(&mut (*conn).gsasl));
    }
    if authused == ((1 as i32) << 6 as i32) as u32 {
        Curl_auth_cleanup_ntlm(Some(&mut (*conn).ntlm));
    }
}
#[no_mangle]
pub unsafe extern "C" fn Curl_sasl_decode_mech<'a1>(
    mut ptr: * const i8,
    mut maxlen: u64,
    mut len: Option<&'a1 mut u64>,
) -> u16 {
    let mut i: u32 = 0;
    let mut c: i8 = 0;
    i = 0 as i32 as u32;
    while !(mechtable[i as usize].name).is_null() {
            ...
            ) == 0
        {
            if !borrow(& len).is_none() {
                *(borrow_mut(&mut len)).unwrap() = mechtable[i as usize].len;
            }
            if maxlen == mechtable[i as usize].len {
...
#[no_mangle]
pub unsafe extern "C" fn Curl_sasl_parse_url_auth_option<'a1>(
    mut sasl: Option<&'a1 mut crate::src::lib::http2::SASL>,
    mut value: * const i8,
    mut len: u64,
) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut mechlen: u64 = 0;
    if len == 0 {
        return CURLE_URL_MALFORMAT;
    }
    if (*(borrow(& sasl)).unwrap()).resetprefs {
        (*(borrow_mut(&mut sasl)).unwrap()).resetprefs = 0 as i32 != 0;
        (*(borrow_mut(&mut sasl)).unwrap()).prefmech = 0 as i32 as u16;
    }
    if strncmp(value, b"*\0" as *const u8 as *const i8, len) == 0 {
        (*(borrow_mut(&mut sasl)).unwrap())
        ...
        if mechbit as i32 != 0 && mechlen == len {
            let mut fresh0 = &mut ((*(borrow_mut(&mut sasl)).unwrap()).prefmech);
...
#[no_mangle]
pub unsafe extern "C" fn Curl_sasl_init<'a1>(
    mut sasl: Option<&'a1 mut crate::src::lib::http2::SASL>,
    mut params: * const crate::src::lib::http2::SASLproto,
) {
    let mut fresh1 = &mut ((*(borrow_mut(&mut sasl)).unwrap()).params);
    *fresh1 = params;
    (*(borrow_mut(&mut sasl)).unwrap()).state = SASL_STOP;
    (*(borrow_mut(&mut sasl)).unwrap()).authmechs = 0 as i32 as u16;
    (*(borrow_mut(&mut sasl)).unwrap())
        .prefmech = (0xffff as i32 & !((1 as i32) << 5 as i32))
        as u16;
    (*(borrow_mut(&mut sasl)).unwrap()).authused = 0 as i32 as u16;
    (*(borrow_mut(&mut sasl)).unwrap()).resetprefs = 1 as i32 != 0;
    (*(borrow_mut(&mut sasl)).unwrap()).mutual_auth = 0 as i32 != 0;
    (*(borrow_mut(&mut sasl)).unwrap()).force_ir = 0 as i32 != 0;
}
unsafe extern "C" fn state(
    mut sasl: * mut crate::src::lib::http2::SASL,
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut newstate: u32,
) {
    (*sasl).state = newstate;
}
unsafe extern "C" fn get_server_message<'a1>(
    mut sasl: * mut crate::src::lib::http2::SASL,
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut out: Option<&'a1 mut crate::src::lib::bufref::bufref>,
) -> u32 {
    ...
    if serverdata.is_null() {
        ...
        Curl_bufref_set(borrow_mut(&mut out), 0 as *const libc::c_void, 0 as i32 as size_t, None);
    } else {
        result = Curl_base64_decode(serverdata, Some(&mut msg), Some(&mut msglen));
        if result as u64 == 0 {
            Curl_bufref_set(
                borrow_mut(&mut out),
...
unsafe extern "C" fn build_message<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut msg: Option<&'a1 mut crate::src::lib::bufref::bufref>,
) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut base64: * mut i8 = 0 as *mut i8;
    let mut base64len: u64 = 0;
    if (Curl_bufref_ptr(borrow(& msg))).is_null() {
        Curl_bufref_set(
            borrow_mut(&mut msg),
    ...
    } else if Curl_bufref_len(borrow(& msg)) == 0 {
        Curl_bufref_set(
            borrow_mut(&mut msg),
            b"=\0" as *const u8 as *const i8 as *const libc::c_void,
            1 as i32 as size_t,
        ...
        result = Curl_base64_encode(
            data,
            Curl_bufref_ptr(borrow(& msg)) as *const i8,
            Curl_bufref_len(borrow(& msg)),
            Some(&mut base64),
            Some(&mut base64len),
        );
        if result as u64 == 0 {
...
#[no_mangle]
pub unsafe extern "C" fn Curl_sasl_can_authenticate<'a1>(
    mut sasl: Option<&'a1 mut crate::src::lib::http2::SASL>,
    mut conn: * mut crate::src::lib::http2::connectdata,
) -> bool {
    ...
    if (*(borrow(& sasl)).unwrap()).authmechs as i32 & (*(borrow(& sasl)).unwrap()).prefmech as i32
        & (1 as i32) << 5 as i32 != 0
    {
        return 1 as i32 != 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn Curl_sasl_start<'a1>(
    mut sasl: * mut crate::src::lib::http2::SASL,
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut conn: * mut crate::src::lib::http2::connectdata,
    mut force_ir: bool,
    mut progress: Option<&'a1 mut u32>,
) -> u32 {
    let mut result: u32 = CURLE_OK;
    let mut enabledmechs: u32 = 0;
    let mut mech: * const i8 = 0 as *const i8;
    let mut resp: crate::src::lib::bufref::bufref = bufref {
        dtor: None,
        ptr: 0 as *const u8,
        len: 0,
    };
    let mut state1: u32 = SASL_STOP;
    let mut state2: u32 = SASL_FINAL;
    ...
    let mut nullmsg: crate::src::lib::bufref::bufref = bufref {
        dtor: None,
        ptr: 0 as *const u8,
        len: 0,
    };
    Curl_bufref_init(Some(&mut nullmsg));
    Curl_bufref_init(Some(&mut resp));
    (*sasl).force_ir = force_ir;
    (*sasl).authused = 0 as i32 as u16;
    enabledmechs = ((*sasl).authmechs as i32 & (*sasl).prefmech as i32)
        as u32;
    *(borrow_mut(&mut progress)).unwrap() = SASL_IDLE;
        ...
        && *((*conn).passwd).offset(0 as i32 as isize) == 0
    {
        ...
        if force_ir as i32 != 0 || ((*data).set).sasl_ir() as i32 != 0 {
            result = Curl_auth_create_external_message((*conn).user, Some(&mut resp));
        }
            ...
            ) as i32 != 0
        {
            ...
            if force_ir as i32 != 0
                || ((*data).set).sasl_ir() as i32 != 0
            {
                result = Curl_auth_create_login_message((*conn).user, Some(&mut resp));
            }
        }
    }
    if result as u64 == 0 && !mech.is_null() {
        if !(Curl_bufref_ptr(Some(&mut resp))).is_null() {
            result = build_message(data, Some(&mut resp));
        }
        if (*(*sasl).params).maxirlen != 0
            && (strlen(mech)).wrapping_add(Curl_bufref_len(Some(&mut resp)))
                > (*(*sasl).params).maxirlen
        {
            Curl_bufref_free(Some(&mut resp));
        }
        if result as u64 == 0 {
            result = ((*(*sasl).params).sendauth)
                .expect(
                    "non-null function pointer",
                )(data, conn, mech, Curl_bufref_ptr(Some(&mut resp)) as *const i8);
        }
        if result as u64 == 0 {
            *(borrow_mut(&mut progress)).unwrap() = SASL_INPROGRESS;
            state(
                sasl,
                data,
                (if !(Curl_bufref_ptr(Some(&mut resp))).is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn Curl_sasl_continue<'a1>(
    mut sasl: * mut crate::src::lib::http2::SASL,
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut conn: * mut crate::src::lib::http2::connectdata,
    mut code: i32,
    ...
    let mut result: u32 = CURLE_OK;
    let mut newstate: u32 = SASL_FINAL;
    let mut resp: crate::src::lib::bufref::bufref = bufref {
        dtor: None,
        ptr: 0 as *const u8,
        len: 0,
    ...
    let mut oauth_bearer: * const i8 = (*data)
        .set
        .str_0[STRING_BEARER as i32 as usize];
    let mut serverdata: crate::src::lib::bufref::bufref = bufref {
        dtor: None,
        ptr: 0 as *const u8,
        len: 0,
    };
    Curl_bufref_init(Some(&mut serverdata));
    Curl_bufref_init(Some(&mut resp));
    *(borrow_mut(&mut progress)).unwrap() = SASL_INPROGRESS;
    if (*sasl).state as u32 == SASL_FINAL as i32 as u32 {
        ...
        *(borrow_mut(&mut progress)).unwrap() = SASL_DONE;
    ...
    match (*sasl).state as u32 {
        ...
        3 => {
            result = Curl_auth_create_login_message((*conn).passwd, Some(&mut resp));
        }
        4 => {
            result = Curl_auth_create_external_message((*conn).user, Some(&mut resp));
        }
        15 => {
            result = get_server_message(sasl, data, Some(&mut serverdata));
            if result as u64 == 0 {
                result = Curl_auth_gsasl_token(
                    data,
                    Some(&mut serverdata),
            ...
            if result as u64 == 0
                && Curl_bufref_len(Some(&mut resp)) > 0 as i32 as u64
            {
                newstate = SASL_GSASL;
            }
        }
        ...
        16 => {
            let mut fresh2 = &mut ((*sasl).authmechs);
            *fresh2 = (*fresh2 as i32 ^ (*sasl).authused as i32)
                as u16;
            return Curl_sasl_start(sasl, data, conn, (*sasl).force_ir, borrow_mut(&mut progress));
        }
        _ => {
            Curl_failf(
                data,
    ...
    Curl_bufref_free(Some(&mut serverdata));
    match result as u32 {
        61 => {
        ...
        0 => {
            ...
            if result as u64 == 0 {
                result = ((*(*sasl).params).sendcont)
                    .expect(
                        "non-null function pointer",
                    )(data, conn, Curl_bufref_ptr(Some(&mut resp)) as *const i8);
            }
        }
        _ => {
            newstate = SASL_STOP;
