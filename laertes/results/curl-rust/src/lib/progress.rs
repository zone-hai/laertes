...
unsafe extern "C" fn time2str(mut r: * mut i8, mut seconds: i64) {
    let mut h: i64 = 0;
    ...
    if h <= 99 as i64 {
        let mut m: i64 = (seconds - h * 3600 as i64)
            / 60 as i64;
        let mut s: i64 = seconds - h * 3600 as i64
            - m * 60 as i64;
        curl_msnprintf(
            r,
            9 as i32 as size_t,
        ...
        let mut d: i64 = seconds / 86400 as i64;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pgrsDone(mut data: * mut crate::src::lib::http2::Curl_easy) -> i32 {
    let mut rc: i32 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pgrsResetTransferSizes(mut data: * mut crate::src::lib::http2::Curl_easy) {
...
pub unsafe extern "C" fn Curl_pgrsTime(
    ...
    mut timer: u32,
) -> crate::src::lib::http2::curltime {
    let mut now: crate::src::lib::http2::curltime = Curl_now();
    let mut delta: Option<&'_ mut i64> = Option::<&'_ mut i64>::None;
    match timer as u32 {
        1 => {
        ...
        2 => {
            ...
            let mut fresh0 = &mut ((*data).progress);
        ...
        3 => {
            delta = Some(&mut (*data).progress.t_nslookup);
        }
        4 => {
            delta = Some(&mut (*data).progress.t_connect);
        }
        5 => {
            delta = Some(&mut (*data).progress.t_appconnect);
        }
        6 => {
            delta = Some(&mut (*data).progress.t_pretransfer);
        }
        7 => {
            delta = Some(&mut (*data).progress.t_starttransfer);
            if ((*data).progress).is_t_startransfer_set() != 0 {
                return now
            } else {
                let mut fresh1 = &mut ((*data).progress);
    ...
    if !borrow(& delta).is_none() {
        let mut us: i64 = Curl_timediff_us(now, (*data).progress.t_startsingle);
        if us < 1 as i32 as i64 {
            us = 1 as i32 as timediff_t;
        }
        *(borrow_mut(&mut delta)).unwrap() += us;
    }
    return now;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pgrsStartNow(mut data: * mut crate::src::lib::http2::Curl_easy) {
    (*data).progress.speeder_c = 0 as i32;
    (*data).progress.start = Curl_now();
    let mut fresh2 = &mut ((*data).progress);
...
pub extern "C" fn Curl_pgrsLimitWaitTime(
    ...
    mut startsize: i64,
    mut limit: i64,
    mut start: crate::src::lib::http2::curltime,
    mut now: crate::src::lib::http2::curltime,
) -> i64 {
    let mut size: i64 = cursize - startsize;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_ratelimit(mut data: * mut crate::src::lib::http2::Curl_easy, mut now: crate::src::lib::http2::curltime) {
...
pub unsafe extern "C" fn Curl_pgrsSetUploadSize(
 ...
 extern "C" fn trspeed(mut size: i64, mut us: i64) -> i64 {
    if us < 1 as i32 as i64 {
...
unsafe extern "C" fn progress_calc(mut data: * mut crate::src::lib::http2::Curl_easy, mut now: crate::src::lib::http2::curltime) -> bool {
    let mut timetoshow: bool = 0 as i32 != 0;
    let p: * mut crate::src::lib::http2::Progress = &mut (*data).progress;
    ...
    if (*p).lastshow != now.tv_sec {
        ...
        let mut fresh3 = &mut ((*p).speeder_c);
        *fresh3 += 1;
...
unsafe extern "C" fn progress_meter(mut data: * mut crate::src::lib::http2::Curl_easy) {
    let mut max5: [[i8; 10]; 6] = [[0; 10]; 6];
    let mut dlpercen: i64 = 0 as i32 as curl_off_t;
    let mut ulpercen: i64 = 0 as i32 as curl_off_t;
    let mut total_percen: i64 = 0 as i32 as curl_off_t;
    let mut total_transfer: i64 = 0;
    let mut total_expected_transfer: i64 = 0;
    let mut time_left: [i8; 10] = [0; 10];
    let mut time_total: [i8; 10] = [0; 10];
    let mut time_spent: [i8; 10] = [0; 10];
    let mut ulestimate: i64 = 0 as i32 as curl_off_t;
    let mut dlestimate: i64 = 0 as i32 as curl_off_t;
    let mut total_estimate: i64 = 0;
    let mut timespent: i64 = (*data).progress.timespent
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pgrsUpdate(mut data: * mut crate::src::lib::http2::Curl_easy) -> i32 {
    let mut now: crate::src::lib::http2::curltime = Curl_now();
