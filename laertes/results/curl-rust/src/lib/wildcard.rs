...
unsafe extern "C" fn fileinfo_dtor(
    mut _user: * mut core::ffi::c_void,
    mut element: * mut core::ffi::c_void,
) {
    Curl_fileinfo_cleanup(element as *mut fileinfo);
}
#[no_mangle]
pub unsafe extern "C" fn Curl_wildcard_init<'a1>(mut wc: Option<&'a1 mut crate::src::lib::http2::WildcardData>) -> u32 {
    Curl_llist_init(
        Some(&mut (*(borrow_mut(&mut wc)).unwrap()).filelist),
        Some(
            fileinfo_dtor,
        ),
    );
    (*(borrow_mut(&mut wc)).unwrap()).state = CURLWC_INIT;
    return CURLE_OK;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_wildcard_dtor<'a1>(mut wc: Option<&'a1 mut crate::src::lib::http2::WildcardData>) {
    if borrow(& wc).is_none() {
        return;
    }
    if ((*(borrow(& wc)).unwrap()).dtor).is_some() {
        ((*(borrow(& wc)).unwrap()).dtor).expect("non-null function pointer")((*(borrow_mut(&mut wc)).unwrap()).protdata);
        let mut fresh0 = &mut ((*(borrow_mut(&mut wc)).unwrap()).dtor);
        *fresh0 = None;
        let mut fresh1 = &mut ((*(borrow_mut(&mut wc)).unwrap()).protdata);
        *fresh1 = 0 as *mut libc::c_void;
    }
    Curl_llist_destroy(&mut (*(borrow_mut(&mut wc)).unwrap()).filelist, 0 as *mut libc::c_void);
    Curl_cfree.expect("non-null function pointer")((*(borrow_mut(&mut wc)).unwrap()).path as *mut libc::c_void);
    let mut fresh2 = &mut ((*(borrow_mut(&mut wc)).unwrap()).path);
    *fresh2 = 0 as *mut i8;
    Curl_cfree.expect("non-null function pointer")((*(borrow_mut(&mut wc)).unwrap()).pattern as *mut libc::c_void);
    let mut fresh3 = &mut ((*(borrow_mut(&mut wc)).unwrap()).pattern);
    *fresh3 = 0 as *mut i8;
    let mut fresh4 = &mut ((*(borrow_mut(&mut wc)).unwrap()).customptr);
