...
pub use crate::src::lib::mprintf::curl_mvsnprintf;
...
unsafe extern "C" fn convert_lineends(
    ...
    let mut inPtr: * mut i8 = 0 as *mut i8;
    let mut outPtr: * mut i8 = 0 as *mut i8;
    if startPtr.is_null() || size < 1 as i32 as u64 {
        return size;
    }
    if ((*data).state).prev_block_had_trailing_cr() != 0 {
        if *startPtr as i32 == '\n' as i32 {
            memmove(
                ...
                startPtr.offset(1 as i32 as isize) as *const libc::c_void,
                size.wrapping_sub(1 as i32 as u64),
            );
            size = size.wrapping_sub(1);
            let mut fresh0 = &mut ((*data).state.crlf_conversions);
            *fresh0 += 1;
        }
        let mut fresh1 = &mut ((*data).state);
        (*fresh1).set_prev_block_had_trailing_cr(0 as i32 as bit);
    }
    outPtr = memchr(startPtr as *const libc::c_void, '\r' as i32, size)
        as *mut i8;
    inPtr = outPtr;
    if !inPtr.is_null() {
        while inPtr < startPtr.offset(size as isize).offset(-(1 as i32 as isize))
        {
            if memcmp(
            ...
            ) == 0 as i32
            {
                ...
                *outPtr = *inPtr;
                let mut fresh2 = &mut ((*data).state.crlf_conversions);
                *fresh2 += 1;
            } else if *inPtr as i32 == '\r' as i32 {
                *outPtr = '\n' as i32 as i8;
            } else {
                *outPtr = *inPtr;
            }
        ...
        if inPtr < startPtr.offset(size as isize) {
            if *inPtr as i32 == '\r' as i32 {
                *outPtr = '\n' as i32 as i8;
                let mut fresh3 = &mut ((*data).state);
                (*fresh3).set_prev_block_had_trailing_cr(1 as i32 as bit);
            } else {
                *outPtr = *inPtr;
            }
            outPtr = outPtr.offset(1);
        ...
        if outPtr < startPtr.offset(size as isize) {
            *outPtr = '\u{0}' as i32 as i8;
        }
        return outPtr.offset_from(startPtr) as i64 as size_t;
    }
    return size;
}
#[no_mangle]
pub extern "C" fn Curl_recv_has_postponed_data(
...
pub unsafe extern "C" fn Curl_infof(
    ...
    if !data.is_null() && ((*data).set).verbose() as i32 != 0 {
        let mut ap: core::ffi::VaListImpl;
        let mut len: u64 = 0;
        let mut buffer: [i8; 2050] = [0; 2050];
        ap = args.clone();
        len = curl_mvsnprintf(
            buffer.as_mut_ptr(),
            2048 as i32 as size_t,
            fmt,
            ap.as_va_list(),
        ) as size_t;
        let mut fresh4 = len;
        ...
        buffer[len as usize] = '\u{0}' as i32 as i8;
...
pub unsafe extern "C" fn Curl_failf(
    ...
    if ((*data).set).verbose() as i32 != 0
        || !((*data).set.errorbuffer).is_null()
    {
        let mut ap: core::ffi::VaListImpl;
        let mut len: u64 = 0;
        let mut error: [i8; 258] = [0; 258];
        ap = args.clone();
        len = curl_mvsnprintf(
            error.as_mut_ptr(),
        ...
        if !((*data).set.errorbuffer).is_null() && ((*data).state).errorbuf() == 0 {
            ...
            let mut fresh5 = &mut ((*data).state);
            (*fresh5).set_errorbuf(1 as i32 as bit);
        }
        let mut fresh6 = len;
        len = len.wrapping_add(1);
        error[fresh6 as usize] = '\n' as i32 as i8;
        error[len as usize] = '\u{0}' as i32 as i8;
        Curl_debug(data, CURLINFO_TEXT, error.as_mut_ptr(), len);
    }
}
#[no_mangle]
pub unsafe extern "C" fn Curl_write<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut sockfd: i32,
    mut mem: * const core::ffi::c_void,
    mut len: u64,
    mut written: Option<&'a1 mut i64>,
) -> u32 {
    ...
    let mut num: i32 = 0;
    conn = (*data).conn;
    num = (sockfd == (*conn).sock[1 as i32 as usize]) as i32;
    bytes_written = ((*conn).send[num as usize])
        .expect("non-null function pointer")(data, num, mem, len, &mut result);
    *(borrow_mut(&mut written)).unwrap() = bytes_written;
    if bytes_written >= 0 as i32 as i64 {
        return CURLE_OK;
    }
    match result as u32 {
        81 => {
            *(borrow_mut(&mut written)).unwrap() = 0 as i32 as ssize_t;
            return CURLE_OK;
        }
...
pub unsafe extern "C" fn Curl_send_plain(
    ...
    if CURLE_OK as i32 != 0 {
        *code = CURLE_OUT_OF_MEMORY;
        return -(1 as i32) as ssize_t;
    }
    bytes_written = send(sockfd, mem, len, MSG_NOSIGNAL as i32);
    *code = CURLE_OK;
    if -(1 as i32) as i64 == bytes_written {
        let mut err: i32 = *__errno_location();
        if 11 as i32 == err || 11 as i32 == err
            || 4 as i32 == err || 115 as i32 == err
        {
            ...
            *code = CURLE_AGAIN;
            ...
            Curl_failf(
                data,
                b"Send failure: %s\0" as *const u8 as *const i8,
                Curl_strerror(
                    err,
                    buffer.as_mut_ptr(),
...
#[no_mangle]
pub unsafe extern "C" fn Curl_write_plain<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut sockfd: i32,
    mut mem: * const core::ffi::c_void,
    mut len: u64,
    ...
    let mut num: i32 = 0;
    num = (sockfd == (*conn).sock[1 as i32 as usize]) as i32;
    *(borrow_mut(&mut written)).unwrap() = Curl_send_plain(data, num, mem, len, &mut result);
    return result;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_recv_plain(
    ...
    nread = recv(sockfd, buf as *mut libc::c_void, len, 0 as i32);
    *code = CURLE_OK;
    if -(1 as i32) as i64 == nread {
        let mut err: i32 = *__errno_location();
        if 11 as i32 == err || 11 as i32 == err
            || 4 as i32 == err
        {
            *code = CURLE_AGAIN;
            ...
            Curl_failf(
                data,
                b"Recv failure: %s\0" as *const u8 as *const i8,
                Curl_strerror(
                    err,
                    buffer.as_mut_ptr(),
                    ::std::mem::size_of::<[i8; 256]>() as u64,
...
unsafe extern "C" fn pausewrite(
    ...
    let mut s: Option<&'_ mut crate::src::lib::http2::UrlState> = Some(&mut (*data).state);
    let mut i: u32 = 0;
    let mut newtype: bool = 1 as i32 != 0;
    Curl_http2_stream_pause(data, 1 as i32 != 0);
    if (*(borrow(& s)).unwrap()).tempcount != 0 {
        i = 0 as i32 as u32;
        while i < (*(borrow(& s)).unwrap()).tempcount {
            if (*(borrow(& s)).unwrap()).tempwrite[i as usize].type_0 == type_0 {
                newtype = 0 as i32 != 0;
                break;
            } else {
                i = i.wrapping_add(1);
    ...
    if newtype {
        Curl_dyn_init(
            &mut (*((*(borrow_mut(&mut s)).unwrap()).tempwrite).as_mut_ptr().offset(i as isize)).b,
            (64 as i32 * 1024 as i32 * 1024 as i32) as size_t,
        );
        (*(borrow_mut(&mut s)).unwrap()).tempwrite[i as usize].type_0 = type_0;
        let mut fresh7 = &mut ((*(borrow_mut(&mut s)).unwrap()).tempcount);
        *fresh7 = (*fresh7).wrapping_add(1);
    }
    if Curl_dyn_addn(
        &mut (*((*(borrow_mut(&mut s)).unwrap()).tempwrite).as_mut_ptr().offset(i as isize)).b,
        ptr as *mut u8 as *const libc::c_void,
        len,
    ) as u64 != 0
    {
        return CURLE_OUT_OF_MEMORY;
    }
    (*(borrow_mut(&mut k)).unwrap()).keepon |= (1 as i32) << 4 as i32;
    return CURLE_OK;
}
unsafe extern "C" fn chop_write(
    ...
    let mut conn: * mut crate::src::lib::http2::connectdata = (*data).conn;
    let mut writeheader: Option<unsafe extern "C"  fn(_: * mut i8,_: u64,_: u64,_: * mut core::ffi::c_void,) -> u64> = None;
    let mut writebody: Option<unsafe extern "C"  fn(_: * mut i8,_: u64,_: u64,_: * mut core::ffi::c_void,) -> u64> = None;
    let mut ptr: * mut i8 = optr;
    let mut len: u64 = olen;
    if len == 0 {
        return CURLE_OK;
    }
    if (*data).req.keepon & (1 as i32) << 4 as i32 != 0 {
        return pausewrite(data, type_0, ptr, len);
    }
    if type_0 & (1 as i32) << 0 as i32 != 0 {
        writebody = (*data).set.fwrite_func;
    }
    if type_0 & (1 as i32) << 1 as i32 != 0
    ...
    while len != 0 {
        let mut chunklen: u64 = if len <= 16384 as i32 as u64 {
            len
        } else {
            16384 as i32 as u64
        };
        if writebody.is_some() {
            ...
            wrote = writebody
                ...
                )(ptr, 1 as i32 as size_t, chunklen, (*data).set.out);
        ...
        len = (len as u64).wrapping_sub(chunklen) as size_t as size_t;
    }
    if writeheader.is_some() {
        let mut wrote_0: u64 = 0;
        ptr = optr;
        len = olen;
        Curl_set_in_callback(data, 1 as i32 != 0);
        wrote_0 = writeheader
            .expect(
                "non-null function pointer",
            )(ptr, 1 as i32 as size_t, len, (*data).set.writeheader);
        Curl_set_in_callback(data, 0 as i32 != 0);
        if 0x10000001 as i32 as u64 == wrote_0 {
            return pausewrite(data, (1 as i32) << 1 as i32, ptr, len);
        }
        if wrote_0 != len {
            Curl_failf(
                data,
                b"Failed writing header\0" as *const u8 as *const i8,
            );
            return CURLE_WRITE_ERROR;
        }
    }
...
pub unsafe extern "C" fn Curl_client_write(
    ...
    if type_0 & (1 as i32) << 0 as i32 != 0
        ...
        && (*conn).proto.ftpc.transfertype as i32 == 'A' as i32
    {
        let mut result: u32 = CURLE_OK as i32 as CURLcode;
        if result as u64 != 0 {
            return result;
        }
...
#[no_mangle]
pub unsafe extern "C" fn Curl_read_plain<'a1>(
    mut sockfd: i32,
    mut buf: * mut i8,
    mut bytesfromsocket: u64,
    mut n: Option<&'a1 mut i64>,
) -> u32 {
    let mut nread: i64 = recv(
        sockfd,
    ...
    if -(1 as i32) as i64 == nread {
        ...
        let return_error: bool = 11 as i32 == err || 11 as i32 == err
            || 4 as i32 == err;
        *(borrow_mut(&mut n)).unwrap() = 0 as i32 as ssize_t;
        if return_error {
            return CURLE_AGAIN;
        }
        return CURLE_RECV_ERROR;
    }
    *(borrow_mut(&mut n)).unwrap() = nread;
    return CURLE_OK;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_read<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut sockfd: i32,
    mut buf: * mut i8,
    mut sizerequested: u64,
    mut n: Option<&'a1 mut i64>,
) -> u32 {
    let mut result: u32 = CURLE_RECV_ERROR;
    let mut nread: i64 = 0 as i32 as ssize_t;
    let mut bytesfromsocket: u64 = 0 as i32 as size_t;
    let mut buffertofill: * mut i8 = 0 as *mut i8;
    let mut conn: * mut crate::src::lib::http2::connectdata = (*data).conn;
    let mut num: i32 = (sockfd == (*conn).sock[1 as i32 as usize])
        as i32;
    *(borrow_mut(&mut n)).unwrap() = 0 as i32 as ssize_t;
...
pub unsafe extern "C" fn Curl_debug(
    ...
    if ((*data).set).verbose() != 0 {
        static mut s_infotype: [[i8; 3]; 7] = unsafe {
            [
                *core::intrinsics::transmute::<&'_ [u8; 3], &'_ [i8; 3]>(b"* \0"),
                *core::intrinsics::transmute::<&'_ [u8; 3], &'_ [i8; 3]>(b"< \0"),
                *core::intrinsics::transmute::<&'_ [u8; 3], &'_ [i8; 3]>(b"> \0"),
                *core::intrinsics::transmute::<&'_ [u8; 3], &'_ [i8; 3]>(b"{ \0"),
                *core::intrinsics::transmute::<&'_ [u8; 3], &'_ [i8; 3]>(b"} \0"),
