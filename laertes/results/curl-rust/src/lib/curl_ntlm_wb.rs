...
extern "C" {
    ...
    fn kill(__pid: i32, __sig: i32) -> i32;
    fn waitpid(
        __pid: i32,
        __stat_loc: * mut i32,
        __options: i32,
...
impl std::default::Default for passwd {
    fn default() -> Self { passwd::new() }
}

unsafe extern "C" fn ntlm_wb_cleanup<'a1>(mut ntlm: Option<&'a1 mut crate::src::lib::http2::ntlmdata>) {
    if (*(borrow(& ntlm)).unwrap()).ntlm_auth_hlpr_socket != -(1 as i32) {
        close((*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_socket);
        (*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_socket = -(1 as i32);
    }
    if (*(borrow(& ntlm)).unwrap()).ntlm_auth_hlpr_pid != 0 {
        let mut i: i32 = 0;
        i = 0 as i32;
        while i < 4 as i32 {
            let mut ret: i32 = waitpid(
                (*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_pid,
                0 as *mut i32,
                1 as i32,
            );
            if ret == (*(borrow(& ntlm)).unwrap()).ntlm_auth_hlpr_pid
            ...
            match i {
                0 => {
                    kill((*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_pid, 15 as i32);
                }
                1 => {
                    Curl_wait_ms(1 as i32 as timediff_t);
                }
                2 => {
                    kill((*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_pid, 9 as i32);
                }
                3 | _ => {}
            }
            i += 1;
        }
        (*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_pid = 0 as i32;
    }
    Curl_cfree
        .expect("non-null function pointer")((*(borrow_mut(&mut ntlm)).unwrap()).challenge as *mut libc::c_void);
    let mut fresh0 = &mut ((*(borrow_mut(&mut ntlm)).unwrap()).challenge);
    *fresh0 = 0 as *mut i8;
    Curl_cfree
        .expect("non-null function pointer")((*(borrow_mut(&mut ntlm)).unwrap()).response as *mut libc::c_void);
    let mut fresh1 = &mut ((*(borrow_mut(&mut ntlm)).unwrap()).response);
    *fresh1 = 0 as *mut i8;
}
unsafe extern "C" fn ntlm_wb_init<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut ntlm: Option<&'a1 mut crate::src::lib::http2::ntlmdata>,
    mut userp: * const i8,
) -> u32 {
    let mut sockfds: [i32; 2] = [0; 2];
    let mut child_pid: i32 = 0;
    let mut username: * const i8 = 0 as *const i8;
    let mut slash: * mut i8 = 0 as *mut i8;
    let mut domain: * mut i8 = 0 as *mut i8;
    let mut ntlm_auth: * const i8 = 0 as *const i8;
    let mut ntlm_auth_alloc: * mut i8 = 0 as *mut i8;
    ...
    if (*(borrow(& ntlm)).unwrap()).ntlm_auth_hlpr_socket != -(1 as i32)
        || (*(borrow(& ntlm)).unwrap()).ntlm_auth_hlpr_pid != 0
    {
        return CURLE_OK;
    }
    username = userp;
    if username.is_null() || *username.offset(0 as i32 as isize) == 0 {
        username = getenv(b"NTLMUSER\0" as *const u8 as *const i8);
        if username.is_null() || *username.offset(0 as i32 as isize) == 0 {
            username = getenv(b"LOGNAME\0" as *const u8 as *const i8);
        }
        if username.is_null() || *username.offset(0 as i32 as isize) == 0 {
            username = getenv(b"USER\0" as *const u8 as *const i8);
        }
        if (username.is_null() || *username.offset(0 as i32 as isize) == 0)
            && getpwuid_r(
                geteuid(),
    ...
    slash = strpbrk(username, b"\\/\0" as *const u8 as *const i8);
    if !slash.is_null() {
        domain = Curl_cstrdup.expect("non-null function pointer")(username);
        if domain.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        slash = domain.offset(slash.offset_from(username) as i64 as isize);
        *slash = '\u{0}' as i32 as i8;
        username = username
            .offset(slash.offset_from(domain) as i64 as isize)
            .offset(1 as i32 as isize);
    }
    ntlm_auth = b"/usr/bin/ntlm_auth\0" as *const u8 as *const i8;
    if access(ntlm_auth, 1 as i32) != 0 as i32 {
        Curl_failf(
            data,
            b"Could not access ntlm_auth: %s errno %d: %s\0" as *const u8
                as *const i8,
            ntlm_auth,
            *__errno_location(),
            Curl_strerror(
        ...
        Curl_failf(
            data,
            b"Could not open socket pair. errno %d: %s\0" as *const u8
                as *const i8,
            *__errno_location(),
            Curl_strerror(
                *__errno_location(),
        ...
        if child_pid == -(1 as i32) {
            close(sockfds[0 as i32 as usize]);
            close(sockfds[1 as i32 as usize]);
            Curl_failf(
                data,
                b"Could not fork. errno %d: %s\0" as *const u8 as *const i8,
                *__errno_location(),
                Curl_strerror(
                    *__errno_location(),
                    buffer.as_mut_ptr(),
            ...
            if child_pid == 0 {
                close(sockfds[0 as i32 as usize]);
                if dup2(sockfds[1 as i32 as usize], 0 as i32)
                    == -(1 as i32)
                {
                    Curl_failf(
                        data,
                        b"Could not redirect child stdin. errno %d: %s\0" as *const u8
                            as *const i8,
                        *__errno_location(),
                        Curl_strerror(
                            *__errno_location(),
                ...
                if dup2(sockfds[1 as i32 as usize], 1 as i32)
                    == -(1 as i32)
                {
                    Curl_failf(
                        data,
                        b"Could not redirect child stdout. errno %d: %s\0" as *const u8
                            as *const i8,
                        *__errno_location(),
                        Curl_strerror(
                            *__errno_location(),
                ...
                if !domain.is_null() {
                    execl(
                        ntlm_auth,
                        ntlm_auth,
                        b"--helper-protocol\0" as *const u8 as *const i8,
                        b"ntlmssp-client-1\0" as *const u8 as *const i8,
                        b"--use-cached-creds\0" as *const u8 as *const i8,
                        b"--username\0" as *const u8 as *const i8,
                        username,
                ...
                Curl_failf(
                    data,
                    b"Could not execl(). errno %d: %s\0" as *const u8
                        as *const i8,
                    *__errno_location(),
                    Curl_strerror(
                        *__errno_location(),
            ...
            close(sockfds[1 as i32 as usize]);
            (*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_socket = sockfds[0 as i32 as usize];
            (*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_pid = child_pid;
            Curl_cfree.expect("non-null function pointer")(domain as *mut libc::c_void);
            Curl_cfree
                .expect(
                    "non-null function pointer",
                )(ntlm_auth_alloc as *mut libc::c_void);
            return CURLE_OK;
        }
    }
    Curl_cfree.expect("non-null function pointer")(domain as *mut libc::c_void);
    Curl_cfree.expect("non-null function pointer")(ntlm_auth_alloc as *mut libc::c_void);
    return CURLE_REMOTE_ACCESS_DENIED;
}
unsafe extern "C" fn ntlm_wb_response<'a1>(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut ntlm: Option<&'a1 mut crate::src::lib::http2::ntlmdata>,
    mut input: * const i8,
    mut state: u32,
    ...
    let mut len_in: u64 = strlen(input);
    let mut len_out: u64 = 0 as i32 as size_t;
    let mut b: crate::src::lib::http2::dynbuf = dynbuf {
        bufr: 0 as *mut i8,
        leng: 0,
        allc: 0,
        toobig: 0,
    ...
    let mut buf: * mut u8 = (*data).state.buffer as *mut u8;
    Curl_dyn_init(&mut b, 100000 as i32 as size_t);
    loop {
        if !(len_in > 0 as i32 as u64) {
            current_block = 11812396948646013369;
            break;
        }
        let mut written: i64 = send(
            (*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_socket,
        ...
        if written == -(1 as i32) as i64 {
            if !(*__errno_location() == 4 as i32) {
                current_block = 119169402052537339;
                break;
            }
        } else {
            input = input.offset(written as isize);
            len_in = (len_in as u64).wrapping_sub(written as u64)
                as size_t as size_t;
        }
    }
    match current_block {
        11812396948646013369 => {
            loop {
                let mut size: i64 = recv(
                    (*(borrow_mut(&mut ntlm)).unwrap()).ntlm_auth_hlpr_socket,
                    buf as *mut libc::c_void,
                    (*data).set.buffer_size as size_t,
                    0 as i32,
                );
                if size == -(1 as i32) as i64 {
                    if !(*__errno_location() == 4 as i32) {
                        current_block = 119169402052537339;
                        break;
                    }
                    ...
                    if Curl_dyn_addn(&mut b, buf as *const libc::c_void, size as size_t)
                        as u64 != 0
                    {
                        current_block = 119169402052537339;
                        break;
                    }
                    len_out = Curl_dyn_len(&mut b);
                    ptr = Curl_dyn_ptr(&mut b);
                    if !(len_out != 0
                        && *ptr
                            .offset(
                                len_out.wrapping_sub(1 as i32 as u64)
                                    as isize,
                            ) as i32 == '\n' as i32)
                    {
                        continue;
            ...
            match current_block {
                ...
                _ => {
                    if !(state as u32
                        == NTLMSTATE_TYPE1 as i32 as u32
                        && len_out == 3 as i32 as u64
                        && *ptr.offset(0 as i32 as isize) as i32
                            == 'P' as i32
                        && *ptr.offset(1 as i32 as isize) as i32
                            == 'W' as i32)
                    {
                        if !(len_out < 4 as i32 as u64) {
                            if !(state as u32
                                == NTLMSTATE_TYPE1 as i32 as u32
                                && (*ptr.offset(0 as i32 as isize) as i32
                                    != 'Y' as i32
                                ...
                                if !(state as u32
                                    ...
                                    && (*ptr.offset(0 as i32 as isize) as i32
                                        ...
                                        || *ptr.offset(2 as i32 as isize) as i32
                                            != ' ' as i32))
                                {
                                    let mut fresh2 = &mut ((*(borrow_mut(&mut ntlm)).unwrap()).response);
                                    *fresh2 = Curl_cstrdup
                                        .expect(
                                            "non-null function pointer",
                                        )(ptr.offset(3 as i32 as isize));
                                    Curl_dyn_free(&mut b);
                                    if ((*(borrow_mut(&mut ntlm)).unwrap()).response).is_null() {
                                        return CURLE_OUT_OF_MEMORY;
                                    }
                                    return CURLE_OK;
                                }
...
#[no_mangle]
pub unsafe extern "C" fn Curl_input_ntlm_wb(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut conn: * mut crate::src::lib::http2::connectdata,
    mut proxy: bool,
    mut header: * const i8,
) -> u32 {
    let mut ntlm: * mut crate::src::lib::http2::ntlmdata = if proxy as i32 != 0 {
        &mut (*conn).proxyntlm
    } else {
        &mut (*conn).ntlm
    };
    let mut state: * mut u32 = if proxy as i32 != 0 {
    ...
    header = header
        .offset(strlen(b"NTLM\0" as *const u8 as *const i8) as isize);
    ...
    if *header != 0 {
        let mut fresh3 = &mut ((*ntlm).challenge);
        *fresh3 = Curl_cstrdup.expect("non-null function pointer")(header);
        if ((*ntlm).challenge).is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        *state = NTLMSTATE_TYPE2;
    } else {
        if *state as u32 == NTLMSTATE_LAST as i32 as u32 {
            Curl_infof(
                data,
                b"NTLM auth restarted\0" as *const u8 as *const i8,
            );
            Curl_http_auth_cleanup_ntlm_wb(conn);
        } else if *state as u32
                == NTLMSTATE_TYPE3 as i32 as u32
            {
            Curl_infof(
                data,
                b"NTLM handshake rejected\0" as *const u8 as *const i8,
            );
            Curl_http_auth_cleanup_ntlm_wb(conn);
...
#[no_mangle]
pub unsafe extern "C" fn Curl_output_ntlm_wb(
    mut data: * mut crate::src::lib::http2::Curl_easy,
    mut conn: * mut crate::src::lib::http2::connectdata,
    mut proxy: bool,
) -> u32 {
    let mut allocuserpwd: Option<&'_ mut * mut i8> = Option::<&'_ mut * mut i8>::None;
    let mut userp: * const i8 = 0 as *const i8;
    let mut ntlm: Option<&'_ mut crate::src::lib::http2::ntlmdata> = Option::<&'_ mut crate::src::lib::http2::ntlmdata>::None;
    let mut state: Option<&'_ mut u32> = Option::<&'_ mut u32>::None;
    let mut authp: Option<&'_ mut crate::src::lib::http2::auth> = Option::<&'_ mut crate::src::lib::http2::auth>::None;
    let mut res: u32 = CURLE_OK;
    if proxy {
        allocuserpwd = Some(&mut (*data).state.aptr.proxyuserpwd);
        userp = (*conn).http_proxy.user;
        ntlm = Some(&mut (*conn).proxyntlm);
        state = Some(&mut (*conn).proxy_ntlm_state);
        authp = Some(&mut (*data).state.authproxy);
    } else {
        allocuserpwd = Some(&mut (*data).state.aptr.userpwd);
        userp = (*conn).user;
        ntlm = Some(&mut (*conn).ntlm);
        state = Some(&mut (*conn).http_ntlm_state);
        authp = Some(&mut (*data).state.authhost);
    }
    (*(borrow_mut(&mut authp)).unwrap()).set_done(0 as i32 as bit);
    if userp.is_null() {
        userp = b"\0" as *const u8 as *const i8;
    }
    let mut current_block_56: u64;
    match *(borrow(& state)).unwrap() as u32 {
        2 => {
            let mut input: * mut i8 = curl_maprintf(
                b"TT %s\n\0" as *const u8 as *const i8,
                (*(borrow(& ntlm)).unwrap()).challenge,
            );
            if input.is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
            res = ntlm_wb_response(data, borrow_mut(&mut ntlm), input, *(borrow_mut(&mut state)).unwrap());
            Curl_cfree.expect("non-null function pointer")(input as *mut libc::c_void);
            if res as u64 != 0 {
                return res;
            }
            Curl_cfree
                .expect("non-null function pointer")(*(borrow_mut(&mut allocuserpwd)).unwrap() as *mut libc::c_void);
            *(borrow_mut(&mut allocuserpwd)).unwrap() = curl_maprintf(
                ...
                (*(borrow(& ntlm)).unwrap()).response,
            );
            *(borrow_mut(&mut state)).unwrap() = NTLMSTATE_TYPE3;
            (*(borrow_mut(&mut authp)).unwrap()).set_done(1 as i32 as bit);
            Curl_http_auth_cleanup_ntlm_wb(conn);
            if (*(borrow_mut(&mut allocuserpwd)).unwrap()).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
            current_block_56 = 6721012065216013753;
        }
        3 => {
            *(borrow_mut(&mut state)).unwrap() = NTLMSTATE_LAST;
            current_block_56 = 13041636594629627168;
        }
        4 => {
            current_block_56 = 13041636594629627168;
        }
        1 | _ => {
            res = ntlm_wb_init(data, borrow_mut(&mut ntlm), userp);
            if res as u64 != 0 {
                return res;
            }
            res = ntlm_wb_response(
                data,
                borrow_mut(&mut ntlm),
                b"YR\n\0" as *const u8 as *const i8,
                *(borrow_mut(&mut state)).unwrap(),
            );
            if res as u64 != 0 {
                return res;
            }
            ...
            *(borrow_mut(&mut allocuserpwd)).unwrap() = curl_maprintf(
            ...
            Curl_cfree
                .expect(
                    "non-null function pointer",
                )((*(borrow_mut(&mut ntlm)).unwrap()).response as *mut libc::c_void);
            let mut fresh4 = &mut ((*(borrow_mut(&mut ntlm)).unwrap()).response);
            *fresh4 = 0 as *mut i8;
            if (*(borrow_mut(&mut allocuserpwd)).unwrap()).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
    ...
    match current_block_56 {
        13041636594629627168 => {
            Curl_cfree
                .expect("non-null function pointer")(*(borrow_mut(&mut allocuserpwd)).unwrap() as *mut libc::c_void);
            *(borrow_mut(&mut allocuserpwd)).unwrap() = 0 as *mut i8;
            (*(borrow_mut(&mut authp)).unwrap()).set_done(1 as i32 as bit);
        }
        _ => {}
    }
...
#[no_mangle]
pub unsafe extern "C" fn Curl_http_auth_cleanup_ntlm_wb(mut conn: * mut crate::src::lib::http2::connectdata) {
    ntlm_wb_cleanup(Some(&mut (*conn).ntlm));
    ntlm_wb_cleanup(Some(&mut (*conn).proxyntlm));
}
