...
extern "C" {
    ...
    fn curlx_dyn_reset(s: * mut crate::src::lib::http2::dynbuf);
...
pub unsafe extern "C" fn parseconfig(
    ...
    let mut operation: * mut crate::src::src::tool_cb_dbg::OperationConfig = (*global).last;
    ...
    if !file.is_null() {
        ...
        let mut buf: crate::src::lib::http2::dynbuf = dynbuf {
            bufr: 0 as *mut i8,
            leng: 0,
            allc: 0,
            toobig: 0,
        ...
        while my_get_line(file, &mut buf, Some(&mut fileerror)) {
            let mut res: i32 = 0;
                ...
                match *line as i32 {
                    ...
                    _ => {
                        ...
                        if res == PARAM_NEXT_OPERATION as i32 {
                            ...
                            {
                                let mut fresh1 = &mut ((*operation).next);
                                *fresh1 = malloc(
                                    ::std::mem::size_of::<OperationConfig>() as u64,
                                ) as *mut OperationConfig;
                                if !((*operation).next).is_null() {
                                    config_init((*operation).next);
                                    let mut fresh2 = &mut ((*(*operation).next).global);
                                    *fresh2 = global;
                                    let mut fresh3 = &mut ((*global).last);
                                    *fresh3 = (*operation).next;
                                    let mut fresh4 = &mut ((*(*operation).next).prev);
                                    *fresh4 = operation;
...
unsafe extern "C" fn my_get_line<'a1>(
    mut fp: * mut crate::src::lib::http2::_IO_FILE,
    mut db: * mut crate::src::lib::http2::dynbuf,
    mut error: Option<&'a1 mut bool>,
) -> bool {
    let mut buf: [i8; 4096] = [0; 4096];
    *(borrow_mut(&mut error)).unwrap() = 0 as i32 != 0;
    loop {
        if (fgets(
            buf.as_mut_ptr(),
        ...
        if curlx_dyn_add(db, buf.as_mut_ptr()) as u64 != 0 {
            *(borrow_mut(&mut error)).unwrap() = 1 as i32 != 0;
            return 0 as i32 != 0;
        }
