...
impl std::default::Default for NameValueUnsigned {
    fn default() -> Self { NameValueUnsigned::new() }
}
...
#[no_mangle]
pub static mut transfers: * mut crate::src::src::tool_cb_hdr::per_transfer<'static> = 0 as *const per_transfer
    as *mut per_transfer;
static mut transfersl: * mut crate::src::src::tool_cb_hdr::per_transfer<'static> = 0 as *const per_transfer as *mut per_transfer;
unsafe extern "C" fn add_per_transfer<'a1>(mut per: * mut * mut crate::src::src::tool_cb_hdr::per_transfer<'a1>) -> u32 where 'a1: 'static {
    ...
    if transfers.is_null() {
        ...
        let mut fresh0 = &mut ((*transfersl).next);
        *fresh0 = p;
        let mut fresh1 = &mut ((*p).prev);
        *fresh1 = transfersl;
        transfersl = p;
    }
    *per = p;
...
unsafe extern "C" fn del_per_transfer<'a1, 'a2>(mut per: * mut crate::src::src::tool_cb_hdr::per_transfer<'a1>) -> * mut crate::src::src::tool_cb_hdr::per_transfer<'a2> where 'a2: 'static, 'a2: 'a1, 'a1: 'a2 {
    ...
    p = (*per).prev;
    if !p.is_null() {
        let mut fresh2 = &mut ((*p).next);
        *fresh2 = n;
    } else {
        transfers = n;
    }
    if !n.is_null() {
        let mut fresh3 = &mut ((*n).prev);
        *fresh3 = p;
    } else {
        transfersl = p;
    }
...
unsafe extern "C" fn pre_transfer<'a1>(
    ...
    if !((*per).uploadfile).is_null() && !stdin_upload((*per).uploadfile) {
        ...
        if uploadfilesize != -(1 as i32) as i64 {
            let mut config: * mut crate::src::src::tool_cb_dbg::OperationConfig = (*per).config;
...
unsafe extern "C" fn post_per_transfer<'a1, 'a2, 'a3>(
    mut global: * mut crate::src::src::tool_cb_dbg::GlobalConfig,
    mut per: * mut crate::src::src::tool_cb_hdr::per_transfer<'a1>,
    mut result: u32,
    ...
    let mut outs: Option<&'_ mut crate::src::src::tool_cb_hdr::OutStruct> = Some(&mut (*per).outs);
    let mut curl: * mut crate::src::lib::http2::Curl_easy = (*per).curl;
    let mut config: * mut crate::src::src::tool_cb_dbg::OperationConfig = (*per).config;
    ...
    *(borrow_mut(&mut delay)).unwrap() = 0 as i32 as i64;
    ...
    if result as u64 == 0 && (*config).xattr as i32 != 0
        && (*(borrow(& outs)).unwrap()).fopened as i32 != 0 && !((*(borrow(& outs)).unwrap()).stream).is_null()
    {
        let mut rc: i32 = fwrite_xattr(curl, fileno((*(borrow_mut(&mut outs)).unwrap()).stream));
        if rc != 0 {
            warnf(
                (*config).global,
    ...
    if result as u64 == 0 && ((*(borrow(& outs)).unwrap()).stream).is_null() && (*(borrow(& outs)).unwrap()).bytes == 0 {
    ...
    if !(*(borrow(& outs)).unwrap()).s_isreg && !((*(borrow(& outs)).unwrap()).stream).is_null() {
        let mut rc_0: i32 = fflush((*(borrow_mut(&mut outs)).unwrap()).stream);
        ...
        if retry as u64 != 0 {
            ...
            if RETRY_HTTP as i32 as u32 == retry as u32 {
                ...
                if retry_after != 0 {
                    ...
                    if (*config).retry_maxtime != 0 {
                        let mut seconds: i64 = tvdiff(tvnow(), (*per).retrystart)
                            / 1000 as i32 as i64;
            ...
            match current_block {
                ...
                _ => {
                    ...
                    let mut fresh4 = &mut ((*per).retry_numretries);
                    *fresh4 -= 1;
                    ...
                    if (*(borrow(& outs)).unwrap()).bytes != 0 && !((*(borrow(& outs)).unwrap()).filename).is_null()
                        && !((*(borrow(& outs)).unwrap()).stream).is_null()
                    {
                        ...
                        fflush((*(borrow_mut(&mut outs)).unwrap()).stream);
                        if ftruncate(fileno((*(borrow_mut(&mut outs)).unwrap()).stream), (*(borrow_mut(&mut outs)).unwrap()).init) != 0 {
                            if (*global).showerror != 0 {
                                curl_mfprintf(
                                    (*global).errors,
    ...
    if (*(borrow(& outs)).unwrap()).fopened as i32 != 0 && !((*(borrow(& outs)).unwrap()).stream).is_null() {
        let mut rc_2: i32 = fclose((*(borrow_mut(&mut outs)).unwrap()).stream);
    ...
    if result as u64 == 0 && (*config).remote_time as i32 != 0
        && (*(borrow(& outs)).unwrap()).s_isreg as i32 != 0 && !((*(borrow(& outs)).unwrap()).filename).is_null()
    {
    ...
    if (*per).heads.alloc_filename {
        ...
        let mut fresh5 = &mut ((*per).heads.filename);
        *fresh5 = 0 as *mut i8;
    }
    ...
    if (*per).etag_save.alloc_filename {
        ...
        let mut fresh6 = &mut ((*per).etag_save.filename);
        *fresh6 = 0 as *mut i8;
    }
    curl_easy_cleanup((*per).curl);
    if (*(borrow(& outs)).unwrap()).alloc_filename {
        free((*(borrow_mut(&mut outs)).unwrap()).filename as *mut libc::c_void);
    }
...
unsafe extern "C" fn single_transfer_cleanup(mut config: * mut crate::src::src::tool_cb_dbg::OperationConfig) {
    if !config.is_null() {
        let mut state: Option<&'_ mut crate::src::src::tool_cb_dbg::State> = Some(&mut (*config).state);
        ...
        free((*(borrow_mut(&mut state)).unwrap()).outfiles as *mut libc::c_void);
        let mut fresh8 = &mut ((*(borrow_mut(&mut state)).unwrap()).outfiles);
        *fresh8 = 0 as *mut i8;
        free((*(borrow_mut(&mut state)).unwrap()).httpgetfields as *mut libc::c_void);
        let mut fresh9 = &mut ((*(borrow_mut(&mut state)).unwrap()).httpgetfields);
        *fresh9 = 0 as *mut i8;
        free((*(borrow_mut(&mut state)).unwrap()).uploadfile as *mut libc::c_void);
...
unsafe extern "C" fn single_transfer<'a1>(
    mut global: * mut crate::src::src::tool_cb_dbg::GlobalConfig,
    ...
    let mut state: * mut crate::src::src::tool_cb_dbg::State = &mut (*config).state;
    ...
    *(borrow_mut(&mut added)).unwrap() = 0 as i32 != 0;
    if !((*config).postfields).is_null() {
        ...
        } else if SetHTTPrequest(config, HTTPREQ_SIMPLEPOST, Some(&mut (*config).httpreq)) != 0
            {
            result = CURLE_FAILED_INIT;
        }
        if result as u64 != 0 {
    ...
    if ((*state).urlnode).is_null() {
        let mut fresh14 = &mut ((*state).urlnode);
        ...
        if ((*urlnode).url).is_null() {
            ...
            *fresh17 = (*urlnode).next;
            ...
            if !(*config).globoff && !infiles.is_null() && inglob.is_null() {
                ...
                let mut fresh19 = &mut ((*config).state.inglob);
                *fresh19 = inglob;
            }
            ...
            if !((*state).up == 0 && infiles.is_null()) {
                if ((*state).uploadfile).is_null() {
                    if !inglob.is_null() {
                        result = glob_next_url(Some(&mut (*state).uploadfile), inglob);
                        if result as u32
                        ...
                        let mut fresh20 = &mut ((*state).uploadfile);
            ...
            if (*state).up < (*state).infilenum {
                ...
                let mut outs: Option<&'_ mut crate::src::src::tool_cb_hdr::OutStruct> = Option::<&'_ mut crate::src::src::tool_cb_hdr::OutStruct>::None;
                let mut input: * mut crate::src::src::tool_cb_hdr::InStruct = 0 as *mut InStruct;
                let mut heads: Option<&'_ mut crate::src::src::tool_cb_hdr::OutStruct> = Option::<&'_ mut crate::src::src::tool_cb_hdr::OutStruct>::None;
                let mut etag_save: Option<&'_ mut crate::src::src::tool_cb_hdr::OutStruct> = Option::<&'_ mut crate::src::src::tool_cb_hdr::OutStruct>::None;
                let mut hdrcbdata: Option<&'_ mut crate::src::src::tool_cb_hdr::HdrCbData<'_>> = Option::<&'_ mut crate::src::src::tool_cb_hdr::HdrCbData<'_>>::None;
                ...
                if result as u32 != 0 || curl.is_null() {
                    ...
                    if !((*state).uploadfile).is_null() {
                        let mut fresh21 = &mut ((*per).uploadfile);
                    ...
                    *(borrow_mut(&mut added)).unwrap() = 1 as i32 != 0;
                    let mut fresh22 = &mut ((*per).config);
                    *fresh22 = config;
                    let mut fresh23 = &mut ((*per).curl);
                    *fresh23 = curl;
                    (*per).urlnum = (*urlnode).num as u32;
                    heads = Some(&mut (*per).heads);
                    ...
                    if !((*config).headerfile).is_null() {
                        ...
                        ) != 0
                        {
                            ...
                            if newfile.is_null() {
                                ...
                                let mut fresh25 = &mut ((*(borrow_mut(&mut heads)).unwrap()).filename);
                                *fresh25 = (*config).headerfile;
                                (*(borrow_mut(&mut heads)).unwrap()).s_isreg = 1 as i32 != 0;
                                (*(borrow_mut(&mut heads)).unwrap()).fopened = 1 as i32 != 0;
                                let mut fresh26 = &mut ((*(borrow_mut(&mut heads)).unwrap()).stream);
                                *fresh26 = newfile;
                            }
                        }
                    }
                    hdrcbdata = Some(&mut (*per).hdrcbdata);
                    outs = Some(&mut (*per).outs);
                    input = &mut (*per).input;
                    let mut fresh27 = &mut ((*per).outfile);
                    *fresh27 = 0 as *mut i8;
                    ...
                    *fresh28 = stdout;
                    if !((*config).etag_compare_file).is_null() {
                        ...
                        let mut file: * mut crate::src::lib::http2::_IO_FILE = fopen(
                        ...
                        if file.is_null() && ((*config).etag_save_file).is_null() {
                            ...
                            if header.is_null() {
                                ...
                                add2list(Some(&mut (*config).headers), header);
                    ...
                    etag_save = Some(&mut (*per).etag_save);
                    ...
                    *fresh29 = stdout;
                    if !((*config).etag_save_file).is_null() {
                        ...
                        {
                            let mut newfile_0: * mut crate::src::lib::http2::_IO_FILE = fopen(
                                (*config).etag_save_file,
                                b"wb\0" as *const u8 as *const i8,
                            );
                            if newfile_0.is_null() {
                                ...
                                let mut fresh30 = &mut ((*(borrow_mut(&mut etag_save)).unwrap()).filename);
                                *fresh30 = (*config).etag_save_file;
                                ...
                                let mut fresh31 = &mut ((*(borrow_mut(&mut etag_save)).unwrap()).stream);
                                *fresh31 = newfile_0;
                            }
                        }
                    }
                    if !((*state).urls).is_null() {
                        result = glob_next_url(Some(&mut (*per).this_url), (*state).urls);
                        if result as u64 != 0 {
                            break;
                        }
                    } else if (*state).li == 0 {
                        let mut fresh32 = &mut ((*per).this_url);
                        *fresh32 = strdup((*urlnode).url);
                        ...
                        let mut fresh33 = &mut ((*per).this_url);
                        *fresh33 = 0 as *mut i8;
                    }
                    if ((*per).this_url).is_null() {
                        break;
                    ...
                    if !((*state).outfiles).is_null() {
                        let mut fresh34 = &mut ((*per).outfile);
                            ...
                            ) != 0
                    {
                            ...
                            && *(*config).output_dir as i32 != 0
                        {
                            ...
                            if d.is_null() {
                                ...
                                free((*per).outfile as *mut libc::c_void);
                                let mut fresh35 = &mut ((*per).outfile);
                                *fresh35 = d;
                            }
                        }
                        ...
                        let _ = (*urlnode).flags & (1 as i32) << 2 as i32 != 0
                        ...
                        if (*config).resume_from != 0 {
                            let mut file_0: * mut crate::src::lib::http2::_IO_FILE = fopen(
                                (*per).outfile,
                                b"ab\0" as *const u8 as *const i8,
                            );
                            if file_0.is_null() {
                                ...
                                *fresh36 = file_0;
                                (*(borrow_mut(&mut outs)).unwrap()).init = (*config).resume_from;
                            }
                        } else {
                    ...
                    if !((*per).uploadfile).is_null() && !stdin_upload((*per).uploadfile)
                    {
                        ...
                        if nurl.is_null() {
                            ...
                            let mut fresh39 = &mut ((*per).this_url);
                            *fresh39 = nurl;
                        }
                    ...
                    if output_expected((*per).this_url, (*per).uploadfile) as i32
                        != 0 && !((*(borrow(& outs)).unwrap()).stream).is_null()
                        && isatty(fileno((*(borrow_mut(&mut outs)).unwrap()).stream)) != 0
                    {
                        let mut fresh41 = &mut ((*global).isatty);
                        *fresh41 = 1 as i32 != 0;
                        let mut fresh42 = &mut ((*global).noprogress);
                        *fresh42 = *fresh41;
                        (*per).noprogress = *fresh42;
                    } else {
                        let mut fresh43 = &mut ((*global).noprogress);
                        *fresh43 = orig_noprogress;
                        (*per).noprogress = *fresh43;
                        (*global).isatty = orig_isatty;
                    }
                    if urlnum > 1 as i32 as u64 && !(*global).mute {
                        let mut fresh44 = &mut ((*per).separator_err);
                        *fresh44 = curl_maprintf(
                        ...
                        if separator != 0 {
                            let mut fresh45 = &mut ((*per).separator);
                    ...
                    if !httpgetfields.is_null() {
                        ...
                        if urlbuffer.is_null() {
                            ...
                            free((*per).this_url as *mut libc::c_void);
                            let mut fresh46 = &mut ((*per).this_url);
                            *fresh46 = 0 as *mut i8;
                            let mut fresh47 = &mut ((*per).this_url);
                            *fresh47 = urlbuffer;
                        }
                    }
                    if ((*global).errors).is_null() {
                        let mut fresh48 = &mut ((*global).errors);
                        *fresh48 = stderr;
                    }
                    let _ = (((*per).outfile).is_null()
                        || strcmp(
                            (*per).outfile,
                            b"-\0" as *const u8 as *const i8,
                        ) == 0) && !(*config).use_ascii;
                    ...
                    let mut fresh49 = &mut ((*input).config);
                    *fresh49 = config;
                    ...
                    match (*config).httpreq as u32 {
                        ...
                        3 => {
                            ...
                            let mut fresh50 = &mut ((*config).mimepost);
                            *fresh50 = 0 as *mut curl_mime;
                            result = tool2curlmime(
                                curl,
                                (*config).mimeroot,
                                Some(&mut (*config).mimepost),
                            );
                            if !(result as u64 != 0) {
                        ...
                        if !((*config).cert).is_null() {
                            if ((*config).cert_type).is_null() {
                                if is_pkcs11_uri((*config).cert) {
                                    let mut fresh51 = &mut ((*config).cert_type);
                                    *fresh51 = strdup(
                        ...
                        if !((*config).key).is_null() {
                            if ((*config).key_type).is_null() {
                                if is_pkcs11_uri((*config).key) {
                                    let mut fresh52 = &mut ((*config).key_type);
                                    *fresh52 = strdup(
                                        b"ENG\0" as *const u8 as *const i8,
                                    );
                                }
                        ...
                        if !((*config).proxy_cert).is_null() {
                            if ((*config).proxy_cert_type).is_null() {
                                if is_pkcs11_uri((*config).proxy_cert) {
                                    let mut fresh53 = &mut ((*config).proxy_cert_type);
                                    *fresh53 = strdup(
                                        b"ENG\0" as *const u8 as *const i8,
                                    );
                                }
                        ...
                        if !((*config).proxy_key).is_null() {
                            if ((*config).proxy_key_type).is_null() {
                                if is_pkcs11_uri((*config).proxy_key) {
                                    let mut fresh54 = &mut ((*config).proxy_key_type);
                                    *fresh54 = strdup(
                                        b"ENG\0" as *const u8 as *const i8,
                                    );
                                }
                ...
                free((*urlnode).outfile as *mut libc::c_void);
                let mut fresh64 = &mut ((*urlnode).outfile);
                *fresh64 = 0 as *mut i8;
                free((*urlnode).infile as *mut libc::c_void);
                let mut fresh65 = &mut ((*urlnode).infile);
                *fresh65 = 0 as *mut i8;
                (*urlnode).flags = 0 as i32;
                glob_cleanup((*state).urls);
                let mut fresh66 = &mut ((*state).urls);
                *fresh66 = 0 as *mut URLGlob;
                (*state).urlnum = 0 as i32 as u64;
                free((*state).outfiles as *mut libc::c_void);
                let mut fresh67 = &mut ((*state).outfiles);
                *fresh67 = 0 as *mut i8;
                free((*state).uploadfile as *mut libc::c_void);
                let mut fresh68 = &mut ((*state).uploadfile);
                *fresh68 = 0 as *mut i8;
                if !((*state).inglob).is_null() {
                    glob_cleanup((*state).inglob);
                    let mut fresh69 = &mut ((*state).inglob);
                    *fresh69 = 0 as *mut URLGlob;
                }
                let mut fresh70 = &mut ((*config).state.urlnode);
...
unsafe extern "C" fn add_parallel_transfers<'a1, 'a2>(
    mut global: * mut crate::src::src::tool_cb_dbg::GlobalConfig,
...
unsafe extern "C" fn parallel_transfers(
    ...
    let mut tick: i64 = time(0 as *mut time_t);
    ...
    while mcode as u64 == 0 && (still_running != 0 || more_transfers as i32 != 0)
    {
        ...
        let mut rc: i32 = 0;
        let mut msg: Option<&'_ mut crate::src::lib::http2::CURLMsg> = Option::<&'_ mut crate::src::lib::http2::CURLMsg>::None;
        ...
        loop {
            ...
            if !borrow(& msg).is_none() {
                ...
                let mut delay: i64 = 0;
                let mut ended: * mut crate::src::src::tool_cb_hdr::per_transfer<'_> = 0 as *mut per_transfer;
                let mut easy: * mut crate::src::lib::http2::Curl_easy = (*(borrow_mut(&mut msg)).unwrap()).easy_handle;
                let mut tres: u32 = (*(borrow_mut(&mut msg)).unwrap()).data.result;
                curl_easy_getinfo(
                    easy,
                    CURLINFO_PRIVATE,
        ...
        if wrapitup {
            ...
            if !checkmore {
                let mut tock: i64 = time(0 as *mut time_t);
                if tick != tock {
                    checkmore = 1 as i32 != 0;
                    tick = tock;
                }
...
unsafe extern "C" fn transfer_per_config<'a1>(
    mut global: * mut crate::src::src::tool_cb_dbg::GlobalConfig,
    mut config: * mut crate::src::src::tool_cb_dbg::OperationConfig,
    mut share: * mut crate::src::lib::asyn_thread::Curl_share,
    mut added: Option<&'a1 mut bool>,
    ...
    *(borrow_mut(&mut added)).unwrap() = 0 as i32 != 0;
            ...
            || !((*config).doh_url).is_null() && !(*config).doh_insecure_ok)
    {
        ...
        let mut tls_backend_info: * mut crate::src::lib::http2::curl_tlssessioninfo = 0
            ...
            != CURLSSLBACKEND_SCHANNEL as i32 as u32
        {
            ...
            if !env.is_null() {
                let mut fresh71 = &mut ((*config).cacert);
                *fresh71 = strdup(env);
                if ((*config).cacert).is_null() {
                ...
                if !env.is_null() {
                    let mut fresh72 = &mut ((*config).capath);
                    *fresh72 = strdup(env);
                    if ((*config).capath).is_null() {
                        curl_free(env as *mut libc::c_void);
                        helpf(
                    ...
                    if !env.is_null() {
                        let mut fresh73 = &mut ((*config).cacert);
                        *fresh73 = strdup(env);
                        if ((*config).cacert).is_null() {
                            curl_free(env as *mut libc::c_void);
                            errorf(
    ...
    if result as u64 == 0 {
        result = single_transfer(global, config, share, capath_from_env, borrow_mut(&mut added));
    }
    return result;
}
unsafe extern "C" fn create_transfer<'a1>(
    mut global: * mut crate::src::src::tool_cb_dbg::GlobalConfig,
    mut share: * mut crate::src::lib::asyn_thread::Curl_share,
    mut added: Option<&'a1 mut bool>,
) -> u32 {
    ...
    *(borrow_mut(&mut added)).unwrap() = 0 as i32 != 0;
    while !((*global).current).is_null() {
        result = transfer_per_config(global, (*global).current, share, borrow_mut(&mut added));
        if !(result as u64 == 0 && !*(borrow(& added)).unwrap()) {
            break;
        }
        let mut fresh74 = &mut ((*global).current);
...
unsafe extern "C" fn run_all_transfers(
    ...
    while !per.is_null() {
        let mut retry: bool = false;
        let mut delay: i64 = 0;
        let mut result2: u32 = post_per_transfer(
            global,
            per,
            result,
            Some(&mut retry),
    ...
    if result as u64 == 0 {
        ...
        if res as u64 != 0 {
            ...
            if result as u64 == 0 {
                ...
                let mut operation: * mut crate::src::src::tool_cb_dbg::OperationConfig = (*global).first;
                ...
                let mut fresh76 = &mut ((*global).current);
