...
pub type writeoutvar<'a> = crate::src::src::tool_writeout::writeoutvar<'a>;
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn ourWriteOutJSON<'a1, 'a2>(
    mut stream: * mut crate::src::lib::http2::_IO_FILE,
    mut mappings: * const crate::src::src::tool_writeout::writeoutvar<'a1>,
    mut per: * mut crate::src::src::tool_cb_hdr::per_transfer<'a2>,
    mut per_result: u32,
) where 'a2: 'a1, 'a1: 'a2 {
    let mut i: i32 = 0;
    fputs(b"{\0" as *const u8 as *const i8, stream);
    ...
    while !((*mappings.offset(i as isize)).name).is_null() {
        if ((*mappings.offset(i as isize)).writefunc).is_some()
            && ((*mappings.offset(i as isize)).writefunc)
                ...
                stream,
                (Some(&*mappings.offset(i as isize))).clone(),
                per,
                per_result,
                1 as i32 != 0,
            ) != 0
