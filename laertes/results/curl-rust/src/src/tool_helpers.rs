...
pub extern "C" fn param2text(mut res: i32) -> * const i8 {
    let mut error: u32 = res as ParameterError;
    match error as u32 {
        9 => {
...
#[no_mangle]
pub unsafe extern "C" fn SetHTTPrequest<'a1>(
    mut config: * mut crate::src::src::tool_cb_dbg::OperationConfig,
    mut req: u32,
    mut store: Option<&'a1 mut u32>,
) -> i32 {
    ...
    if *(borrow(& store)).unwrap() as u32 == HTTPREQ_UNSPEC as i32 as u32
        || *(borrow(& store)).unwrap() as u32 == req as u32
    {
        *(borrow_mut(&mut store)).unwrap() = req;
        return 0 as i32;
    }
    warnf(
        (*config).global,
        ...
        reqname[*(borrow(& store)).unwrap() as usize],
    );
    return 1 as i32;
}
#[no_mangle]
