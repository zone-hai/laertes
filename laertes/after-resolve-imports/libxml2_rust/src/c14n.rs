...
pub use crate::src::xpath::xmlXPathNodeSetContains;
...
pub type xmlC14NIsVisibleCallback = Option::<
    unsafe extern "C" fn(*mut libc::c_void, xmlNodePtr, xmlNodePtr) -> i32,
>;
...
unsafe extern "C" fn xmlC14NVisibleNsStackAdd(
    ...
    if ((*cur).nsTab).is_null() && ((*cur).nodeTab).is_null() {
        ...
        *fresh2 = tmp as *mut xmlNsPtr;
        ...
        let fresh3 = &mut ((*cur).nodeTab);
        *fresh3 = tmp as *mut xmlNodePtr;
        (*cur).nsMax = tmpSize;
    }
    let fresh4 = &mut (*((*cur).nsTab).offset((*cur).nsCurEnd as isize));
    *fresh4 = ns;
    let fresh5 = &mut (*((*cur).nodeTab).offset((*cur).nsCurEnd as isize));
    *fresh5 = node;
    let fresh6 = &mut ((*cur).nsCurEnd);
    *fresh6 += 1;
}
...
unsafe extern "C" fn xmlC14NIsXmlNs(mut ns: xmlNsPtr) -> i32 {
    return (!ns.is_null()
        && xmlStrEqual(
            (*ns).prefix,
...
unsafe extern "C" fn xmlC14NIsXmlAttr(mut attr: xmlAttrPtr) -> i32 {
            ...
            if parent_visible != 0 {
                ...
                if !xml_base_attr.is_null() {
                    ...
                    if !xml_base_attr.is_null() {
                        ...
                        let fresh9 = &mut ((*xml_base_attr).next);
...
unsafe extern "C" fn xmlC14NNewCtx(
    ...
    let fresh10 = &mut ((*ctx).doc);
    *fresh10 = doc;
    (*ctx).with_comments = with_comments;
    let fresh11 = &mut ((*ctx).is_visible_callback);
    *fresh11 = is_visible_callback;
    let fresh12 = &mut ((*ctx).user_data);
    *fresh12 = user_data;
    let fresh13 = &mut ((*ctx).buf);
    *fresh13 = buf;
    (*ctx).parent_is_doc = 1 as i32;
    (*ctx).pos = XMLC14N_BEFORE_DOCUMENT_ELEMENT;
    let fresh14 = &mut ((*ctx).ns_rendered);
    *fresh14 = xmlC14NVisibleNsStackCreate();
    ...
    if (*ctx).mode as u32
        == XML_C14N_EXCLUSIVE_1_0 as i32 as u32
    {
        let fresh15 = &mut ((*ctx).inclusive_ns_prefixes);
...
pub unsafe extern "C" fn xmlC14NDocDumpMemory(
    ...
    *doc_txt_ptr = 0 as *mut xmlChar;
    ...
    xmlOutputBufferClose(buf);
    if (*doc_txt_ptr).is_null() && ret >= 0 as i32 {
        xmlC14NErrMemory(
