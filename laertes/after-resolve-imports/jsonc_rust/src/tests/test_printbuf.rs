...
unsafe extern "C" fn test_printbuf_memappend(mut before_resize: *mut i32) {
    let mut pb: *mut printbuf = 0 as *mut printbuf;
    ...
    *before_resize = (*pb).bpos - 1 as i32;
    printf(
    ...
    printbuf_free(pb);
    pb = printbuf_new();
    let mut data: *mut i8 = malloc(*before_resize as u64)
        as *mut i8;
    memset(data as *mut libc::c_void, 'X' as i32, *before_resize as u64);
    ...
    printbuf_free(pb);
    pb = printbuf_new();
    data = malloc((*before_resize + 1 as i32) as u64)
        as *mut i8;
    memset(
    ...
    if (*pb).size - (*pb).bpos > *before_resize + 1 as i32 {
        memcpy(
            ((*pb).buf).offset((*pb).bpos as isize) as *mut libc::c_void,
            data as *const libc::c_void,
            (*before_resize + 1 as i32) as u64,
        );
        (*pb).bpos += *before_resize + 1 as i32;
        *((*pb).buf).offset((*pb).bpos as isize) = '\u{0}' as i32 as i8;
    } else {
        printbuf_memappend(pb, data, *before_resize + 1 as i32);
    }
    printf(
...
unsafe extern "C" fn test_sprintbuf(mut before_resize: i32) {
    ...
    printf(
        b"%s: starting test\n\0" as *const u8 as *const i8,
        (*::std::mem::transmute::<&[u8; 15], &[i8; 15]>(b"test_sprintbuf\0"))
            .as_ptr(),
    );
    pb = printbuf_new();
    printf(b"Buffer length: %d\n\0" as *const u8 as *const i8, (*pb).bpos);
