...
pub type json_c_visit_userfunc = crate::src::json_visit::json_c_visit_userfunc;
#[derive(Copy, Clone)]
#[repr(C)]
...
unsafe extern "C" fn test_incremental_parse() {
    ...
    while !(incremental_steps[ii as usize].string_to_parse).is_null() {
        ...
        let mut length: i32 = (*step).length;
        let mut expected_char_offset: size_t = 0;
        json_tokener_set_flags(tok, (*step).tok_flags);
        if length == -(1 as i32) {
            length = strlen((*step).string_to_parse) as i32;
        }
        if (*step).char_offset == -(1 as i32) {
            ...
            != json_tokener_success as i32 as u32
        {
            ...
            } else if jerr as u32 != (*step).expected_error as u32 {
                printf(
        ...
        if (*step).reset_tokener & 1 as i32 != 0 {
