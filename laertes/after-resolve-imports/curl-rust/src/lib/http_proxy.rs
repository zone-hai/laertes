...
pub unsafe extern "C" fn Curl_proxy_connect(
        ...
        && ((*conn).bits).httpproxy() as i32 != 0
    {
        ...
        let fresh0 = &mut ((*data).state.aptr.proxyuserpwd);
        *fresh0 = 0 as *mut i8;
    }
    return CURLE_OK;
}
...
unsafe extern "C" fn connect_init(
    ...
    if !reinit {
        ...
        let fresh1 = &mut ((*conn).connect_state);
        *fresh1 = s;
        Curl_dyn_init(&mut (*s).rcvbuf, 16384 as i32 as size_t);
        let fresh2 = &mut ((*s).prot_save);
        *fresh2 = (*data).req.p.http;
        let fresh3 = &mut ((*data).req.p.http);
...
unsafe extern "C" fn connect_done(mut data: *mut Curl_easy) {
    ...
    if (*s).tunnel_state as u32 != TUNNEL_EXIT as i32 as u32 {
        ...
        let fresh4 = &mut ((*data).req.p.http);
        *fresh4 = (*s).prot_save;
        let fresh5 = &mut ((*s).prot_save);
...
unsafe extern "C" fn CONNECT(
    ...
    loop {
            ...
            == HTTPSEND_REQUEST as i32 as u32
        {
            if (*s).nsend == 0 {
                ...
                let fresh8 = &mut ((*k).upload_fromhere);
        ...
        while (*s).keepon as u64 != 0 {
            ...
            if result as u64 != 0 {
                ...
                let fresh13 = &mut ((*data).info.header_size);
    ...
    if (*data).info.httpproxycode / 100 as i32 != 2 as i32 {
        if (*s).close_connection() as i32 != 0 && !((*data).req.newurl).is_null()
        {
            ...
            let fresh15 = &mut ((*data).req.newurl);
            *fresh15 = 0 as *mut i8;
    ...
    let fresh16 = &mut ((*data).state.aptr.proxyuserpwd);
    *fresh16 = 0 as *mut i8;
    let fresh17 = &mut ((*data).state.aptr.proxyuserpwd);
    *fresh17 = 0 as *mut i8;
    let fresh18 = &mut ((*data).state.authproxy);
    (*fresh18).set_done(1 as i32 as bit);
    let fresh19 = &mut ((*data).state.authproxy);
    ...
    let fresh20 = &mut ((*data).req);
    (*fresh20).set_ignorebody(0 as i32 as bit);
    let fresh21 = &mut ((*conn).bits);
...
pub unsafe extern "C" fn Curl_connect_free(mut data: *mut Curl_easy) {
    ...
    if !s.is_null() {
        ...
        let fresh22 = &mut ((*conn).connect_state);
