...
pub use crate::src::lib::warnless::curlx_uztosz;
...
pub static mut Curl_handler_gopher: Curl_handler = unsafe {
    {
        let mut init = Curl_handler {
            scheme: b"GOPHER\0" as *const u8 as *const i8,
            ...
            attach: None,
            defport: 70 as i32,
            protocol: ((1 as i32) << 25 as i32) as u32,
            family: ((1 as i32) << 25 as i32) as u32,
...
pub static mut Curl_handler_gophers: Curl_handler = unsafe {
    {
        let mut init = Curl_handler {
            scheme: b"GOPHERS\0" as *const u8 as *const i8,
            setup_connection: None,
            do_it: Some(
                gopher_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            ...
            attach: None,
            defport: 70 as i32,
            protocol: ((1 as i32) << 29 as i32) as u32,
            family: ((1 as i32) << 25 as i32) as u32,
...
unsafe extern "C" fn gopher_connecting(
    ...
    let mut result: CURLcode = Curl_ssl_connect(data, conn, 0 as i32);
    if result as u64 != 0 {
...
unsafe extern "C" fn gopher_do(
    ...
    let mut gopherpath: *mut i8 = 0 as *mut i8;
    let mut path: *mut i8 = (*data).state.up.path;
    let mut query: *mut i8 = (*data).state.up.query;
    let mut sel: *mut i8 = 0 as *mut i8;
    let mut sel_org: *mut i8 = 0 as *mut i8;
    ...
    *done = 1 as i32 != 0;
    ...
    if strlen(gopherpath) <= 2 as i32 as u64 {
        sel = b"\0" as *const u8 as *const i8 as *mut i8;
        len = strlen(sel);
        Curl_cfree.expect("non-null function pointer")(gopherpath as *mut libc::c_void);
    } else {
        let mut newp: *mut i8 = 0 as *mut i8;
        newp = gopherpath;
        newp = newp.offset(2 as i32 as isize);
    ...
    k = curlx_uztosz(len);
    while !(strlen(sel) < 1 as i32 as u64) {
        result = Curl_write(
            data,
            sockfd,
        ...
        if k < 1 as i32 as i64 {
            break;
        }
        timeout_ms = Curl_timeleft(data, 0 as *mut curltime, 0 as i32 != 0);
        if timeout_ms < 0 as i32 as i64 {
            result = CURLE_OPERATION_TIMEDOUT;
            break;
        } else {
            if timeout_ms == 0 {
                timeout_ms = 0x7fffffffffffffff as i64;
            }
    ...
    if result as u64 != 0 {
        Curl_failf(
            data,
            b"Failed sending Gopher request\0" as *const u8 as *const i8,
        );
        return result;
    }
    result = Curl_client_write(
