...
unsafe extern "C" fn getrpacketevent(mut packet: *const tftp_packet) -> u16 {
    return ((*((*packet).data).offset(0 as i32 as isize) as i32)
        << 8 as i32
        | *((*packet).data).offset(1 as i32 as isize) as i32)
        as u16;
}
unsafe extern "C" fn getrpacketblock(mut packet: *const tftp_packet) -> u16 {
    return ((*((*packet).data).offset(2 as i32 as isize) as i32)
        << 8 as i32
        | *((*packet).data).offset(3 as i32 as isize) as i32)
        as u16;
}
...
unsafe extern "C" fn tftp_option_get(
    ...
    *value = &*buf
        .offset(
            ((strlen
                as unsafe extern "C" fn(*const i8) -> u64)(*option))
        ...
        0 | 7 => {
            ...
            *fresh0 += 1;
            ...
            if ((*data).set).upload() != 0 {
                ...
                let fresh1 = &mut ((*(*state).data).req.upload_fromhere);
...
unsafe extern "C" fn tftp_rx(
    ...
    match event as i32 {
        3 => {
            rblock = getrpacketblock(&mut (*state).rpacket) as i32;
...
unsafe extern "C" fn tftp_tx(
    ...
    match event as i32 {
        4 | 6 => {
            if event as i32 == TFTP_EVENT_ACK as i32 {
                let mut rblock: i32 = getrpacketblock(&mut (*state).rpacket)
                    as i32;
            ...
            let fresh5 = &mut ((*(*state).data).req.upload_fromhere);
            *fresh5 = ((*state).spacket.data as *mut i8)
            ...
            loop {
                ...
                let fresh6 = &mut ((*(*state).data).req.upload_fromhere);
            ...
            let fresh7 = &mut ((*k).writebytecount);
            *fresh7 += (*state).sbytes as i64;
            Curl_pgrsSetUploadCounter(data, (*k).writebytecount);
        }
        7 => {
            let fresh8 = &mut ((*state).retries);
            *fresh8 += 1;
            Curl_infof(
                data,
                b"Timeout waiting for block %d ACK.  Retries = %d\0" as *const u8
 ...
 extern "C" fn tftp_translate_code(mut error: tftp_error_t) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
...
unsafe extern "C" fn tftp_disconnect(
    ...
    if !state.is_null() {
        ...
        let fresh9 = &mut ((*state).rpacket.data);
        *fresh9 = 0 as *mut u8;
        Curl_cfree
            .expect(
                "non-null function pointer",
            )((*state).spacket.data as *mut libc::c_void);
        let fresh10 = &mut ((*state).spacket.data);
        *fresh10 = 0 as *mut u8;
...
unsafe extern "C" fn tftp_connect(
    ...
    blksize = 512 as i32;
    let fresh11 = &mut ((*conn).proto.tftpc);
    *fresh11 = Curl_ccalloc
    ...
    if ((*state).rpacket.data).is_null() {
        let fresh12 = &mut ((*state).rpacket.data);
    ...
    if ((*state).spacket.data).is_null() {
        let fresh13 = &mut ((*state).spacket.data);
...
unsafe extern "C" fn tftp_receive_packet(mut data: *mut Curl_easy) -> CURLcode {
    ...
    if (*state).rbytes < 4 as i32 {
        ...
        let mut event: u16 = getrpacketevent(&mut (*state).rpacket);
        ...
        match (*state).event as i32 {
            3 => {
                if (*state).rbytes > 4 as i32
                    && (*state).block as i32 + 1 as i32
                        & 0xffff as i32
                        == getrpacketblock(&mut (*state).rpacket) as i32
                {
                    ...
                    let fresh16 = &mut ((*k).bytecount);
                    *fresh16 += ((*state).rbytes - 4 as i32) as i64;
                    Curl_pgrsSetDownloadCounter(data, (*k).bytecount);
                }
            }
            5 => {
                let mut error: u16 = getrpacketblock(&mut (*state).rpacket);
                let mut str: *mut i8 = ((*state).rpacket.data
                    as *mut i8)
                    .offset(4 as i32 as isize);
                let mut strn: size_t = ((*state).rbytes - 4 as i32) as size_t;
