...
pub use crate::src::src::tool_hugehelp::internal_state;
...
unsafe extern "C" fn gzip_unencode_write(
    ...
    match (*zp).zlib_init as u32 {
        ...
        3 => {
            let fresh17 = &mut ((*z).next_in);
            *fresh17 = buf as *mut Bytef;
            (*z).avail_in = nbytes as uInt;
            return process_trailer(data, zp);
        }
        5 | _ => {
            let fresh18 = &mut ((*z).next_in);
...
unsafe extern "C" fn new_unencoding_writer(
    ...
    if !writer.is_null() {
        let fresh21 = &mut ((*writer).handler);
        *fresh21 = handler;
        let fresh22 = &mut ((*writer).downstream);
        *fresh22 = downstream;
...
pub unsafe extern "C" fn Curl_unencode_cleanup(mut data: *mut Curl_easy) {
    ...
    while !writer.is_null() {
        let fresh23 = &mut ((*k).writer_stack);
    ...
    loop {
            ...
            ) != 0
        {
            ...
            if ((*k).writer_stack).is_null() {
                let fresh24 = &mut ((*k).writer_stack);
            ...
            let fresh25 = &mut ((*k).writer_stack);
            *fresh25 = writer;
        }
        if !(*enclist != 0) {
            break;
