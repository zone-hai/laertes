...
unsafe extern "C" fn Curl_shuffle_addr(
    ...
    let num_addrs: i32 = Curl_num_addresses(*addr);
    if num_addrs > 1 as i32 {
        ...
        if !nodes.is_null() {
            ...
            let rnd_size: size_t = (num_addrs as u64)
                .wrapping_mul(::std::mem::size_of::<u32>() as u64);
            let fresh6 = &mut (*nodes.offset(0 as i32 as isize));
            *fresh6 = *addr;
            i = 1 as i32;
            while i < num_addrs {
                let fresh7 = &mut (*nodes.offset(i as isize));
...
pub unsafe extern "C" fn Curl_cache_addr(
    ...
    *fresh12 = addr;
    ...
    dns = dns2;
    let fresh13 = &mut ((*dns).inuse);
    *fresh13 += 1;
    return dns;
}
unsafe extern "C" fn get_localhost6(mut port: i32) -> *mut Curl_addrinfo {
    let mut ca: *mut Curl_addrinfo = 0 as *mut Curl_addrinfo;
    let ss_size: size_t = ::std::mem::size_of::<sockaddr_in6>() as u64;
    let hostlen: size_t = strlen(b"localhost\0" as *const u8 as *const i8);
    ...
    let fresh14 = &mut ((*ca).ai_next);
    *fresh14 = 0 as *mut Curl_addrinfo;
    let fresh15 = &mut ((*ca).ai_addr);
    *fresh15 = (ca as *mut i8)
    ...
    let fresh16 = &mut ((*ca).ai_canonname);
    *fresh16 = ((*ca).ai_addr as *mut i8).offset(ss_size as isize);
    strcpy((*ca).ai_canonname, b"localhost\0" as *const u8 as *const i8);
    return ca;
}
unsafe extern "C" fn get_localhost(mut port: i32) -> *mut Curl_addrinfo {
    let mut ca: *mut Curl_addrinfo = 0 as *mut Curl_addrinfo;
    let ss_size: size_t = ::std::mem::size_of::<sockaddr_in>() as u64;
    let hostlen: size_t = strlen(b"localhost\0" as *const u8 as *const i8);
    ...
    *fresh17 = (ca as *mut i8)
    ...
    let fresh18 = &mut ((*ca).ai_canonname);
    *fresh18 = ((*ca).ai_addr as *mut i8).offset(ss_size as isize);
    strcpy((*ca).ai_canonname, b"localhost\0" as *const u8 as *const i8);
    let fresh19 = &mut ((*ca).ai_next);
...
pub unsafe extern "C" fn Curl_resolv_timeout(
    ...
    *entry = 0 as *mut Curl_dns_entry;
    ...
    rc = Curl_resolv(data, hostname, port, 1 as i32 != 0, entry);
    return rc;
}
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn Curl_mk_dnscache(mut hash: *mut Curl_hash) -> i32 {
...
pub unsafe extern "C" fn Curl_loadhostpairs(mut data: *mut Curl_easy) -> CURLcode {
    ...
    let fresh27 = &mut ((*data).state.resolve);
...
pub unsafe extern "C" fn Curl_once_resolved(
    ...
    if !((*data).state.async_0.dns).is_null() {
        let fresh28 = &mut ((*conn).dns_entry);
        *fresh28 = (*data).state.async_0.dns;
        let fresh29 = &mut ((*data).state.async_0.dns);
