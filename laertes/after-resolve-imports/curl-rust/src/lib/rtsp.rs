...
unsafe extern "C" fn rtsp_setup_connection(
    ...
    rtsp = Curl_ccalloc
        ...
        as *mut RTSP;
    let fresh0 = &mut ((*data).req.p.rtsp);
    *fresh0 = rtsp;
    if rtsp.is_null() {
...
unsafe extern "C" fn rtsp_disconnect(
    ...
    let fresh1 = &mut ((*conn).proto.rtspc.rtp_buf);
    *fresh1 = 0 as *mut i8;
    return CURLE_OK;
}
    ...
    *done = 1 as i32 != 0;
        ...
        && p_transport.is_null()
    {
        if !((*data).set.str_0[STRING_RTSP_TRANSPORT as i32 as usize]).is_null()
        {
            ...
            let fresh6 = &mut ((*data).state.aptr.rtsp_transport);
            *fresh6 = 0 as *mut i8;
            let fresh7 = &mut ((*data).state.aptr.rtsp_transport);
    ...
    if rtspreq as u32 == RTSPREQ_DESCRIBE as i32 as u32 {
            ...
            && !((*data).set.str_0[STRING_ENCODING as i32 as usize]).is_null()
        {
            ...
            let fresh8 = &mut ((*data).state.aptr.accept_encoding);
            *fresh8 = 0 as *mut i8;
            let fresh9 = &mut ((*data).state.aptr.accept_encoding);
        ...
        .is_null() && !((*data).state.aptr.uagent).is_null()
    {
        ...
        let fresh10 = &mut ((*data).state.aptr.uagent);
        *fresh10 = 0 as *mut i8;
        let fresh11 = &mut ((*data).state.aptr.uagent);
        *fresh11 = 0 as *mut i8;
    ...
    let fresh12 = &mut ((*data).state.aptr.ref_0);
    *fresh12 = 0 as *mut i8;
    if !((*data).state.referer).is_null()
        && (Curl_checkheaders(data, b"Referer\0" as *const u8 as *const i8))
            .is_null()
    {
        let fresh13 = &mut ((*data).state.aptr.ref_0);
        *fresh13 = curl_maprintf(
            b"Referer: %s\r\n\0" as *const u8 as *const i8,
            (*data).state.referer,
        );
    } else {
        let fresh14 = &mut ((*data).state.aptr.ref_0);
        *fresh14 = 0 as *mut i8;
    }
                ...
                | RTSPREQ_RECORD as i32) as u32 != 0
    {
            ...
            .is_null() && !((*data).state.range).is_null()
        {
            ...
            let fresh15 = &mut ((*data).state.aptr.rangeline);
            *fresh15 = 0 as *mut i8;
            let fresh16 = &mut ((*data).state.aptr.rangeline);
    ...
    let fresh17 = &mut ((*data).state.aptr.userpwd);
    *fresh17 = 0 as *mut i8;
    let fresh18 = &mut ((*data).state.aptr.userpwd);
    *fresh18 = 0 as *mut i8;
    if result as u64 != 0 {
        return result;
    }
    ...
    let fresh22 = &mut ((*data).state.rtsp_next_client_CSeq);
    *fresh22 += 1;
...
unsafe extern "C" fn rtsp_rtp_readwrite(
    ...
    if !((*rtspc).rtp_buf).is_null() {
        ...
        if newptr.is_null() {
            let fresh23 = &mut ((*rtspc).rtp_buf);
            *fresh23 = 0 as *mut i8;
            (*rtspc).rtp_bufsize = 0 as i32 as ssize_t;
            return CURLE_OUT_OF_MEMORY;
        }
        let fresh24 = &mut ((*rtspc).rtp_buf);
        *fresh24 = newptr;
        memcpy(
        ...
        let fresh25 = &mut ((*rtspc).rtp_bufsize);
        *fresh25 += *nread;
        rtp = (*rtspc).rtp_buf;
                ...
                *readmore = 1 as i32 != 0;
                ...
                if result as u64 != 0 {
                    ...
                    *readmore = 0 as i32 != 0;
                    ...
                    let fresh26 = &mut ((*rtspc).rtp_buf);
                    *fresh26 = 0 as *mut i8;
                    let fresh27 = &mut ((*rtspc).rtp_buf);
                    *fresh27 = 0 as *mut i8;
                    (*rtspc).rtp_bufsize = 0 as i32 as ssize_t;
                    return result;
                }
        ...
        && *rtp.offset(0 as i32 as isize) as i32 == '$' as i32
    {
        ...
        if scratch.is_null() {
            ...
            let fresh28 = &mut ((*rtspc).rtp_buf);
            *fresh28 = 0 as *mut i8;
            let fresh29 = &mut ((*rtspc).rtp_buf);
            *fresh29 = 0 as *mut i8;
            (*rtspc).rtp_bufsize = 0 as i32 as ssize_t;
            return CURLE_OUT_OF_MEMORY;
        }
        ...
        let fresh30 = &mut ((*rtspc).rtp_buf);
        *fresh30 = 0 as *mut i8;
        let fresh31 = &mut ((*rtspc).rtp_buf);
        *fresh31 = scratch;
        (*rtspc).rtp_bufsize = rtp_dataleft;
        *nread = 0 as i32 as ssize_t;
        return CURLE_OK;
    }
    let fresh32 = &mut ((*k).str_0);
    *fresh32 = (*fresh32).offset((*nread - rtp_dataleft) as isize);
    rtp_dataleft > 0 as i32 as i64;
    *nread = rtp_dataleft;
    Curl_cfree
        .expect("non-null function pointer")((*rtspc).rtp_buf as *mut libc::c_void);
    let fresh33 = &mut ((*rtspc).rtp_buf);
    *fresh33 = 0 as *mut i8;
    let fresh34 = &mut ((*rtspc).rtp_buf);
    *fresh34 = 0 as *mut i8;
    (*rtspc).rtp_bufsize = 0 as i32 as ssize_t;
    return CURLE_OK;
}
