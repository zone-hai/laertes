...
unsafe extern "C" fn pop3_endofresp(
    ...
    if (*pop3c).state as u32 == POP3_CAPA as i32 as u32 {
...
unsafe extern "C" fn pop3_perform_capa(
    ...
    let mut pop3c: *mut pop3_conn = &mut (*conn).proto.pop3c;
    (*pop3c).sasl.authmechs = 0 as i32 as u16;
    (*pop3c).sasl.authused = 0 as i32 as u16;
    (*pop3c).tls_supported = 0 as i32 != 0;
...
unsafe extern "C" fn pop3_perform_apop(
    ...
    Curl_MD5_update(
        ctxt,
        (*pop3c).apoptimestamp as *const u8,
...
unsafe extern "C" fn pop3_state_capa_resp(
    ...
    if pop3code == '*' as i32 {
        ...
        {
            (*pop3c).tls_supported = 1 as i32 != 0;
        ...
        } else if pop3code == '+' as i32 && (*pop3c).tls_supported as i32 != 0 {
...
unsafe extern "C" fn pop3_multi_statemach(
        ...
        && !(*pop3c).ssldone
    {
        ...
        if result as u32 != 0 || !(*pop3c).ssldone {
            return result;
        }
    }
    ...
    *done = if (*pop3c).state as u32 == POP3_STOP as i32 as u32
    {
        1 as i32
    } else {
        0 as i32
...
unsafe extern "C" fn pop3_block_statemach(
    ...
    let mut result: CURLcode = CURLE_OK;
    let mut pop3c: *mut pop3_conn = &mut (*conn).proto.pop3c;
    while (*pop3c).state as u32 != POP3_STOP as i32 as u32
        && result as u64 == 0
    {
        result = Curl_pp_statemach(
            data,
...
unsafe extern "C" fn pop3_disconnect(
    ...
    Curl_sasl_cleanup(conn, (*pop3c).sasl.authused as u32);
    Curl_cfree
        .expect(
            "non-null function pointer",
        )((*pop3c).apoptimestamp as *mut libc::c_void);
...
unsafe extern "C" fn pop3_parse_url_options(mut conn: *mut connectdata) -> CURLcode {
    ...
    (*pop3c).sasl.resetprefs = 1 as i32 != 0;
    while result as u64 == 0 && !ptr.is_null() && *ptr as i32 != 0 {
        ...
        ) != 0
        {
                ...
                ) != 0
            {
                ...
                (*pop3c).sasl.prefmech = 0 as i32 as u16;
                result = CURLE_OK;
            }
        } else {
    ...
    if (*pop3c).preftype != ((1 as i32) << 1 as i32) as u32 {
        match (*pop3c).sasl.prefmech as i32 {
            0 => {
                (*pop3c).preftype = 0 as i32 as u32;
            }
            65503 => {
                (*pop3c).preftype = !(0 as u32);
            }
...
pub unsafe extern "C" fn Curl_pop3_write(
    ...
    let mut last: size_t = 0 as i32 as size_t;
    let mut i: size_t = 0;
    i = 0 as i32 as size_t;
    while i < nread {
        ...
        match *str.offset(i as isize) as i32 {
            13 => {
                ...
                } else if (*pop3c).eob == 3 as i32 as u64 {
                    let ref mut fresh15 = (*pop3c).eob;
                    *fresh15 = (*fresh15).wrapping_add(1);
                } else {
                    (*pop3c).eob = 1 as i32 as size_t;
                }
            }
            10 => {
                if (*pop3c).eob == 1 as i32 as u64
                    || (*pop3c).eob == 4 as i32 as u64
                {
                    let ref mut fresh16 = (*pop3c).eob;
                    *fresh16 = (*fresh16).wrapping_add(1);
                } else {
                    (*pop3c).eob = 0 as i32 as size_t;
                }
            }
            46 => {
                if (*pop3c).eob == 2 as i32 as u64 {
