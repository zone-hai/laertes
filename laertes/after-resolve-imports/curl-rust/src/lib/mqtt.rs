...
unsafe extern "C" fn mqtt_send(
    ...
    if len != n as size_t {
        let mut nsend: size_t = len.wrapping_sub(n as u64);
...
unsafe extern "C" fn add_passwd(
    ...
    let conn_flags_pos: size_t = (remain_pos + 8 as i32) as size_t;
...
unsafe extern "C" fn mqtt_connect(mut data: *mut Curl_easy) -> CURLcode {
    ...
    let mut packetlen: size_t = 0 as i32 as size_t;
    let mut payloadlen: size_t = 0 as i32 as size_t;
    let mut start_user: size_t = 0 as i32 as size_t;
    let mut start_pwd: size_t = 0 as i32 as size_t;
    let mut client_id: [i8; 13] = *::std::mem::transmute::<
        &[u8; 13],
        &mut [i8; 13],
    >(b"curl\0\0\0\0\0\0\0\0\0");
    let clen: size_t = strlen(b"curl\0" as *const u8 as *const i8);
    ...
    if !(result as u64 != 0) {
        ...
        if nread < 3 as i32 as i64 {
            result = CURLE_WEIRD_SERVER_REPLY;
                ...
                != (*mqtt).packetid >> 8 as i32
                    & 0xff as i32 as u32
                || readbuf[1 as i32 as usize] as u32
                    != (*mqtt).packetid & 0xff as i32 as u32
...
unsafe extern "C" fn mqtt_decode_len(
    ...
    let mut mult: size_t = 1 as i32 as size_t;
    let mut i: size_t = 0;
...
unsafe extern "C" fn mqtt_read_publish(
    ...
    match current_block {
        4488286894823169796 => {
            ...
            result = Curl_read(data, sockfd, pkt as *mut i8, rest, &mut nread);
            if result as u64 != 0 {
