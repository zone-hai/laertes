...
unsafe extern "C" fn mqtt_setup_conn(
    ...
    let fresh0 = &mut ((*data).req.p.mqtt);
    *fresh0 = mq;
    return CURLE_OK;
}
unsafe extern "C" fn mqtt_send(
    ...
    if len != n as size_t {
        ...
        let fresh1 = &mut ((*mq).sendleftovers);
        *fresh1 = sendleftovers;
        (*mq).nsend = nsend;
    } else {
        let fresh2 = &mut ((*mq).sendleftovers);
        *fresh2 = 0 as *mut i8;
...
unsafe extern "C" fn add_passwd(
    ...
    let fresh3 = &mut (*pkt.offset(conn_flags_pos as isize));
...
unsafe extern "C" fn add_user(
    ...
    let fresh4 = &mut (*pkt.offset(conn_flags_pos as isize));
...
unsafe extern "C" fn mqtt_connect(mut data: *mut Curl_easy) -> CURLcode {
    ...
    let fresh5 = &mut ((*data).state.aptr.user);
    *fresh5 = 0 as *mut i8;
    Curl_cfree
        .expect(
            "non-null function pointer",
        )((*data).state.aptr.passwd as *mut libc::c_void);
    let fresh6 = &mut ((*data).state.aptr.passwd);
    *fresh6 = 0 as *mut i8;
    return result;
}
...
unsafe extern "C" fn mqtt_subscribe(mut data: *mut Curl_easy) -> CURLcode {
    ...
    if !(result as u64 != 0) {
        let fresh7 = &mut ((*conn).proto.mqtt.packetid);
