...
pub unsafe extern "C" fn Curl_range(mut data: *mut Curl_easy) -> CURLcode {
        ...
        && !((*data).state.range).is_null()
    {
        ...
        if from_t as u32 == CURL_OFFT_FLOW as i32 as u32 {
        ...
        to_t = curlx_strtoofft(ptr, &mut ptr2, 0 as i32, &mut to);
        if to_t as u32 == CURL_OFFT_FLOW as i32 as u32 {
            return CURLE_RANGE_ERROR;
        }
        if to_t as u32 == CURL_OFFT_INVAL as i32 as u32
            && from_t as u64 == 0
        {
            (*data).state.resume_from = from;
        } else if from_t as u32
                == CURL_OFFT_INVAL as i32 as u32 && to_t as u64 == 0
            {
            ...
            totalsize = to - from;
            if totalsize == 0x7fffffffffffffff as i64 {
                return CURLE_RANGE_ERROR;
            }
            (*data).req.maxdownload = totalsize + 1 as i32 as i64;
            (*data).state.resume_from = from;
        }
    } else {
