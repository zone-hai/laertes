...
unsafe extern "C" fn oldap_setup_connection(
    ...
    (*li).proto = proto;
    let fresh0 = &mut ((*conn).proto.ldapc);
    *fresh0 = li;
...
unsafe extern "C" fn oldap_connecting(
        ...
        & ((1 as i32) << 0 as i32) as u32 != 0
    {
        ...
        if !(*li).sslinst {
            ...
            let fresh2 = &mut ((*li).recv);
            *fresh2 = (*conn).recv[0 as i32 as usize];
            let fresh3 = &mut ((*li).send);
    ...
    let fresh4 = &mut ((*conn).recv[0 as i32 as usize]);
...
unsafe extern "C" fn oldap_disconnect(
    ...
    if !li.is_null() {
        if !((*li).ld).is_null() {
            ...
            let fresh5 = &mut ((*li).ld);
            *fresh5 = 0 as *mut LDAP;
        }
        let fresh6 = &mut ((*conn).proto.ldapc);
...
unsafe extern "C" fn oldap_do(
    ...
    (*lr).msgid = msgid;
    let fresh7 = &mut ((*data).req.p.ldap);
    *fresh7 = lr;
    Curl_setup_transfer(
        data,
        0 as i32,
...
unsafe extern "C" fn oldap_done(
    ...
    if !lr.is_null() {
        ...
        let fresh8 = &mut ((*data).req.p.ldap);
        ...
        if msgtype as u64 == 0x65 as u32 as ber_tag_t {
            ...
            if !(msgtype as u64 != 0x64 as u32 as ber_tag_t) {
                ...
                *fresh9 += 1;
                ...
                let fresh10 = &mut ((*data).req.bytecount);
                *fresh10 = (*fresh10 as u64)
                    ...
                    if bvals.is_null() {
                        ...
                        *fresh11 = (*fresh11 as u64)
                        ...
                        while !((*bvals.offset(i as isize)).bv_val).is_null() {
                            ...
                            let fresh12 = &mut ((*data).req.bytecount);
                            *fresh12 = (*fresh12 as u64)
                            ...
                            if binary != 0 || binval != 0 {
                                ...
                                let fresh13 = &mut ((*data).req.bytecount);
                                *fresh13 += 2 as i32 as i64;
                                if val_b64_sz > 0 as i32 as u64 {
                                    ...
                                    let fresh14 = &mut ((*data).req.bytecount);
                                ...
                                let fresh15 = &mut ((*data).req.bytecount);
                            ...
                            let fresh16 = &mut ((*data).req.bytecount);
                            *fresh16 += 1;
                            i += 1;
                        }
                        ber_memfree(bvals as *mut libc::c_void);
                        ...
                        let fresh17 = &mut ((*data).req.bytecount);
                        *fresh17 += 1;
                    }
                    rc = ldap_get_attribute_ber((*li).ld, ent, ber, &mut bv, &mut bvals);
                }
                ...
                let fresh18 = &mut ((*data).req.bytecount);
                *fresh18 += 1;
                ber_free(ber, 0 as i32);
            }
