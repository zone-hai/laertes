...
unsafe extern "C" fn convert_lineends(
    ...
    if ((*data).state).prev_block_had_trailing_cr() != 0 {
        if *startPtr as i32 == '\n' as i32 {
            ...
            let fresh0 = &mut ((*data).state.crlf_conversions);
            *fresh0 += 1;
        }
        let fresh1 = &mut ((*data).state);
    ...
    if !inPtr.is_null() {
        while inPtr < startPtr.offset(size as isize).offset(-(1 as i32 as isize))
        {
            ...
            ) == 0 as i32
            {
                ...
                *outPtr = *inPtr;
                let fresh2 = &mut ((*data).state.crlf_conversions);
                *fresh2 += 1;
...
unsafe extern "C" fn chop_write(
    ...
    while len != 0 {
        let mut chunklen: size_t = if len <= 16384 as i32 as u64 {
            len
        } else {
            16384 as i32 as u64
        };
...
pub unsafe extern "C" fn Curl_client_write(
    ...
    {
        let mut result: CURLcode = CURLE_OK as i32 as CURLcode;
        if result as u64 != 0 {
            return result;
        }
...
pub unsafe extern "C" fn Curl_read(
    ...
    let mut nread: ssize_t = 0 as i32 as ssize_t;
    let mut bytesfromsocket: size_t = 0 as i32 as size_t;
...
pub unsafe extern "C" fn Curl_debug(
    ...
    if ((*data).set).verbose() != 0 {
        static mut s_infotype: [[i8; 3]; 7] = unsafe {
            [
                *::std::mem::transmute::<&[u8; 3], &[i8; 3]>(b"* \0"),
                *::std::mem::transmute::<&[u8; 3], &[i8; 3]>(b"< \0"),
                *::std::mem::transmute::<&[u8; 3], &[i8; 3]>(b"> \0"),
                *::std::mem::transmute::<&[u8; 3], &[i8; 3]>(b"{ \0"),
                *::std::mem::transmute::<&[u8; 3], &[i8; 3]>(b"} \0"),
