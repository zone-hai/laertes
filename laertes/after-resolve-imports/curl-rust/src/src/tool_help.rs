...
pub type __compar_fn_t = crate::src::lib::cookie::__compar_fn_t;
#[derive(Copy, Clone)]
#[repr(C)]
...
pub unsafe extern "C" fn tool_version_info() {
    ...
    if (*curlinfo).features != 0 {
        let mut featp: [*mut i8; 29] = [0 as *mut i8; 29];
        let mut numfeat: size_t = 0 as i32 as size_t;
        let mut i: u32 = 0;
                ...
                .wrapping_div(::std::mem::size_of::<feat>() as u64)
        {
            if (*curlinfo).features & feats[i as usize].bitmask != 0 {
                ...
                featp[fresh0 as usize] = feats[i as usize].name as *mut i8;
            }
            i = i.wrapping_add(1);
        }
        qsort(
            &mut *featp.as_mut_ptr().offset(0 as i32 as isize)
