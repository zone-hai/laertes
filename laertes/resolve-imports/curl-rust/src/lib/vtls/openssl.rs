...
unsafe extern "C" fn ossl_closeone(
    ...
    if !((*backend).handle).is_null() {
        ...
        let ref mut fresh2 = (*(*conn).ssl[0 as i32 as usize].backend).logger;
        *fresh2 = data;
        SSL_read(
            (*backend).handle,
...
unsafe extern "C" fn ossl_connect_step1(
    ...
    *fresh8 = SSL_CTX_new(req_method);
    ...
    if ((*data).set.fdebug).is_some() && ((*data).set).verbose() as i32 != 0 {
        ...
        let ref mut fresh9 = (*(*conn).ssl[0 as i32 as usize].backend).logger;
        *fresh9 = data;
    }
...
unsafe extern "C" fn ossl_send(
    ...
    let ref mut fresh24 = (*(*conn).ssl[0 as i32 as usize].backend).logger;
    *fresh24 = data;
...
unsafe extern "C" fn ossl_recv(
    ...
    let ref mut fresh25 = (*(*conn).ssl[0 as i32 as usize].backend).logger;
    *fresh25 = data;
