...
unsafe extern "C" fn ntlm_wb_cleanup(mut ntlm: *mut ntlmdata) {
    if (*ntlm).ntlm_auth_hlpr_socket != -(1 as i32) {
        close((*ntlm).ntlm_auth_hlpr_socket);
        (*ntlm).ntlm_auth_hlpr_socket = -(1 as i32);
    }
    if (*ntlm).ntlm_auth_hlpr_pid != 0 {
        let mut i: i32 = 0;
        i = 0 as i32;
        while i < 4 as i32 {
            let mut ret: pid_t = waitpid(
                (*ntlm).ntlm_auth_hlpr_pid,
                0 as *mut i32,
                1 as i32,
            );
            if ret == (*ntlm).ntlm_auth_hlpr_pid
            ...
            match i {
                0 => {
                    kill((*ntlm).ntlm_auth_hlpr_pid, 15 as i32);
                }
                1 => {
                    Curl_wait_ms(1 as i32 as timediff_t);
                }
                2 => {
                    kill((*ntlm).ntlm_auth_hlpr_pid, 9 as i32);
                }
                3 | _ => {}
            }
            i += 1;
        }
        (*ntlm).ntlm_auth_hlpr_pid = 0 as i32;
    }
    Curl_cfree
        .expect("non-null function pointer")((*ntlm).challenge as *mut libc::c_void);
    let ref mut fresh0 = (*ntlm).challenge;
    *fresh0 = 0 as *mut i8;
    Curl_cfree
        .expect("non-null function pointer")((*ntlm).response as *mut libc::c_void);
    let ref mut fresh1 = (*ntlm).response;
    *fresh1 = 0 as *mut i8;
}
unsafe extern "C" fn ntlm_wb_init(
    mut data: *mut Curl_easy,
    mut ntlm: *mut ntlmdata,
    mut userp: *const i8,
) -> CURLcode {
    let mut sockfds: [curl_socket_t; 2] = [0; 2];
    ...
    if access(ntlm_auth, 1 as i32) != 0 as i32 {
        ...
        if child_pid == -(1 as i32) {
            ...
            close(sockfds[1 as i32 as usize]);
            (*ntlm).ntlm_auth_hlpr_socket = sockfds[0 as i32 as usize];
...
unsafe extern "C" fn ntlm_wb_response(
    mut data: *mut Curl_easy,
    mut ntlm: *mut ntlmdata,
    mut input: *const i8,
    mut state: curlntlm,
    ...
    let mut len_in: size_t = strlen(input);
    let mut len_out: size_t = 0 as i32 as size_t;
    let mut b: dynbuf = dynbuf {
        bufr: 0 as *mut i8,
        leng: 0,
        allc: 0,
        toobig: 0,
                                ...
                                if !(state as u32
                                    ...
                                    && (*ptr.offset(0 as i32 as isize) as i32
                                        ...
                                        || *ptr.offset(2 as i32 as isize) as i32
                                            != ' ' as i32))
                                {
                                    let ref mut fresh2 = (*ntlm).response;
                                    *fresh2 = Curl_cstrdup
                                        .expect(
                                            "non-null function pointer",
                                        )(ptr.offset(3 as i32 as isize));
                                    Curl_dyn_free(&mut b);
                                    if ((*ntlm).response).is_null() {
                                        return CURLE_OUT_OF_MEMORY;
                                    }
                                    return CURLE_OK;
                                }
...
pub unsafe extern "C" fn Curl_input_ntlm_wb(
    ...
    let mut ntlm: *mut ntlmdata = if proxy as i32 != 0 {
        &mut (*conn).proxyntlm
    } else {
        &mut (*conn).ntlm
    };
    let mut state: *mut curlntlm = if proxy as i32 != 0 {
    ...
    if *header != 0 {
        let ref mut fresh3 = (*ntlm).challenge;
...
pub unsafe extern "C" fn Curl_output_ntlm_wb(
    ...
    let mut allocuserpwd: *mut *mut i8 = 0 as *mut *mut i8;
        ...
        1 | _ => {
            ...
            *allocuserpwd = curl_maprintf(
            ...
            Curl_cfree
                .expect(
                    "non-null function pointer",
                )((*ntlm).response as *mut libc::c_void);
            let ref mut fresh4 = (*ntlm).response;
            *fresh4 = 0 as *mut i8;
            if (*allocuserpwd).is_null() {
                return CURLE_OUT_OF_MEMORY;
            }
...
#[no_mangle]
pub unsafe extern "C" fn Curl_http_auth_cleanup_ntlm_wb(mut conn: *mut connectdata) {
    ntlm_wb_cleanup(&mut (*conn).ntlm);
    ntlm_wb_cleanup(&mut (*conn).proxyntlm);
}
