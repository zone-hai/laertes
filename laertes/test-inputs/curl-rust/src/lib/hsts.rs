...
pub unsafe extern "C" fn Curl_hsts_init() -> *mut hsts {
    let mut h: *mut hsts = Curl_ccalloc
        ...
        )(::std::mem::size_of::<hsts>() as libc::c_ulong, 1 as libc::c_int as size_t)
        as *mut hsts;
    if !h.is_null() {
...
unsafe extern "C" fn hsts_free(mut e: *mut stsentry) {
    Curl_cfree
        .expect(
            "non-null function pointer",
        )((*e).host as *mut libc::c_char as *mut libc::c_void);
...
unsafe extern "C" fn hsts_entry() -> *mut stsentry {
    return Curl_ccalloc
        .expect(
            "non-null function pointer",
        )(::std::mem::size_of::<stsentry>() as libc::c_ulong, 1 as libc::c_int as size_t)
        as *mut stsentry;
}
...
pub unsafe extern "C" fn Curl_hsts_parse(
    ...
    let mut p: *const libc::c_char = header;
    let mut expires: curl_off_t = 0 as libc::c_int as curl_off_t;
    let mut gotma: bool = 0 as libc::c_int != 0;
    let mut gotinc: bool = 0 as libc::c_int != 0;
    let mut subdomains: bool = 0 as libc::c_int != 0;
    ...
    loop {
        while *p as libc::c_int != 0
            && Curl_isspace(*p as libc::c_uchar as libc::c_int) != 0
        {
            p = p.offset(1);
        }
        ...
        ) != 0
        {
            ...
            offt = curlx_strtoofft(p, &mut endp, 10 as libc::c_int, &mut expires);
            if offt as libc::c_uint == CURL_OFFT_FLOW as libc::c_int as libc::c_uint {
                expires = 0x7fffffffffffffff as libc::c_long;
            ...
            subdomains = 1 as libc::c_int != 0;
            p = p.offset(17 as libc::c_int as isize);
            gotinc = 1 as libc::c_int != 0;
        } else {
    ...
    if expires == 0 {
        sts = Curl_hsts(h, hostname, 0 as libc::c_int != 0);
        if !sts.is_null() {
    ...
    if 0x7fffffffffffffff as libc::c_long - now < expires {
        expires = 0x7fffffffffffffff as libc::c_long;
    } else {
        expires += now;
    }
    ...
    if !h.is_null() {
        ...
        while !e.is_null() {
            ...
            if (*sts).expires <= now {
                ...
                if subdomain as libc::c_int != 0
                    && (*sts).includeSubDomains as libc::c_int != 0
                {
                    ...
                    if ntail < hlen {
                        ...
                        if *hostname
                            .offset(
                                offs.wrapping_sub(1 as libc::c_int as libc::c_ulong)
                                    as isize,
...
unsafe extern "C" fn hsts_push(
    ...
    e.name = (*sts).host as *mut libc::c_char;
    e.namelen = strlen((*sts).host);
    e.set_includeSubDomains((*sts).includeSubDomains as libc::c_uint);
    if (*sts).expires != 0x7fffffffffffffff as libc::c_long {
        ...
        curl_msnprintf(
            ...
            b"%d%02d%02d %02d:%02d:%02d\0" as *const u8 as *const libc::c_char,
        ...
        strcpy(
            (e.expire).as_mut_ptr(),
            b"unlimited\0" as *const u8 as *const libc::c_char,
        );
    }
    ...
    *stop = sc as libc::c_uint != CURLSTS_OK as libc::c_int as libc::c_uint;
    return (if sc as libc::c_uint == CURLSTS_FAIL as libc::c_int as libc::c_uint {
...
unsafe extern "C" fn hsts_out(mut sts: *mut stsentry, mut fp: *mut FILE) -> CURLcode {
    ...
    if (*sts).expires != 0x7fffffffffffffff as libc::c_long {
        ...
        curl_mfprintf(
            ...
            if (*sts).includeSubDomains as libc::c_int != 0 {
...
pub unsafe extern "C" fn Curl_hsts_save(
    ...
    if !((*h).flags as libc::c_long
        & ((1 as libc::c_int) << 1 as libc::c_int) as libc::c_long != 0 || file.is_null()
        || *file.offset(0 as libc::c_int as isize) == 0)
    {
        if Curl_rand_hex(
            data,
    ...
    if ((*data).set.hsts_write).is_some() {
        ...
        i.total = (*h).list.size;
        i.index = 0 as libc::c_int as size_t;
        e = (*h).list.head;
        while !e.is_null() {
            ...
            if result as libc::c_uint != 0 || stop as libc::c_int != 0 {
                break;
            }
...
unsafe extern "C" fn hsts_add(
    ...
    let mut host: [libc::c_char; 257] = [0; 257];
    let mut date: [libc::c_char; 65] = [0; 65];
    let mut rc: libc::c_int = 0;
    rc = sscanf(
        line,
        b"%256s \"%64[^\"]\"\0" as *const u8 as *const libc::c_char,
        host.as_mut_ptr(),
        date.as_mut_ptr(),
    );
    if 2 as libc::c_int == rc {
        ...
        let mut result: CURLcode = CURLE_OK;
        let mut p: *mut libc::c_char = host.as_mut_ptr();
        let mut subdomain: bool = 0 as libc::c_int != 0;
        if *p.offset(0 as libc::c_int as isize) as libc::c_int == '.' as i32 {
            p = p.offset(1);
            subdomain = 1 as libc::c_int != 0;
        }
...
unsafe extern "C" fn hsts_pull(mut data: *mut Curl_easy, mut h: *mut hsts) -> CURLcode {
    if ((*data).set.hsts_read).is_some() {
        ...
        loop {
            ...
            e
                .namelen = (::std::mem::size_of::<[libc::c_char; 257]>()
                as libc::c_ulong)
                .wrapping_sub(1 as libc::c_int as libc::c_ulong);
            e.set_includeSubDomains(0 as libc::c_int as libc::c_uint);
            e.expire[0 as libc::c_int as usize] = 0 as libc::c_int as libc::c_char;
            *(e.name)
            ...
            if sc as libc::c_uint == CURLSTS_OK as libc::c_int as libc::c_uint {
                let mut expires: time_t = 0;
                let mut result: CURLcode = CURLE_OK;
                if *(e.name).offset(0 as libc::c_int as isize) == 0 {
                    return CURLE_BAD_FUNCTION_ARGUMENT;
                }
                if e.expire[0 as libc::c_int as usize] != 0 {
                ...
                result = hsts_create(
                    h,
                    e.name,
                    if e.includeSubDomains() as libc::c_int != 0 {
                        1 as libc::c_int
                    } else {
                        0 as libc::c_int
                    } != 0,
            ...
            } else if sc as libc::c_uint == CURLSTS_FAIL as libc::c_int as libc::c_uint {
