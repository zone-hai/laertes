...
pub type zlibInitState = libc::c_uint;
...
unsafe extern "C" fn exit_zlib(
    ...
    if *zlib_init as libc::c_uint == ZLIB_GZIP_HEADER as libc::c_int as libc::c_uint {
        Curl_cfree
            .expect("non-null function pointer")((*z).next_in as *mut libc::c_void);
        let ref mut fresh0 = (*z).next_in;
        *fresh0 = 0 as *mut Bytef;
    }
    if *zlib_init as libc::c_uint != ZLIB_UNINIT as libc::c_int as libc::c_uint {
        if inflateEnd(z) != 0 as libc::c_int
            && result as libc::c_uint == CURLE_OK as libc::c_int as libc::c_uint
        {
...
unsafe extern "C" fn process_trailer(
    ...
    *fresh1 = (*fresh1 as libc::c_uint).wrapping_sub(len) as uInt as uInt;
    let ref mut fresh2 = (*z).avail_in;
    *fresh2 = (*fresh2 as libc::c_uint).wrapping_sub(len) as uInt as uInt;
    let ref mut fresh3 = (*z).next_in;
    *fresh3 = (*fresh3).offset(len as isize);
    if (*z).avail_in != 0 {
        result = CURLE_WRITE_ERROR;
    }
    if result as libc::c_uint != 0 || (*zp).trailerlen == 0 {
...
unsafe extern "C" fn inflate_stream(
    ...
    let mut result: CURLcode = CURLE_OK;
    let mut decomp: *mut libc::c_char = 0 as *mut libc::c_char;
    if (*zp).zlib_init as libc::c_uint != ZLIB_INIT as libc::c_int as libc::c_uint
        && (*zp).zlib_init as libc::c_uint
            != ZLIB_INFLATING as libc::c_int as libc::c_uint
        && (*zp).zlib_init as libc::c_uint
            != ZLIB_INIT_GZIP as libc::c_int as libc::c_uint
        && (*zp).zlib_init as libc::c_uint
            != ZLIB_GZIP_INFLATING as libc::c_int as libc::c_uint
    {
        return exit_zlib(data, z, &mut (*zp).zlib_init, CURLE_WRITE_ERROR);
    }
    decomp = Curl_cmalloc
        .expect("non-null function pointer")(16384 as libc::c_int as size_t)
        as *mut libc::c_char;
    if decomp.is_null() {
    ...
    while !done {
        ...
        *fresh4 = decomp as *mut Bytef;
        (*z).avail_out = 16384 as libc::c_int as uInt;
        status = inflate(z, 5 as libc::c_int);
        if (*z).avail_out != 16384 as libc::c_int as libc::c_uint {
            if status == 0 as libc::c_int || status == 1 as libc::c_int {
                ...
                result = Curl_unencode_write(
                    ...
                    (*writer).downstream,
                    decomp,
                    (16384 as libc::c_int as libc::c_uint).wrapping_sub((*z).avail_out)
                        as size_t,
                );
                if result as u64 != 0 {
        ...
        match status {
            ...
            -3 => {
                if (*zp).zlib_init as libc::c_uint
                    == ZLIB_INIT as libc::c_int as libc::c_uint
                {
                    inflateEnd(z);
                    if inflateInit2_(
                        z,
                    ...
                    ) == 0 as libc::c_int
                    {
                        ...
                        *fresh5 = orig_in;
                        ...
                        (*zp).trailerlen = 4 as libc::c_int as uInt;
                        done = 0 as libc::c_int != 0;
                        current_block_21 = 10692455896603418738;
                    } else {
    ...
    if nread != 0
        && (*zp).zlib_init as libc::c_uint == ZLIB_INIT as libc::c_int as libc::c_uint
    {
        (*zp).zlib_init = started;
    }
    return result;
...
unsafe extern "C" fn deflate_init_writer(
    ...
    *fresh6 = ::std::mem::transmute::<
        Option::<unsafe extern "C" fn(voidpf, libc::c_uint, libc::c_uint) -> voidpf>,
        alloc_func,
    >(
        Some(
            zalloc_cb
...
unsafe extern "C" fn deflate_unencode_write(
    ...
    *fresh8 = buf as *mut Bytef;
    ...
    if (*zp).zlib_init as libc::c_uint
        == ZLIB_EXTERNAL_TRAILER as libc::c_int as libc::c_uint
    {
...
static mut deflate_encoding: content_encoding = unsafe {
    {
        let mut init = content_encoding {
            name: b"deflate\0" as *const u8 as *const libc::c_char,
            ...
            paramsize: ::std::mem::size_of::<zlib_params>() as libc::c_ulong,
        };
        init
    }
};
unsafe extern "C" fn gzip_init_writer(
    ...
    *fresh10 = ::std::mem::transmute::<
        ...
        free_func,
    >(Some(zfree_cb as unsafe extern "C" fn(voidpf, voidpf) -> ()));
    if strcmp(zlibVersion(), b"1.2.0.4\0" as *const u8 as *const libc::c_char)
        >= 0 as libc::c_int
    {
        if inflateInit2_(
            z,
        ...
        (*zp).trailerlen = 8 as libc::c_int as uInt;
        (*zp).zlib_init = ZLIB_INIT;
    }
    return CURLE_OK;
}
unsafe extern "C" fn check_gzip_header(
    ...
    method = *data.offset(2 as libc::c_int as isize) as libc::c_int;
    flags = *data.offset(3 as libc::c_int as isize) as libc::c_int;
    if method != 8 as libc::c_int || flags & 0xe0 as libc::c_int != 0 as libc::c_int {
        return GZIP_BAD;
    }
    len -= 10 as libc::c_int as libc::c_long;
    data = data.offset(10 as libc::c_int as isize);
    if flags & 0x4 as libc::c_int != 0 {
        let mut extra_len: ssize_t = 0;
        if len < 2 as libc::c_int as libc::c_long {
            return GZIP_UNDERFLOW;
        }
        extra_len = ((*data.offset(1 as libc::c_int as isize) as libc::c_int)
            << 8 as libc::c_int | *data.offset(0 as libc::c_int as isize) as libc::c_int)
            as ssize_t;
        if len < extra_len + 2 as libc::c_int as libc::c_long {
            return GZIP_UNDERFLOW;
        }
        len -= extra_len + 2 as libc::c_int as libc::c_long;
        data = data.offset((extra_len + 2 as libc::c_int as libc::c_long) as isize);
    }
    if flags & 0x8 as libc::c_int != 0 {
        while len != 0 && *data as libc::c_int != 0 {
            len -= 1;
            data = data.offset(1);
        }
        if len == 0 || *data as libc::c_int != 0 {
            return GZIP_UNDERFLOW;
        }
        len -= 1;
        data = data.offset(1);
...
unsafe extern "C" fn gzip_unencode_write(
    ...
    let mut zp: *mut zlib_params = &mut (*writer).params as *mut *mut libc::c_void
        as *mut zlib_params;
    let mut z: *mut z_stream = &mut (*zp).z;
    if (*zp).zlib_init as libc::c_uint == ZLIB_INIT_GZIP as libc::c_int as libc::c_uint {
        let ref mut fresh11 = (*z).next_in;
        *fresh11 = buf as *mut Bytef;
        (*z).avail_in = nbytes as uInt;
        return inflate_stream(data, writer, ZLIB_INIT_GZIP);
    }
    match (*zp).zlib_init as libc::c_uint {
        1 => {
            ...
            ) as libc::c_uint
            {
                0 => {
                    ...
                    *fresh12 = (buf as *mut Bytef).offset(hlen as isize);
                    (*z).avail_in = nbytes.wrapping_sub(hlen as libc::c_ulong) as uInt;
        ...
        4 => {
            ...
            *fresh14 = (*fresh14 as libc::c_uint).wrapping_add(nbytes as uInt) as uInt
                as uInt;
                ...
                as libc::c_uint
            {
                0 => {
                    ...
                    *fresh16 = (buf as *mut Bytef)
                    ...
                    (*z).avail_in = ((*z).avail_in as libc::c_long - hlen_0) as uInt;
                    (*zp).zlib_init = ZLIB_GZIP_INFLATING;
                }
                2 => return CURLE_OK,
                1 | _ => {
    ...
    if (*z).avail_in == 0 as libc::c_int as libc::c_uint {
        return CURLE_OK;
    }
...
static mut gzip_encoding: content_encoding = unsafe {
    {
        let mut init = content_encoding {
            ...
            alias: b"x-gzip\0" as *const u8 as *const libc::c_char,
            init_writer: Some(
                gzip_init_writer
                    as unsafe extern "C" fn(
                        *mut Curl_easy,
...
unsafe extern "C" fn identity_init_writer(
    ...
    return (if !((*writer).downstream).is_null() {
        ...
        CURLE_WRITE_ERROR as libc::c_int
    }) as CURLcode;
}
...
unsafe extern "C" fn identity_close_writer(
    mut data: *mut Curl_easy,
    mut writer: *mut contenc_writer,
) {}
static mut identity_encoding: content_encoding = unsafe {
    {
        let mut init = content_encoding {
            name: b"identity\0" as *const u8 as *const libc::c_char,
            alias: b"none\0" as *const u8 as *const libc::c_char,
            init_writer: Some(
            ...
            paramsize: 0 as libc::c_int as size_t,
        };
        init
    }
};
...
#[no_mangle]
pub unsafe extern "C" fn Curl_all_content_encodings() -> *mut libc::c_char {
    ...
    let mut ace: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    while !(*cep).is_null() {
        ...
        ) == 0
        {
            len = (len as libc::c_ulong)
                .wrapping_add(
                    (strlen((*ce).name)).wrapping_add(2 as libc::c_int as libc::c_ulong),
                ) as size_t as size_t;
        }
        cep = cep.offset(1);
    }
    ...
    ace = Curl_cmalloc.expect("non-null function pointer")(len) as *mut libc::c_char;
    if !ace.is_null() {
        let mut p: *mut libc::c_char = ace;
        cep = encodings.as_ptr();
        while !(*cep).is_null() {
            ce = *cep;
            if Curl_strcasecompare(
        ...
        *p.offset(-(2 as libc::c_int) as isize) = '\u{0}' as i32 as libc::c_char;
    }
    return ace;
}
...
unsafe extern "C" fn client_unencode_write(
    ...
    if nbytes == 0 || (*k).ignorebody() as libc::c_int != 0 {
        return CURLE_OK;
    }
...
unsafe extern "C" fn client_close_writer(
    mut data: *mut Curl_easy,
    mut writer: *mut contenc_writer,
) {}
...
unsafe extern "C" fn error_unencode_write(
    ...
    let mut all: *mut libc::c_char = Curl_all_content_encodings();
    if all.is_null() {
...
unsafe extern "C" fn error_close_writer(
    mut data: *mut Curl_easy,
    mut writer: *mut contenc_writer,
) {}
...
unsafe extern "C" fn new_unencoding_writer(
    ...
    let mut sz: size_t = (16 as libc::c_ulong).wrapping_add((*handler).paramsize);
    let mut writer: *mut contenc_writer = Curl_ccalloc
        .expect("non-null function pointer")(1 as libc::c_int as size_t, sz)
...
pub unsafe extern "C" fn Curl_build_unencoding_stack(
    mut data: *mut Curl_easy,
    mut enclist: *const libc::c_char,
    mut maybechunked: libc::c_int,
) -> CURLcode {
    let mut k: *mut SingleRequest = &mut (*data).req;
    loop {
        let mut name: *const libc::c_char = 0 as *const libc::c_char;
        let mut namelen: size_t = 0;
        while Curl_isspace(*enclist as libc::c_uchar as libc::c_int) != 0
            || *enclist as libc::c_int == ',' as i32
        {
            enclist = enclist.offset(1);
        }
        name = enclist;
        namelen = 0 as libc::c_int as size_t;
        while *enclist as libc::c_int != 0 && *enclist as libc::c_int != ',' as i32 {
            if Curl_isspace(*enclist as libc::c_uchar as libc::c_int) == 0 {
                namelen = (enclist.offset_from(name) as libc::c_long
                    + 1 as libc::c_int as libc::c_long) as size_t;
            }
            enclist = enclist.offset(1);
        }
        if maybechunked != 0 && namelen == 7 as libc::c_int as libc::c_ulong
        ...
        {
            (*k).set_chunk(1 as libc::c_int as bit);
