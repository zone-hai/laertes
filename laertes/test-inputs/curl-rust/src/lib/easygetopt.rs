...
extern "C" {
    ...
    static mut Curl_easyopts: [curl_easyoption; 0];
}
...
unsafe extern "C" fn lookup(
    ...
    if !name.is_null() || id as libc::c_uint != 0 {
        let mut o: *mut curl_easyoption = &mut *Curl_easyopts
            .as_mut_ptr()
            .offset(0 as libc::c_int as isize) as *mut curl_easyoption;
        loop {
            if !name.is_null() {
                if Curl_strcasecompare((*o).name, name) != 0 {
                    return o;
                }
            } else if (*o).id as libc::c_uint == id as libc::c_uint
                    && (*o).flags
...
pub unsafe extern "C" fn curl_easy_option_by_id(
    ...
    return lookup(0 as *const libc::c_char, id);
}
#[no_mangle]
pub unsafe extern "C" fn curl_easy_option_next(
    ...
    if !prev.is_null() && !((*prev).name).is_null() {
        ...
        return &mut *Curl_easyopts.as_mut_ptr().offset(0 as libc::c_int as isize)
