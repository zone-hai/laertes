...
extern "C" {
    ...
    fn gsasl_strerror(err: libc::c_int) -> *const libc::c_char;
    fn gsasl_init(ctx: *mut *mut Gsasl) -> libc::c_int;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_auth_gsasl_is_supported(
    mut data: *mut Curl_easy,
    mut mech: *const libc::c_char,
    mut gsasl: *mut gsasldata,
) -> bool {
    let mut res: libc::c_int = 0;
    res = gsasl_init(&mut (*gsasl).ctx);
    ...
    res = gsasl_client_start((*gsasl).ctx, mech, &mut (*gsasl).client);
    if res != GSASL_OK as libc::c_int {
...
#[no_mangle]
pub unsafe extern "C" fn Curl_auth_gsasl_start(
    mut data: *mut Curl_easy,
...
#[no_mangle]
pub unsafe extern "C" fn Curl_auth_gsasl_token(
    mut data: *mut Curl_easy,
    ...
    res = gsasl_step(
        (*gsasl).client,
        Curl_bufref_ptr(chlg) as *const libc::c_char,
        Curl_bufref_len(chlg),
        &mut response,
        &mut outlen,
    );
    if res != GSASL_OK as libc::c_int && res != GSASL_NEEDS_MORE as libc::c_int {
        Curl_failf(
            data,
            b"GSASL step: %s\n\0" as *const u8 as *const libc::c_char,
            gsasl_strerror(res),
    ...
    Curl_bufref_set(
        ...
        outlen,
        Some(gsasl_free as unsafe extern "C" fn(*mut libc::c_void) -> ()),
    );
    return CURLE_OK;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_auth_gsasl_cleanup(mut gsasl: *mut gsasldata) {
    gsasl_finish((*gsasl).client);
    let ref mut fresh0 = (*gsasl).client;
    *fresh0 = 0 as *mut Gsasl_session;
    gsasl_done((*gsasl).ctx);
    let ref mut fresh1 = (*gsasl).ctx;
