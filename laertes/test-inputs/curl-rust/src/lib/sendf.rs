...
unsafe extern "C" fn convert_lineends(
    mut data: *mut Curl_easy,
    mut startPtr: *mut libc::c_char,
    mut size: size_t,
) -> size_t {
    let mut inPtr: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut outPtr: *mut libc::c_char = 0 as *mut libc::c_char;
    if startPtr.is_null() || size < 1 as libc::c_int as libc::c_ulong {
        return size;
    }
    if ((*data).state).prev_block_had_trailing_cr() != 0 {
        if *startPtr as libc::c_int == '\n' as i32 {
            memmove(
                startPtr as *mut libc::c_void,
                startPtr.offset(1 as libc::c_int as isize) as *const libc::c_void,
                size.wrapping_sub(1 as libc::c_int as libc::c_ulong),
            );
            size = size.wrapping_sub(1);
            let ref mut fresh0 = (*data).state.crlf_conversions;
            *fresh0 += 1;
        }
        let ref mut fresh1 = (*data).state;
        (*fresh1).set_prev_block_had_trailing_cr(0 as libc::c_int as bit);
    }
    outPtr = memchr(startPtr as *const libc::c_void, '\r' as i32, size)
        as *mut libc::c_char;
    inPtr = outPtr;
    if !inPtr.is_null() {
        while inPtr < startPtr.offset(size as isize).offset(-(1 as libc::c_int as isize))
        {
            if memcmp(
            ...
            ) == 0 as libc::c_int
            {
                ...
                *outPtr = *inPtr;
                let ref mut fresh2 = (*data).state.crlf_conversions;
                *fresh2 += 1;
            } else if *inPtr as libc::c_int == '\r' as i32 {
        ...
        if inPtr < startPtr.offset(size as isize) {
            if *inPtr as libc::c_int == '\r' as i32 {
                *outPtr = '\n' as i32 as libc::c_char;
                let ref mut fresh3 = (*data).state;
                (*fresh3).set_prev_block_had_trailing_cr(1 as libc::c_int as bit);
            } else {
                *outPtr = *inPtr;
            }
            outPtr = outPtr.offset(1);
        }
        if outPtr < startPtr.offset(size as isize) {
            *outPtr = '\u{0}' as i32 as libc::c_char;
        }
        return outPtr.offset_from(startPtr) as libc::c_long as size_t;
    }
    return size;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_recv_has_postponed_data(
...
#[no_mangle]
pub unsafe extern "C" fn Curl_infof(
    mut data: *mut Curl_easy,
    mut fmt: *const libc::c_char,
    mut args: ...
) {
    if !data.is_null() && ((*data).set).verbose() as libc::c_int != 0 {
        let mut ap: ::std::ffi::VaListImpl;
        let mut len: size_t = 0;
        let mut buffer: [libc::c_char; 2050] = [0; 2050];
        ap = args.clone();
        len = curl_mvsnprintf(
            buffer.as_mut_ptr(),
        ...
        buffer[fresh4 as usize] = '\n' as i32 as libc::c_char;
        buffer[len as usize] = '\u{0}' as i32 as libc::c_char;
        Curl_debug(data, CURLINFO_TEXT, buffer.as_mut_ptr(), len);
    }
}
#[no_mangle]
pub unsafe extern "C" fn Curl_failf(
    mut data: *mut Curl_easy,
    mut fmt: *const libc::c_char,
    mut args: ...
) {
    if ((*data).set).verbose() as libc::c_int != 0
        || !((*data).set.errorbuffer).is_null()
    {
        let mut ap: ::std::ffi::VaListImpl;
        let mut len: size_t = 0;
        let mut error: [libc::c_char; 258] = [0; 258];
        ap = args.clone();
        len = curl_mvsnprintf(
            error.as_mut_ptr(),
        ...
        if !((*data).set.errorbuffer).is_null() && ((*data).state).errorbuf() == 0 {
            strcpy((*data).set.errorbuffer, error.as_mut_ptr());
            let ref mut fresh5 = (*data).state;
            (*fresh5).set_errorbuf(1 as libc::c_int as bit);
        }
        let fresh6 = len;
        len = len.wrapping_add(1);
        error[fresh6 as usize] = '\n' as i32 as libc::c_char;
        error[len as usize] = '\u{0}' as i32 as libc::c_char;
        Curl_debug(data, CURLINFO_TEXT, error.as_mut_ptr(), len);
    }
}
#[no_mangle]
pub unsafe extern "C" fn Curl_write(
    mut data: *mut Curl_easy,
    mut sockfd: curl_socket_t,
    mut mem: *const libc::c_void,
    mut len: size_t,
    mut written: *mut ssize_t,
) -> CURLcode {
    ...
    conn = (*data).conn;
    num = (sockfd == (*conn).sock[1 as libc::c_int as usize]) as libc::c_int;
    bytes_written = ((*conn).send[num as usize])
        .expect("non-null function pointer")(data, num, mem, len, &mut result);
    *written = bytes_written;
    ...
    match result as libc::c_uint {
        81 => {
            *written = 0 as libc::c_int as ssize_t;
            return CURLE_OK;
        }
        0 => return CURLE_SEND_ERROR,
        _ => return result,
    };
}
#[no_mangle]
pub unsafe extern "C" fn Curl_send_plain(
    mut data: *mut Curl_easy,
    mut num: libc::c_int,
    mut mem: *const libc::c_void,
    mut len: size_t,
    mut code: *mut CURLcode,
) -> ssize_t {
    let mut conn: *mut connectdata = 0 as *mut connectdata;
    let mut sockfd: curl_socket_t = 0;
    let mut bytes_written: ssize_t = 0;
    conn = (*data).conn;
    sockfd = (*conn).sock[num as usize];
    if CURLE_OK as libc::c_int != 0 {
        *code = CURLE_OUT_OF_MEMORY;
        return -(1 as libc::c_int) as ssize_t;
    }
    bytes_written = send(sockfd, mem, len, MSG_NOSIGNAL as libc::c_int);
    *code = CURLE_OK;
    if -(1 as libc::c_int) as libc::c_long == bytes_written {
        let mut err: libc::c_int = *__errno_location();
        if 11 as libc::c_int == err || 11 as libc::c_int == err
            || 4 as libc::c_int == err || 115 as libc::c_int == err
        {
            ...
            *code = CURLE_AGAIN;
            ...
            Curl_failf(
                data,
                b"Send failure: %s\0" as *const u8 as *const libc::c_char,
                Curl_strerror(
                    err,
                    buffer.as_mut_ptr(),
...
#[no_mangle]
pub unsafe extern "C" fn Curl_write_plain(
    mut data: *mut Curl_easy,
    mut sockfd: curl_socket_t,
    mut mem: *const libc::c_void,
    mut len: size_t,
    ...
    let mut num: libc::c_int = 0;
    num = (sockfd == (*conn).sock[1 as libc::c_int as usize]) as libc::c_int;
    *written = Curl_send_plain(data, num, mem, len, &mut result);
    return result;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_recv_plain(
    mut data: *mut Curl_easy,
    mut num: libc::c_int,
    ...
    nread = recv(sockfd, buf as *mut libc::c_void, len, 0 as libc::c_int);
    *code = CURLE_OK;
    if -(1 as libc::c_int) as libc::c_long == nread {
        let mut err: libc::c_int = *__errno_location();
        if 11 as libc::c_int == err || 11 as libc::c_int == err
            || 4 as libc::c_int == err
        {
            *code = CURLE_AGAIN;
            ...
            Curl_failf(
                data,
                b"Recv failure: %s\0" as *const u8 as *const libc::c_char,
                Curl_strerror(
                    err,
                    buffer.as_mut_ptr(),
                    ::std::mem::size_of::<[libc::c_char; 256]>() as libc::c_ulong,
...
unsafe extern "C" fn pausewrite(
    ...
    let mut newtype: bool = 1 as libc::c_int != 0;
    Curl_http2_stream_pause(data, 1 as libc::c_int != 0);
    if (*s).tempcount != 0 {
        i = 0 as libc::c_int as libc::c_uint;
        while i < (*s).tempcount {
            if (*s).tempwrite[i as usize].type_0 == type_0 {
                newtype = 0 as libc::c_int != 0;
                break;
            } else {
                i = i.wrapping_add(1);
            }
    ...
    if newtype {
        Curl_dyn_init(
            &mut (*((*s).tempwrite).as_mut_ptr().offset(i as isize)).b,
            (64 as libc::c_int * 1024 as libc::c_int * 1024 as libc::c_int) as size_t,
        );
        (*s).tempwrite[i as usize].type_0 = type_0;
        let ref mut fresh7 = (*s).tempcount;
        *fresh7 = (*fresh7).wrapping_add(1);
    }
    if Curl_dyn_addn(
        &mut (*((*s).tempwrite).as_mut_ptr().offset(i as isize)).b,
        ptr as *mut libc::c_uchar as *const libc::c_void,
        len,
    ) as u64 != 0
    {
        return CURLE_OUT_OF_MEMORY;
    }
    (*k).keepon |= (1 as libc::c_int) << 4 as libc::c_int;
    return CURLE_OK;
}
unsafe extern "C" fn chop_write(
    mut data: *mut Curl_easy,
    mut type_0: libc::c_int,
    ...
    let mut conn: *mut connectdata = (*data).conn;
    let mut writeheader: curl_write_callback = None;
    let mut writebody: curl_write_callback = None;
    let mut ptr: *mut libc::c_char = optr;
    let mut len: size_t = olen;
    if len == 0 {
        return CURLE_OK;
    }
    if (*data).req.keepon & (1 as libc::c_int) << 4 as libc::c_int != 0 {
        return pausewrite(data, type_0, ptr, len);
    }
    if type_0 & (1 as libc::c_int) << 0 as libc::c_int != 0 {
        writebody = (*data).set.fwrite_func;
    }
    if type_0 & (1 as libc::c_int) << 1 as libc::c_int != 0
        && (((*data).set.fwrite_header).is_some()
            || !((*data).set.writeheader).is_null())
    {
        writeheader = if ((*data).set.fwrite_header).is_some() {
    ...
    while len != 0 {
        let mut chunklen: size_t = if len <= 16384 as libc::c_int as libc::c_ulong {
            len
        } else {
            16384 as libc::c_int as libc::c_ulong
        };
        if writebody.is_some() {
            let mut wrote: size_t = 0;
            Curl_set_in_callback(data, 1 as libc::c_int != 0);
            wrote = writebody
                .expect(
                    "non-null function pointer",
                )(ptr, 1 as libc::c_int as size_t, chunklen, (*data).set.out);
            ...
            if 0x10000001 as libc::c_int as libc::c_ulong == wrote {
                    ...
                    & ((1 as libc::c_int) << 4 as libc::c_int) as libc::c_uint != 0
                {
                    Curl_failf(
                        data,
                        b"Write callback asked for PAUSE when not supported!\0"
            ...
            if wrote != chunklen {
                Curl_failf(
                    data,
                    b"Failure writing output to destination\0" as *const u8
                        as *const libc::c_char,
                );
                return CURLE_WRITE_ERROR;
            }
        }
        ptr = ptr.offset(chunklen as isize);
        len = (len as libc::c_ulong).wrapping_sub(chunklen) as size_t as size_t;
    }
    if writeheader.is_some() {
        let mut wrote_0: size_t = 0;
        ptr = optr;
        len = olen;
        Curl_set_in_callback(data, 1 as libc::c_int != 0);
        wrote_0 = writeheader
            .expect(
                "non-null function pointer",
            )(ptr, 1 as libc::c_int as size_t, len, (*data).set.writeheader);
        Curl_set_in_callback(data, 0 as libc::c_int != 0);
        if 0x10000001 as libc::c_int as libc::c_ulong == wrote_0 {
            return pausewrite(data, (1 as libc::c_int) << 1 as libc::c_int, ptr, len);
        }
        if wrote_0 != len {
            Curl_failf(
                data,
                b"Failed writing header\0" as *const u8 as *const libc::c_char,
            );
            return CURLE_WRITE_ERROR;
        }
    }
...
#[no_mangle]
pub unsafe extern "C" fn Curl_client_write(
    mut data: *mut Curl_easy,
    mut type_0: libc::c_int,
    ...
    if type_0 & (1 as libc::c_int) << 0 as libc::c_int != 0
        ...
        && (*conn).proto.ftpc.transfertype as libc::c_int == 'A' as i32
    {
        let mut result: CURLcode = CURLE_OK as libc::c_int as CURLcode;
        if result as u64 != 0 {
            return result;
        }
        len = convert_lineends(data, ptr, len);
    }
    return chop_write(data, type_0, ptr, len);
}
#[no_mangle]
pub unsafe extern "C" fn Curl_read_plain(
    mut sockfd: curl_socket_t,
    mut buf: *mut libc::c_char,
    mut bytesfromsocket: size_t,
    mut n: *mut ssize_t,
) -> CURLcode {
    ...
    if -(1 as libc::c_int) as libc::c_long == nread {
        let err: libc::c_int = *__errno_location();
        let return_error: bool = 11 as libc::c_int == err || 11 as libc::c_int == err
            || 4 as libc::c_int == err;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_read(
    mut data: *mut Curl_easy,
    mut sockfd: curl_socket_t,
    mut buf: *mut libc::c_char,
    mut sizerequested: size_t,
    mut n: *mut ssize_t,
) -> CURLcode {
    let mut result: CURLcode = CURLE_RECV_ERROR;
    let mut nread: ssize_t = 0 as libc::c_int as ssize_t;
    let mut bytesfromsocket: size_t = 0 as libc::c_int as size_t;
    let mut buffertofill: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut conn: *mut connectdata = (*data).conn;
    let mut num: libc::c_int = (sockfd == (*conn).sock[1 as libc::c_int as usize])
        as libc::c_int;
    *n = 0 as libc::c_int as ssize_t;
    bytesfromsocket = if sizerequested < (*data).set.buffer_size as size_t {
        sizerequested
    } else {
        (*data).set.buffer_size as size_t
    };
    buffertofill = buf;
    nread = ((*conn).recv[num as usize])
        .expect(
            "non-null function pointer",
        )(data, num, buffertofill, bytesfromsocket, &mut result);
    if nread < 0 as libc::c_int as libc::c_long {
        return result;
    }
    *n += nread;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_debug(
    mut data: *mut Curl_easy,
    ...
    if ((*data).set).verbose() != 0 {
        static mut s_infotype: [[libc::c_char; 3]; 7] = unsafe {
            [
                *::std::mem::transmute::<&[u8; 3], &[libc::c_char; 3]>(b"* \0"),
                *::std::mem::transmute::<&[u8; 3], &[libc::c_char; 3]>(b"< \0"),
                *::std::mem::transmute::<&[u8; 3], &[libc::c_char; 3]>(b"> \0"),
                *::std::mem::transmute::<&[u8; 3], &[libc::c_char; 3]>(b"{ \0"),
                *::std::mem::transmute::<&[u8; 3], &[libc::c_char; 3]>(b"} \0"),
        ...
        if ((*data).set.fdebug).is_some() {
            Curl_set_in_callback(data, 1 as libc::c_int != 0);
            rc = (Some(((*data).set.fdebug).expect("non-null function pointer")))
                .expect(
                    "non-null function pointer",
                )(data, type_0, ptr, size, (*data).set.debugdata);
            Curl_set_in_callback(data, 0 as libc::c_int != 0);
        } else {
            match type_0 as libc::c_uint {
                0 | 2 | 1 => {
                    fwrite(
                        (s_infotype[type_0 as usize]).as_ptr() as *const libc::c_void,
                        2 as libc::c_int as libc::c_ulong,
