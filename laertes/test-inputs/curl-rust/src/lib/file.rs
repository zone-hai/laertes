...
#[no_mangle]
pub static mut Curl_handler_file: Curl_handler = unsafe {
    {
        let mut init = Curl_handler {
            scheme: b"FILE\0" as *const u8 as *const libc::c_char,
            setup_connection: Some(
                file_setup_connection
                    as unsafe extern "C" fn(*mut Curl_easy, *mut connectdata) -> CURLcode,
            ),
            do_it: Some(
                file_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            done: Some(
                file_done
            ...
            protocol: ((1 as libc::c_int) << 10 as libc::c_int) as libc::c_uint,
            family: ((1 as libc::c_int) << 10 as libc::c_int) as libc::c_uint,
...
unsafe extern "C" fn file_setup_connection(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
) -> CURLcode {
    let ref mut fresh0 = (*data).req.p.file;
    *fresh0 = Curl_ccalloc
        .expect(
            "non-null function pointer",
        )(
        1 as libc::c_int as size_t,
        ::std::mem::size_of::<FILEPROTO>() as libc::c_ulong,
    ) as *mut FILEPROTO;
    if ((*data).req.p.file).is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    return CURLE_OK;
}
unsafe extern "C" fn file_connect(
    mut data: *mut Curl_easy,
    mut done: *mut bool,
) -> CURLcode {
    let mut real_path: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut file: *mut FILEPROTO = (*data).req.p.file;
    let mut fd: libc::c_int = 0;
    let mut real_path_len: size_t = 0;
    ...
    if !(memchr(real_path as *const libc::c_void, 0 as libc::c_int, real_path_len))
        .is_null()
    {
        Curl_cfree.expect("non-null function pointer")(real_path as *mut libc::c_void);
        real_path = 0 as *mut libc::c_char;
        return CURLE_URL_MALFORMAT;
    }
    fd = open(real_path, 0 as libc::c_int);
    let ref mut fresh1 = (*file).path;
    *fresh1 = real_path;
    let ref mut fresh2 = (*file).freepath;
    *fresh2 = real_path;
    (*file).fd = fd;
    if ((*data).set).upload() == 0 && fd == -(1 as libc::c_int) {
        Curl_failf(
            data,
            b"Couldn't open file %s\0" as *const u8 as *const libc::c_char,
            (*data).state.up.path,
        );
        file_done(data, CURLE_FILE_COULDNT_READ_FILE, 0 as libc::c_int != 0);
        return CURLE_FILE_COULDNT_READ_FILE;
    }
    *done = 1 as libc::c_int != 0;
    return CURLE_OK;
}
unsafe extern "C" fn file_done(
    mut data: *mut Curl_easy,
    mut status: CURLcode,
    mut premature: bool,
) -> CURLcode {
    ...
    if !file.is_null() {
        Curl_cfree
            .expect("non-null function pointer")((*file).freepath as *mut libc::c_void);
        let ref mut fresh3 = (*file).freepath;
        *fresh3 = 0 as *mut libc::c_char;
        let ref mut fresh4 = (*file).path;
        *fresh4 = 0 as *mut libc::c_char;
        if (*file).fd != -(1 as libc::c_int) {
            close((*file).fd);
        }
...
unsafe extern "C" fn file_disconnect(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut dead_connection: bool,
) -> CURLcode {
    return file_done(data, CURLE_OK, 0 as libc::c_int != 0);
}
unsafe extern "C" fn file_upload(mut data: *mut Curl_easy) -> CURLcode {
    let mut file: *mut FILEPROTO = (*data).req.p.file;
    let mut dir: *const libc::c_char = strchr((*file).path, '/' as i32);
    let mut fd: libc::c_int = 0;
    let mut mode: libc::c_int = 0;
    let mut result: CURLcode = CURLE_OK;
    let mut buf: *mut libc::c_char = (*data).state.buffer;
    let mut bytecount: curl_off_t = 0 as libc::c_int as curl_off_t;
    ...
    let mut buf2: *const libc::c_char = 0 as *const libc::c_char;
    let ref mut fresh5 = (*data).req.upload_fromhere;
    *fresh5 = buf;
    if dir.is_null() {
        return CURLE_FILE_COULDNT_READ_FILE;
    }
    if *dir.offset(1 as libc::c_int as isize) == 0 {
        return CURLE_FILE_COULDNT_READ_FILE;
    }
    if (*data).state.resume_from != 0 {
        mode = 0o1 as libc::c_int | 0o100 as libc::c_int | 0o2000 as libc::c_int;
    } else {
        mode = 0o1 as libc::c_int | 0o100 as libc::c_int | 0o1000 as libc::c_int;
    }
    ...
    if (*data).state.resume_from < 0 as libc::c_int as libc::c_long {
        if fstat(fd, &mut file_stat) != 0 {
            ...
            Curl_failf(
                data,
                b"Can't get the size of %s\0" as *const u8 as *const libc::c_char,
                (*file).path,
            );
            return CURLE_WRITE_ERROR;
        }
        (*data).state.resume_from = file_stat.st_size;
    }
    while result as u64 == 0 {
        let mut nread: size_t = 0;
        let mut nwrite: size_t = 0;
        let mut readcount: size_t = 0;
        ...
        nread = readcount;
        if (*data).state.resume_from != 0 {
            if nread as curl_off_t <= (*data).state.resume_from {
                let ref mut fresh6 = (*data).state.resume_from;
                *fresh6 = (*fresh6 as libc::c_ulong).wrapping_sub(nread) as curl_off_t
                    as curl_off_t;
                nread = 0 as libc::c_int as size_t;
                buf2 = buf;
            } else {
                buf2 = buf.offset((*data).state.resume_from as isize);
                nread = (nread as libc::c_ulong)
                    .wrapping_sub((*data).state.resume_from as size_t) as size_t
                    as size_t;
        ...
        nwrite = write(fd, buf2 as *const libc::c_void, nread) as size_t;
        if nwrite != nread {
            result = CURLE_SEND_ERROR;
            break;
        } else {
            bytecount = (bytecount as libc::c_ulong).wrapping_add(nread) as curl_off_t
                as curl_off_t;
    ...
    if result as u64 == 0 && Curl_pgrsUpdate(data) != 0 {
        result = CURLE_ABORTED_BY_CALLBACK;
    }
    close(fd);
    return result;
}
unsafe extern "C" fn file_do(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    ...
    let mut expected_size: curl_off_t = -(1 as libc::c_int) as curl_off_t;
    let mut size_known: bool = false;
    let mut fstated: bool = 0 as libc::c_int != 0;
    let mut buf: *mut libc::c_char = (*data).state.buffer;
    let mut bytecount: curl_off_t = 0 as libc::c_int as curl_off_t;
    let mut fd: libc::c_int = 0;
    let mut file: *mut FILEPROTO = 0 as *mut FILEPROTO;
    *done = 1 as libc::c_int != 0;
    ...
    fd = (*file).fd;
    if -(1 as libc::c_int) != fstat(fd, &mut statbuf) {
        ...
        fstated = 1 as libc::c_int != 0;
    }
    if fstated as libc::c_int != 0 && ((*data).state.range).is_null()
    ...
    if fstated {
        ...
        let mut header: [libc::c_char; 80] = [0; 80];
        let mut headerlen: libc::c_int = 0;
        let mut accept_ranges: [libc::c_char; 24] = *::std::mem::transmute::<
            &[u8; 24],
            &mut [libc::c_char; 24],
        >(b"Accept-ranges: bytes\r\n\0\0");
        if expected_size >= 0 as libc::c_int as libc::c_long {
            ...
            result = Curl_client_write(
                data,
                (1 as libc::c_int) << 1 as libc::c_int,
                accept_ranges.as_mut_ptr(),
                strlen(accept_ranges.as_mut_ptr()),
            );
    ...
    if (*data).state.resume_from < 0 as libc::c_int as libc::c_long {
        if !fstated {
            Curl_failf(
                data,
                b"Can't get the size of file.\0" as *const u8 as *const libc::c_char,
            );
            return CURLE_READ_ERROR;
        }
        let ref mut fresh7 = (*data).state.resume_from;
        *fresh7 += statbuf.st_size;
    }
    if (*data).state.resume_from > 0 as libc::c_int as libc::c_long {
        if (*data).state.resume_from <= expected_size {
            ...
            Curl_failf(
                data,
                b"failed to resume file:// transfer\0" as *const u8
                    as *const libc::c_char,
            );
            return CURLE_BAD_DOWNLOAD_RESUME;
        }
    }
    if (*data).req.maxdownload > 0 as libc::c_int as libc::c_long {
        expected_size = (*data).req.maxdownload;
    }
    if !fstated || expected_size <= 0 as libc::c_int as libc::c_long {
        size_known = 0 as libc::c_int != 0;
    } else {
        size_known = 1 as libc::c_int != 0;
    }
    if size_known {
        Curl_pgrsSetDownloadSize(data, expected_size);
    }
    if (*data).state.resume_from != 0 {
        if (*data).state.resume_from
            != lseek(fd, (*data).state.resume_from, 0 as libc::c_int)
        {
            return CURLE_BAD_DOWNLOAD_RESUME;
        }
    }
    ...
    while result as u64 == 0 {
        ...
        let mut bytestoread: size_t = 0;
        if size_known {
            bytestoread = if expected_size < (*data).set.buffer_size {
                curlx_sotouz(expected_size)
            } else {
                (*data).set.buffer_size as size_t
            };
        } else {
            bytestoread = ((*data).set.buffer_size - 1 as libc::c_int as libc::c_long)
                as size_t;
        }
        nread = read(fd, buf as *mut libc::c_void, bytestoread);
        if nread > 0 as libc::c_int as libc::c_long {
            *buf.offset(nread as isize) = 0 as libc::c_int as libc::c_char;
        }
        if nread <= 0 as libc::c_int as libc::c_long
            || size_known as libc::c_int != 0
                && expected_size == 0 as libc::c_int as libc::c_long
        {
            break;
        }
        bytecount += nread;
