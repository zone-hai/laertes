...
#[no_mangle]
pub unsafe extern "C" fn Curl_speedinit(mut data: *mut Curl_easy) {
    memset(
        &mut (*data).state.keeps_speed as *mut curltime as *mut libc::c_void,
        0 as libc::c_int,
        ::std::mem::size_of::<curltime>() as libc::c_ulong,
    );
}
#[no_mangle]
pub unsafe extern "C" fn Curl_speedcheck(
    ...
    if (*data).progress.current_speed >= 0 as libc::c_int as libc::c_long
        && (*data).set.low_speed_time != 0
    {
        if (*data).progress.current_speed < (*data).set.low_speed_limit {
            if (*data).state.keeps_speed.tv_sec == 0 {
                (*data).state.keeps_speed = now;
            } else {
                let mut howlong: timediff_t = Curl_timediff(
                    now,
                    (*data).state.keeps_speed,
                );
                if howlong
                    >= (*data).set.low_speed_time * 1000 as libc::c_int as libc::c_long
                {
                    Curl_failf(
                        data,
                        b"Operation too slow. Less than %ld bytes/sec transferred the last %ld seconds\0"
    ...
    if (*data).set.low_speed_limit != 0 {
        Curl_expire(data, 1000 as libc::c_int as timediff_t, EXPIRE_SPEEDCHECK);
    }
    return CURLE_OK;
}
