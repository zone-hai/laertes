...
extern "C" {
    ...
    fn Curl_timediff_us(newer: curltime, older: curltime) -> timediff_t;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pgrsDone(mut data: *mut Curl_easy) -> libc::c_int {
    let mut rc: libc::c_int = 0;
    (*data).progress.lastshow = 0 as libc::c_int as time_t;
    rc = Curl_pgrsUpdate(data);
    if rc != 0 {
        return rc;
    }
    if (*data).progress.flags & (1 as libc::c_int) << 4 as libc::c_int == 0
        && ((*data).progress).callback() == 0
    {
        curl_mfprintf((*data).set.err, b"\n\0" as *const u8 as *const libc::c_char);
    }
    (*data).progress.speeder_c = 0 as libc::c_int;
...
pub unsafe extern "C" fn Curl_pgrsTime(
    ...
    match timer as libc::c_uint {
        ...
        2 => {
            ...
            (*fresh0).set_is_t_startransfer_set(0 as libc::c_int as bit);
        }
        9 => {
        ...
        7 => {
            ...
            if ((*data).progress).is_t_startransfer_set() != 0 {
                ...
                (*fresh1).set_is_t_startransfer_set(1 as libc::c_int as bit);
            }
        }
        8 => {}
        10 => {
    ...
    if !delta.is_null() {
        ...
        if us < 1 as libc::c_int as libc::c_long {
            us = 1 as libc::c_int as timediff_t;
        }
        *delta += us;
    }
    return now;
...
pub unsafe extern "C" fn Curl_pgrsStartNow(mut data: *mut Curl_easy) {
    ...
    (*fresh2).set_is_t_startransfer_set(0 as libc::c_int as bit);
    (*data).progress.ul_limit_start = (*data).progress.start;
    (*data).progress.dl_limit_start = (*data).progress.start;
    (*data).progress.ul_limit_size = 0 as libc::c_int as curl_off_t;
    (*data).progress.dl_limit_size = 0 as libc::c_int as curl_off_t;
    (*data).progress.downloaded = 0 as libc::c_int as curl_off_t;
    (*data).progress.uploaded = 0 as libc::c_int as curl_off_t;
    (*data).progress.flags
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pgrsLimitWaitTime(
    ...
    if size < 0x7fffffffffffffff as libc::c_long / 1000 as libc::c_int as libc::c_long {
        minimum = 1000 as libc::c_long * size / limit;
    } else {
        minimum = size / limit;
        if minimum
            < 0x7fffffffffffffff as libc::c_long / 1000 as libc::c_int as libc::c_long
        {
            minimum *= 1000 as libc::c_int as libc::c_long;
        } else {
            minimum = 0x7fffffffffffffff as libc::c_long;
        }
    }
...
pub unsafe extern "C" fn Curl_pgrsSetDownloadSize(
    ...
    if size >= 0 as libc::c_int as libc::c_long {
        ...
        (*data).progress.flags |= (1 as libc::c_int) << 6 as libc::c_int;
    } else {
        (*data).progress.size_dl = 0 as libc::c_int as curl_off_t;
        (*data).progress.flags &= !((1 as libc::c_int) << 6 as libc::c_int);
    };
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pgrsSetUploadSize(
    ...
    if size >= 0 as libc::c_int as libc::c_long {
        ...
        (*data).progress.flags |= (1 as libc::c_int) << 5 as libc::c_int;
    } else {
        (*data).progress.size_ul = 0 as libc::c_int as curl_off_t;
        (*data).progress.flags &= !((1 as libc::c_int) << 5 as libc::c_int);
    };
}
unsafe extern "C" fn trspeed(mut size: curl_off_t, mut us: curl_off_t) -> curl_off_t {
    if us < 1 as libc::c_int as libc::c_long {
        return size * 1000000 as libc::c_int as libc::c_long
    } else if size
            < 0x7fffffffffffffff as libc::c_long / 1000000 as libc::c_int as libc::c_long
        {
        return size * 1000000 as libc::c_int as libc::c_long / us
    } else if us >= 1000000 as libc::c_int as libc::c_long {
        return size / (us / 1000000 as libc::c_int as libc::c_long)
    } else {
        return 0x7fffffffffffffff as libc::c_long
    };
}
unsafe extern "C" fn progress_calc(mut data: *mut Curl_easy, mut now: curltime) -> bool {
    let mut timetoshow: bool = 0 as libc::c_int != 0;
    ...
    if (*p).lastshow != now.tv_sec {
        let mut countindex: libc::c_int = 0;
        let mut nowindex: libc::c_int = (*p).speeder_c
            % (5 as libc::c_int + 1 as libc::c_int);
        (*p).lastshow = now.tv_sec;
        timetoshow = 1 as libc::c_int != 0;
        (*p).speeder[nowindex as usize] = (*p).downloaded + (*p).uploaded;
        (*p).speeder_time[nowindex as usize] = now;
        let ref mut fresh3 = (*p).speeder_c;
        *fresh3 += 1;
        countindex = (if (*p).speeder_c >= 5 as libc::c_int + 1 as libc::c_int {
            5 as libc::c_int + 1 as libc::c_int
        } else {
            (*p).speeder_c
        }) - 1 as libc::c_int;
        if countindex != 0 {
            let mut checkindex: libc::c_int = 0;
            let mut span_ms: timediff_t = 0;
            let mut amount: curl_off_t = 0;
            checkindex = if (*p).speeder_c >= 5 as libc::c_int + 1 as libc::c_int {
                (*p).speeder_c % (5 as libc::c_int + 1 as libc::c_int)
            } else {
                0 as libc::c_int
            };
            span_ms = Curl_timediff(now, (*p).speeder_time[checkindex as usize]);
            if 0 as libc::c_int as libc::c_long == span_ms {
                span_ms = 1 as libc::c_int as timediff_t;
            }
            amount = (*p).speeder[nowindex as usize] - (*p).speeder[checkindex as usize];
            if amount > 4294967 as libc::c_long {
                (*p)
                    .current_speed = (amount as libc::c_double
                    / (span_ms as libc::c_double / 1000.0f64)) as curl_off_t;
            } else {
                (*p).current_speed = amount * 1000 as libc::c_long / span_ms;
            }
        } else {
...
unsafe extern "C" fn progress_meter(mut data: *mut Curl_easy) {
    let mut max5: [[libc::c_char; 10]; 6] = [[0; 10]; 6];
    let mut dlpercen: curl_off_t = 0 as libc::c_int as curl_off_t;
    let mut ulpercen: curl_off_t = 0 as libc::c_int as curl_off_t;
    let mut total_percen: curl_off_t = 0 as libc::c_int as curl_off_t;
    ...
    let mut ulestimate: curl_off_t = 0 as libc::c_int as curl_off_t;
    let mut dlestimate: curl_off_t = 0 as libc::c_int as curl_off_t;
    ...
    if (*data).progress.flags & (1 as libc::c_int) << 7 as libc::c_int == 0 {
        ...
        (*data).progress.flags |= (1 as libc::c_int) << 7 as libc::c_int;
    }
    if (*data).progress.flags & (1 as libc::c_int) << 5 as libc::c_int != 0
        && (*data).progress.ulspeed > 0 as libc::c_long
    {
        ulestimate = (*data).progress.size_ul / (*data).progress.ulspeed;
        if (*data).progress.size_ul > 10000 as libc::c_long {
            ulpercen = (*data).progress.uploaded
                / ((*data).progress.size_ul / 100 as libc::c_long);
        } else if (*data).progress.size_ul > 0 as libc::c_long {
            ulpercen = (*data).progress.uploaded * 100 as libc::c_int as libc::c_long
                / (*data).progress.size_ul;
        }
    }
    if (*data).progress.flags & (1 as libc::c_int) << 6 as libc::c_int != 0
        && (*data).progress.dlspeed > 0 as libc::c_long
    {
        dlestimate = (*data).progress.size_dl / (*data).progress.dlspeed;
        if (*data).progress.size_dl > 10000 as libc::c_long {
            dlpercen = (*data).progress.downloaded
                / ((*data).progress.size_dl / 100 as libc::c_long);
        } else if (*data).progress.size_dl > 0 as libc::c_long {
            dlpercen = (*data).progress.downloaded * 100 as libc::c_int as libc::c_long
    ...
    time2str(
        ...
        if total_estimate > 0 as libc::c_int as libc::c_long {
    ...
    if total_expected_transfer > 10000 as libc::c_long {
        total_percen = total_transfer / (total_expected_transfer / 100 as libc::c_long);
    } else if total_expected_transfer > 0 as libc::c_long {
        total_percen = total_transfer * 100 as libc::c_int as libc::c_long
    ...
    curl_mfprintf(
        ...
        max5data(
            total_expected_transfer,
            (max5[2 as libc::c_int as usize]).as_mut_ptr(),
        ),
        dlpercen,
        max5data(
            (*data).progress.downloaded,
            (max5[0 as libc::c_int as usize]).as_mut_ptr(),
        ),
        ulpercen,
        max5data(
            (*data).progress.uploaded,
            (max5[1 as libc::c_int as usize]).as_mut_ptr(),
        ),
        max5data(
            (*data).progress.dlspeed,
            (max5[3 as libc::c_int as usize]).as_mut_ptr(),
        ),
        max5data(
            (*data).progress.ulspeed,
            (max5[4 as libc::c_int as usize]).as_mut_ptr(),
        ),
        ...
        max5data(
            ...
            (max5[5 as libc::c_int as usize]).as_mut_ptr(),
        ),
    );
    fflush((*data).set.err);
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pgrsUpdate(mut data: *mut Curl_easy) -> libc::c_int {
    let mut now: curltime = Curl_now();
    ...
    if (*data).progress.flags & (1 as libc::c_int) << 4 as libc::c_int == 0 {
        if ((*data).set.fxferinfo).is_some() {
            ...
            if result != 0x10000001 as libc::c_int {
                if result != 0 {
                    Curl_failf(
                        data,
                        b"Callback aborted\0" as *const u8 as *const libc::c_char,
                    );
                }
                return result;
            }
            ...
            result_0 = ((*data).set.fprogress)
                ...
                (*data).progress.downloaded as libc::c_double,
                (*data).progress.size_ul as libc::c_double,
                (*data).progress.uploaded as libc::c_double,
            );
            Curl_set_in_callback(data, 0 as libc::c_int != 0);
            if result_0 != 0x10000001 as libc::c_int {
                if result_0 != 0 {
                    Curl_failf(
                        data,
                        b"Callback aborted\0" as *const u8 as *const libc::c_char,
