...
unsafe extern "C" fn socksstate(mut data: *mut Curl_easy, mut state: connect_t) {
    let mut conn: *mut connectdata = (*data).conn;
    let mut oldstate: connect_t = (*conn).cnnct.state;
...
pub unsafe extern "C" fn Curl_SOCKS_getsock(
    ...
    *sock.offset(0 as libc::c_int as isize) = (*conn).sock[sockindex as usize];
    match (*conn).cnnct.state as libc::c_uint {
        10 | 4 | 8 | 15 | 16 => {
            rc = (1 as libc::c_int) << 0 as libc::c_int;
        }
        _ => {
            rc = (1 as libc::c_int) << 16 as libc::c_int + 0 as libc::c_int;
        }
    }
    return rc;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_SOCKS4(
    mut proxy_user: *const libc::c_char,
    ...
    let mut conn: *mut connectdata = (*data).conn;
    let protocol4a: bool = if (*conn).socks_proxy.proxytype as libc::c_uint
    ...
    let mut socksreq: *mut libc::c_uchar = (*data).state.buffer as *mut libc::c_uchar;
    let mut result: CURLcode = CURLE_OK;
    let mut sockfd: curl_socket_t = (*conn).sock[sockindex as usize];
    let mut sx: *mut connstate = &mut (*conn).cnnct;
    let mut dns: *mut Curl_dns_entry = 0 as *mut Curl_dns_entry;
    let mut actualread: ssize_t = 0;
    let mut written: ssize_t = 0;
    if !((*sx).state as libc::c_uint >= CONNECT_SOCKS_INIT as libc::c_int as libc::c_uint
        && ((*sx).state as libc::c_uint) < CONNECT_DONE as libc::c_int as libc::c_uint)
        && !*done
    {
        socksstate(data, CONNECT_SOCKS_INIT);
    }
    let mut current_block_107: u64;
    match (*sx).state as libc::c_uint {
        1 => {
            ...
            Curl_infof(
                data,
                b"SOCKS4 communication to %s:%d\0" as *const u8 as *const libc::c_char,
                hostname,
                remote_port,
            );
            *socksreq
                .offset(0 as libc::c_int as isize) = 4 as libc::c_int as libc::c_uchar;
            *socksreq
                .offset(1 as libc::c_int as isize) = 1 as libc::c_int as libc::c_uchar;
            *socksreq
                .offset(
                    2 as libc::c_int as isize,
                ) = (remote_port >> 8 as libc::c_int & 0xff as libc::c_int)
                as libc::c_uchar;
            *socksreq
                .offset(
                    3 as libc::c_int as isize,
                ) = (remote_port & 0xff as libc::c_int) as libc::c_uchar;
            if !protocol4a {
                let mut rc: resolve_t = Curl_resolv(
                    data,
                    hostname,
                    remote_port,
                    0 as libc::c_int != 0,
                ...
                if rc as libc::c_int == CURLRESOLV_ERROR as libc::c_int {
        ...
        10 => {
            ...
            if !dns.is_null() {
                let ref mut fresh0 = (*data).state.async_0.dns;
                *fresh0 = dns;
                let ref mut fresh1 = (*data).state.async_0;
                (*fresh1).set_done(1 as libc::c_int as bit);
                Curl_infof(
                    data,
                    b"Hostname '%s' was found\0" as *const u8 as *const libc::c_char,
                    hostname,
                );
                socksstate(data, CONNECT_RESOLVED);
            } else {
    ...
    match current_block_107 {
        1526476230797115868 => {
            let mut hp: *mut Curl_addrinfo = 0 as *mut Curl_addrinfo;
            if !dns.is_null() {
                hp = (*dns).addr;
                while !hp.is_null() && (*hp).ai_family != 2 as libc::c_int {
                    hp = (*hp).ai_next;
                }
                if !hp.is_null() {
                    let mut saddr_in: *mut sockaddr_in = 0 as *mut sockaddr_in;
    ...
    match current_block_107 {
        10721739657148430059 => {
            *socksreq
                .offset(8 as libc::c_int as isize) = 0 as libc::c_int as libc::c_uchar;
            if !proxy_user.is_null() {
                let mut plen: size_t = strlen(proxy_user);
                if plen
                ...
                memcpy(
                    socksreq.offset(8 as libc::c_int as isize) as *mut libc::c_void,
                    proxy_user as *const libc::c_void,
                    plen.wrapping_add(1 as libc::c_int as libc::c_ulong),
                );
            }
            let mut packetsize: size_t = (9 as libc::c_int as libc::c_ulong)
                .wrapping_add(
                    strlen(
                        (socksreq as *mut libc::c_char).offset(8 as libc::c_int as isize),
                    ),
                );
            if protocol4a {
                let mut hostnamelen: size_t = 0 as libc::c_int as size_t;
                *socksreq
                    .offset(
                        4 as libc::c_int as isize,
                ...
                *socksreq
                ...
                if hostnamelen <= 255 as libc::c_int as libc::c_ulong {
                    strcpy(
                        (socksreq as *mut libc::c_char).offset(packetsize as isize),
                        hostname,
                    );
                } else {
                    Curl_failf(
                ...
                packetsize = (packetsize as libc::c_ulong).wrapping_add(hostnamelen)
                    as size_t as size_t;
            }
            let ref mut fresh2 = (*sx).outp;
            *fresh2 = socksreq;
            (*sx).outstanding = packetsize as ssize_t;
    ...
    match current_block_107 {
        4542371696367325799 => {
            result = Curl_write_plain(
                data,
                sockfd,
                (*sx).outp as *mut libc::c_char as *const libc::c_void,
                (*sx).outstanding as size_t,
                &mut written,
            );
            if result as libc::c_uint != 0
                && CURLE_AGAIN as libc::c_int as libc::c_uint != result as libc::c_uint
            {
                Curl_failf(
                    data,
            ...
            if written != (*sx).outstanding {
                let ref mut fresh3 = (*sx).outstanding;
                *fresh3 -= written;
                let ref mut fresh4 = (*sx).outp;
                *fresh4 = (*fresh4).offset(written as isize);
                return CURLPX_OK;
            }
            (*sx).outstanding = 8 as libc::c_int as ssize_t;
            let ref mut fresh5 = (*sx).outp;
            *fresh5 = socksreq;
                ...
                && CURLE_AGAIN as libc::c_int as libc::c_uint != result as libc::c_uint
            {
                ...
                if result as u64 == 0 && actualread == 0 {
                    ...
                    if actualread != (*sx).outstanding {
                        let ref mut fresh6 = (*sx).outstanding;
                        *fresh6 -= actualread;
                        let ref mut fresh7 = (*sx).outp;
    ...
    if *socksreq.offset(0 as libc::c_int as isize) != 0 {
        Curl_failf(
            data,
    ...
    match *socksreq.offset(1 as libc::c_int as isize) as libc::c_int {
        90 => {
            Curl_infof(
                data,
                b"SOCKS4%s request granted.\0" as *const u8 as *const libc::c_char,
                if protocol4a as libc::c_int != 0 {
                    b"a\0" as *const u8 as *const libc::c_char
                } else {
                    b"\0" as *const u8 as *const libc::c_char
        ...
        91 => {
            Curl_failf(
                ...
                *socksreq.offset(4 as libc::c_int as isize) as libc::c_int,
                *socksreq.offset(5 as libc::c_int as isize) as libc::c_int,
                *socksreq.offset(6 as libc::c_int as isize) as libc::c_int,
                *socksreq.offset(7 as libc::c_int as isize) as libc::c_int,
                (*socksreq.offset(2 as libc::c_int as isize) as libc::c_int)
                    << 8 as libc::c_int
                    | *socksreq.offset(3 as libc::c_int as isize) as libc::c_int,
                *socksreq.offset(1 as libc::c_int as isize) as libc::c_int,
            );
...
#[no_mangle]
pub unsafe extern "C" fn Curl_SOCKS5(
    mut proxy_user: *const libc::c_char,
    mut proxy_password: *const libc::c_char,
    mut hostname: *const libc::c_char,
    mut remote_port: libc::c_int,
    mut sockindex: libc::c_int,
    mut data: *mut Curl_easy,
    ...
    let mut conn: *mut connectdata = (*data).conn;
    let mut socksreq: *mut libc::c_uchar = (*data).state.buffer as *mut libc::c_uchar;
    let mut dest: [libc::c_char; 256] = *::std::mem::transmute::<
        &[u8; 256],
        &mut [libc::c_char; 256],
    >(
    ...
    let mut result: CURLcode = CURLE_OK;
    let mut sockfd: curl_socket_t = (*conn).sock[sockindex as usize];
    let mut socks5_resolve_local: bool = if (*conn).socks_proxy.proxytype as libc::c_uint
    ...
    let hostname_len: size_t = strlen(hostname);
    let mut len: ssize_t = 0 as libc::c_int as ssize_t;
    let auth: libc::c_ulong = (*data).set.socks5auth;
    let mut allow_gssapi: bool = 0 as libc::c_int != 0;
    let mut sx: *mut connstate = &mut (*conn).cnnct;
    let mut dns: *mut Curl_dns_entry = 0 as *mut Curl_dns_entry;
    if !((*sx).state as libc::c_uint >= CONNECT_SOCKS_INIT as libc::c_int as libc::c_uint
        && ((*sx).state as libc::c_uint) < CONNECT_DONE as libc::c_int as libc::c_uint)
    ...
    match (*sx).state as libc::c_uint {
        1 => {
            ...
            if !socks5_resolve_local
                && hostname_len > 255 as libc::c_int as libc::c_ulong
            {
                Curl_infof(
                    data,
                ...
                socks5_resolve_local = 1 as libc::c_int != 0;
            }
            if auth
            ...
            if auth & (1 as libc::c_int as libc::c_ulong) << 0 as libc::c_int == 0 {
                proxy_user = 0 as *const libc::c_char;
            }
            idx = 0 as libc::c_int;
            let fresh8 = idx;
            idx = idx + 1;
            *socksreq.offset(fresh8 as isize) = 5 as libc::c_int as libc::c_uchar;
            idx += 1;
            let fresh9 = idx;
            idx = idx + 1;
            *socksreq.offset(fresh9 as isize) = 0 as libc::c_int as libc::c_uchar;
            if allow_gssapi {
                let fresh10 = idx;
                idx = idx + 1;
                *socksreq.offset(fresh10 as isize) = 1 as libc::c_int as libc::c_uchar;
            }
            if !proxy_user.is_null() {
                let fresh11 = idx;
                idx = idx + 1;
                *socksreq.offset(fresh11 as isize) = 2 as libc::c_int as libc::c_uchar;
            }
            *socksreq
                .offset(
                    1 as libc::c_int as isize,
                ) = (idx - 2 as libc::c_int) as libc::c_uchar;
            result = Curl_write_plain(
                data,
                sockfd,
                socksreq as *mut libc::c_char as *const libc::c_void,
                idx as size_t,
                &mut written,
            );
            if result as libc::c_uint != 0
            ...
            if written != idx as libc::c_long {
                socksstate(data, CONNECT_SOCKS_SEND);
                (*sx).outstanding = idx as libc::c_long - written;
                let ref mut fresh12 = (*sx).outp;
                *fresh12 = &mut *socksreq.offset(written as isize) as *mut libc::c_uchar;
                return CURLPX_OK;
            }
            socksstate(data, CONNECT_SOCKS_READ);
        ...
        2 => {
            ...
            if written != (*sx).outstanding {
                let ref mut fresh13 = (*sx).outstanding;
                *fresh13 -= written;
                let ref mut fresh14 = (*sx).outp;
        ...
        10 => {
            ...
            if !dns.is_null() {
                let ref mut fresh27 = (*data).state.async_0.dns;
                *fresh27 = dns;
                let ref mut fresh28 = (*data).state.async_0;
                (*fresh28).set_done(1 as libc::c_int as bit);
                Curl_infof(
                    data,
                    b"SOCKS5: hostname '%s' found\0" as *const u8 as *const libc::c_char,
                    hostname,
                );
            }
            if dns.is_null() {
    ...
    match current_block_276 {
        3729150195017652645 => {
            (*sx).outstanding = 2 as libc::c_int as ssize_t;
            let ref mut fresh15 = (*sx).outp;
            *fresh15 = socksreq;
            current_block_276 = 17113687173598995549;
        }
        _ => {}
    ...
    match current_block_276 {
        17113687173598995549 => {
                ...
                && CURLE_AGAIN as libc::c_int as libc::c_uint != result as libc::c_uint
            {
                ...
                Curl_failf(
                    data,
                    b"Connection to proxy closed\0" as *const u8 as *const libc::c_char,
                );
                return CURLPX_CLOSED;
            } else if actualread != (*sx).outstanding {
                let ref mut fresh16 = (*sx).outstanding;
                *fresh16 -= actualread;
                let ref mut fresh17 = (*sx).outp;
                *fresh17 = (*fresh17).offset(actualread as isize);
                return CURLPX_OK;
            } else if *socksreq.offset(0 as libc::c_int as isize) as libc::c_int
                    != 5 as libc::c_int
                {
                Curl_failf(
                    data,
                ...
                return CURLPX_BAD_VERSION;
            } else if *socksreq.offset(1 as libc::c_int as isize) as libc::c_int
                    == 0 as libc::c_int
                {
                socksstate(data, CONNECT_REQ_INIT);
                current_block_276 = 6765806975593869225;
            } else {
                if *socksreq.offset(1 as libc::c_int as isize) as libc::c_int
                    == 2 as libc::c_int
                {
    ...
    match current_block_276 {
        6177381118916711871 => {
            let mut proxy_user_len: size_t = 0;
            let mut proxy_password_len: size_t = 0;
            if !proxy_user.is_null() && !proxy_password.is_null() {
                proxy_user_len = strlen(proxy_user);
                proxy_password_len = strlen(proxy_password);
            } else {
                proxy_user_len = 0 as libc::c_int as size_t;
                proxy_password_len = 0 as libc::c_int as size_t;
            }
            len = 0 as libc::c_int as ssize_t;
            let fresh18 = len;
            len = len + 1;
            *socksreq.offset(fresh18 as isize) = 1 as libc::c_int as libc::c_uchar;
            let fresh19 = len;
            len = len + 1;
            *socksreq.offset(fresh19 as isize) = proxy_user_len as libc::c_uchar;
            if !proxy_user.is_null() && proxy_user_len != 0 {
                if proxy_user_len >= 255 as libc::c_int as libc::c_ulong {
                    Curl_failf(
                        data,
            ...
            len = (len as libc::c_ulong).wrapping_add(proxy_user_len) as ssize_t
                as ssize_t;
            let fresh20 = len;
            len = len + 1;
            *socksreq.offset(fresh20 as isize) = proxy_password_len as libc::c_uchar;
            if !proxy_password.is_null() && proxy_password_len != 0 {
                if proxy_password_len > 255 as libc::c_int as libc::c_ulong {
                    Curl_failf(
                        data,
            ...
            len = (len as libc::c_ulong).wrapping_add(proxy_password_len) as ssize_t
                as ssize_t;
            socksstate(data, CONNECT_AUTH_SEND);
            (*sx).outstanding = len;
            let ref mut fresh21 = (*sx).outp;
            *fresh21 = socksreq;
            current_block_276 = 2986675497777506938;
        }
        _ => {}
    ...
    match current_block_276 {
        2986675497777506938 => {
            ...
            if (*sx).outstanding != written {
                let ref mut fresh22 = (*sx).outstanding;
                *fresh22 -= written;
                let ref mut fresh23 = (*sx).outp;
                *fresh23 = (*fresh23).offset(written as isize);
                return CURLPX_OK;
            }
            let ref mut fresh24 = (*sx).outp;
            *fresh24 = socksreq;
            (*sx).outstanding = 2 as libc::c_int as ssize_t;
        ...
        5249903830285462583 => {
                ...
                && CURLE_AGAIN as libc::c_int as libc::c_uint != result as libc::c_uint
            {
                ...
                if result as u64 == 0 && actualread == 0 {
                    ...
                    if actualread != (*sx).outstanding {
                        let ref mut fresh25 = (*sx).outstanding;
                        *fresh25 -= actualread;
                        let ref mut fresh26 = (*sx).outp;
                        *fresh26 = (*fresh26).offset(actualread as isize);
                        return CURLPX_OK;
                    } else {
                        if *socksreq.offset(1 as libc::c_int as isize) != 0 {
                            Curl_failf(
                                data,
                                b"User was rejected by the SOCKS5 server (%d %d).\0"
                                    as *const u8 as *const libc::c_char,
                                *socksreq.offset(0 as libc::c_int as isize) as libc::c_int,
                                *socksreq.offset(1 as libc::c_int as isize) as libc::c_int,
                            );
    ...
    match current_block_276 {
        4992401317151232712 => {
            ...
            let fresh36 = len;
            len = len + 1;
            *socksreq.offset(fresh36 as isize) = 5 as libc::c_int as libc::c_uchar;
            let fresh37 = len;
            len = len + 1;
            *socksreq.offset(fresh37 as isize) = 1 as libc::c_int as libc::c_uchar;
            let fresh38 = len;
            len = len + 1;
            *socksreq.offset(fresh38 as isize) = 0 as libc::c_int as libc::c_uchar;
            if !socks5_resolve_local {
                let fresh39 = len;
                len = len + 1;
                *socksreq.offset(fresh39 as isize) = 3 as libc::c_int as libc::c_uchar;
                let fresh40 = len;
                len = len + 1;
                *socksreq
                    .offset(
                        fresh40 as isize,
                    ) = hostname_len as libc::c_char as libc::c_uchar;
                memcpy(
                    &mut *socksreq.offset(len as isize) as *mut libc::c_uchar
                        as *mut libc::c_void,
                    hostname as *const libc::c_void,
                    hostname_len,
                );
                len = (len as libc::c_ulong).wrapping_add(hostname_len) as ssize_t
                    as ssize_t;
                Curl_infof(
                    data,
        ...
        6938158527927677584 => {
            ...
            let fresh29 = len;
            len = len + 1;
            *socksreq.offset(fresh29 as isize) = 5 as libc::c_int as libc::c_uchar;
            let fresh30 = len;
            len = len + 1;
            *socksreq.offset(fresh30 as isize) = 1 as libc::c_int as libc::c_uchar;
            let fresh31 = len;
            len = len + 1;
            *socksreq.offset(fresh31 as isize) = 0 as libc::c_int as libc::c_uchar;
            if (*hp).ai_family == 2 as libc::c_int {
                let mut i: libc::c_int = 0;
                let mut saddr_in: *mut sockaddr_in = 0 as *mut sockaddr_in;
                let fresh32 = len;
                len = len + 1;
                *socksreq.offset(fresh32 as isize) = 1 as libc::c_int as libc::c_uchar;
                saddr_in = (*hp).ai_addr as *mut libc::c_void as *mut sockaddr_in;
                i = 0 as libc::c_int;
            ...
            } else if (*hp).ai_family == 10 as libc::c_int {
                let mut i_0: libc::c_int = 0;
                let mut saddr_in6: *mut sockaddr_in6 = 0 as *mut sockaddr_in6;
                let fresh34 = len;
                len = len + 1;
                *socksreq.offset(fresh34 as isize) = 4 as libc::c_int as libc::c_uchar;
                ...
                while i_0 < 16 as libc::c_int {
                    ...
                    *socksreq
                        ...
                        as *mut [uint8_t; 16] as *mut libc::c_uchar)
                        .offset(i_0 as isize);
                    i_0 += 1;
                }
                Curl_infof(
    ...
    match current_block_276 {
        1406533880978078420 => {
            ...
            if (*sx).outstanding != written {
                let ref mut fresh44 = (*sx).outstanding;
                *fresh44 -= written;
                let ref mut fresh45 = (*sx).outp;
                *fresh45 = (*fresh45).offset(written as isize);
                return CURLPX_OK;
            }
            (*sx).outstanding = 10 as libc::c_int as ssize_t;
            let ref mut fresh46 = (*sx).outp;
            *fresh46 = socksreq;
        ...
        17002506680330119909 => {
                ...
                && CURLE_AGAIN as libc::c_int as libc::c_uint != result as libc::c_uint
            {
                ...
                if result as u64 == 0 && actualread == 0 {
                    ...
                    if actualread != (*sx).outstanding {
                        let ref mut fresh47 = (*sx).outstanding;
                        *fresh47 -= actualread;
                        let ref mut fresh48 = (*sx).outp;
                        *fresh48 = (*fresh48).offset(actualread as isize);
                        return CURLPX_OK;
                    }
                }
            }
            if *socksreq.offset(0 as libc::c_int as isize) as libc::c_int
                != 5 as libc::c_int
            {
                Curl_failf(
                    data,
                ...
                if *socksreq.offset(1 as libc::c_int as isize) != 0 {
                    let mut rc_0: CURLproxycode = CURLPX_REPLY_UNASSIGNED;
                    let mut code: libc::c_int = *socksreq
                    ...
                    if code < 9 as libc::c_int {
                        static mut lookup: [CURLproxycode; 9] = [
                            CURLPX_OK,
            ...
            if *socksreq.offset(3 as libc::c_int as isize) as libc::c_int
                == 3 as libc::c_int
            {
                let mut addrlen: libc::c_int = *socksreq
                    .offset(4 as libc::c_int as isize) as libc::c_int;
                len = (5 as libc::c_int + addrlen + 2 as libc::c_int) as ssize_t;
            } else if *socksreq.offset(3 as libc::c_int as isize) as libc::c_int
                    == 4 as libc::c_int
                {
                len = (4 as libc::c_int + 16 as libc::c_int + 2 as libc::c_int)
                    as ssize_t;
            } else if *socksreq.offset(3 as libc::c_int as isize) as libc::c_int
                    == 1 as libc::c_int
                {
                len = (4 as libc::c_int + 4 as libc::c_int + 2 as libc::c_int)
                    as ssize_t;
            } else {
                Curl_failf(
                    data,
            ...
            if len > 10 as libc::c_int as libc::c_long {
                (*sx).outstanding = len - 10 as libc::c_int as libc::c_long;
                let ref mut fresh49 = (*sx).outp;
                *fresh49 = &mut *socksreq.offset(10 as libc::c_int as isize)
                    as *mut libc::c_uchar;
        ...
        5405868585544072297 => {
                ...
                && CURLE_AGAIN as libc::c_int as libc::c_uint != result as libc::c_uint
            {
                ...
                if result as u64 == 0 && actualread == 0 {
                    ...
                    if actualread != (*sx).outstanding {
                        let ref mut fresh50 = (*sx).outstanding;
                        *fresh50 -= actualread;
                        let ref mut fresh51 = (*sx).outp;
    ...
    Curl_infof(data, b"SOCKS5 request granted.\0" as *const u8 as *const libc::c_char);
    *done = 1 as libc::c_int != 0;
    return CURLPX_OK;
}
