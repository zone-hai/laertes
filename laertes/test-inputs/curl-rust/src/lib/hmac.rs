...
static mut hmac_ipad: libc::c_uchar = 0x36 as libc::c_int as libc::c_uchar;
static mut hmac_opad: libc::c_uchar = 0x5c as libc::c_int as libc::c_uchar;
#[no_mangle]
pub unsafe extern "C" fn Curl_HMAC_init(
    mut hashparams: *const HMAC_params,
    mut key: *const libc::c_uchar,
    mut keylen: libc::c_uint,
) -> *mut HMAC_context {
    let mut i: size_t = 0;
    let mut ctxt: *mut HMAC_context = 0 as *mut HMAC_context;
    let mut hkey: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut b: libc::c_uchar = 0;
    i = (::std::mem::size_of::<HMAC_context>() as libc::c_ulong)
        .wrapping_add(
            (2 as libc::c_int as libc::c_uint).wrapping_mul((*hashparams).hmac_ctxtsize)
                as libc::c_ulong,
        )
        .wrapping_add((*hashparams).hmac_resultlen as libc::c_ulong);
    ...
    *fresh1 = ctxt.offset(1 as libc::c_int as isize) as *mut libc::c_void;
    let ref mut fresh2 = (*ctxt).hmac_hashctxt2;
    *fresh2 = ((*ctxt).hmac_hashctxt1 as *mut libc::c_char)
    ...
    if keylen > (*hashparams).hmac_maxkeylen {
        ...
        hkey = ((*ctxt).hmac_hashctxt2 as *mut libc::c_uchar)
    ...
    while i < keylen as libc::c_ulong {
        b = (*key as libc::c_int ^ hmac_ipad as libc::c_int) as libc::c_uchar;
        (Some(((*hashparams).hmac_hupdate).expect("non-null function pointer")))
            .expect(
                "non-null function pointer",
            )((*ctxt).hmac_hashctxt1, &mut b, 1 as libc::c_int as libc::c_uint);
        let fresh3 = key;
        key = key.offset(1);
        b = (*fresh3 as libc::c_int ^ hmac_opad as libc::c_int) as libc::c_uchar;
        (Some(((*hashparams).hmac_hupdate).expect("non-null function pointer")))
            .expect(
                "non-null function pointer",
            )((*ctxt).hmac_hashctxt2, &mut b, 1 as libc::c_int as libc::c_uint);
        i = i.wrapping_add(1);
    }
    while i < (*hashparams).hmac_maxkeylen as libc::c_ulong {
        (Some(((*hashparams).hmac_hupdate).expect("non-null function pointer")))
            .expect(
                "non-null function pointer",
            )((*ctxt).hmac_hashctxt1, &hmac_ipad, 1 as libc::c_int as libc::c_uint);
        (Some(((*hashparams).hmac_hupdate).expect("non-null function pointer")))
            .expect(
                "non-null function pointer",
            )((*ctxt).hmac_hashctxt2, &hmac_opad, 1 as libc::c_int as libc::c_uint);
        i = i.wrapping_add(1);
    }
    return ctxt;
}
...
pub unsafe extern "C" fn Curl_HMAC_final(
    ...
    let mut hashparams: *const HMAC_params = (*ctxt).hmac_hash;
    if result.is_null() {
        result = ((*ctxt).hmac_hashctxt2 as *mut libc::c_uchar)
    ...
    Curl_cfree
        .expect(
            "non-null function pointer",
        )(ctxt as *mut libc::c_char as *mut libc::c_void);
    return 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_hmacit(
    ...
    let mut ctxt: *mut HMAC_context = Curl_HMAC_init(
        hashparams,
        key,
        curlx_uztoui(keylen),
    );
