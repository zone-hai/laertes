...
extern "C" {
    ...
    fn Curl_psl_release(easy: *mut Curl_easy);
    fn Curl_psl_use(easy: *mut Curl_easy) -> *const psl_ctx_t;
    fn psl_is_cookie_domain_acceptable(
        psl: *const psl_ctx_t,
        hostname: *const libc::c_char,
        cookie_domain: *const libc::c_char,
    ) -> libc::c_int;
...
unsafe extern "C" fn tailmatch(
    mut cooke_domain: *const libc::c_char,
...
unsafe extern "C" fn pathmatch(
    ...
    if !pos.is_null() {
        *pos = 0 as libc::c_int as libc::c_char;
    }
    if 0 as libc::c_int as libc::c_ulong == strlen(uri_path)
        || *uri_path.offset(0 as libc::c_int as isize) as libc::c_int != '/' as i32
    {
        strstore(&mut uri_path, b"/\0" as *const u8 as *const libc::c_char);
        if uri_path.is_null() {
            return 0 as libc::c_int != 0;
        }
    }
    ...
    } else if *uri_path.offset(cookie_path_len as isize) as libc::c_int == '/' as i32 {
        ret = 1 as libc::c_int != 0;
    } else {
        ret = 0 as libc::c_int != 0;
    }
    ...
    if !domain.is_null() {
        ...
        if !last.is_null() {
            first = Curl_memrchr(
                ...
                '.' as i32,
                last.offset_from(domain) as libc::c_long as size_t,
            ) as *const libc::c_char;
            if !first.is_null() {
                first = first.offset(1);
                len = (len as libc::c_ulong)
                    .wrapping_sub(
                        first.offset_from(domain) as libc::c_long as libc::c_ulong,
                    ) as size_t as size_t;
            }
        }
    }
...
unsafe extern "C" fn cookie_hash_domain(
    ...
    let mut end: *const libc::c_char = domain.offset(len as isize);
    ...
    while domain < end {
        ...
        h ^= Curl_raw_toupper(*fresh0) as libc::c_ulong;
    }
    return h.wrapping_rem(256 as libc::c_int as libc::c_ulong);
}
unsafe extern "C" fn cookiehash(domain: *const libc::c_char) -> size_t {
    let mut top: *const libc::c_char = 0 as *const libc::c_char;
    let mut len: size_t = 0;
    if domain.is_null() || Curl_host_is_ipnum(domain) as libc::c_int != 0 {
...
unsafe extern "C" fn sanitize_cookie_path(
    ...
    let mut len: size_t = 0;
    let mut new_path: *mut libc::c_char = Curl_cstrdup
        .expect("non-null function pointer")(cookie_path);
    if new_path.is_null() {
        return 0 as *mut libc::c_char;
    }
    len = strlen(new_path);
    if *new_path.offset(0 as libc::c_int as isize) as libc::c_int == '"' as i32 {
        memmove(
            new_path as *mut libc::c_void,
            new_path.offset(1 as libc::c_int as isize) as *const libc::c_void,
            len,
        );
        len = len.wrapping_sub(1);
    }
    if len != 0
        && *new_path.offset(len.wrapping_sub(1 as libc::c_int as libc::c_ulong) as isize)
    ...
    if *new_path.offset(0 as libc::c_int as isize) as libc::c_int != '/' as i32 {
        strstore(&mut new_path, b"/\0" as *const u8 as *const libc::c_char);
        return new_path;
    }
    if len != 0
        && *new_path.offset(len.wrapping_sub(1 as libc::c_int as libc::c_ulong) as isize)
    ...
    if !list.is_null() {
        ...
        while !list.is_null() {
            ...
            if newcookies.is_null() {
                ...
                let ref mut fresh1 = (*data).cookies;
                *fresh1 = newcookies;
            }
            list = (*list).next;
        }
        curl_slist_free_all((*data).state.cookielist);
        let ref mut fresh2 = (*data).state.cookielist;
...
unsafe extern "C" fn remove_expired(mut cookies: *mut CookieInfo) {
    ...
    if now < (*cookies).next_expiration
        && (*cookies).next_expiration != 0x7fffffffffffffff as libc::c_long
    {
        return
    } else {
        (*cookies).next_expiration = 0x7fffffffffffffff as libc::c_long;
    ...
    while i < 256 as libc::c_int as libc::c_uint {
        ...
        while !co.is_null() {
            ...
            if (*co).expires != 0 && (*co).expires < now {
                if pv.is_null() {
                    let ref mut fresh3 = (*cookies).cookies[i as usize];
                    *fresh3 = (*co).next;
                } else {
                    let ref mut fresh4 = (*pv).next;
                    *fresh4 = (*co).next;
                }
                let ref mut fresh5 = (*cookies).numcookies;
                *fresh5 -= 1;
                freecookie(co);
            } else {
...
unsafe extern "C" fn bad_domain(mut domain: *const libc::c_char) -> bool {
...
pub unsafe extern "C" fn Curl_cookie_add(
    ...
    let mut replace_old: bool = 0 as libc::c_int != 0;
    let mut badcookie: bool = 0 as libc::c_int != 0;
    let mut myhash: size_t = 0;
    co = Curl_ccalloc
        .expect(
            "non-null function pointer",
        )(1 as libc::c_int as size_t, ::std::mem::size_of::<Cookie>() as libc::c_ulong)
        as *mut Cookie;
    if co.is_null() {
    ...
    if httpheader {
        ...
        let mut what: [libc::c_char; 4096] = [0; 4096];
        let mut ptr: *const libc::c_char = 0 as *const libc::c_char;
        let mut semiptr: *const libc::c_char = 0 as *const libc::c_char;
        let mut linelength: size_t = strlen(lineptr);
        if linelength > 5000 as libc::c_int as libc::c_ulong {
            Curl_cfree.expect("non-null function pointer")(co as *mut libc::c_void);
            return 0 as *mut Cookie;
        }
        semiptr = strchr(lineptr, ';' as i32);
        while *lineptr as libc::c_int != 0
            && (*lineptr as libc::c_uchar as libc::c_int == ' ' as i32
                || *lineptr as libc::c_uchar as libc::c_int == '\t' as i32)
        {
        ...
        ptr = lineptr;
        loop {
            what[0 as libc::c_int as usize] = 0 as libc::c_int as libc::c_char;
            name[0 as libc::c_int as usize] = what[0 as libc::c_int as usize];
            if 1 as libc::c_int
                <= sscanf(
                    ptr,
                    b"%4095[^;\r\n=] =%4095[^;\r\n]\0" as *const u8
            ...
            {
                let mut whatptr: *const libc::c_char = 0 as *const libc::c_char;
                ...
                let mut endofn: *const libc::c_char = &*ptr.offset(nlen as isize)
                    as *const libc::c_char;
                if nlen >= (4096 as libc::c_int - 1 as libc::c_int) as libc::c_ulong
                    || len >= (4096 as libc::c_int - 1 as libc::c_int) as libc::c_ulong
                    || nlen.wrapping_add(len) > 4096 as libc::c_int as libc::c_ulong
                {
                    freecookie(co);
                    Curl_infof(
                        data,
                ...
                sep = if *endofn as libc::c_int == '=' as i32 {
                    1 as libc::c_int
                } else {
                    0 as libc::c_int
                } != 0;
                if nlen != 0 {
                    ...
                    if *endofn as libc::c_uchar as libc::c_int == ' ' as i32
                        || *endofn as libc::c_uchar as libc::c_int == '\t' as i32
                    {
                        while *endofn as libc::c_int != 0
                            && (*endofn as libc::c_uchar as libc::c_int == ' ' as i32
                                || *endofn as libc::c_uchar as libc::c_int == '\t' as i32)
                            && nlen != 0
                        {
                        ...
                        name[nlen as usize] = 0 as libc::c_int as libc::c_char;
                    }
                }
                while len != 0
                    && (what[len.wrapping_sub(1 as libc::c_int as libc::c_ulong)
                        as usize] as libc::c_uchar as libc::c_int == ' ' as i32
                        || what[len.wrapping_sub(1 as libc::c_int as libc::c_ulong)
                            as usize] as libc::c_uchar as libc::c_int == '\t' as i32)
                {
                    what[len.wrapping_sub(1 as libc::c_int as libc::c_ulong)
                ...
                while *whatptr as libc::c_int != 0
                    && (*whatptr as libc::c_uchar as libc::c_int == ' ' as i32
                        || *whatptr as libc::c_uchar as libc::c_int == '\t' as i32)
                {
                    whatptr = whatptr.offset(1);
                }
                if nlen > 3 as libc::c_int as libc::c_ulong
                    && name[0 as libc::c_int as usize] as libc::c_int == '_' as i32
                    && name[1 as libc::c_int as usize] as libc::c_int == '_' as i32
                {
                    if strncmp(
                ...
                if ((*co).name).is_null() {
                    if !sep {
                        badcookie = 1 as libc::c_int != 0;
                        break;
                    } else {
                        let ref mut fresh8 = (*co).name;
                        *fresh8 = Curl_cstrdup
                            .expect("non-null function pointer")(name.as_mut_ptr());
                        let ref mut fresh9 = (*co).value;
                        *fresh9 = Curl_cstrdup
                    ...
                    {
                        if secure as libc::c_int != 0 || !(*c).running {
                            (*co).secure = 1 as libc::c_int != 0;
                        } else {
                            badcookie = 1 as libc::c_int != 0;
                            break;
                        }
                ...
                if !done {
                    ...
                    ) != 0
                    {
                        ...
                        if '.' as i32
                            == *whatptr.offset(0 as libc::c_int as isize) as libc::c_int
                        {
                            whatptr = whatptr.offset(1);
                        }
                        ...
                        if domain.is_null()
                            || is_ip as libc::c_int != 0 && strcmp(whatptr, domain) == 0
                            || !is_ip && tailmatch(whatptr, domain) as libc::c_int != 0
                        {
                            ...
                            if ((*co).domain).is_null() {
                                ...
                                (*co).tailmatch = 1 as libc::c_int != 0;
                            }
                        } else {
                            badcookie = 1 as libc::c_int != 0;
                            Curl_infof(
            ...
            if semiptr.is_null() || *semiptr == 0 {
                semiptr = 0 as *const libc::c_char;
            } else {
                ptr = semiptr.offset(1 as libc::c_int as isize);
                while *ptr as libc::c_int != 0
                    && (*ptr as libc::c_uchar as libc::c_int == ' ' as i32
                        || *ptr as libc::c_uchar as libc::c_int == '\t' as i32)
                {
                    ptr = ptr.offset(1);
                }
                semiptr = strchr(ptr, ';' as i32);
                if semiptr.is_null() && *ptr as libc::c_int != 0 {
        ...
        if !((*co).maxage).is_null() {
            ...
            offt = curlx_strtoofft(
                if *(*co).maxage as libc::c_int == '"' as i32 {
                    &mut *((*co).maxage).offset(1 as libc::c_int as isize)
                } else {
                    &mut *((*co).maxage).offset(0 as libc::c_int as isize)
                },
                0 as *mut *mut libc::c_char,
                10 as libc::c_int,
                &mut (*co).expires,
            );
            if offt as libc::c_uint == CURL_OFFT_FLOW as libc::c_int as libc::c_uint {
                (*co).expires = 0x7fffffffffffffff as libc::c_long;
            } else if offt as u64 == 0 {
                if (*co).expires == 0 {
                    (*co).expires = 1 as libc::c_int as curl_off_t;
                } else if 0x7fffffffffffffff as libc::c_long - now < (*co).expires {
                    (*co).expires = 0x7fffffffffffffff as libc::c_long;
                } else {
                    let ref mut fresh11 = (*co).expires;
                    *fresh11 += now;
                }
            }
        } else if !((*co).expirestr).is_null() {
            (*co).expires = Curl_getdate_capped((*co).expirestr);
            if (*co).expires == 0 as libc::c_int as libc::c_long {
                (*co).expires = 1 as libc::c_int as curl_off_t;
            } else if (*co).expires < 0 as libc::c_int as libc::c_long {
                (*co).expires = 0 as libc::c_int as curl_off_t;
            }
        }
        if !badcookie && ((*co).domain).is_null() {
            if !domain.is_null() {
                let ref mut fresh12 = (*co).domain;
        ...
        if !badcookie && ((*co).path).is_null() && !path.is_null() {
            let mut queryp: *mut libc::c_char = strchr(path, '?' as i32);
            let mut endslash: *mut libc::c_char = 0 as *mut libc::c_char;
            if queryp.is_null() {
                ...
                endslash = Curl_memrchr(
                    ...
                    '/' as i32,
                    queryp.offset_from(path) as libc::c_long as size_t,
                ) as *mut libc::c_char;
            }
            if !endslash.is_null() {
                let mut pathlen: size_t = (endslash.offset_from(path) as libc::c_long
                    + 1 as libc::c_int as libc::c_long) as size_t;
                let ref mut fresh13 = (*co).path;
                *fresh13 = Curl_cmalloc
                ...
                if !((*co).path).is_null() {
                    ...
                    *((*co).path)
                        .offset(pathlen as isize) = 0 as libc::c_int as libc::c_char;
                    let ref mut fresh14 = (*co).spath;
        ...
        if badcookie as libc::c_int != 0 || ((*co).name).is_null() {
            freecookie(co);
            return 0 as *mut Cookie;
        }
    } else {
        let mut ptr_0: *mut libc::c_char = 0 as *mut libc::c_char;
        let mut firstptr: *mut libc::c_char = 0 as *mut libc::c_char;
        ...
        {
            lineptr = lineptr.offset(10 as libc::c_int as isize);
            (*co).httponly = 1 as libc::c_int != 0;
        }
        if *lineptr.offset(0 as libc::c_int as isize) as libc::c_int == '#' as i32 {
            Curl_cfree.expect("non-null function pointer")(co as *mut libc::c_void);
            return 0 as *mut Cookie;
        }
        ptr_0 = strchr(lineptr, '\r' as i32);
        if !ptr_0.is_null() {
            *ptr_0 = 0 as libc::c_int as libc::c_char;
        }
        ...
        while !ptr_0.is_null() && !badcookie {
            ...
            match fields {
                0 => {
                    if *ptr_0.offset(0 as libc::c_int as isize) as libc::c_int
                        == '.' as i32
                    {
                        ptr_0 = ptr_0.offset(1);
                    }
                    let ref mut fresh15 = (*co).domain;
                ...
                2 => {
                    if strcmp(b"TRUE\0" as *const u8 as *const libc::c_char, ptr_0) != 0
                        && strcmp(b"FALSE\0" as *const u8 as *const libc::c_char, ptr_0)
                            != 0
                    {
                        let ref mut fresh16 = (*co).path;
                        *fresh16 = Curl_cstrdup
                            .expect("non-null function pointer")(ptr_0);
                        if ((*co).path).is_null() {
                            badcookie = 1 as libc::c_int != 0;
                        } else {
                            let ref mut fresh17 = (*co).spath;
            ...
            match current_block_187 {
                15249084414975911002 => {
                    (*co).secure = 0 as libc::c_int != 0;
                    if Curl_strcasecompare(
                        ptr_0,
                        b"TRUE\0" as *const u8 as *const libc::c_char,
                    ) != 0
                    {
                        if secure as libc::c_int != 0 || (*c).running as libc::c_int != 0
                        {
                            (*co).secure = 1 as libc::c_int != 0;
                        } else {
                            badcookie = 1 as libc::c_int != 0;
        ...
        if 6 as libc::c_int == fields {
            let ref mut fresh24 = (*co).value;
        ...
        if !badcookie && 7 as libc::c_int != fields {
            badcookie = 1 as libc::c_int != 0;
        }
        if badcookie {
            freecookie(co);
    ...
    if (*co).prefix as libc::c_int & (1 as libc::c_int) << 0 as libc::c_int != 0 {
        if !(*co).secure {
            freecookie(co);
            return 0 as *mut Cookie;
        }
    }
    if (*co).prefix as libc::c_int & (1 as libc::c_int) << 1 as libc::c_int != 0 {
        if (*co).secure as libc::c_int != 0 && !((*co).path).is_null()
            && strcmp((*co).path, b"/\0" as *const u8 as *const libc::c_char)
                == 0 as libc::c_int && !(*co).tailmatch
        {} else {
            freecookie(co);
            return 0 as *mut Cookie;
        }
    }
    if !(*c).running && (*c).newsession as libc::c_int != 0 && (*co).expires == 0 {
        freecookie(co);
        return 0 as *mut Cookie;
    }
    (*co).livecookie = (*c).running;
    let ref mut fresh25 = (*c).lastct;
    *fresh25 += 1;
            ...
            && !Curl_host_is_ipnum((*co).domain))
    {
        ...
        let mut acceptable: libc::c_int = 0;
        if !psl.is_null() {
            acceptable = psl_is_cookie_domain_acceptable(psl, domain, (*co).domain);
            Curl_psl_release(data);
        } else {
            acceptable = !bad_domain(domain) as libc::c_int;
        }
        if acceptable == 0 {
            Curl_infof(
                data,
        ...
        if Curl_strcasecompare((*clist).name, (*co).name) != 0 {
            if !((*clist).domain).is_null() && !((*co).domain).is_null() {
                if Curl_strcasecompare((*clist).domain, (*co).domain) != 0
                    && (*clist).tailmatch as libc::c_int
                        == (*co).tailmatch as libc::c_int
                {
                    replace_old = 1 as libc::c_int != 0;
                }
            ...
            if replace_old {
                if !((*clist).spath).is_null() && !((*co).spath).is_null() {
                    if (*clist).secure as libc::c_int != 0 && !(*co).secure && !secure {
                        let mut cllen: size_t = 0;
                        let mut sep_0: *const libc::c_char = 0 as *const libc::c_char;
                        sep_0 = strchr(
                            ((*clist).spath).offset(1 as libc::c_int as isize),
                            '/' as i32,
                        );
                        if !sep_0.is_null() {
                            cllen = sep_0.offset_from((*clist).spath) as libc::c_long
                                as size_t;
                        } else {
            ...
            if replace_old as libc::c_int != 0 && !(*co).livecookie
                && (*clist).livecookie as libc::c_int != 0
            {
                freecookie(co);
                return 0 as *mut Cookie;
            }
            if replace_old {
                let ref mut fresh26 = (*co).next;
                *fresh26 = (*clist).next;
    ...
    if (*c).running {
        Curl_infof(
            ...
            b"%s cookie %s=\"%s\" for domain %s, path %s, expire %ld\0" as *const u8
                as *const libc::c_char,
            if replace_old as libc::c_int != 0 {
                b"Replaced\0" as *const u8 as *const libc::c_char
            } else {
    ...
    if !replace_old {
        if !lastc.is_null() {
            let ref mut fresh27 = (*lastc).next;
            *fresh27 = co;
        } else {
            let ref mut fresh28 = (*c).cookies[myhash as usize];
            *fresh28 = co;
        }
        let ref mut fresh29 = (*c).numcookies;
        *fresh29 += 1;
    }
...
pub unsafe extern "C" fn Curl_cookie_init(
    ...
    let mut fromfile: bool = 1 as libc::c_int != 0;
    ...
    if inc.is_null() {
        c = Curl_ccalloc
            ...
            ::std::mem::size_of::<CookieInfo>() as libc::c_ulong,
        ) as *mut CookieInfo;
        if c.is_null() {
            return 0 as *mut CookieInfo;
        }
        let ref mut fresh30 = (*c).filename;
        *fresh30 = Curl_cstrdup
        ...
        if ((*c).filename).is_null() {
            ...
            (*c).next_expiration = 0x7fffffffffffffff as libc::c_long;
    ...
    match current_block {
        11650488183268122163 => {
            (*c).running = 0 as libc::c_int != 0;
            if !file.is_null()
                && strcmp(file, b"-\0" as *const u8 as *const libc::c_char) == 0
            {
                fp = stdin;
                fromfile = 0 as libc::c_int != 0;
            } else if !file.is_null() && *file == 0 {
                fp = 0 as *mut FILE;
            } else {
                fp = if !file.is_null() {
                    fopen(file, b"r\0" as *const u8 as *const libc::c_char)
                } else {
                    0 as *mut FILE
                };
            }
            ...
            if !fp.is_null() {
                ...
                line = Curl_cmalloc
                    .expect("non-null function pointer")(5000 as libc::c_int as size_t)
                    as *mut libc::c_char;
                if line.is_null() {
                    current_block = 3471203105459249385;
                } else {
                    while !(Curl_get_line(line, 5000 as libc::c_int, fp)).is_null() {
                        if curl_strnequal(
                        ...
                        {
                            lineptr = &mut *line.offset(11 as libc::c_int as isize)
                                as *mut libc::c_char;
                            headerline = 1 as libc::c_int != 0;
                        } else {
                            lineptr = line;
                            headerline = 0 as libc::c_int != 0;
                        }
                        while *lineptr as libc::c_int != 0
                            && (*lineptr as libc::c_uchar as libc::c_int == ' ' as i32
                                || *lineptr as libc::c_uchar as libc::c_int == '\t' as i32)
            ...
            match current_block {
                3471203105459249385 => {}
                _ => {
                    (*c).running = 1 as libc::c_int != 0;
                    if !data.is_null() {
                        let ref mut fresh31 = (*data).state;
                        (*fresh31).set_cookie_engine(1 as libc::c_int as bit);
                    }
                    return c;
                }
            }
    ...
    if fromfile as libc::c_int != 0 && !fp.is_null() {
        fclose(fp);
    }
    return 0 as *mut CookieInfo;
}
unsafe extern "C" fn cookie_sort(
    ...
    if l1 != l2 {
        return if l2 > l1 { 1 as libc::c_int } else { -(1 as libc::c_int) };
    }
...
unsafe extern "C" fn dup_cookie(mut src: *mut Cookie) -> *mut Cookie {
    ...
    let mut d: *mut Cookie = Curl_ccalloc
        .expect(
            "non-null function pointer",
        )(::std::mem::size_of::<Cookie>() as libc::c_ulong, 1 as libc::c_int as size_t)
        as *mut Cookie;
    if !d.is_null() {
        if !((*src).expirestr).is_null() {
            let ref mut fresh32 = (*d).expirestr;
...
pub unsafe extern "C" fn Curl_cookie_getlist(
    ...
    let mut matches: size_t = 0 as libc::c_int as size_t;
    let mut is_ip: bool = false;
    ...
    loop {
        ...
        if if (*co).secure as libc::c_int != 0 {
            secure as libc::c_int
        } else {
            1 as libc::c_int
        } != 0
        {
            if ((*co).domain).is_null()
                || (*co).tailmatch as libc::c_int != 0 && !is_ip
                    && tailmatch((*co).domain, host) as libc::c_int != 0
                || (!(*co).tailmatch || is_ip as libc::c_int != 0)
                    && Curl_strcasecompare(host, (*co).domain) != 0
            {
                if ((*co).spath).is_null()
                    || pathmatch((*co).spath, path) as libc::c_int != 0
                {
                    newco = dup_cookie(co);
                    if newco.is_null() {
                    ...
                    let ref mut fresh40 = (*newco).next;
                    *fresh40 = mainco;
                    mainco = newco;
        ...
        4808432441040389987 => {
            if matches != 0 {
                ...
                array = Curl_cmalloc
                    ...
                    (::std::mem::size_of::<*mut Cookie>() as libc::c_ulong)
                ...
                if array.is_null() {
                    ...
                    while !co.is_null() {
                        let fresh41 = i;
                        i = i.wrapping_add(1);
                        let ref mut fresh42 = *array.offset(fresh41 as isize);
                        *fresh42 = co;
                        co = (*co).next;
                    }
                    qsort(
                    ...
                    mainco = *array.offset(0 as libc::c_int as isize);
                    i = 0 as libc::c_int as size_t;
                    while i < matches.wrapping_sub(1 as libc::c_int as libc::c_ulong) {
                        let ref mut fresh43 = (**array.offset(i as isize)).next;
                        *fresh43 = *array
                            .offset(
                    ...
                    let ref mut fresh44 = (**array
                        .offset(
                            matches.wrapping_sub(1 as libc::c_int as libc::c_ulong)
                                as isize,
                        ))
                        .next;
...
pub unsafe extern "C" fn Curl_cookie_clearall(mut cookies: *mut CookieInfo) {
    if !cookies.is_null() {
        ...
        while i < 256 as libc::c_int as libc::c_uint {
            ...
            let ref mut fresh45 = (*cookies).cookies[i as usize];
            *fresh45 = 0 as *mut Cookie;
            i = i.wrapping_add(1);
        }
        (*cookies).numcookies = 0 as libc::c_int as libc::c_long;
    }
}
#[no_mangle]
...
pub unsafe extern "C" fn Curl_cookie_clearsess(mut cookies: *mut CookieInfo) {
    ...
    while i < 256 as libc::c_int as libc::c_uint {
        if !((*cookies).cookies[i as usize]).is_null() {
            ...
            while !curr.is_null() {
                ...
                if (*curr).expires == 0 {
                    ...
                    if prev == curr {
                        ...
                        let ref mut fresh46 = (*prev).next;
                        *fresh46 = next;
                    }
                    freecookie(curr);
                    let ref mut fresh47 = (*cookies).numcookies;
                    *fresh47 -= 1;
                } else {
                    prev = curr;
                }
            ...
            let ref mut fresh48 = (*cookies).cookies[i as usize];
            *fresh48 = first;
        }
        i = i.wrapping_add(1);
    }
...
unsafe extern "C" fn get_netscape_format(mut co: *const Cookie) -> *mut libc::c_char {
    return curl_maprintf(
        b"%s%s%s\t%s\t%s\t%s\t%ld\t%s\t%s\0" as *const u8 as *const libc::c_char,
        if (*co).httponly as libc::c_int != 0 {
            b"#HttpOnly_\0" as *const u8 as *const libc::c_char
        } else {
            b"\0" as *const u8 as *const libc::c_char
        },
        if (*co).tailmatch as libc::c_int != 0 && !((*co).domain).is_null()
            && *((*co).domain).offset(0 as libc::c_int as isize) as libc::c_int
                != '.' as i32
        {
            b".\0" as *const u8 as *const libc::c_char
        } else {
        ...
        if !((*co).domain).is_null() {
            (*co).domain as *const libc::c_char
        } else {
            b"unknown\0" as *const u8 as *const libc::c_char
        },
        if (*co).tailmatch as libc::c_int != 0 {
            b"TRUE\0" as *const u8 as *const libc::c_char
        } else {
            b"FALSE\0" as *const u8 as *const libc::c_char
        },
        if !((*co).path).is_null() {
            (*co).path as *const libc::c_char
        } else {
            b"/\0" as *const u8 as *const libc::c_char
        },
        if (*co).secure as libc::c_int != 0 {
            b"TRUE\0" as *const u8 as *const libc::c_char
        } else {
            b"FALSE\0" as *const u8 as *const libc::c_char
        },
...
unsafe extern "C" fn cookie_output(
    ...
    let mut use_stdout: bool = 0 as libc::c_int != 0;
    let mut tempstore: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut error: CURLcode = CURLE_OK;
    if c.is_null() {
        return CURLE_OK;
    }
    remove_expired(c);
    if strcmp(b"-\0" as *const u8 as *const libc::c_char, filename) == 0 {
        out = stdout;
        use_stdout = 1 as libc::c_int != 0;
        current_block = 15652330335145281839;
    } else {
        let mut randsuffix: [libc::c_uchar; 9] = [0; 9];
        if Curl_rand_hex(
            data,
        ...
        tempstore = curl_maprintf(
            b"%s.%s.tmp\0" as *const u8 as *const libc::c_char,
            filename,
            randsuffix.as_mut_ptr(),
        );
        ...
        out = fopen(tempstore, b"w\0" as *const u8 as *const libc::c_char);
        if out.is_null() {
    ...
    match current_block {
        15652330335145281839 => {
            ...
            if (*c).numcookies != 0 {
                let mut i: libc::c_uint = 0;
                let mut nvalid: size_t = 0 as libc::c_int as size_t;
                let mut array: *mut *mut Cookie = 0 as *mut *mut Cookie;
                array = Curl_ccalloc
                    .expect(
                        "non-null function pointer",
                    ...
                    1 as libc::c_int as size_t,
                    (::std::mem::size_of::<*mut Cookie>() as libc::c_ulong)
                        .wrapping_mul((*c).numcookies as libc::c_ulong),
                ) as *mut *mut Cookie;
                if array.is_null() {
                    ...
                    while i < 256 as libc::c_int as libc::c_uint {
                        ...
                        while !co.is_null() {
                            if !((*co).domain).is_null() {
                                ...
                                let ref mut fresh50 = *array.offset(fresh49 as isize);
                                *fresh50 = co;
                            }
                            co = (*co).next;
                        }
                    ...
                    i = 0 as libc::c_int as libc::c_uint;
                    loop {
                        if !((i as libc::c_ulong) < nvalid) {
                            current_block = 18435049525520518667;
                            break;
                        }
                        let mut format_ptr: *mut libc::c_char = get_netscape_format(
...
unsafe extern "C" fn cookie_list(mut data: *mut Curl_easy) -> *mut curl_slist {
    ...
    if ((*data).cookies).is_null()
        || (*(*data).cookies).numcookies == 0 as libc::c_int as libc::c_long
    {
...
pub unsafe extern "C" fn Curl_flush_cookies(
    ...
    if !((*data).set.str_0[STRING_COOKIEJAR as libc::c_int as usize]).is_null() {
        ...
        res = cookie_output(
            data,
            (*data).cookies,
            (*data).set.str_0[STRING_COOKIEJAR as libc::c_int as usize],
        );
        if res as u64 != 0 {
            Curl_infof(
                data,
        ...
        if cleanup as libc::c_int != 0 && !((*data).state.cookielist).is_null() {
            curl_slist_free_all((*data).state.cookielist);
            let ref mut fresh51 = (*data).state.cookielist;
            *fresh51 = 0 as *mut curl_slist;
        }
        Curl_share_lock(data, CURL_LOCK_DATA_COOKIE, CURL_LOCK_ACCESS_SINGLE);
    }
    if cleanup as libc::c_int != 0
        && (((*data).share).is_null() || (*data).cookies != (*(*data).share).cookies)
    {
        Curl_cookie_cleanup((*data).cookies);
        let ref mut fresh52 = (*data).cookies;
