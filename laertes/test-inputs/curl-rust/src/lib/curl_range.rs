...
#[no_mangle]
pub unsafe extern "C" fn Curl_range(mut data: *mut Curl_easy) -> CURLcode {
    let mut from: curl_off_t = 0;
    let mut to: curl_off_t = 0;
    let mut ptr: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut ptr2: *mut libc::c_char = 0 as *mut libc::c_char;
    if ((*data).state).use_range() as libc::c_int != 0
        && !((*data).state.range).is_null()
    {
        let mut from_t: CURLofft = CURL_OFFT_OK;
        let mut to_t: CURLofft = CURL_OFFT_OK;
        ...
        if from_t as libc::c_uint == CURL_OFFT_FLOW as libc::c_int as libc::c_uint {
        ...
        to_t = curlx_strtoofft(ptr, &mut ptr2, 0 as libc::c_int, &mut to);
        if to_t as libc::c_uint == CURL_OFFT_FLOW as libc::c_int as libc::c_uint {
            return CURLE_RANGE_ERROR;
        }
        if to_t as libc::c_uint == CURL_OFFT_INVAL as libc::c_int as libc::c_uint
            && from_t as u64 == 0
        {
            (*data).state.resume_from = from;
        } else if from_t as libc::c_uint
                == CURL_OFFT_INVAL as libc::c_int as libc::c_uint && to_t as u64 == 0
            {
            (*data).req.maxdownload = to;
            (*data).state.resume_from = -to;
        } else {
            let mut totalsize: curl_off_t = 0;
            if from > to {
                return CURLE_RANGE_ERROR;
            }
            totalsize = to - from;
            if totalsize == 0x7fffffffffffffff as libc::c_long {
                return CURLE_RANGE_ERROR;
            }
            (*data).req.maxdownload = totalsize + 1 as libc::c_int as libc::c_long;
            (*data).state.resume_from = from;
        }
    } else {
