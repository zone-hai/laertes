...
extern "C" {
    ...
    fn psl_latest(fname: *const libc::c_char) -> *mut psl_ctx_t;
...
pub unsafe extern "C" fn Curl_psl_destroy(mut pslcache: *mut PslCache) {
    if !((*pslcache).psl).is_null() {
        ...
        let ref mut fresh0 = (*pslcache).psl;
        *fresh0 = 0 as *const psl_ctx_t;
        (*pslcache).dynamic = 0 as libc::c_int != 0;
    }
}
...
pub unsafe extern "C" fn Curl_psl_use(mut easy: *mut Curl_easy) -> *const psl_ctx_t {
    ...
    if ((*pslcache).psl).is_null() || (*pslcache).expires <= now {
        ...
        now = now_seconds();
        if ((*pslcache).psl).is_null() || (*pslcache).expires <= now {
            let mut dynamic: bool = 0 as libc::c_int != 0;
            let mut expires: time_t = 0x7fffffffffffffff as libc::c_long;
            psl = psl_latest(0 as *const libc::c_char);
            dynamic = !psl.is_null();
            expires = if now
                < 0x7fffffffffffffff as libc::c_long
                    - (72 as libc::c_int * 3600 as libc::c_int) as libc::c_long
            {
                now + (72 as libc::c_int * 3600 as libc::c_int) as libc::c_long
            } else {
                0x7fffffffffffffff as libc::c_long
            };
            ...
            if !psl.is_null() {
                ...
                let ref mut fresh1 = (*pslcache).psl;
                *fresh1 = psl;
