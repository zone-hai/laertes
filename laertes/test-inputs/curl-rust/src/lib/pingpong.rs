...
pub unsafe extern "C" fn Curl_pp_state_timeout(
    ...
    let mut response_time: timediff_t = if (*data).set.server_response_timeout != 0 {
    ...
    if (*data).set.timeout != 0 && !disconnecting {
        let mut timeout2_ms: timediff_t = (*data).set.timeout
...
pub unsafe extern "C" fn Curl_pp_statemach(
    ...
    let mut timeout_ms: timediff_t = Curl_pp_state_timeout(data, pp, disconnecting);
    let mut result: CURLcode = CURLE_OK;
    if timeout_ms <= 0 as libc::c_int as libc::c_long {
        Curl_failf(
            data,
            b"server response timeout\0" as *const u8 as *const libc::c_char,
        );
    ...
    if block {
        ...
        interval_ms = 0 as libc::c_int as timediff_t;
    }
    if Curl_ssl_data_pending(conn, 0 as libc::c_int) {
        rc = 1 as libc::c_int;
        ...
        rc = Curl_socket_check(
            if (*pp).sendleft != 0 { -(1 as libc::c_int) } else { sock },
            -(1 as libc::c_int),
            if (*pp).sendleft != 0 { sock } else { -(1 as libc::c_int) },
            interval_ms,
        );
    }
    if block {
    ...
    if rc == -(1 as libc::c_int) {
        Curl_failf(data, b"select/poll error\0" as *const u8 as *const libc::c_char);
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_init(mut data: *mut Curl_easy, mut pp: *mut pingpong) {
    (*pp).nread_resp = 0 as libc::c_int as size_t;
    let ref mut fresh0 = (*pp).linestart_resp;
    *fresh0 = (*data).state.buffer;
    (*pp).pending_resp = 1 as libc::c_int != 0;
    (*pp).response = Curl_now();
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_setup(mut pp: *mut pingpong) {
    Curl_dyn_init(
        &mut (*pp).sendbuf,
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_vsendf(
    mut data: *mut Curl_easy,
    mut pp: *mut pingpong,
    ...
    Curl_dyn_reset(&mut (*pp).sendbuf);
    result = Curl_dyn_vaddf(&mut (*pp).sendbuf, fmt, args.as_va_list());
    if result as u64 != 0 {
        return result;
    }
    ...
    write_len = Curl_dyn_len(&mut (*pp).sendbuf);
    s = Curl_dyn_ptr(&mut (*pp).sendbuf);
    ...
    if bytes_written != write_len as ssize_t {
        let ref mut fresh1 = (*pp).sendthis;
        *fresh1 = s;
        (*pp).sendsize = write_len;
        (*pp).sendleft = write_len.wrapping_sub(bytes_written as libc::c_ulong);
    } else {
        let ref mut fresh2 = (*pp).sendthis;
        *fresh2 = 0 as *mut libc::c_char;
        let ref mut fresh3 = (*pp).sendsize;
        *fresh3 = 0 as libc::c_int as size_t;
        (*pp).sendleft = *fresh3;
        (*pp).response = Curl_now();
    }
    return CURLE_OK;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_sendf(
    mut data: *mut Curl_easy,
    mut pp: *mut pingpong,
    mut fmt: *const libc::c_char,
    mut args: ...
    ...
    ap = args.clone();
    result = Curl_pp_vsendf(data, pp, fmt, ap.as_va_list());
    return result;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_readresp(
    mut data: *mut Curl_easy,
    ...
    *size = 0 as libc::c_int as size_t;
    ...
    perline = ptr.offset_from((*pp).linestart_resp) as libc::c_long;
    while (*pp).nread_resp < (*data).set.buffer_size as size_t
        && (keepon as libc::c_int != 0 && result as u64 == 0)
    {
        if !((*pp).cache).is_null() {
            ...
            *fresh4 = 0 as *mut libc::c_char;
            ...
            result = Curl_read(
                ...
                sockfd,
                ptr,
                ((*data).set.buffer_size as libc::c_ulong)
            ...
            if result as u64 == 0 && gotbytes > 0 as libc::c_int as libc::c_long {
                result = CURLE_OK as libc::c_int as CURLcode;
            }
            if result as u64 != 0 {
        ...
        if keepon {
            if gotbytes <= 0 as libc::c_int as libc::c_long {
                keepon = 0 as libc::c_int != 0;
                result = CURLE_RECV_ERROR;
                Curl_failf(
                    data,
                    b"response reading failed\0" as *const u8 as *const libc::c_char,
                );
            } else {
                let mut i: ssize_t = 0;
                let mut clipamount: ssize_t = 0 as libc::c_int as ssize_t;
                let mut restart: bool = 0 as libc::c_int != 0;
                let ref mut fresh5 = (*data).req.headerbytecount;
                *fresh5 += gotbytes;
                let ref mut fresh6 = (*pp).nread_resp;
                *fresh6 = (*fresh6 as libc::c_ulong)
                    .wrapping_add(gotbytes as libc::c_ulong) as size_t as size_t;
                ...
                while i < gotbytes {
                    ...
                    if *ptr as libc::c_int == '\n' as i32 {
                        ...
                        {
                            let mut n: size_t = ptr.offset_from((*pp).linestart_resp)
                            ...
                            *buf.offset(n as isize) = 0 as libc::c_int as libc::c_char;
                            keepon = 0 as libc::c_int != 0;
                            let ref mut fresh7 = (*pp).linestart_resp;
                            *fresh7 = ptr.offset(1 as libc::c_int as isize);
                            i += 1;
                            *size = (*pp).nread_resp;
                            (*pp).nread_resp = 0 as libc::c_int as size_t;
                            break;
                        } else {
                            perline = 0 as libc::c_int as ssize_t;
                            let ref mut fresh8 = (*pp).linestart_resp;
                            *fresh8 = ptr.offset(1 as libc::c_int as isize);
                        }
                    }
                    ptr = ptr.offset(1);
                    i += 1;
                ...
                if !keepon && i != gotbytes {
                    ...
                    if perline == gotbytes
                        && gotbytes
                            > (*data).set.buffer_size / 2 as libc::c_int as libc::c_long
                    {
                        Curl_infof(
                            data,
                        ...
                        restart = 1 as libc::c_int != 0;
                        clipamount = 40 as libc::c_int as ssize_t;
                ...
                if restart {
                    (*pp).nread_resp = 0 as libc::c_int as size_t;
                    let ref mut fresh10 = (*pp).linestart_resp;
                    *fresh10 = buf;
                    ptr = *fresh10;
                    perline = 0 as libc::c_int as ssize_t;
                }
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_getsock(
    mut data: *mut Curl_easy,
    mut pp: *mut pingpong,
...
pub unsafe extern "C" fn Curl_pp_flushsend(
    ...
    let mut sock: curl_socket_t = (*conn).sock[0 as libc::c_int as usize];
    let mut result: CURLcode = Curl_write(
        data,
        sock,
        ((*pp).sendthis)
    ...
    if written != (*pp).sendleft as ssize_t {
        let ref mut fresh11 = (*pp).sendleft;
        *fresh11 = (*fresh11 as libc::c_ulong).wrapping_sub(written as libc::c_ulong)
            as size_t as size_t;
    } else {
        let ref mut fresh12 = (*pp).sendthis;
        *fresh12 = 0 as *mut libc::c_char;
        let ref mut fresh13 = (*pp).sendsize;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_disconnect(mut pp: *mut pingpong) -> CURLcode {
    Curl_dyn_free(&mut (*pp).sendbuf);
    Curl_cfree.expect("non-null function pointer")((*pp).cache as *mut libc::c_void);
    let ref mut fresh14 = (*pp).cache;
    *fresh14 = 0 as *mut libc::c_char;
    return CURLE_OK;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_pp_moredata(mut pp: *mut pingpong) -> bool {
