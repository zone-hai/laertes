...
unsafe extern "C" fn ntlm_wb_cleanup(mut ntlm: *mut ntlmdata) {
    if (*ntlm).ntlm_auth_hlpr_socket != -(1 as libc::c_int) {
        close((*ntlm).ntlm_auth_hlpr_socket);
        (*ntlm).ntlm_auth_hlpr_socket = -(1 as libc::c_int);
    }
    if (*ntlm).ntlm_auth_hlpr_pid != 0 {
        ...
        while i < 4 as libc::c_int {
            ...
            match i {
                0 => {
                    kill((*ntlm).ntlm_auth_hlpr_pid, 15 as libc::c_int);
                }
                1 => {
                    Curl_wait_ms(1 as libc::c_int as timediff_t);
                }
                2 => {
                    kill((*ntlm).ntlm_auth_hlpr_pid, 9 as libc::c_int);
                }
                3 | _ => {}
            }
            i += 1;
        }
        (*ntlm).ntlm_auth_hlpr_pid = 0 as libc::c_int;
    }
    Curl_cfree
...
unsafe extern "C" fn ntlm_wb_init(
    ...
    let mut ntlm_auth: *const libc::c_char = 0 as *const libc::c_char;
    let mut ntlm_auth_alloc: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    username = userp;
    if username.is_null() || *username.offset(0 as libc::c_int as isize) == 0 {
        username = getenv(b"NTLMUSER\0" as *const u8 as *const libc::c_char);
        if username.is_null() || *username.offset(0 as libc::c_int as isize) == 0 {
            username = getenv(b"LOGNAME\0" as *const u8 as *const libc::c_char);
        }
        if username.is_null() || *username.offset(0 as libc::c_int as isize) == 0 {
            username = getenv(b"USER\0" as *const u8 as *const libc::c_char);
        }
        if (username.is_null() || *username.offset(0 as libc::c_int as isize) == 0)
            && getpwuid_r(
                geteuid(),
    ...
    slash = strpbrk(username, b"\\/\0" as *const u8 as *const libc::c_char);
    if !slash.is_null() {
        domain = Curl_cstrdup.expect("non-null function pointer")(username);
        if domain.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        slash = domain.offset(slash.offset_from(username) as libc::c_long as isize);
        *slash = '\u{0}' as i32 as libc::c_char;
        username = username
            .offset(slash.offset_from(domain) as libc::c_long as isize)
            .offset(1 as libc::c_int as isize);
    }
    ntlm_auth = b"/usr/bin/ntlm_auth\0" as *const u8 as *const libc::c_char;
    if access(ntlm_auth, 1 as libc::c_int) != 0 as libc::c_int {
        Curl_failf(
            data,
        ...
        if child_pid == -(1 as libc::c_int) {
            close(sockfds[0 as libc::c_int as usize]);
            close(sockfds[1 as libc::c_int as usize]);
            Curl_failf(
                data,
                b"Could not fork. errno %d: %s\0" as *const u8 as *const libc::c_char,
                *__errno_location(),
                Curl_strerror(
                    *__errno_location(),
                    buffer.as_mut_ptr(),
            ...
            if child_pid == 0 {
                close(sockfds[0 as libc::c_int as usize]);
                if dup2(sockfds[1 as libc::c_int as usize], 0 as libc::c_int)
                    == -(1 as libc::c_int)
                {
                    Curl_failf(
                        data,
                ...
                if dup2(sockfds[1 as libc::c_int as usize], 1 as libc::c_int)
                    == -(1 as libc::c_int)
                {
                    Curl_failf(
                        data,
                ...
                if !domain.is_null() {
                    execl(
                        ntlm_auth,
                        ntlm_auth,
                        b"--helper-protocol\0" as *const u8 as *const libc::c_char,
                        b"ntlmssp-client-1\0" as *const u8 as *const libc::c_char,
                        b"--use-cached-creds\0" as *const u8 as *const libc::c_char,
            ...
            close(sockfds[1 as libc::c_int as usize]);
            (*ntlm).ntlm_auth_hlpr_socket = sockfds[0 as libc::c_int as usize];
...
unsafe extern "C" fn ntlm_wb_response(
    ...
    let mut len_out: size_t = 0 as libc::c_int as size_t;
    ...
    let mut buf: *mut libc::c_uchar = (*data).state.buffer as *mut libc::c_uchar;
    Curl_dyn_init(&mut b, 100000 as libc::c_int as size_t);
    loop {
        if !(len_in > 0 as libc::c_int as libc::c_ulong) {
        ...
        if written == -(1 as libc::c_int) as libc::c_long {
            ...
            len_in = (len_in as libc::c_ulong).wrapping_sub(written as libc::c_ulong)
                as size_t as size_t;
        }
    }
    match current_block {
            ...
            loop {
                ...
                if size == -(1 as libc::c_int) as libc::c_long {
                    ...
                    if !(len_out != 0
                        && *ptr
                            .offset(
                                len_out.wrapping_sub(1 as libc::c_int as libc::c_ulong)
                                    as isize,
                            ) as libc::c_int == '\n' as i32)
                    {
                        continue;
...
pub unsafe extern "C" fn Curl_input_ntlm_wb(
    ...
    let mut ntlm: *mut ntlmdata = if proxy as libc::c_int != 0 {
        &mut (*conn).proxyntlm
    } else {
        &mut (*conn).ntlm
    };
    let mut state: *mut curlntlm = if proxy as libc::c_int != 0 {
...
pub unsafe extern "C" fn Curl_output_ntlm_wb(
    ...
    match *state as libc::c_uint {
        2 => {
            let mut input: *mut libc::c_char = curl_maprintf(
