...
extern "C" {
    ...
    fn Curl_ssl_kill_session(session: *mut Curl_ssl_session);
...
pub unsafe extern "C" fn curl_share_init() -> *mut CURLSH {
    let mut share: *mut Curl_share = Curl_ccalloc
        .expect(
            "non-null function pointer",
        )(
        1 as libc::c_int as size_t,
        ::std::mem::size_of::<Curl_share>() as libc::c_ulong,
    ) as *mut Curl_share;
    if !share.is_null() {
        (*share).magic = 0x7e117a1e as libc::c_int as libc::c_uint;
        (*share).specifier
            |= ((1 as libc::c_int) << CURL_LOCK_DATA_SHARE as libc::c_int)
                as libc::c_uint;
        if Curl_mk_dnscache(&mut (*share).hostcache) != 0 {
...
pub unsafe extern "C" fn curl_share_setopt(
    mut share: *mut Curl_share,
    mut option: CURLSHoption,
    mut args: ...
) -> CURLSHcode {
    let mut param: ::std::ffi::VaListImpl;
    let mut type_0: libc::c_int = 0;
    let mut lockfunc: curl_lock_function = None;
    let mut unlockfunc: curl_unlock_function = None;
    let mut ptr: *mut libc::c_void = 0 as *mut libc::c_void;
    let mut res: CURLSHcode = CURLSHE_OK;
    if !(!share.is_null() && (*share).magic == 0x7e117a1e as libc::c_int as libc::c_uint)
    {
    ...
    match option as libc::c_uint {
        1 => {
            type_0 = param.arg::<libc::c_int>();
            match type_0 {
                2 => {
                    if ((*share).cookies).is_null() {
                        let ref mut fresh0 = (*share).cookies;
                ...
                4 => {
                    if ((*share).sslsession).is_null() {
                        (*share).max_ssl_sessions = 8 as libc::c_int as size_t;
                        let ref mut fresh1 = (*share).sslsession;
                        *fresh1 = Curl_ccalloc
                            .expect(
                                "non-null function pointer",
                            )(
                        ...
                        (*share).sessionage = 0 as libc::c_int as libc::c_long;
                        if ((*share).sslsession).is_null() {
                            res = CURLSHE_NOMEM;
                        }
                    }
                ...
                5 => {
                    if Curl_conncache_init(&mut (*share).conn_cache, 103 as libc::c_int)
                        != 0
                    {
                        res = CURLSHE_NOMEM;
                    }
            ...
            if res as u64 == 0 {
                (*share).specifier |= ((1 as libc::c_int) << type_0) as libc::c_uint;
            }
        }
        2 => {
            type_0 = param.arg::<libc::c_int>();
            ...
            match type_0 {
                2 => {
                    if !((*share).cookies).is_null() {
                        ...
                        let ref mut fresh2 = (*share).cookies;
                ...
                4 => {
                    ...
                    let ref mut fresh3 = (*share).sslsession;
        ...
        3 => {
            ...
            let ref mut fresh4 = (*share).lockfunc;
            *fresh4 = lockfunc;
        }
        4 => {
            unlockfunc = ::std::mem::transmute(
                param
                    .arg::<
                    *mut unsafe extern "C" fn(
                        *mut CURL,
            ...
            let ref mut fresh5 = (*share).unlockfunc;
            *fresh5 = unlockfunc;
        }
        5 => {
            ptr = param.arg::<*mut libc::c_void>();
            let ref mut fresh6 = (*share).clientdata;
            *fresh6 = ptr;
        }
        _ => {
            res = CURLSHE_BAD_OPTION;
...
#[no_mangle]
pub unsafe extern "C" fn curl_share_cleanup(mut share: *mut Curl_share) -> CURLSHcode {
    if !(!share.is_null() && (*share).magic == 0x7e117a1e as libc::c_int as libc::c_uint)
    {
        return CURLSHE_INVALID;
    }
    ...
    Curl_conncache_close_all_connections(&mut (*share).conn_cache);
    Curl_conncache_destroy(&mut (*share).conn_cache);
    ...
    if !((*share).sslsession).is_null() {
        ...
        while i < (*share).max_ssl_sessions {
            Curl_ssl_kill_session(&mut *((*share).sslsession).offset(i as isize));
            i = i.wrapping_add(1);
        }
        Curl_cfree
            .expect(
    ...
    (*share).magic = 0 as libc::c_int as libc::c_uint;
    Curl_cfree.expect("non-null function pointer")(share as *mut libc::c_void);
    return CURLSHE_OK;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_share_lock(
    ...
    mut type_0: curl_lock_data,
    mut accesstype: curl_lock_access,
) -> CURLSHcode {
    let mut share: *mut Curl_share = (*data).share;
    if share.is_null() {
        return CURLSHE_INVALID;
    }
    if (*share).specifier
        & ((1 as libc::c_int) << type_0 as libc::c_uint) as libc::c_uint != 0
    {
        if ((*share).lockfunc).is_some() {
            ((*share).lockfunc)
                .expect(
