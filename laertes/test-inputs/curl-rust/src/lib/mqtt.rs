...
#[no_mangle]
pub static mut Curl_handler_mqtt: Curl_handler = unsafe {
    {
        let mut init = Curl_handler {
            scheme: b"MQTT\0" as *const u8 as *const libc::c_char,
            setup_connection: Some(
                mqtt_setup_conn
                    as unsafe extern "C" fn(*mut Curl_easy, *mut connectdata) -> CURLcode,
            ),
            do_it: Some(
                mqtt_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            done: None,
            do_more: None,
            connect_it: None,
            connecting: None,
            doing: Some(
                mqtt_doing as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            proto_getsock: None,
            doing_getsock: Some(
                mqtt_getsock
            ...
            attach: None,
            defport: 1883 as libc::c_int,
            protocol: ((1 as libc::c_int) << 28 as libc::c_int) as libc::c_uint,
            family: ((1 as libc::c_int) << 28 as libc::c_int) as libc::c_uint,
            flags: 0 as libc::c_int as libc::c_uint,
        };
        init
    }
};
unsafe extern "C" fn mqtt_setup_conn(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
) -> CURLcode {
    let mut mq: *mut MQTT = 0 as *mut MQTT;
    mq = Curl_ccalloc
        .expect(
            "non-null function pointer",
        )(1 as libc::c_int as size_t, ::std::mem::size_of::<MQTT>() as libc::c_ulong)
        as *mut MQTT;
    if mq.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    let ref mut fresh0 = (*data).req.p.mqtt;
    *fresh0 = mq;
    return CURLE_OK;
}
unsafe extern "C" fn mqtt_send(
    mut data: *mut Curl_easy,
    ...
    let mut mq: *mut MQTT = (*data).req.p.mqtt;
    let mut n: ssize_t = 0;
    result = Curl_write(data, sockfd, buf as *const libc::c_void, len, &mut n);
    if result as u64 == 0 {
        Curl_debug(data, CURLINFO_HEADER_OUT, buf, n as size_t);
    }
    if len != n as size_t {
        let mut nsend: size_t = len.wrapping_sub(n as libc::c_ulong);
        let mut sendleftovers: *mut libc::c_char = Curl_memdup(
            &mut *buf.offset(n as isize) as *mut libc::c_char as *const libc::c_void,
            nsend,
        ) as *mut libc::c_char;
        if sendleftovers.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        let ref mut fresh1 = (*mq).sendleftovers;
        *fresh1 = sendleftovers;
        (*mq).nsend = nsend;
    } else {
        let ref mut fresh2 = (*mq).sendleftovers;
        *fresh2 = 0 as *mut libc::c_char;
        (*mq).nsend = 0 as libc::c_int as size_t;
    }
    return result;
}
unsafe extern "C" fn mqtt_getsock(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
...
unsafe extern "C" fn mqtt_encode_len(
    mut buf: *mut libc::c_char,
    mut len: size_t,
) -> libc::c_int {
    let mut encoded: libc::c_uchar = 0;
    let mut i: libc::c_int = 0;
    i = 0 as libc::c_int;
    while len > 0 as libc::c_int as libc::c_ulong && i < 4 as libc::c_int {
        encoded = len.wrapping_rem(0x80 as libc::c_int as libc::c_ulong)
            as libc::c_uchar;
        len = (len as libc::c_ulong).wrapping_div(0x80 as libc::c_int as libc::c_ulong)
            as size_t as size_t;
        if len != 0 {
            encoded = (encoded as libc::c_int | 0x80 as libc::c_int) as libc::c_uchar;
        }
        *buf.offset(i as isize) = encoded as libc::c_char;
        i += 1;
    }
    return i;
}
unsafe extern "C" fn add_passwd(
    mut passwd: *const libc::c_char,
    plen: size_t,
    mut pkt: *mut libc::c_char,
    start: size_t,
    mut remain_pos: libc::c_int,
) -> libc::c_int {
    let conn_flags_pos: size_t = (remain_pos + 8 as libc::c_int) as size_t;
    if plen > 0xffff as libc::c_int as libc::c_ulong {
        return 1 as libc::c_int;
    }
    let ref mut fresh3 = *pkt.offset(conn_flags_pos as isize);
    *fresh3 = (*fresh3 as libc::c_int | 0x40 as libc::c_int) as libc::c_char;
    *pkt
        .offset(
            start as isize,
        ) = (plen >> 8 as libc::c_int & 0xff as libc::c_int as libc::c_ulong)
        as libc::c_char;
    *pkt
        .offset(
            start.wrapping_add(1 as libc::c_int as libc::c_ulong) as isize,
        ) = (plen & 0xff as libc::c_int as libc::c_ulong) as libc::c_char;
    memcpy(
        &mut *pkt.offset(start.wrapping_add(2 as libc::c_int as libc::c_ulong) as isize)
...
unsafe extern "C" fn add_user(
    ...
    let conn_flags_pos: size_t = (remain_pos + 8 as libc::c_int) as size_t;
    if ulen > 0xffff as libc::c_int as libc::c_ulong {
        return 1 as libc::c_int;
    }
    let ref mut fresh4 = *pkt.offset(conn_flags_pos as isize);
    *fresh4 = (*fresh4 as libc::c_int | 0x80 as libc::c_int) as libc::c_uchar;
    *pkt
        .offset(
            start as isize,
        ) = (ulen >> 8 as libc::c_int & 0xff as libc::c_int as libc::c_ulong)
        as libc::c_uchar;
    *pkt
        .offset(
            start.wrapping_add(1 as libc::c_int as libc::c_ulong) as isize,
        ) = (ulen & 0xff as libc::c_int as libc::c_ulong) as libc::c_uchar;
    memcpy(
        &mut *pkt.offset(start.wrapping_add(2 as libc::c_int as libc::c_ulong) as isize)
...
unsafe extern "C" fn add_client_id(
    mut client_id: *const libc::c_char,
    client_id_len: size_t,
    mut pkt: *mut libc::c_char,
    start: size_t,
) -> libc::c_int {
    if client_id_len != 12 as libc::c_int as libc::c_ulong {
        return 1 as libc::c_int;
    }
    *pkt.offset(start as isize) = 0 as libc::c_int as libc::c_char;
    *pkt
        .offset(
            start.wrapping_add(1 as libc::c_int as libc::c_ulong) as isize,
    ...
    memcpy(
        &mut *pkt.offset(start.wrapping_add(2 as libc::c_int as libc::c_ulong) as isize)
            as *mut libc::c_char as *mut libc::c_void,
        client_id as *const libc::c_void,
        12 as libc::c_int as libc::c_ulong,
    );
    return 0 as libc::c_int;
}
unsafe extern "C" fn init_connpack(
    mut packet: *mut libc::c_char,
    mut remain: *mut libc::c_char,
    mut remain_pos: libc::c_int,
) -> libc::c_int {
    *packet.offset(0 as libc::c_int as isize) = 0x10 as libc::c_int as libc::c_char;
    memcpy(
        &mut *packet.offset(1 as libc::c_int as isize) as *mut libc::c_char
            as *mut libc::c_void,
        remain as *const libc::c_void,
        remain_pos as libc::c_ulong,
    );
    *packet
        .offset(
            (remain_pos + 1 as libc::c_int) as isize,
        ) = 0 as libc::c_int as libc::c_char;
    *packet
        .offset(
            (remain_pos + 2 as libc::c_int) as isize,
        ) = 0x4 as libc::c_int as libc::c_char;
    *packet
        .offset((remain_pos + 3 as libc::c_int) as isize) = 'M' as i32 as libc::c_char;
    *packet
        .offset((remain_pos + 4 as libc::c_int) as isize) = 'Q' as i32 as libc::c_char;
    *packet
        .offset((remain_pos + 5 as libc::c_int) as isize) = 'T' as i32 as libc::c_char;
    *packet
        .offset((remain_pos + 6 as libc::c_int) as isize) = 'T' as i32 as libc::c_char;
    *packet
        .offset(
            (remain_pos + 7 as libc::c_int) as isize,
        ) = 0x4 as libc::c_int as libc::c_char;
    *packet
        .offset(
            (remain_pos + 8 as libc::c_int) as isize,
        ) = 0x2 as libc::c_int as libc::c_char;
    *packet
        .offset(
            (remain_pos + 9 as libc::c_int) as isize,
        ) = 0 as libc::c_int as libc::c_char;
    *packet
        .offset(
            (remain_pos + 10 as libc::c_int) as isize,
        ) = 0x3c as libc::c_int as libc::c_char;
    return remain_pos + 10 as libc::c_int;
}
unsafe extern "C" fn mqtt_connect(mut data: *mut Curl_easy) -> CURLcode {
    let mut current_block: u64;
    let mut result: CURLcode = CURLE_OK;
    let mut pos: libc::c_int = 0 as libc::c_int;
    let mut rc: libc::c_int = 0 as libc::c_int;
    let mut remain_pos: libc::c_int = 0 as libc::c_int;
    let mut remain: [libc::c_char; 4] = [0 as libc::c_int as libc::c_char, 0, 0, 0];
    let mut packetlen: size_t = 0 as libc::c_int as size_t;
    let mut payloadlen: size_t = 0 as libc::c_int as size_t;
    let mut start_user: size_t = 0 as libc::c_int as size_t;
    let mut start_pwd: size_t = 0 as libc::c_int as size_t;
    let mut client_id: [libc::c_char; 13] = *::std::mem::transmute::<
        &[u8; 13],
        &mut [libc::c_char; 13],
    >(b"curl\0\0\0\0\0\0\0\0\0");
    let clen: size_t = strlen(b"curl\0" as *const u8 as *const libc::c_char);
    let mut packet: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut username: *const libc::c_char = if !((*data).state.aptr.user).is_null() {
        (*data).state.aptr.user as *const libc::c_char
    } else {
        b"\0" as *const u8 as *const libc::c_char
    };
    let ulen: size_t = strlen(username);
    let mut passwd: *const libc::c_char = if !((*data).state.aptr.passwd).is_null() {
        (*data).state.aptr.passwd as *const libc::c_char
    } else {
        b"\0" as *const u8 as *const libc::c_char
    };
    let plen: size_t = strlen(passwd);
    payloadlen = ulen
        .wrapping_add(plen)
        .wrapping_add(12 as libc::c_int as libc::c_ulong)
        .wrapping_add(2 as libc::c_int as libc::c_ulong);
    if ulen != 0 {
        payloadlen = (payloadlen as libc::c_ulong)
    ...
    remain_pos = mqtt_encode_len(
        remain.as_mut_ptr(),
        payloadlen.wrapping_add(10 as libc::c_int as libc::c_ulong),
    );
    packetlen = payloadlen
        .wrapping_add(10 as libc::c_int as libc::c_ulong)
        .wrapping_add(remain_pos as libc::c_ulong)
        .wrapping_add(1 as libc::c_int as libc::c_ulong);
    if packetlen > 268435455 as libc::c_int as libc::c_ulong {
        return CURLE_WEIRD_SERVER_REPLY;
    }
    packet = Curl_cmalloc.expect("non-null function pointer")(packetlen)
        as *mut libc::c_char;
    if packet.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    memset(packet as *mut libc::c_void, 0 as libc::c_int, packetlen);
    pos = init_connpack(packet, remain.as_mut_ptr(), remain_pos);
    result = Curl_rand_hex(
        data,
        &mut *client_id.as_mut_ptr().offset(clen as isize) as *mut libc::c_char
            as *mut libc::c_uchar,
        (12 as libc::c_int as libc::c_ulong)
            .wrapping_sub(clen)
            .wrapping_add(1 as libc::c_int as libc::c_ulong),
    ...
    if rc != 0 {
        Curl_failf(
            data,
            b"Client ID length mismatched: [%lu]\0" as *const u8 as *const libc::c_char,
            strlen(client_id.as_mut_ptr()),
        );
        ...
        Curl_infof(
            data,
            b"Using client id '%s'\0" as *const u8 as *const libc::c_char,
            client_id.as_mut_ptr(),
        );
        start_user = (pos + 3 as libc::c_int + 12 as libc::c_int) as size_t;
        start_pwd = start_user.wrapping_add(ulen);
        if ulen != 0 {
            start_pwd = (start_pwd as libc::c_ulong)
                .wrapping_add(2 as libc::c_int as libc::c_ulong) as size_t as size_t;
            rc = add_user(
                username,
                ulen,
                packet as *mut libc::c_uchar,
                start_user,
                remain_pos,
            );
            if rc != 0 {
                Curl_failf(
                    data,
                    b"Username is too large: [%lu]\0" as *const u8
                        as *const libc::c_char,
                    ulen,
                );
                result = CURLE_WEIRD_SERVER_REPLY;
                current_block = 10351409997398868566;
            } else {
            ...
            _ => {
                if plen != 0 {
                    ...
                    if rc != 0 {
                        Curl_failf(
                            data,
                            b"Password is too large: [%lu]\0" as *const u8
                                as *const libc::c_char,
                            plen,
                        );
                        result = CURLE_WEIRD_SERVER_REPLY;
    ...
    if !packet.is_null() {
        Curl_cfree.expect("non-null function pointer")(packet as *mut libc::c_void);
    }
    Curl_cfree
        .expect(
            "non-null function pointer",
        )((*data).state.aptr.user as *mut libc::c_void);
    let ref mut fresh5 = (*data).state.aptr.user;
    *fresh5 = 0 as *mut libc::c_char;
    Curl_cfree
        .expect(
            "non-null function pointer",
        )((*data).state.aptr.passwd as *mut libc::c_void);
    let ref mut fresh6 = (*data).state.aptr.passwd;
    *fresh6 = 0 as *mut libc::c_char;
    return result;
}
unsafe extern "C" fn mqtt_disconnect(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    result = mqtt_send(
        data,
        b"\xE0\0\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
        2 as libc::c_int as size_t,
    );
    return result;
}
unsafe extern "C" fn mqtt_verify_connack(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = (*data).conn;
    let mut sockfd: curl_socket_t = (*conn).sock[0 as libc::c_int as usize];
    let mut readbuf: [libc::c_uchar; 2] = [0; 2];
    let mut nread: ssize_t = 0;
    result = Curl_read(
        data,
        sockfd,
        readbuf.as_mut_ptr() as *mut libc::c_char,
        2 as libc::c_int as size_t,
        &mut nread,
    );
    if !(result as u64 != 0) {
        ...
        if nread < 2 as libc::c_int as libc::c_long {
            result = CURLE_WEIRD_SERVER_REPLY;
        } else if readbuf[0 as libc::c_int as usize] as libc::c_int != 0 as libc::c_int
                || readbuf[1 as libc::c_int as usize] as libc::c_int != 0 as libc::c_int
            {
            Curl_failf(
                data,
                b"Expected %02x%02x but got %02x%02x\0" as *const u8
                    as *const libc::c_char,
                0 as libc::c_int,
                0 as libc::c_int,
                readbuf[0 as libc::c_int as usize] as libc::c_int,
                readbuf[1 as libc::c_int as usize] as libc::c_int,
            );
            result = CURLE_WEIRD_SERVER_REPLY;
        }
    }
...
unsafe extern "C" fn mqtt_get_topic(
    mut data: *mut Curl_easy,
    mut topic: *mut *mut libc::c_char,
    mut topiclen: *mut size_t,
) -> CURLcode {
    let mut path: *mut libc::c_char = (*data).state.up.path;
    if strlen(path) > 1 as libc::c_int as libc::c_ulong {
    ...
    Curl_failf(
        data,
        b"No MQTT topic found. Forgot to URL encode it?\0" as *const u8
            as *const libc::c_char,
    );
    return CURLE_URL_MALFORMAT;
}
unsafe extern "C" fn mqtt_subscribe(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut topic: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut topiclen: size_t = 0;
    let mut packet: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut packetlen: size_t = 0;
    let mut encodedsize: [libc::c_char; 4] = [0; 4];
    let mut n: size_t = 0;
    let mut conn: *mut connectdata = (*data).conn;
    result = mqtt_get_topic(data, &mut topic, &mut topiclen);
    if !(result as u64 != 0) {
        let ref mut fresh7 = (*conn).proto.mqtt.packetid;
        *fresh7 = (*fresh7).wrapping_add(1);
        packetlen = topiclen.wrapping_add(5 as libc::c_int as libc::c_ulong);
        n = mqtt_encode_len(encodedsize.as_mut_ptr(), packetlen) as size_t;
        packetlen = (packetlen as libc::c_ulong)
            .wrapping_add(n.wrapping_add(1 as libc::c_int as libc::c_ulong)) as size_t
            as size_t;
        packet = Curl_cmalloc.expect("non-null function pointer")(packetlen)
            as *mut libc::c_uchar;
        if packet.is_null() {
            ...
            *packet
            ...
            memcpy(
                &mut *packet.offset(1 as libc::c_int as isize) as *mut libc::c_uchar
                    as *mut libc::c_void,
                encodedsize.as_mut_ptr() as *const libc::c_void,
                n,
            );
            *packet
                .offset(
                    (1 as libc::c_int as libc::c_ulong).wrapping_add(n) as isize,
                ) = ((*conn).proto.mqtt.packetid >> 8 as libc::c_int
                & 0xff as libc::c_int as libc::c_uint) as libc::c_uchar;
            *packet
                .offset(
                    (2 as libc::c_int as libc::c_ulong).wrapping_add(n) as isize,
                ) = ((*conn).proto.mqtt.packetid & 0xff as libc::c_int as libc::c_uint)
                as libc::c_uchar;
            *packet
                .offset(
                    (3 as libc::c_int as libc::c_ulong).wrapping_add(n) as isize,
                ) = (topiclen >> 8 as libc::c_int & 0xff as libc::c_int as libc::c_ulong)
                as libc::c_uchar;
            *packet
                .offset(
                    (4 as libc::c_int as libc::c_ulong).wrapping_add(n) as isize,
                ) = (topiclen & 0xff as libc::c_int as libc::c_ulong) as libc::c_uchar;
            memcpy(
                &mut *packet
                    .offset((5 as libc::c_int as libc::c_ulong).wrapping_add(n) as isize)
                    as *mut libc::c_uchar as *mut libc::c_void,
                topic as *const libc::c_void,
                topiclen,
            );
            *packet
                .offset(
                    (5 as libc::c_int as libc::c_ulong)
                        .wrapping_add(n)
                        .wrapping_add(topiclen) as isize,
                ) = 0 as libc::c_int as libc::c_uchar;
            result = mqtt_send(data, packet as *mut libc::c_char, packetlen);
        }
    }
    Curl_cfree.expect("non-null function pointer")(topic as *mut libc::c_void);
    Curl_cfree.expect("non-null function pointer")(packet as *mut libc::c_void);
    return result;
}
unsafe extern "C" fn mqtt_verify_suback(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut conn: *mut connectdata = (*data).conn;
    let mut sockfd: curl_socket_t = (*conn).sock[0 as libc::c_int as usize];
    let mut readbuf: [libc::c_uchar; 3] = [0; 3];
    let mut nread: ssize_t = 0;
    let mut mqtt: *mut mqtt_conn = &mut (*conn).proto.mqtt;
    result = Curl_read(
        data,
        sockfd,
        readbuf.as_mut_ptr() as *mut libc::c_char,
    ...
    if !(result as u64 != 0) {
        ...
        if nread < 3 as libc::c_int as libc::c_long {
            result = CURLE_WEIRD_SERVER_REPLY;
        } else if readbuf[0 as libc::c_int as usize] as libc::c_uint
                != (*mqtt).packetid >> 8 as libc::c_int
                    & 0xff as libc::c_int as libc::c_uint
                || readbuf[1 as libc::c_int as usize] as libc::c_uint
                    != (*mqtt).packetid & 0xff as libc::c_int as libc::c_uint
                || readbuf[2 as libc::c_int as usize] as libc::c_int != 0 as libc::c_int
            {
            result = CURLE_WEIRD_SERVER_REPLY;
        }
    }
...
unsafe extern "C" fn mqtt_publish(mut data: *mut Curl_easy) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    let mut payload: *mut libc::c_char = (*data).set.postfields as *mut libc::c_char;
    let mut payloadlen: size_t = 0;
    let mut topic: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut topiclen: size_t = 0;
    let mut pkt: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut i: size_t = 0 as libc::c_int as size_t;
    let mut remaininglength: size_t = 0;
    let mut encodelen: size_t = 0;
    let mut encodedbytes: [libc::c_char; 4] = [0; 4];
    let mut postfieldsize: curl_off_t = (*data).set.postfieldsize;
    if payload.is_null() {
        return CURLE_BAD_FUNCTION_ARGUMENT;
    }
    if postfieldsize < 0 as libc::c_int as libc::c_long {
        payloadlen = strlen(payload);
    } else {
        payloadlen = postfieldsize as size_t;
    }
    result = mqtt_get_topic(data, &mut topic, &mut topiclen);
    if !(result as u64 != 0) {
        remaininglength = payloadlen
            .wrapping_add(2 as libc::c_int as libc::c_ulong)
            .wrapping_add(topiclen);
        encodelen = mqtt_encode_len(encodedbytes.as_mut_ptr(), remaininglength)
            as size_t;
        pkt = Curl_cmalloc
            .expect(
                "non-null function pointer",
        ...
        if pkt.is_null() {
            ...
            let fresh8 = i;
            i = i.wrapping_add(1);
            *pkt.offset(fresh8 as isize) = 0x30 as libc::c_int as libc::c_uchar;
            memcpy(
                &mut *pkt.offset(i as isize) as *mut libc::c_uchar as *mut libc::c_void,
                encodedbytes.as_mut_ptr() as *const libc::c_void,
                encodelen,
            );
            i = (i as libc::c_ulong).wrapping_add(encodelen) as size_t as size_t;
            let fresh9 = i;
            i = i.wrapping_add(1);
            *pkt
                .offset(
            ...
            *pkt
            ...
            i = (i as libc::c_ulong).wrapping_add(topiclen) as size_t as size_t;
            memcpy(
                &mut *pkt.offset(i as isize) as *mut libc::c_uchar as *mut libc::c_void,
                payload as *const libc::c_void,
                payloadlen,
            );
            i = (i as libc::c_ulong).wrapping_add(payloadlen) as size_t as size_t;
            result = mqtt_send(data, pkt as *mut libc::c_char, i);
        }
    }
    Curl_cfree.expect("non-null function pointer")(pkt as *mut libc::c_void);
    Curl_cfree.expect("non-null function pointer")(topic as *mut libc::c_void);
    return result;
}
unsafe extern "C" fn mqtt_decode_len(
    mut buf: *mut libc::c_uchar,
    mut buflen: size_t,
    mut lenbytes: *mut size_t,
) -> size_t {
    let mut len: size_t = 0 as libc::c_int as size_t;
    let mut mult: size_t = 1 as libc::c_int as size_t;
    let mut i: size_t = 0;
    let mut encoded: libc::c_uchar = 128 as libc::c_int as libc::c_uchar;
    i = 0 as libc::c_int as size_t;
    while i < buflen && encoded as libc::c_int & 128 as libc::c_int != 0 {
        encoded = *buf.offset(i as isize);
        len = (len as libc::c_ulong)
            .wrapping_add(
                ((encoded as libc::c_int & 127 as libc::c_int) as libc::c_ulong)
                    .wrapping_mul(mult),
            ) as size_t as size_t;
        mult = (mult as libc::c_ulong).wrapping_mul(128 as libc::c_int as libc::c_ulong)
            as size_t as size_t;
        i = i.wrapping_add(1);
    }
    if !lenbytes.is_null() {
...
unsafe extern "C" fn mqstate(
    mut data: *mut Curl_easy,
    mut state: mqttstate,
    mut nextstate: mqttstate,
) {
    let mut conn: *mut connectdata = (*data).conn;
    let mut mqtt: *mut mqtt_conn = &mut (*conn).proto.mqtt;
    (*mqtt).state = state;
    if state as libc::c_uint == MQTT_FIRST as libc::c_int as libc::c_uint {
        (*mqtt).nextstate = nextstate;
    }
}
unsafe extern "C" fn mqtt_read_publish(
    mut data: *mut Curl_easy,
    mut done: *mut bool,
) -> CURLcode {
    let mut current_block: u64;
    ...
    let mut sockfd: curl_socket_t = (*conn).sock[0 as libc::c_int as usize];
    let mut nread: ssize_t = 0;
    let mut pkt: *mut libc::c_uchar = (*data).state.buffer as *mut libc::c_uchar;
    let mut remlen: size_t = 0;
    let mut mqtt: *mut mqtt_conn = &mut (*conn).proto.mqtt;
    let mut mq: *mut MQTT = (*data).req.p.mqtt;
    let mut packet: libc::c_uchar = 0;
    match (*mqtt).state as libc::c_uint {
        4 => {
            current_block = 16063587588522528629;
        }
        3 | 5 => {
            packet = ((*mq).firstbyte as libc::c_int & 0xf0 as libc::c_int)
                as libc::c_uchar;
            if packet as libc::c_int == 0x30 as libc::c_int {
                mqstate(data, MQTT_PUB_REMAIN, MQTT_NOSTATE);
                remlen = (*mq).remaining_length;
                Curl_infof(
                    data,
                    b"Remaining length: %zd bytes\0" as *const u8 as *const libc::c_char,
                    remlen,
                );
                if (*data).set.max_filesize != 0
                    && remlen as curl_off_t > (*data).set.max_filesize
                {
                    Curl_failf(
                        data,
                        b"Maximum file size exceeded\0" as *const u8
                            as *const libc::c_char,
                    );
                    result = CURLE_FILESIZE_EXCEEDED;
                    current_block = 1715196665109041652;
                } else {
                    Curl_pgrsSetDownloadSize(data, remlen as curl_off_t);
                    (*data).req.bytecount = 0 as libc::c_int as curl_off_t;
                    (*data).req.size = remlen as curl_off_t;
                    (*mq).npacket = remlen;
                    current_block = 4488286894823169796;
                }
            } else if packet as libc::c_int == 0x90 as libc::c_int {
                mqstate(data, MQTT_SUBACK_COMING, MQTT_NOSTATE);
                current_block = 16063587588522528629;
            } else {
                if packet as libc::c_int == 0xe0 as libc::c_int {
                    Curl_infof(
                        data,
                        b"Got DISCONNECT\0" as *const u8 as *const libc::c_char,
                    );
    ...
    match current_block {
        4488286894823169796 => {
            ...
            let mut rest: size_t = (*mq).npacket;
            if rest > (*data).set.buffer_size as size_t {
                rest = (*data).set.buffer_size as size_t;
            }
            result = Curl_read(data, sockfd, pkt as *mut libc::c_char, rest, &mut nread);
            if result as u64 != 0 {
                if CURLE_AGAIN as libc::c_int as libc::c_uint == result as libc::c_uint {
                    Curl_infof(
                        data,
                        b"EEEE AAAAGAIN\0" as *const u8 as *const libc::c_char,
                    );
                }
            } else if nread == 0 {
                Curl_infof(
                    data,
                    b"server disconnected\0" as *const u8 as *const libc::c_char,
                );
                ...
                Curl_debug(
                    data,
                    CURLINFO_DATA_IN,
                    pkt as *mut libc::c_char,
                    nread as size_t,
                );
                let ref mut fresh11 = (*mq).npacket;
                *fresh11 = (*fresh11 as libc::c_ulong)
                    .wrapping_sub(nread as libc::c_ulong) as size_t as size_t;
                let ref mut fresh12 = (*k).bytecount;
                *fresh12 += nread;
...
unsafe extern "C" fn mqtt_do(mut data: *mut Curl_easy, mut done: *mut bool) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    *done = 0 as libc::c_int != 0;
    ...
    if result as u64 != 0 {
        Curl_failf(
            data,
            b"Error %d sending MQTT CONN request\0" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn mqtt_doing(
    ...
    *done = 0 as libc::c_int != 0;
    if (*mq).nsend != 0 {
        let mut ptr: *mut libc::c_char = (*mq).sendleftovers;
        result = mqtt_send(data, (*mq).sendleftovers, (*mq).nsend);
        Curl_cfree.expect("non-null function pointer")(ptr as *mut libc::c_void);
        if result as u64 != 0 {
            return result;
        }
    }
    Curl_infof(
        data,
        b"mqtt_doing: state [%d]\0" as *const u8 as *const libc::c_char,
        (*mqtt).state as libc::c_int,
    );
    ...
    match (*mqtt).state as libc::c_uint {
        0 => {
            result = Curl_read(
                data,
                sockfd,
                &mut (*mq).firstbyte as *mut libc::c_uchar as *mut libc::c_char,
                1 as libc::c_int as size_t,
                &mut nread,
            );
            if nread == 0 {
                current_block_42 = 2500484646272006982;
            } else {
                Curl_debug(
                    data,
                    CURLINFO_HEADER_IN,
                ...
                (*mq).npacket = 0 as libc::c_int as size_t;
                mqstate(data, MQTT_REMAINING_LENGTH, MQTT_NOSTATE);
                current_block_42 = 9618971279722602572;
            }
        }
        1 => {
            current_block_42 = 9618971279722602572;
        ...
        _ => {
            Curl_failf(
                data,
                b"State not handled yet\0" as *const u8 as *const libc::c_char,
            );
            *done = 1 as libc::c_int != 0;
            current_block_42 = 2500484646272006982;
        }
    ...
    match current_block_42 {
        9618971279722602572 => {
            loop {
                ...
                let ref mut fresh13 = (*mq).npacket;
                let fresh14 = *fresh13;
                *fresh13 = (*fresh13).wrapping_add(1);
                *pkt.offset(fresh14 as isize) = byte;
                if !(byte as libc::c_int & 0x80 as libc::c_int != 0
                    && (*mq).npacket < 4 as libc::c_int as libc::c_ulong)
                {
                    break;
                }
            }
            if nread != 0 && byte as libc::c_int & 0x80 as libc::c_int != 0 {
                result = CURLE_WEIRD_SERVER_REPLY;
            }
            if !(result as u64 != 0) {
                (*mq)
                    .remaining_length = mqtt_decode_len(
                    &mut *pkt.offset(0 as libc::c_int as isize),
                    (*mq).npacket,
                    0 as *mut size_t,
                );
                (*mq).npacket = 0 as libc::c_int as size_t;
                if (*mq).remaining_length != 0 {
                    mqstate(data, (*mqtt).nextstate, MQTT_NOSTATE);
                } else {
                    mqstate(data, MQTT_FIRST, MQTT_FIRST);
                    if (*mq).firstbyte as libc::c_int == 0xe0 as libc::c_int {
                        Curl_infof(
                            data,
                            b"Got DISCONNECT\0" as *const u8 as *const libc::c_char,
                        );
