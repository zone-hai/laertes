...
extern "C" {
    ...
    fn Curl_getaddrinfo_ex(
        nodename: *const libc::c_char,
        servname: *const libc::c_char,
        hints: *const addrinfo,
        result: *mut *mut Curl_addrinfo,
    ) -> libc::c_int;
    fn Curl_resolver_error(data: *mut Curl_easy) -> CURLcode;
    fn Curl_addrinfo_callback(
        data: *mut Curl_easy,
        status: libc::c_int,
        ai: *mut Curl_addrinfo,
    ) -> CURLcode;
    ...
    fn Curl_thread_create(
        func: Option::<unsafe extern "C" fn(*mut libc::c_void) -> libc::c_uint>,
        arg: *mut libc::c_void,
    ) -> *mut pthread_t;
    fn Curl_thread_destroy(hnd: *mut pthread_t);
    fn Curl_thread_join(hnd: *mut *mut pthread_t) -> libc::c_int;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_resolver_global_init() -> libc::c_int {
    return CURLE_OK as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_resolver_global_cleanup() {}
#[no_mangle]
pub unsafe extern "C" fn Curl_resolver_init(
    ...
    *resolver = Curl_ccalloc
        ...
        )(1 as libc::c_int as size_t, ::std::mem::size_of::<resdata>() as libc::c_ulong);
    if (*resolver).is_null() {
...
unsafe extern "C" fn destroy_thread_sync_data(mut tsd: *mut thread_sync_data) {
    ...
    if (*tsd).sock_pair[1 as libc::c_int as usize] != -(1 as libc::c_int) {
        close((*tsd).sock_pair[1 as libc::c_int as usize]);
    }
    memset(
        tsd as *mut libc::c_void,
        0 as libc::c_int,
        ::std::mem::size_of::<thread_sync_data>() as libc::c_ulong,
    );
}
unsafe extern "C" fn init_thread_sync_data(
    ...
    let ref mut fresh0 = (*tsd).td;
    *fresh0 = td;
    (*tsd).port = port;
    (*tsd).done = 1 as libc::c_int;
    (*tsd).hints = *hints;
    let ref mut fresh1 = (*tsd).mtx;
    *fresh1 = Curl_cmalloc
        .expect(
            "non-null function pointer",
    ...
    if !((*tsd).mtx).is_null() {
        ...
        if socketpair(
            ...
            0 as libc::c_int,
            &mut *((*tsd).sock_pair).as_mut_ptr().offset(0 as libc::c_int as isize),
        ) < 0 as libc::c_int
        {
            (*tsd).sock_pair[0 as libc::c_int as usize] = -(1 as libc::c_int);
            (*tsd).sock_pair[1 as libc::c_int as usize] = -(1 as libc::c_int);
        } else {
            (*tsd).sock_error = CURLE_OK as libc::c_int;
            let ref mut fresh2 = (*tsd).hostname;
...
unsafe extern "C" fn getaddrinfo_complete(mut data: *mut Curl_easy) -> libc::c_int {
    let mut tsd: *mut thread_sync_data = conn_thread_sync_data(data);
    let mut rc: libc::c_int = 0;
    rc = Curl_addrinfo_callback(data, (*tsd).sock_error, (*tsd).res) as libc::c_int;
    let ref mut fresh3 = (*tsd).res;
    *fresh3 = 0 as *mut Curl_addrinfo;
    return rc;
}
unsafe extern "C" fn getaddrinfo_thread(mut arg: *mut libc::c_void) -> libc::c_uint {
    let mut tsd: *mut thread_sync_data = arg as *mut thread_sync_data;
    let mut td: *mut thread_data = (*tsd).td;
    let mut service: [libc::c_char; 12] = [0; 12];
    let mut rc: libc::c_int = 0;
    ...
    if rc != 0 {
        ...
        if (*tsd).sock_error == 0 as libc::c_int {
            (*tsd).sock_error = -(10 as libc::c_int);
        }
    }
    ...
    if (*tsd).done != 0 {
        ...
        if (*tsd).sock_pair[1 as libc::c_int as usize] != -(1 as libc::c_int) {
            ...
            if send(
                (*tsd).sock_pair[1 as libc::c_int as usize],
...
unsafe extern "C" fn destroy_async_data(mut async_0: *mut Curl_async) {
    if !((*async_0).tdata).is_null() {
        ...
        let mut sock_rd: curl_socket_t = (*td).tsd.sock_pair[0 as libc::c_int as usize];
        let mut data: *mut Curl_easy = (*td).tsd.data;
        pthread_mutex_lock((*td).tsd.mtx);
        done = (*td).tsd.done;
        (*td).tsd.done = 1 as libc::c_int;
...
unsafe extern "C" fn init_resolve_thread(
    ...
    let mut err: libc::c_int = 12 as libc::c_int;
    let mut asp: *mut Curl_async = &mut (*data).state.async_0;
    let ref mut fresh6 = (*data).state.async_0.tdata;
    *fresh6 = td;
    if !td.is_null() {
        (*asp).port = port;
        (*asp).set_done(0 as libc::c_int as bit);
        (*asp).status = 0 as libc::c_int;
        let ref mut fresh7 = (*asp).dns;
        *fresh7 = 0 as *mut Curl_dns_entry;
        let ref mut fresh8 = (*td).thread_hnd;
        *fresh8 = 0 as *mut pthread_t;
        if init_thread_sync_data(td, hostname, port, hints) == 0 {
            let ref mut fresh9 = (*asp).tdata;
            *fresh9 = 0 as *mut thread_data;
            ...
            let ref mut fresh10 = (*asp).hostname;
            *fresh10 = Curl_cstrdup.expect("non-null function pointer")(hostname);
            if !((*asp).hostname).is_null() {
                (*td).tsd.done = 0 as libc::c_int;
                let ref mut fresh11 = (*td).thread_hnd;
                *fresh11 = Curl_thread_create(
                    Some(
                        getaddrinfo_thread
                            as unsafe extern "C" fn(*mut libc::c_void) -> libc::c_uint,
                    ),
...
unsafe extern "C" fn thread_wait_resolv(
    ...
    let ref mut fresh12 = (*data).state.async_0;
    (*fresh12).set_done(1 as libc::c_int as bit);
    if !entry.is_null() {
        *entry = (*data).state.async_0.dns;
    }
    if ((*data).state.async_0.dns).is_null() && report as libc::c_int != 0 {
...
pub unsafe extern "C" fn Curl_resolver_kill(mut data: *mut Curl_easy) {
    ...
    if !td.is_null() && !((*td).thread_hnd).is_null() {
        thread_wait_resolv(data, 0 as *mut *mut Curl_dns_entry, 0 as libc::c_int != 0);
    } else {
...
pub unsafe extern "C" fn Curl_resolver_wait_resolv(
    ...
    return thread_wait_resolv(data, entry, 1 as libc::c_int != 0);
}
#[no_mangle]
    ...
    if done != 0 {
        ...
        *entry = (*data).state.async_0.dns;
        ...
        if elapsed < 0 as libc::c_int as libc::c_long {
            elapsed = 0 as libc::c_int as timediff_t;
        }
        if (*td).poll_interval == 0 as libc::c_int as libc::c_uint {
            (*td).poll_interval = 1 as libc::c_int as libc::c_uint;
        } else if elapsed >= (*td).interval_end {
            let ref mut fresh13 = (*td).poll_interval;
            *fresh13 = (*fresh13).wrapping_mul(2 as libc::c_int as libc::c_uint);
        }
        if (*td).poll_interval > 250 as libc::c_int as libc::c_uint {
            (*td).poll_interval = 250 as libc::c_int as libc::c_uint;
        }
        (*td).interval_end = elapsed + (*td).poll_interval as libc::c_long;
...
pub unsafe extern "C" fn Curl_resolver_getsock(
    ...
    if !td.is_null() {
        *socks
            ...
            ) = (*td).tsd.sock_pair[0 as libc::c_int as usize];
        let ref mut fresh14 = (*td).tsd.data;
        *fresh14 = data;
        ret_val = (1 as libc::c_int) << 0 as libc::c_int;
    } else {
        ms = Curl_timediff(Curl_now(), (*reslv).start);
        if ms < 3 as libc::c_int as libc::c_long {
            milli = 0 as libc::c_int as timediff_t;
        } else if ms <= 50 as libc::c_int as libc::c_long {
            milli = ms / 3 as libc::c_int as libc::c_long;
        } else if ms <= 250 as libc::c_int as libc::c_long {
            milli = 50 as libc::c_int as timediff_t;
        } else {
            milli = 200 as libc::c_int as timediff_t;
        }
...
pub unsafe extern "C" fn Curl_resolver_getaddrinfo(
    ...
    let mut pf: libc::c_int = 2 as libc::c_int;
    let mut reslv: *mut resdata = (*data).state.async_0.resolver as *mut resdata;
    *waitp = 0 as libc::c_int;
    ...
    hints.ai_family = pf;
    hints
        .ai_socktype = if (*(*data).conn).transport as libc::c_uint
    ...
    Curl_failf(
        data,
        b"getaddrinfo() thread failed to start\0" as *const u8 as *const libc::c_char,
    );
    return 0 as *mut Curl_addrinfo;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_set_dns_servers(
    mut data: *mut Curl_easy,
    mut servers: *mut libc::c_char,
) -> CURLcode {
    return CURLE_NOT_BUILT_IN;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_set_dns_interface(
    mut data: *mut Curl_easy,
...
#[no_mangle]
pub unsafe extern "C" fn Curl_set_dns_local_ip4(
    mut data: *mut Curl_easy,
    mut local_ip4: *const libc::c_char,
) -> CURLcode {
    return CURLE_NOT_BUILT_IN;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_set_dns_local_ip6(
    mut data: *mut Curl_easy,
    mut local_ip6: *const libc::c_char,
) -> CURLcode {
    return CURLE_NOT_BUILT_IN;
}
