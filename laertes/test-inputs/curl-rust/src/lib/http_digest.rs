...
extern "C" {
    ...
    fn Curl_auth_decode_digest_http_message(
        chlg: *const libc::c_char,
        digest: *mut digestdata,
    ) -> CURLcode;
    fn Curl_auth_create_digest_http_message(
        data: *mut Curl_easy,
    ...
    fn Curl_auth_digest_cleanup(digest: *mut digestdata);
...
pub unsafe extern "C" fn Curl_input_digest(
    ...
    if curl_strnequal(
        ...
        || Curl_isspace(
            *header.offset(6 as libc::c_int as isize) as libc::c_uchar as libc::c_int,
        ) == 0
    {
        return CURLE_BAD_CONTENT_ENCODING;
    }
    header = header
        .offset(strlen(b"Digest\0" as *const u8 as *const libc::c_char) as isize);
...
pub unsafe extern "C" fn Curl_output_digest(
    ...
    *allocuserpwd = 0 as *mut libc::c_char;
    ...
    if (*authp).iestyle() != 0 {
        ...
        if !tmp.is_null() {
            let mut urilen: size_t = tmp.offset_from(uripath as *mut libc::c_char)
    ...
    if tmp.is_null() {
        path = Curl_cstrdup
            .expect("non-null function pointer")(uripath as *mut libc::c_char)
            as *mut libc::c_uchar;
    }
    if path.is_null() {
