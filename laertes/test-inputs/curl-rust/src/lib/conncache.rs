...
unsafe extern "C" fn conn_llist_dtor(
    ...
    let mut conn: *mut connectdata = element as *mut connectdata;
    let ref mut fresh0 = (*conn).bundle;
    *fresh0 = 0 as *mut connectbundle;
}
unsafe extern "C" fn bundle_create(mut bundlep: *mut *mut connectbundle) -> CURLcode {
    *bundlep = Curl_cmalloc
        .expect(
            "non-null function pointer",
        )(::std::mem::size_of::<connectbundle>() as libc::c_ulong) as *mut connectbundle;
    if (*bundlep).is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    (**bundlep).num_connections = 0 as libc::c_int as size_t;
    (**bundlep).multiuse = 0 as libc::c_int;
    Curl_llist_init(
        &mut (**bundlep).conn_list,
        ::std::mem::transmute::<
...
unsafe extern "C" fn bundle_destroy(mut bundle: *mut connectbundle) {
    if bundle.is_null() {
        return;
    }
...
unsafe extern "C" fn bundle_add_conn(
    ...
    let ref mut fresh1 = (*conn).bundle;
    *fresh1 = bundle;
    let ref mut fresh2 = (*bundle).num_connections;
...
unsafe extern "C" fn bundle_remove_conn(
    ...
    while !curr.is_null() {
        if (*curr).ptr == conn as *mut libc::c_void {
            ...
            let ref mut fresh3 = (*bundle).num_connections;
            *fresh3 = (*fresh3).wrapping_sub(1);
            let ref mut fresh4 = (*conn).bundle;
...
unsafe extern "C" fn free_bundle_hash_entry(mut freethis: *mut libc::c_void) {
    let mut b: *mut connectbundle = freethis as *mut connectbundle;
    bundle_destroy(b);
}
#[no_mangle]
pub unsafe extern "C" fn Curl_conncache_init(
    mut connc: *mut conncache,
    mut size: libc::c_int,
) -> libc::c_int {
    let mut rc: libc::c_int = 0;
    let ref mut fresh5 = (*connc).closure_handle;
    *fresh5 = curl_easy_init();
    ...
    rc = Curl_hash_init(
        ...
        Some(free_bundle_hash_entry as unsafe extern "C" fn(*mut libc::c_void) -> ()),
    );
    if rc != 0 {
        Curl_close(&mut (*connc).closure_handle);
    } else {
        let ref mut fresh6 = (*(*connc).closure_handle).state.conn_cache;
        *fresh6 = connc;
    }
    return rc;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_conncache_destroy(mut connc: *mut conncache) {
    if !connc.is_null() {
        Curl_hash_destroy(&mut (*connc).hash);
    }
}
unsafe extern "C" fn hashkey(
    mut conn: *mut connectdata,
    mut buf: *mut libc::c_char,
    mut len: size_t,
    mut hostp: *mut *const libc::c_char,
) {
    let mut hostname: *const libc::c_char = 0 as *const libc::c_char;
    let mut port: libc::c_long = (*conn).remote_port as libc::c_long;
    if ((*conn).bits).httpproxy() as libc::c_int != 0
        && ((*conn).bits).tunnel_proxy() == 0
    {
        hostname = (*conn).http_proxy.host.name;
        port = (*conn).port as libc::c_long;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_conncache_size(mut data: *mut Curl_easy) -> size_t {
...
#[no_mangle]
pub unsafe extern "C" fn Curl_conncache_find_bundle(
    mut data: *mut Curl_easy,
    mut conn: *mut connectdata,
    mut connc: *mut conncache,
    mut hostp: *mut *const libc::c_char,
...
unsafe extern "C" fn conncache_add_bundle(
    ...
    let mut p: *mut libc::c_void = Curl_hash_add(
        &mut (*connc).hash,
        key as *mut libc::c_void,
        strlen(key),
        bundle as *mut libc::c_void,
    );
    return if !p.is_null() { 1 as libc::c_int } else { 0 as libc::c_int } != 0;
}
unsafe extern "C" fn conncache_remove_bundle(
    ...
    Curl_hash_start_iterate(&mut (*connc).hash, &mut iter);
...
#[no_mangle]
pub unsafe extern "C" fn Curl_conncache_add_conn(mut data: *mut Curl_easy) -> CURLcode {
    let mut current_block: u64;
    let mut result: CURLcode = CURLE_OK;
    let mut bundle: *mut connectbundle = 0 as *mut connectbundle;
    let mut conn: *mut connectdata = (*data).conn;
    let mut connc: *mut conncache = (*data).state.conn_cache;
    ...
    if bundle.is_null() {
        let mut rc: libc::c_int = 0;
        let mut key: [libc::c_char; 128] = [0; 128];
        result = bundle_create(&mut bundle);
        if result as u64 != 0 {
    ...
    match current_block {
        17407779659766490442 => {
            ...
            let ref mut fresh7 = (*connc).next_connection_id;
            let fresh8 = *fresh7;
            *fresh7 = *fresh7 + 1;
            (*conn).connection_id = fresh8;
            let ref mut fresh9 = (*connc).num_conn;
...
pub unsafe extern "C" fn Curl_conncache_remove_conn(
    ...
    let mut bundle: *mut connectbundle = (*conn).bundle;
    let mut connc: *mut conncache = (*data).state.conn_cache;
    if !bundle.is_null() {
        if lock {
            if !((*data).share).is_null() {
        ...
        if (*bundle).num_connections == 0 as libc::c_int as libc::c_ulong {
            conncache_remove_bundle(connc, bundle);
        }
        let ref mut fresh10 = (*conn).bundle;
        *fresh10 = 0 as *mut connectbundle;
        if !connc.is_null() {
            let ref mut fresh11 = (*connc).num_conn;
...
pub unsafe extern "C" fn Curl_conncache_foreach(
    ...
    while !he.is_null() {
        ...
        curr = (*bundle).conn_list.head;
        while !curr.is_null() {
            let mut conn: *mut connectdata = (*curr).ptr as *mut connectdata;
            curr = (*curr).next;
            if 1 as libc::c_int
...
unsafe extern "C" fn conncache_find_first_connection(
    mut connc: *mut conncache,
) -> *mut connectdata {
    let mut iter: Curl_hash_iterator = Curl_hash_iterator {
        hash: 0 as *mut Curl_hash,
...
pub unsafe extern "C" fn Curl_conncache_return_conn(
    ...
    let mut maxconnects: size_t = (if (*(*data).multi).maxconnects
        < 0 as libc::c_int as libc::c_long
    {
        ((*(*data).multi).num_easy * 4 as libc::c_int) as libc::c_long
    } else {
        (*(*data).multi).maxconnects
    }) as size_t;
    let mut conn_candidate: *mut connectdata = 0 as *mut connectdata;
    (*conn).lastused = Curl_now();
    if maxconnects > 0 as libc::c_int as libc::c_ulong
        && Curl_conncache_size(data) > maxconnects
    {
        ...
        if !conn_candidate.is_null() {
            Curl_disconnect(data, conn_candidate, 0 as libc::c_int != 0);
        }
    }
    return if conn_candidate == conn { 0 as libc::c_int } else { 1 as libc::c_int } != 0;
}
#[no_mangle]
pub unsafe extern "C" fn Curl_conncache_extract_bundle(
    ...
    let mut curr: *mut Curl_llist_element = 0 as *mut Curl_llist_element;
    let mut highscore: timediff_t = -(1 as libc::c_int) as timediff_t;
    let mut score: timediff_t = 0;
    ...
    if !conn_candidate.is_null() {
        ...
        let ref mut fresh12 = (*(*data).state.conn_cache).num_conn;
...
pub unsafe extern "C" fn Curl_conncache_extract_oldest(
    ...
    let mut conn_candidate: *mut connectdata = 0 as *mut connectdata;
    let mut bundle: *mut connectbundle = 0 as *mut connectbundle;
    let mut bundle_candidate: *mut connectbundle = 0 as *mut connectbundle;
    now = Curl_now();
    if !((*data).share).is_null() {
        Curl_share_lock(data, CURL_LOCK_DATA_CONNECT, CURL_LOCK_ACCESS_SINGLE);
    }
    ...
    if !conn_candidate.is_null() {
        ...
        let ref mut fresh13 = (*connc).num_conn;
...
#[no_mangle]
pub unsafe extern "C" fn Curl_conncache_close_all_connections(
    mut connc: *mut conncache,
) {
    let mut conn: *mut connectdata = 0 as *mut connectdata;
    let mut buffer: [libc::c_char; 1025] = [0; 1025];
    if ((*connc).closure_handle).is_null() {
        return;
    }
    let ref mut fresh14 = (*(*connc).closure_handle).state.buffer;
    *fresh14 = buffer.as_mut_ptr();
    (*(*connc).closure_handle).set.buffer_size = 1024 as libc::c_int as libc::c_long;
    ...
    while !conn.is_null() {
        ...
        sigpipe_ignore((*connc).closure_handle, &mut pipe_st);
        Curl_conncontrol(conn, 1 as libc::c_int);
        Curl_conncache_remove_conn((*connc).closure_handle, conn, 1 as libc::c_int != 0);
        Curl_disconnect((*connc).closure_handle, conn, 0 as libc::c_int != 0);
        sigpipe_restore(&mut pipe_st);
        conn = conncache_find_first_connection(connc);
    }
    let ref mut fresh15 = (*(*connc).closure_handle).state.buffer;
    *fresh15 = 0 as *mut libc::c_char;
    if !((*connc).closure_handle).is_null() {
        let mut pipe_st_0: sigpipe_ignore = sigpipe_ignore {
            old_pipe_act: sigaction {
                __sigaction_handler: C2RustUnnamed_6 {
                    sa_handler: None,
                },
        ...
        sigpipe_ignore((*connc).closure_handle, &mut pipe_st_0);
        Curl_hostcache_clean(
            (*connc).closure_handle,
            (*(*connc).closure_handle).dns.hostcache,
        );
        Curl_close(&mut (*connc).closure_handle);
        sigpipe_restore(&mut pipe_st_0);
    }
}
