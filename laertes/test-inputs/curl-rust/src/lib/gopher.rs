...
extern "C" {
    ...
    fn curlx_uztosz(uznum: size_t) -> ssize_t;
...
#[no_mangle]
pub static mut Curl_handler_gopher: Curl_handler = unsafe {
    {
        let mut init = Curl_handler {
            scheme: b"GOPHER\0" as *const u8 as *const libc::c_char,
            setup_connection: None,
            do_it: Some(
                gopher_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            done: None,
            do_more: None,
            connect_it: None,
            ...
            attach: None,
            defport: 70 as libc::c_int,
            protocol: ((1 as libc::c_int) << 25 as libc::c_int) as libc::c_uint,
            family: ((1 as libc::c_int) << 25 as libc::c_int) as libc::c_uint,
...
#[no_mangle]
pub static mut Curl_handler_gophers: Curl_handler = unsafe {
    {
        let mut init = Curl_handler {
            scheme: b"GOPHERS\0" as *const u8 as *const libc::c_char,
            setup_connection: None,
            do_it: Some(
                gopher_do as unsafe extern "C" fn(*mut Curl_easy, *mut bool) -> CURLcode,
            ),
            ...
            attach: None,
            defport: 70 as libc::c_int,
            protocol: ((1 as libc::c_int) << 29 as libc::c_int) as libc::c_uint,
            family: ((1 as libc::c_int) << 25 as libc::c_int) as libc::c_uint,
...
unsafe extern "C" fn gopher_connect(
    mut data: *mut Curl_easy,
    mut done: *mut bool,
) -> CURLcode {
    return CURLE_OK;
}
unsafe extern "C" fn gopher_connecting(
    mut data: *mut Curl_easy,
    mut done: *mut bool,
) -> CURLcode {
    let mut conn: *mut connectdata = (*data).conn;
    let mut result: CURLcode = Curl_ssl_connect(data, conn, 0 as libc::c_int);
    if result as u64 != 0 {
...
unsafe extern "C" fn gopher_do(
    ...
    let mut gopherpath: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut path: *mut libc::c_char = (*data).state.up.path;
    let mut query: *mut libc::c_char = (*data).state.up.query;
    let mut sel: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut sel_org: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    *done = 1 as libc::c_int != 0;
    if !query.is_null() {
        gopherpath = curl_maprintf(
            b"%s?%s\0" as *const u8 as *const libc::c_char,
            path,
            query,
        );
    } else {
        gopherpath = Curl_cstrdup.expect("non-null function pointer")(path);
    }
    if gopherpath.is_null() {
        return CURLE_OUT_OF_MEMORY;
    }
    if strlen(gopherpath) <= 2 as libc::c_int as libc::c_ulong {
        sel = b"\0" as *const u8 as *const libc::c_char as *mut libc::c_char;
        len = strlen(sel);
        Curl_cfree.expect("non-null function pointer")(gopherpath as *mut libc::c_void);
    } else {
        let mut newp: *mut libc::c_char = 0 as *mut libc::c_char;
        newp = gopherpath;
        newp = newp.offset(2 as libc::c_int as isize);
    ...
    k = curlx_uztosz(len);
    while !(strlen(sel) < 1 as libc::c_int as libc::c_ulong) {
        result = Curl_write(
            data,
            sockfd,
        ...
        k -= amount;
        sel = sel.offset(amount as isize);
        if k < 1 as libc::c_int as libc::c_long {
            break;
        }
        timeout_ms = Curl_timeleft(data, 0 as *mut curltime, 0 as libc::c_int != 0);
        if timeout_ms < 0 as libc::c_int as libc::c_long {
            result = CURLE_OPERATION_TIMEDOUT;
            break;
        } else {
            if timeout_ms == 0 {
                timeout_ms = 0x7fffffffffffffff as libc::c_long;
            }
    ...
    Curl_cfree.expect("non-null function pointer")(sel_org as *mut libc::c_void);
    if result as u64 == 0 {
        result = Curl_write(
            data,
            sockfd,
    ...
    if result as u64 != 0 {
        Curl_failf(
            data,
            b"Failed sending Gopher request\0" as *const u8 as *const libc::c_char,
        );
        return result;
    }
    result = Curl_client_write(
