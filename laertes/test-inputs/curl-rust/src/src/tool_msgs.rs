...
unsafe extern "C" fn voutf(
    ...
    let mut width: size_t = (79 as libc::c_int as libc::c_ulong)
        .wrapping_sub(strlen(prefix));
    if !(*config).mute {
        let mut len: size_t = 0;
        let mut ptr: *mut libc::c_char = 0 as *mut libc::c_char;
        let mut print_buffer: *mut libc::c_char = 0 as *mut libc::c_char;
        ...
        while len > 0 as libc::c_int as libc::c_ulong {
            ...
            if len > width {
                ...
                while Curl_isspace(
                    *ptr.offset(cut as isize) as libc::c_uchar as libc::c_int,
                ) == 0 && cut != 0
                {
                    cut = cut.wrapping_sub(1);
                }
                if 0 as libc::c_int as libc::c_ulong == cut {
                    cut = width.wrapping_sub(1 as libc::c_int as libc::c_ulong);
                }
                fwrite(
                    ptr as *const libc::c_void,
                    cut.wrapping_add(1 as libc::c_int as libc::c_ulong),
                    1 as libc::c_int as libc::c_ulong,
                    (*config).errors,
                );
                fputs(b"\n\0" as *const u8 as *const libc::c_char, (*config).errors);
                ptr = ptr
                    .offset(
                        cut.wrapping_add(1 as libc::c_int as libc::c_ulong) as isize,
                    );
                len = (len as libc::c_ulong)
                    .wrapping_sub(cut.wrapping_add(1 as libc::c_int as libc::c_ulong))
                    as size_t as size_t;
            } else {
...
pub unsafe extern "C" fn helpf(
    ...
    if !fmt.is_null() {
        let mut ap: ::std::ffi::VaListImpl;
        ap = args.clone();
        fputs(b"curl: \0" as *const u8 as *const libc::c_char, errors);
...
pub unsafe extern "C" fn errorf(
    ...
    if !(*config).mute {
        ...
        voutf(
            config,
            b"curl: \0" as *const u8 as *const libc::c_char,
            fmt,
            ap.as_va_list(),
        );
    }
