...
static mut mappings: [xattr_mapping; 4] = [
    {
        let mut init = xattr_mapping {
            attr: b"user.xdg.referrer.url\0" as *const u8 as *const libc::c_char,
    ...
    {
        let mut init = xattr_mapping {
            attr: b"user.xdg.origin.url\0" as *const u8 as *const libc::c_char,
    ...
    {
        let mut init = xattr_mapping {
            attr: b"user.mime_type\0" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn stripcredentials(mut url: *mut *mut libc::c_char) -> bool {
    let mut u: *mut CURLU = 0 as *mut CURLU;
    let mut uc: CURLUcode = CURLUE_OK;
    let mut nurl: *mut libc::c_char = 0 as *mut libc::c_char;
    u = curl_url();
    if !u.is_null() {
        uc = curl_url_set(u, CURLUPART_URL, *url, 0 as libc::c_int as libc::c_uint);
        if !(uc as u64 != 0) {
            uc = curl_url_set(
                u,
                CURLUPART_USER,
...
pub unsafe extern "C" fn fwrite_xattr(
    ...
    while err == 0 as libc::c_int && !(mappings[i as usize].attr).is_null() {
        ...
        if result as u64 == 0 && !value.is_null() {
            let mut freeptr: bool = 0 as libc::c_int != 0;
            if CURLINFO_EFFECTIVE_URL as libc::c_int as libc::c_uint
                == mappings[i as usize].info as libc::c_uint
            {
