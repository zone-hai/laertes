...
pub unsafe extern "C" fn htmlSetMetaEncoding(
    ...
    if !encoding.is_null() {
        ...
        newcontent[(::std::mem::size_of::<[libc::c_char; 100]>() as libc::c_ulong)
...
pub unsafe extern "C" fn htmlNodeDumpFileFormat(
    ...
    htmlNodeDumpFormatOutput(buf, doc, cur, 0 as *const libc::c_char, format);
...
unsafe extern "C" fn htmlDtdDumpOutput(
    ...
    if !((*cur).ExternalID).is_null() {
        ...
        if !((*cur).SystemID).is_null() {
            ...
            && xmlStrcmp(
                (*cur).SystemID,
                b"about:legacy-compat\0" as *const u8 as *const libc::c_char
                    as *mut xmlChar,
            ) != 0
        {
        xmlOutputBufferWriteString(
        ...
        match (*cur).type_0 as libc::c_uint {
            13 | 9 => {
                if !((*(cur as xmlDocPtr)).intSubset).is_null() {
                    htmlDtdDumpOutput(buf, cur as xmlDocPtr, 0 as *const libc::c_char);
                }
                if !((*cur).children).is_null() {
            ...
            1 => {
                if (*cur).parent != parent && !((*cur).children).is_null() {
                    ...
                    if !info.is_null() && (*info).empty as libc::c_int != 0 {
                        ...
                        if !info.is_null()
                            && (*info).saveEndTag as libc::c_int != 0 as libc::c_int
                            && xmlStrcmp(
                        ...
                        if format != 0 && !info.is_null() && (*info).isinline == 0
                            ...
                            && *((*cur).name).offset(0 as libc::c_int as isize)
                        ...
                        && (*info).isinline == 0
                    {
                        if (*(*cur).next).type_0 as libc::c_uint
                            ...
                            && *((*parent).name).offset(0 as libc::c_int as isize)
                                as libc::c_int != 'p' as i32
                        {
                            xmlOutputBufferWriteString(
                                buf,
...
pub unsafe extern "C" fn htmlNodeDumpOutput(
    ...
    htmlNodeDumpFormatOutput(buf, doc, cur, 0 as *const libc::c_char, 1 as libc::c_int);
}
#[no_mangle]
