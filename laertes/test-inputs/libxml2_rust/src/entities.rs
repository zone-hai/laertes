...
unsafe extern "C" fn xmlCreateEntity(
    ...
    ret = xmlMalloc
        .expect(
            "non-null function pointer",
        )(::std::mem::size_of::<xmlEntity>() as libc::c_ulong) as xmlEntityPtr;
    if ret.is_null() {
    ...
    memset(
        ...
        0 as libc::c_int,
        ::std::mem::size_of::<xmlEntity>() as libc::c_ulong,
    );
    (*ret).type_0 = XML_ENTITY_DECL;
    (*ret).checked = 0 as libc::c_int;
    (*ret).etype = type_0 as xmlEntityType;
    if dict.is_null() {
        let ref mut fresh0 = (*ret).name;
        *fresh0 = xmlStrdup(name);
        if !ExternalID.is_null() {
            let ref mut fresh1 = (*ret).ExternalID;
            *fresh1 = xmlStrdup(ExternalID);
        }
        if !SystemID.is_null() {
            let ref mut fresh2 = (*ret).SystemID;
            *fresh2 = xmlStrdup(SystemID);
        }
    } else {
        let ref mut fresh3 = (*ret).name;
        *fresh3 = xmlDictLookup(dict, name, -(1 as libc::c_int));
        if !ExternalID.is_null() {
            let ref mut fresh4 = (*ret).ExternalID;
            *fresh4 = xmlDictLookup(dict, ExternalID, -(1 as libc::c_int));
        }
        if !SystemID.is_null() {
            let ref mut fresh5 = (*ret).SystemID;
            *fresh5 = xmlDictLookup(dict, SystemID, -(1 as libc::c_int));
        }
    }
    if !content.is_null() {
        (*ret).length = xmlStrlen(content);
        if !dict.is_null() && (*ret).length < 5 as libc::c_int {
            let ref mut fresh6 = (*ret).content;
    ...
    let ref mut fresh9 = (*ret).URI;
    *fresh9 = 0 as *const xmlChar;
    let ref mut fresh10 = (*ret).orig;
    ...
    match type_0 {
        1 | 2 | 3 => {
            ...
            if !predef.is_null() {
                ...
                if type_0 == XML_INTERNAL_GENERAL_ENTITY as libc::c_int
                    && !content.is_null()
                {
                    let mut c: libc::c_int = *((*predef).content)
                        .offset(0 as libc::c_int as isize) as libc::c_int;
                    if *content.offset(0 as libc::c_int as isize) as libc::c_int == c
                        && *content.offset(1 as libc::c_int as isize) as libc::c_int
                            == 0 as libc::c_int
                        && (c == '>' as i32 || c == '\'' as i32 || c == '"' as i32)
                    {
                        valid = 1 as libc::c_int;
                    } else if *content.offset(0 as libc::c_int as isize) as libc::c_int
                            == '&' as i32
                            && *content.offset(1 as libc::c_int as isize) as libc::c_int
                                == '#' as i32
                        {
                        if *content.offset(2 as libc::c_int as isize) as libc::c_int
                            == 'x' as i32
                        {
                            ...
                            ref_0[0 as libc::c_int
                                as usize] = *hex
                                .offset(
                                    (c / 16 as libc::c_int % 16 as libc::c_int) as isize,
                                );
                            ref_0[1 as libc::c_int
                                as usize] = *hex.offset((c % 16 as libc::c_int) as isize);
                            if xmlStrcasecmp(
                                &*content.offset(3 as libc::c_int as isize),
                                ref_0.as_mut_ptr(),
                            ) == 0 as libc::c_int
                            {
                                valid = 1 as libc::c_int;
                            ...
                            let mut ref_1: [xmlChar; 4] = *::std::mem::transmute::<
                                ...
                                &mut [xmlChar; 4],
                            >(b"00;\0");
                            ref_1[0 as libc::c_int
                                as usize] = ('0' as i32
                                + c / 10 as libc::c_int % 10 as libc::c_int) as xmlChar;
                            ref_1[1 as libc::c_int
                                as usize] = ('0' as i32 + c % 10 as libc::c_int) as xmlChar;
                            if xmlStrEqual(
                                &*content.offset(2 as libc::c_int as isize),
                                ref_1.as_mut_ptr(),
                            ) != 0
                            {
                                valid = 1 as libc::c_int;
            ...
            if ((*dtd).entities).is_null() {
                let ref mut fresh11 = (*dtd).entities;
                *fresh11 = xmlHashCreateDict(0 as libc::c_int, dict)
                    as *mut libc::c_void;
            }
        ...
        4 | 5 => {
            if ((*dtd).pentities).is_null() {
                let ref mut fresh12 = (*dtd).pentities;
                *fresh12 = xmlHashCreateDict(0 as libc::c_int, dict)
                    as *mut libc::c_void;
            }
...
pub unsafe extern "C" fn xmlAddDtdEntity(
    ...
    let ref mut fresh14 = (*ret).parent;
    *fresh14 = dtd;
    let ref mut fresh15 = (*ret).doc;
    *fresh15 = (*dtd).doc;
    if ((*dtd).last).is_null() {
        let ref mut fresh16 = (*dtd).last;
        *fresh16 = ret as xmlNodePtr;
        let ref mut fresh17 = (*dtd).children;
        *fresh17 = *fresh16;
    } else {
        let ref mut fresh18 = (*(*dtd).last).next;
        *fresh18 = ret as xmlNodePtr;
        let ref mut fresh19 = (*ret).prev;
        *fresh19 = (*dtd).last;
        let ref mut fresh20 = (*dtd).last;
...
pub unsafe extern "C" fn xmlAddDocEntity(
    ...
    let ref mut fresh21 = (*ret).parent;
    *fresh21 = dtd;
    let ref mut fresh22 = (*ret).doc;
    *fresh22 = (*dtd).doc;
    if ((*dtd).last).is_null() {
        let ref mut fresh23 = (*dtd).last;
        *fresh23 = ret as xmlNodePtr;
        let ref mut fresh24 = (*dtd).children;
        *fresh24 = *fresh23;
    } else {
        let ref mut fresh25 = (*(*dtd).last).next;
        *fresh25 = ret as xmlNodePtr;
        let ref mut fresh26 = (*ret).prev;
        *fresh26 = (*dtd).last;
        let ref mut fresh27 = (*dtd).last;
...
unsafe extern "C" fn xmlEncodeEntitiesInternal(
    ...
    buffer = xmlMalloc
        .expect(
            "non-null function pointer",
        )(buffer_size.wrapping_mul(::std::mem::size_of::<xmlChar>() as libc::c_ulong))
        as *mut xmlChar;
    if buffer.is_null() {
        xmlEntitiesErrMemory(
    ...
    's_62: loop {
        ...
        if indx.wrapping_add(100 as libc::c_int as libc::c_ulong) > buffer_size {
        ...
        if *cur as libc::c_int == '<' as i32 {
                ...
                }
            {
                while cur != end {
                    ...
                    *fresh30 = *fresh29;
                    ...
                    if !(indx.wrapping_add(100 as libc::c_int as libc::c_ulong)
                        > buffer_size)
                    {
                        continue;
                    }
            ...
            *fresh54 = *cur;
            ...
            if !doc.is_null() && !((*doc).encoding).is_null() || html != 0 {
                ...
                *fresh55 = *cur;
                ...
                if *cur.offset(0 as libc::c_int as isize) as libc::c_int
                    & 0xc0 as libc::c_int != 0xc0 as libc::c_int
                    ...
                    || *cur.offset(0 as libc::c_int as isize) as libc::c_int
                        ...
                        && *cur.offset(3 as libc::c_int as isize) as libc::c_int
                            & 0xc0 as libc::c_int != 0x80 as libc::c_int
                    || *cur.offset(0 as libc::c_int as isize) as libc::c_int
                        & 0xf8 as libc::c_int == 0xf8 as libc::c_int
                {
                    xmlEntitiesErr(
                        XML_CHECK_NOT_UTF8,
                    ...
                    if !doc.is_null() {
                        let ref mut fresh56 = (*doc).encoding;
                        *fresh56 = xmlStrdup(
                    ...
                    buf[(::std::mem::size_of::<[libc::c_char; 11]>() as libc::c_ulong)
                        ...
                        }) == 0
                    {
                        ...
                        if !doc.is_null() {
                            let ref mut fresh59 = (*doc).encoding;
                            *fresh59 = xmlStrdup(
                                b"ISO-8859-1\0" as *const u8 as *const libc::c_char
                                    as *mut xmlChar,
                            );
                        ...
                        buf[(::std::mem::size_of::<[libc::c_char; 11]>()
                            as libc::c_ulong)
                            .wrapping_sub(1 as libc::c_int as libc::c_ulong)
                            as usize] = 0 as libc::c_int as libc::c_char;
                        ptr = buf.as_mut_ptr();
            ...
            buf_0[(::std::mem::size_of::<[libc::c_char; 11]>() as libc::c_ulong)
                .wrapping_sub(1 as libc::c_int as libc::c_ulong)
                as usize] = 0 as libc::c_int as libc::c_char;
...
pub unsafe extern "C" fn xmlEncodeSpecialChars(
    ...
    loop {
        ...
        let mut indx: size_t = out.offset_from(buffer) as libc::c_long as size_t;
        if indx.wrapping_add(10 as libc::c_int as libc::c_ulong) > buffer_size {
            let mut tmp: *mut xmlChar = 0 as *mut xmlChar;
            let mut new_size: size_t = buffer_size
                .wrapping_mul(2 as libc::c_int as libc::c_ulong);
            if new_size < buffer_size {
