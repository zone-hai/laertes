...
unsafe extern "C" fn xmlXIncludeNewRef(
    ...
    ret = xmlMalloc
        ...
        )(::std::mem::size_of::<xmlXIncludeRef>() as libc::c_ulong) as xmlXIncludeRefPtr;
    if ret.is_null() {
    ...
    memset(
        ...
        0 as libc::c_int,
        ::std::mem::size_of::<xmlXIncludeRef>() as libc::c_ulong,
    );
    if URI.is_null() {
    ...
    if (*ctxt).incMax == 0 as libc::c_int {
        ...
        *fresh8 = xmlMalloc
            ...
            ((*ctxt).incMax as libc::c_ulong)
                .wrapping_mul(
                    ::std::mem::size_of::<xmlXIncludeRefPtr>() as libc::c_ulong,
                ),
...
pub unsafe extern "C" fn xmlXIncludeNewContext(
    ...
    ret = xmlMalloc
        .expect(
            "non-null function pointer",
        )(::std::mem::size_of::<xmlXIncludeCtxt>() as libc::c_ulong)
...
unsafe extern "C" fn xmlXIncludeURLPop(mut ctxt: xmlXIncludeCtxtPtr) {
    ...
    *fresh21 -= 1;
    if (*ctxt).urlNr > 0 as libc::c_int {
        ...
        *fresh22 = *((*ctxt).urlTab).offset(((*ctxt).urlNr - 1 as libc::c_int) as isize);
    } else {
...
unsafe extern "C" fn xmlXIncludeParseFile(
    ...
    *fresh25 = (*ctxt)._private;
    ...
    if !URL.is_null()
        && strcmp(URL, b"-\0" as *const u8 as *const libc::c_char) == 0 as libc::c_int
    {
        URL = b"./-\0" as *const u8 as *const libc::c_char;
    }
...
unsafe extern "C" fn xmlXIncludeAddTxt(
    ...
    if (*ctxt).txtMax == 0 as libc::c_int {
        ...
        *fresh43 = xmlMalloc
            ...
            ((*ctxt).txtMax as libc::c_ulong)
                .wrapping_mul(::std::mem::size_of::<xmlURL>() as libc::c_ulong),
        ) as *mut xmlURL;
        if ((*ctxt).txturlTab).is_null() {
            xmlXIncludeErrMemory(
                ctxt,
    ...
    match current_block {
        5689316957504528238 => {
            ...
            *fresh62 = xmlGetCharEncodingHandler(enc);
            ...
            's_281: while xmlParserInputBufferRead(buf, 128 as libc::c_int)
                > 0 as libc::c_int
            {
...
unsafe extern "C" fn xmlXIncludeLoadFallback(
    ...
    let mut oldNbErrors: libc::c_int = (*ctxt).nbErrors;
    if fallback.is_null()
    ...
    if !((*fallback).children).is_null() {
        ...
        *fresh65 = xmlStrdup((*ctxt).base);
        ...
        (*newctxt).incTotal = (*ctxt).incTotal;
        if xmlXIncludeDoProcess(newctxt, (*ctxt).doc, fallback, 1 as libc::c_int)
            < 0 as libc::c_int
        {
            ret = -(1 as libc::c_int);
        }
    ...
    (**((*ctxt).incTab).offset(nr as isize)).fallback = 1 as libc::c_int;
    return ret;
}
...
unsafe extern "C" fn xmlXIncludeIncludeNode(
    ...
    *fresh70 = 0 as xmlNodePtr;
    (**((*ctxt).incTab).offset(nr as isize)).emptyFb = 0 as libc::c_int;
...
pub unsafe extern "C" fn xmlXIncludeProcessTreeFlagsData(
    ...
    *fresh73 = xmlStrdup((*(*tree).doc).URL as *mut xmlChar);
    ...
    ret = xmlXIncludeDoProcess(ctxt, (*tree).doc, tree, 0 as libc::c_int);
...
pub unsafe extern "C" fn xmlXIncludeProcessNode(
    ...
    ret = xmlXIncludeDoProcess(ctxt, (*node).doc, node, 0 as libc::c_int);
    if ret >= 0 as libc::c_int && (*ctxt).nbErrors > 0 as libc::c_int {
        ret = -(1 as libc::c_int);
    }
    return ret;
