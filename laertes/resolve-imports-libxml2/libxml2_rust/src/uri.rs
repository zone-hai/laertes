...
unsafe extern "C" fn xmlParse3986Scheme(
    ...
    if !uri.is_null() {
        ...
        let fresh0 = &mut ((*uri).scheme);
        *fresh0 = xmlStrndup(
            *str as *const xmlChar,
            cur.offset_from(*str) as i64 as i32,
        ) as *mut i8;
    }
    *str = cur;
    return 0 as i32;
...
unsafe extern "C" fn xmlParse3986Fragment(
    ...
    if !uri.is_null() {
        ...
        if (*uri).cleanup & 2 as i32 != 0 {
            let fresh1 = &mut ((*uri).fragment);
            *fresh1 = xmlStrndup(
                *str as *const xmlChar,
                cur.offset_from(*str) as i64 as i32,
            ) as *mut i8;
        } else {
            let fresh2 = &mut ((*uri).fragment);
...
unsafe extern "C" fn xmlParse3986Query(
    ...
    if !uri.is_null() {
        ...
        if (*uri).cleanup & 2 as i32 != 0 {
            let fresh3 = &mut ((*uri).query);
            *fresh3 = xmlStrndup(
                *str as *const xmlChar,
                cur.offset_from(*str) as i64 as i32,
            ) as *mut i8;
        } else {
            let fresh4 = &mut ((*uri).query);
...
unsafe extern "C" fn xmlParse3986Userinfo(
    ...
    if *cur as i32 == '@' as i32 {
        if !uri.is_null() {
            ...
            if (*uri).cleanup & 2 as i32 != 0 {
                let fresh6 = &mut ((*uri).user);
                *fresh6 = xmlStrndup(
                    *str as *const xmlChar,
                    cur.offset_from(*str) as i64 as i32,
                ) as *mut i8;
            } else {
                let fresh7 = &mut ((*uri).user);
...
unsafe extern "C" fn xmlParse3986PathAbEmpty(
    ...
    if !uri.is_null() {
        ...
        if *str != cur {
            if (*uri).cleanup & 2 as i32 != 0 {
                let fresh12 = &mut ((*uri).path);
                *fresh12 = xmlStrndup(
                    *str as *const xmlChar,
                    cur.offset_from(*str) as i64 as i32,
                ) as *mut i8;
            } else {
                let fresh13 = &mut ((*uri).path);
...
unsafe extern "C" fn xmlParse3986PathAbsolute(
    ...
    if !uri.is_null() {
        ...
        if cur != *str {
            if (*uri).cleanup & 2 as i32 != 0 {
                let fresh15 = &mut ((*uri).path);
                *fresh15 = xmlStrndup(
                    *str as *const xmlChar,
                    cur.offset_from(*str) as i64 as i32,
                ) as *mut i8;
            } else {
                let fresh16 = &mut ((*uri).path);
...
unsafe extern "C" fn xmlParse3986PathRootless(
    ...
    if !uri.is_null() {
        ...
        if cur != *str {
            if (*uri).cleanup & 2 as i32 != 0 {
                let fresh18 = &mut ((*uri).path);
                *fresh18 = xmlStrndup(
                    *str as *const xmlChar,
                    cur.offset_from(*str) as i64 as i32,
                ) as *mut i8;
            } else {
                let fresh19 = &mut ((*uri).path);
...
unsafe extern "C" fn xmlParse3986PathNoScheme(
    ...
    if !uri.is_null() {
        ...
        if cur != *str {
            if (*uri).cleanup & 2 as i32 != 0 {
                let fresh21 = &mut ((*uri).path);
                *fresh21 = xmlStrndup(
                    *str as *const xmlChar,
                    cur.offset_from(*str) as i64 as i32,
                ) as *mut i8;
            } else {
                let fresh22 = &mut ((*uri).path);
...
#[no_mangle]
pub unsafe extern "C" fn xmlParseURI(mut str: *const i8) -> xmlURIPtr {
...
unsafe extern "C" fn xmlSaveUriRealloc(
    ...
    let mut tmp: i32 = 0;
    if *max > 1024 as i32 * 1024 as i32 {
        xmlURIErrMemory(
                            ...
                            if ret != 0 as i32 {
                                ...
                                if !res.is_null() {
                                            ...
                                            && ((*ref_0).server).is_null())
                                    {
                                        ...
                                        if !((*bas).scheme).is_null() {
                                            let fresh121 = &mut ((*res).scheme);
                                            *fresh121 = xmlMemStrdup
                                                .expect("non-null function pointer")((*bas).scheme);
                                        }
                                        if !((*ref_0).query_raw).is_null() {
                                            let fresh122 = &mut ((*res).query_raw);
                                            *fresh122 = xmlMemStrdup
                                                .expect("non-null function pointer")((*ref_0).query_raw);
                                        } else if !((*ref_0).query).is_null() {
                                            let fresh123 = &mut ((*res).query);
                                            *fresh123 = xmlMemStrdup
                                                .expect("non-null function pointer")((*ref_0).query);
                                        }
                                        if !((*ref_0).fragment).is_null() {
                                            let fresh124 = &mut ((*res).fragment);
                                            ...
                                            || !((*ref_0).server).is_null()
                                        {
                                            ...
                                            if !((*bas).authority).is_null() {
                                                let fresh129 = &mut ((*res).authority);
                                                *fresh129 = xmlMemStrdup
                                                ...
                                                {
                                                if !((*bas).server).is_null() {
                                                    let fresh130 = &mut ((*res).server);
                                                    *fresh130 = xmlMemStrdup
                                                        .expect("non-null function pointer")((*bas).server);
                                                }
                                                if !((*bas).user).is_null() {
                                                    let fresh131 = &mut ((*res).user);
